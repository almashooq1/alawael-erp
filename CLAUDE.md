...
- 15 `describe.skip` blocks in `backend/tests/eStamp.test.js` + `eSignaturePdf.test.js` — supertest integration tests parked because `safeMount` swallows route-load failures in isolated Jest env. Fix `safeMount` OR migrate these to a test-DB setup.
