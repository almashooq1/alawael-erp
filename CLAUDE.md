...
- **5 dormant route files** identified 2026-05-20 by dead-route audit, **re-classified 2026-05-20** after side-by-side endpoint review. Real Express routers (574-1500 LOC, 8-45 endpoints each), referenced ONLY from their own auto-generated `tests/unit/*.routes.test.js` — never `require()`-d by `app.js`, `routes/_registry.js`, or any `routes/registries/*.js`. Last human change to each was 3 weeks ago (only lint sweeps since). Per-file verdict:
  ...
  - ~~`digital-wallet.routes.js` (269 LOC, 15 endpoints — `/stats`, `/coupons`, `/:id/{topup,debit,transfer,statement,block,unblock,apply-coupon,redeem-loyalty,loyalty-history}`)~~ — **mounted 2026-05-21** in `routes/registries/finance.registry.js` (W225b). Self-protecting (auth + branchScope + per-wallet ownership guard via `requireWalletAccess`) → plain `dualMount`, not `dualMountAuth`. 5/5 existing unit tests pass post-mount. The pre-existing `scheduler/wallet.scheduler.js` cron (registered in `server.js:196`) was maintaining wallet data alone; the HTTP API now exposes the same data to users.
