...
- ~~4 monolithic backend files~~ — **FULLY CLEARED 2026-05-23 across the W277-W278 session**. All 4 now <3000 LOC:
  ...
  - `services/importExportPro.service.js` 3,882 → **2,656** (-31.6%) — pure-data extract: `MODULE_REGISTRY` + `SYSTEM_TEMPLATES` (~1,170 LOC of field descriptors) moved to `services/importExport/{module-registry,system-templates}.js`. Zero risk (no method moved). Pass 2+ candidates (format-specific exporters) deferred.
