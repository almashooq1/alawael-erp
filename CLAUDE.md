...
## Pre-push prevention stack (5 gates, Cycle 11+12+13 — 2026-05-26/27/28)

`.husky/pre-push` runs 5 static drift checks BEFORE the heavy `quality:push` block. Each was extracted from a real silent-break that cost ~24h of CI red OR a recurring per-session cleanup tax. Combined runtime <15s; failure messages include the one-command fix. Order in the hook (independent, sequential):
...
- Gate 5: re-number the offending commit via `git commit --amend` (or interactive rebase) to the "suggested next free wave" the script prints. For coordinated multi-repo waves (rare), bypass with `CHECK_WAVE_SKIP=1 git push` and document the intent in the commit body. Env override: `CHECK_WAVE_OTHER_REPO_PATH=<absolute path>` for non-default sibling layouts.

**On-demand audit scripts (NOT in pre-push, ship 2026-05-28 alongside gate 5)**:

- `npm run check:dormant-modules` (`backend/scripts/check-dormant-modules.js`) — builds a token-index over every `.js` in `backend/` (~1s) and surfaces every `routes/<x>.routes.js` + `services/<x>.service.js` whose `baseNoExt` token appears in NO other file. v1 surfaced 35 candidates against an empty baseline; W340 cleanup waves can baseline + ratchet-DOWN per the W325c pattern. Models intentionally out of scope (mongoose-registered name often ≠ filename).
- `npm run check:memory-health` (`backend/scripts/check-memory-health.js`) — scans `~/.claude/projects/<slug>/memory/`. Three tiers: `OK / WARN / FAIL`. `FAIL` (exit 1) when MEMORY.md ≥ 400 lines OR ≥ 250 KB OR any single memory file ≥ 200 KB. `--strict` promotes `WARN` to `FAIL` for tighter cleanup waves. Env override: `CLAUDE_MEMORY_DIR=<path>` or `--dir=<path>`.

**Adding a 6th gate** (when a new silent-break class shows up 3+ times):
...
- `no-duplicate-model-registration-wave340.test.js` — W340 no duplicate `mongoose.model('X', schema)` registration in >1 source file (detects 3 patterns: direct `mongoose.model(...)` + per-connection `connection.model(...)` / `conn.model(...)` / `db.model(...)` + helper-wrapped `reg/getOrCreate/registerModel/...`; CONNECTION_REGISTRATION_RE added in AF-2 commit `abb24eac6`). Baseline ratchet pattern (cumulative as of 2026-05-26): **47 entries at peak** (post-AF-2 regex extension, was 52 pre-extension); **W341/W342/W343/W347 cleared 11** (seeders + HR services + REGISTRATION_ALLOWLIST for Referral/Task/AuditLog); **AF-1/AF-2/AF-3 + 4 ratchet waves cleared 12 more** (Permission+Role+TrafficAccident via empty-shim migration `98988fe75`+`a56c9feb0`; Vehicle+VehicleTrip+VehicleMaintenance+TransportRoute+Beneficiary+EmailLog via dormant-service lookup `b12fd9c8b`; BranchSetting+GlobalSetting+LifecyclePolicy via ALLOWLIST `6e5cb00b8`+`f08fe12a7`). **Current baseline: ~35 entries** (38 dual-real candidates pending per-entity work + ~5 still in KNOWN_DUPLICATE_REGISTRATIONS; subtract any newly-ALLOWLISTed). **ADR-021 = framework** with 4-pattern decision tree A/B/C/D; **ADR-022/023/024 = the three per-entity applications** (Pattern D rename for ApprovalRequest / ReportTemplate / WorkflowInstance). Tier 1 framework now COMPLETE — all 4 entries have a documented decision path (3 ADRs + AuditLog ALLOWLIST stopgap). PRs unblocked pending stakeholder Q1-Q4 sign-off per each ADR. **ADR-030** (2026-05-26) formalizes the wire-up-vs-delete question for the dormant modules surfaced via AF-2 (5000+ LOC in vehicles/, partial in communication/, permissions/).
...
Event contracts / DDD integration (W374+, **93 assertions / 13 suites** as of 2026-05-25 W408 — covers BOTH registries + producer-side behavioral verification + producer→bus→subscriber integration bridge + 5 new producer surfaces from the W400-W404 closure arc + W407 third-subscriber-file guard + W408 strict-envelope verification for LIVE-registry producers):

- `domain-event-contracts-wave382.test.js` — drift guard for `backend/events/contracts/domainEventContracts.js` (LIVE registry actually loaded by `startup/integrationBus.js`; W377 follow-up confirmed dddEventContracts.js is structurally orphaned). Combined structural + dead-contract guard (13 assertions). **W400-W404 sequence reduced KNOWN_DEAD_CONTRACTS 19 → 0** ✅: W394 modelEventBridge cleared most via post-save hooks; W400 wired `system.error.occurred` in errorHandler 5xx publish; W401 added `services/finance/budgetThresholdSweeper.js` (env-gated `ENABLE_BUDGET_THRESHOLD_SWEEPER=true`, daily 24h cadence); W402 added `services/hr/absenceDetectionSweeper.js` (env-gated `ENABLE_ABSENCE_DETECTION_SWEEPER=true`, scans yesterday's HRAttendanceRecord rows); W403 added `cachingService.js` `_emitCacheInvalidated()` hook on clear() + invalidateByPattern() (single-key delete deliberately silent to avoid TTL/LRU spam); W404 added 3 modelEventBridge mappings (Prescription / RiskSnapshot / PayrollPeriod) + new `predicate(doc)` mapping field for conditional emit (RiskSnapshot uses it to emit only on tierDelta='escalated' or first-snapshot high/critical). Adding ANY new dead contract now fails CI; baseline locked at zero.
- `subscriber-coverage-live-wave392.test.js` — LIVE-side subscriber-must-have-producer drift guard. **W400-W404 reduced KNOWN_LIVE_ORPHAN_SUBSCRIBERS 3 → 0** ✅ via the same surfaces (W401 closed `finance.budget.threshold_reached`, W402 closed `attendance.absence.detected`, W404 closed `medical.risk.alert_raised` via RiskSnapshot predicate-gated mapping). Subscriber side now matches producer side at zero.
- `budget-threshold-sweeper-wave401.test.js` (8 unit tests), `absence-detection-sweeper-wave402.test.js` (9), `cache-invalidation-hook-wave403.test.js` (7), `model-event-bridge-wave404.test.js` (14) — behavioral tests for each new producer surface. Together: 38 new assertions plus the W382 + W392 ratchet-down assertions.
...
**W340 drift-guard blind spot — CLOSED** (discovered + closed 2026-05-25 in AF-1 + AF-2): the original W340 regex matched `mongoose.model(...)` + helper-wrapped registrations only — `connection.model(...)` / `conn.model(...)` / `db.model(...)` were invisible. AF-1 deleted the worst offender (`workflow/workflow-engine.js` — 701 LOC dead code registering `WorkflowInstance` via `connection.model`). AF-2 extended the regex with `CONNECTION_REGISTRATION_RE = /\b(?:connection|conn|db)\.model\s*\(\s*['"]([^'"]+)['"]\s*,/g` and surfaced **9 hidden duplicates** (all baselined per ratchet-down pattern): `Beneficiary` + `EmailLog` + `Permission` + `Role` + `TrafficAccident` + `TransportRoute` + `Vehicle` + `VehicleMaintenance` + `VehicleTrip`. The anti-pattern in all 9: canonical schema at `models/<X>.js` + local registration in `<domain>/*-service.js` via `connection.model`. Each needs (a) `module.exports = require('../models/X')` re-export, (b) ALLOWLIST with per-connection-isolation justification, or (c) Pattern D rename per ADR-021 — pick per future ratchet wave. The blind spot is now CLOSED: any new `connection.model('X', ...)` registration of an already-canonical name will fail W340 in CI.
...
Proven on W325c — **58 phantoms → 0 across ~12 waves** (W324 / W326 / W327+W328 / W329 / W333 / W335 / W336 / W337-built-CapaItem / + REF_ALLOWLIST decisions for Company / ComplianceControl / DisabilityRehabilitation). The pattern works for ANY large pre-existing tech-debt sweep. Now also proven on W340 (52 → 47 → 35 baseline; W341/W342/W343/W347 cleared 11 entries in 4 follow-up waves; AF-1/AF-2/AF-3 cycle 2026-05-26 cleared 12 more via three new sub-patterns: **empty-shim** + **dormant-service** + **defensive-lookup-fallback**).
...
- **Markdown lint config (W399, `.markdownlint.json` at repo root)**: 5 rules intentionally disabled to match this codebase's docs style — `MD013` (line-length 80 — Arabic+English mixed docs with RTL tables exceed 80 cols routinely), `MD029` (ol-prefix sequential — MODULE_AUDIT numbers items 1-10 across section dividers to match original priority list), `MD033` (no-inline-html — occasional layout need), `MD041` (first-line H1 — `~/.claude/projects/.../memory/MEMORY.md` spec mandates no H1), `MD060` (table-pipe alignment — Arabic-content tables render correctly without column alignment). Kept: `MD024` set to `siblings_only` (duplicate H2s in different sections OK). **Adding a new docs warning**: if `npx markdownlint-cli2 <file>` flags it, first decide — is it a real readability issue (fix in file) or a project-wide style preference (add to `.markdownlint.json`)? Don't disable a rule globally to silence one occurrence — fix the occurrence instead.
