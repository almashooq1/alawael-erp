...
- **2 actionable stale TODOs in `routes/parent-portal-v1.routes.js`** (sweep 2026-05-21). Both have working backend infrastructure that the parent-portal route hasn't been connected to yet:
  ...
  - **Line 1131-1135** (`GET /messages/threads`): returns `[]` with `// TODO(phase-16): query MessageThread where participants include this guardian.` The MessageThread model + `routes/messages.routes.js` are live. Fix is one query: `MessageThread.find({ participants: userId }).sort({ updatedAt: -1 }).lean()` + contract-match the UI shape. Slightly invasive — need to know what each thread object should expose.
