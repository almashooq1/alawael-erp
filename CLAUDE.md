...
Architecture / module-dependency (W354+, **9 assertions / 1 suite** as of 2026-05-24 W354):

- `module-dependency-direction-wave354.test.js` — enforces `docs/architecture/MODULE_DEPENDENCY_RULES.md` doctrine on `backend/domains/`. Three checks: (1) tier-direction of declared `dependencies: [...]` in `BaseDomainModule` constructors respects the 8-tier ordering per ADR-025 (platform-core → beneficiary-360 → ... → quality-risk-governance); (2) DFS on declared graph asserts zero cycles; (3) cross-domain `require('../../X/{models,services,repositories,routes,validators}/...')` must go through the target's `index.js` facade, not internals. 9 tests across 4 describes + 2 sanity. Baseline snapshot: 8 tier-direction violations baselined (workflow→{core,episodes,timeline}; family→{sessions,goals,care-plans}; dashboards→quality; reports→quality), 0 cycles, 3 unique facade-bypass target modules (all in `domains/workflow/services/JourneyService.js`). Baseline-ratchet pattern (W325c/W340 lineage): new violations fail CI + stale baseline entries fail CI. **ADR-025 has 5 open stakeholder questions** (Q1 workflow placement, Q2 reports/dashboards→quality direction, Q3 research tier, Q4 hr split, Q5 legacy areas) that gate further baseline shrinkage. Static analysis only (no mongoose, no DB).
