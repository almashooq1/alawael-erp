...
- **5 dormant route files** identified 2026-05-20 by dead-route audit. Real Express routers (574-1500 LOC, 8-45 endpoints each), referenced ONLY from their own auto-generated `tests/unit/*.routes.test.js` — never `require()`-d by `app.js`, `routes/_registry.js`, or any `routes/registries/*.js`. Last human change to each was 3 weeks ago (only lint sweeps since). Files: `biometric-attendance.routes.js` (28 endpoints, ZKTeco system 37), `digital-wallet.routes.js` (15), `payment-gateway.routes.js` (8), `smart-assessment-engine.routes.js` (45 — likely superseded by W206 assessment-engine), `smart-insurance.routes.js` (21 — overlaps with `insurance.routes.js`+`insuranceClaims.routes.js`). **DO NOT auto-mount** — financial + clinical risk. The correct path is per-file: confirm it's superseded → move to `_archived/`, OR confirm it's genuinely-missing → wire mount + run security review. Treat the same as the W205/W205b cases that turned out to be real (mount), but verify before mounting.
