...
- `backend/tests/unit/documentWorkflow.engine.test.js` + `documentTemplates.engine.test.js` — 3 `it.skip` blocks remain (createWorkflow, createTemplate, duplicateTemplate). All 3 fail with `TypeError: X is not a constructor` because the engines capture `mongoose.model('X')` at module load time, before any test can intercept it. Fix path: `jest.mock` the model module pre-require, or refactor engines to lazy model lookup. (Previously a 4th `.skip` here cited `ReferenceError: escalatedTo`; verified 2026-05-19 that source is fine and re-enabled — passes now.)
