...
- 15 `describe.skip` blocks in `backend/tests/eStamp.test.js` + `eSignaturePdf.test.js` — three-step investigation 2026-05-19 narrowed the real blocker. (1) Original "safeMount fails silently" claim is **wrong** (safeMount logs at error level, routes are mounted). (2) Suspected 401 from missing JWT — **wrong**: route is `/public/verify` which is unauthenticated by design. (3) **Actual blocker**: response is **503 Service Unavailable** because the route handler throws when the underlying Mongoose models query an unavailable DB. Test env has `USE_MOCK_DB=true` + `DISABLE_REDIS=true` (jest.env.js), and `safeError` translates the thrown query to 503. Fix path: provide a real test DB (MongoMemoryServer is already wired via jest.globalSetup.js — verify whether the EStamp models register correctly under it) **OR** `jest.mock` the model modules before require. JWT helper does **not** help. Separately, `npx jest` doesn't auto-discover `backend/jest.config.js` from CWD on this machine — must pass `--config=jest.config.js` explicitly OR install jest into `backend/node_modules` (`devDependencies` declares it but it's not installed locally).
