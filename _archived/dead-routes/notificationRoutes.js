...
const { whatsappIntegration } = require('../services/whatsapp-integration.service');
const {
  sendWhatsAppText,
  sendWhatsAppImage,
 ...
 * إرسال إشعار موحد
 */
router.post('/send', authorize(['admin', 'system_admin']), async (req, res) => {
  try {
 ...
 * إرسال إشعار فوري
 */
router.post('/send-immediate', authorize(['admin', 'system_admin']), async (req, res) => {
  try {
 ...
 */
router.post('/send-bulk', authorize(['admin', 'system_admin']), async (req, res) => {
  try {
 ...
 * إرسال رسالة واتس آب
 */
router.post('/whatsapp/send', authorize(['admin', 'system_admin']), async (req, res) => {
  try {
    ...
    switch (type) {
      case 'image':
        result = await sendWhatsAppImage(phoneNumber, mediaUrl, message);
        break;
      case 'document':
        result = await sendWhatsAppDocument(phoneNumber, mediaUrl, message);
        break;
      default:
        result = await sendWhatsAppText(phoneNumber, message);
    }
