/**
 * W502 — close UNAUTH exposure on 20 route mounts (defense-in-depth).
 *
 ...
 *
 * Audit result (pre-W502): 21 such mounts. Catastrophic findings:
 *
...
// Slugs that MUST mount via dualMountAuth (W502 fix targets)
const W502_AUTHED_SLUGS = [
  'core',
  'workflow',
  'family',
  'behavior',
...
describe('W502 — mount-layer authentication on 20 previously-unauth routes', () => {
  describe.each(W502_AUTHED_SLUGS)('slug %s', slug => {
    test('uses dualMountAuth (not bare dualMount)', () => {
      ...
      // Strip line comments so historical "Pre-W502 had: dualMount(app, '...'" doc-comments don't false-positive.
      const code = src
        .split('\n')
