/**
 * beneficiary-lifecycle-beneficiary-side-effects-window-route-wave607.test.js
 *   — W607.
 *
 * Covers the W607 additive `?windowDays=N` time-bound on the
 * BENEFICIARY-level operational roll-up
 *   GET /api/v1/beneficiary-lifecycle/beneficiaries/:beneficiaryId/side-effects-summary
 * introduced in W599. Mirrors the branch endpoint W604: the window bounds
 * the scan to the beneficiary's transitions whose best-effort timestamp
 * (executedAt → requestedAt → createdAt → updatedAt) falls within the last
 * N days. Records WITHOUT any timestamp are kept (never silently dropped).
 * Omitted/invalid windowDays => whole journey, backward compatible with W599.
 *
...
const BEN = 'BEN-1';
const PATH = `/api/v1/beneficiary-lifecycle/beneficiaries/${BEN}/side-effects-summary`;

const DAY = 24 * 60 * 60 * 1000;
const daysAgo = n => new Date(Date.now() - n * DAY).toISOString();

// One beneficiary's journey: four executed transitions at different ages,
// plus one row with NO timestamp at all (must always be retained).
...
describe('W607 GET /beneficiaries/:beneficiaryId/side-effects-summary?windowDays=N', () => {
  test('no windowDays => whole journey (backward compatible with W599)', async () => {
    const { app } = makeApp();
    const r = await request(app).get(PATH);
    expect(r.status).toBe(200);
    const d = r.body.data;
    expect(d.beneficiaryId).toBe(BEN);
  ...
  test('empty windowDays string is treated as omitted (whole journey)', async () => {
    const { app } = makeApp();
