...
beforeAll(async () => {
  ...
  INSP.a = (await seedInsp(BRANCH_A, 'A')).insertedId;
  INSP.b = (await seedInsp(BRANCH_B, 'B')).insertedId;
});
...
describe('W1604 — HSE safety-incident branch isolation', () => {
  ...
  it('GET /inspections — restricted user lists only own-branch inspections', async () => {
    ...
    expect(r.status).toBe(200);
    const ids = r.body.data.map(d => String(d._id));
    expect(ids).toContain(String(INSP.a));
    expect(ids).not.toContain(String(INSP.b));
  });

  it('PUT /inspections/:id — restricted user 404s on a foreign-branch inspection', async () => {
    const r = await request(app).put(`${base}/inspections/${INSP.b}`).send({ area: 'x' });
    expect(r.status).toBe(404);
  });

  it('DELETE /inspections/:id — restricted user 404s on a foreign-branch inspection', async () => {
    mockUser.u = { _id: USER_A, id: String(USER_A), role: 'admin' };
    const r = await request(app).delete(`${base}/inspections/${INSP.b}`);
    expect(r.status).toBe(404);
  });
  ...
  it('static: branchFilter scoping + strip helpers wired', () => {
    ...
    expect(src).toMatch(/const stripInspection =/);
