...
const ctx = (rows, now) => ({
  models: { PayEquitySnapshot: model(rows) },
  now: now || new Date('2026-06-15'),
});
...
describe('W1197 pay-equity alert rule — evaluate predicate', () => {
  ...
  test('alerts on a reportable gap over the ceiling, NOT on a suppressed one', async () => {
    const reportable = snap({
      _id: 'sGap',
      equityScore: 95,
      genderGap: { reportable: true, medianGapPct: 25, direction: 'female' },
    });
    const suppressed = snap({
      _id: 'sSup',
      branchId: 'bB',
      equityScore: 95,
      nationalityGap: { reportable: false, medianGapPct: 40, direction: 'saudi' },
    });
  ...
  test('department-scoped snapshots are deduped independently of the branch-scoped one', async () => {
    const branchScope = snap({
      _id: 'b',
      branchId: 'bA',
      equityScore: 92,
      scope: { level: 'branch', department: null },
    });
    const deptScope = snap({
      _id: 'd',
      branchId: 'bA',
      equityScore: 40,
      scope: { level: 'department', department: 'PT' },
    });
