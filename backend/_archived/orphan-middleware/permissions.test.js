...
// Auto-generated unit test for middleware/permissions.js
...
let mw;
try { mw = require('../../middleware/permissions'); } catch (e) { mw = null; }

describe('middleware/permissions.js', () => {
  ...
  test('checkRole exists and is callable', async () => {
    if (!mw || typeof mw.checkRole !== 'function') return;
    const req = mockReq();
    const res = mockRes();
    const next = jest.fn();
    try {
      const result = mw.checkRole(req, res, next);
      if (result && typeof result === 'function') {
        // Factory pattern — invoke returned middleware
        try { await result(req, res, next); } catch (e) { /* expected */ }
      } else if (result && typeof result.then === 'function') {
