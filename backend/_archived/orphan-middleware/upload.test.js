...
// Auto-generated unit test for middleware/upload.js
...
let mw;
try { mw = require('../../middleware/upload'); } catch (e) { mw = null; }
...
describe('middleware/upload.js', () => {
  ...
  test('uploadsDir exists and is callable', async () => {
    if (!mw || typeof mw.uploadsDir !== 'function') return;
    const req = mockReq();
    const res = mockRes();
    const next = jest.fn();
    try {
      const result = mw.uploadsDir(req, res, next);
      if (result && typeof result === 'function') {
        // Factory pattern — invoke returned middleware
        try { await result(req, res, next); } catch (e) { /* expected */ }
      } else if (result && typeof result.then === 'function') {
