    ...
    relatedDocuments: [
      {
        ...
        relationship: {
          type: String,
          enum: ['parent', 'child', 'reference', 'attachment', 'version'],
        },
      },
    ],
...
// Indexes
// documentNumber already has unique:true → implicit index
...
class ArchiveService {
  ...
  async uploadDocument(file, metadata, options = {}) {
    ...
    const retentionDays =
      archiveConfig.retention.categories[metadata.category.main] || archiveConfig.retention.default;
  ...
  validateFile(file) {
    ...
    if (file.size > archiveConfig.storage.maxFileSize) {
      throw new Error(
        `File size exceeds maximum allowed (${archiveConfig.storage.maxFileSize / 1024 / 1024}MB)`
      );
    }
  ...
  async searchDocuments(query, options = {}) {
    ...
    const [documents, total] = await Promise.all([
      this.Document.find(filter).sort({ 'audit.createdAt': -1 }).skip(skip).limit(limit),
  ...
  async processRetention() {
    ...
    for (const doc of expired) {
      ...
      logger.info(`Document ${doc.documentNumber} marked as expired`);
    }
  ...
  async getStatistics(tenantId) {
    ...
    const [total, byCategory, byStatus, byClassification, totalSize] = await Promise.all([
      this.Document.countDocuments(filter),
    ...
    return {
      ...
      byClassification: byClassification.reduce(
