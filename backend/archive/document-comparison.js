      ...
      structuralChanges: [
        {
          type: {
            type: String,
            enum: ['section_added', 'section_removed', 'section_moved', 'order_changed'],
          },
          section: String,
          details: String,
        },
...
class DocumentComparisonService {
  ...
  async compareDocuments(originalDoc, modifiedDoc, options = {}) {
    ...
    try {
      ...
      comparison.error = 'حدث خطأ داخلي';
      await comparison.save();
    }
    ...
    for (const item of diff) {
      if (item.type !== 'equal') {
        changes.push({
          ...
          significance:
            item.value.length > 100 ? 'major' : item.value.length > 20 ? 'moderate' : 'minor',
        });
      }
