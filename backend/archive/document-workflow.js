    ...
    stages: [
      {
        ...
        type: {
          type: String,
          enum: ['start', 'review', 'approval', 'parallel', 'end'],
          required: true,
        },
        ...
        actions: [
          {
            ...
            type: {
              type: String,
              enum: ['approve', 'reject', 'delegate', 'comment', 'attach', 'custom'],
            },
            nextStage: String,
...
const WorkflowInstanceSchema = new mongoose.Schema(
  {
    ...
    status: {
      type: String,
      enum: ['running', 'completed', 'cancelled', 'paused'],
      default: 'running',
    },
...
class DocumentWorkflowService {
        ...
        stages: [
          {
            ...
            order: 1,
            type: 'start',
            actions: [{ name: 'إرسال للمراجعة', type: 'approve', nextStage: 'review' }],
          },
          {
            name: 'review',
            order: 2,
            type: 'review',
            assignees: { type: 'role', value: ['reviewer'] },
            actions: [
          ...
          },
          {
            ...
            order: 3,
            type: 'approval',
            assignees: { type: 'role', value: ['approver'] },
            actions: [
      ...
      },
      {
        ...
        stages: [
          { name: 'draft', order: 1, type: 'start' },
          {
            ...
            order: 2,
            type: 'review',
            assignees: { type: 'department', value: ['legal'] },
          ...
          },
          {
            ...
            order: 3,
            type: 'review',
            assignees: { type: 'department', value: ['finance'] },
          ...
          },
          {
            ...
            order: 4,
            type: 'approval',
            assignees: { type: 'role', value: ['manager', 'ceo'] },
      ...
      },
      {
        ...
        stages: [
          { name: 'submitted', order: 1, type: 'start' },
          {
            ...
            order: 2,
            type: 'review',
            assignees: { type: 'role', value: ['accountant'] },
            actions: [
  ...
  async startWorkflow(workflowName, documentId, options = {}) {
    const definition = await this.WorkflowDefinition.findOne({
      name: workflowName,
      isActive: true,
    });
    if (!definition) {
  ...
  async sendNotifications(instance, stage, event) {
    ...
    logger.info(
      `Notification: ${event} for stage ${stage.name} to ${instance.currentAssignees.join(', ')}`
    );
  }
  ...
  async getDocumentWorkflows(documentId) {
    return this.WorkflowInstance.find({ documentId }).sort({ startedAt: -1 });
  }
