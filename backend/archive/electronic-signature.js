...
const SignatureRequestSchema = new mongoose.Schema(
  {
    ...
    requestId: {
      type: String,
      unique: true,
      default: () => `SR-${Date.now()}-${crypto.randomBytes(4).toString('hex')}`,
    },
...
class ElectronicSignatureService {
  ...
  async notifySigner(request, signer) {
    ...
    logger.info(`Notifying signer: ${signer.email} for request ${request.requestId}`);
  }
  ...
  async verifySignature(signatureId) {
    ...
    const now = new Date();
    const isValid =
      signature.status === 'active' && now >= signature.validFrom && now <= signature.validUntil;
  ...
  async getDocumentSignatures(documentId) {
    return this.DigitalSignature.find({ documentId, status: 'active' }).sort({ createdAt: -1 });
  }
