...
class OCRService {
  ...
  async processQueue() {
    ...
    while (this.jobQueue.length > 0) {
      ...
      try {
        ...
        logger.error(`OCR Job ${job.id} failed:`, error.message);
        job.status = 'failed';
  ...
  async processWithTesseract(filePath, options) {
    ...
    return new Promise((resolve, reject) => {
      ...
      const args = [filePath, outputPath, '-l', langParam, '--oem', '3', '--psm', '3'];
  ...
  getStatistics() {
    ...
    return {
      ...
      pending,
      processing,
      successRate: this.jobQueue.length > 0 ? (completed / (completed + failed)) * 100 : 0,
    };
  }
}
...
function ocrMiddleware(options = {}) {
  return async (req, res, next) => {
    ...
    try {
      ...
      logger.error('OCR middleware error:', error.message);
      next();
    }
  };
}
