...
class BranchManagementService extends EventEmitter {
  ...
  async initialize(connection) {
    ...
    this.BranchPerformanceLog = connection.model(
      'BranchPerformanceLog',
      BranchPerformanceLogSchema
    );

    logger.info('✅ Branch Management Service initialized');
  }
  ...
  async getBranch(branchId) {
    return this.Branch.findById(branchId).populate('parentBranch');
  }
  ...
  async updateBranchPerformanceSummary(branchId) {
    ...
    const totalTransactions = logs.reduce(
      (sum, log) => sum + (log.metrics.transactionsCount || 0),
      0
    );
  ...
  async getPerformanceReport(branchId, startDate, endDate) {
    ...
    const summary = {
      ...
      averageWaitTime: this.calculateAverage(
        logs.map(l => l.metrics.averageWaitTime).filter(Boolean)
      ),
      customerComplaints: logs.reduce((sum, log) => sum + (log.metrics.customerComplaints || 0), 0),
      customerCompliments: logs.reduce(
        (sum, log) => sum + (log.metrics.customerCompliments || 0),
        0
      ),
    };
  ...
  async getStatistics(tenantId) {
    ...
    const [total, byType, byStatus, byRegion, activeStaff] = await Promise.all([
      this.Branch.countDocuments(filter),
      this.Branch.aggregate([{ $match: filter }, { $group: { _id: '$type', count: { $sum: 1 } } }]),
    ...
    return {
      ...
      byRegion: byRegion.reduce(
        (acc, item) => ({ ...acc, [item._id || 'غير محدد']: item.count }),
        {}
      ),
