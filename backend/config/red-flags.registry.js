...
const RED_FLAGS = Object.freeze([
  ...
  {
    id: 'operational.therapist.license.expired',
    nameEn: 'Therapist SCFHS license — EXPIRED (blocks new session assignment)',
    nameAr: 'ترخيص المعالج (الهيئة) — منتهي الصلاحية (يمنع تخصيص جلسات جديدة)',
    domain: 'operational',
    severity: 'critical',
    category: 'absence',
    trigger: {
      source: {
        service: 'hrCredentialService',
        method: 'expiredLicensesForBeneficiary',
        path: 'count',
      },
      condition: {
        operator: '>=',
    ...
    response: {
      blocking: true,
      notify: [ROLES.HR_MANAGER, ROLES.CLINICAL_DIRECTOR, ROLES.COMPLIANCE_OFFICER],
      escalateTo: ROLES.CLINICAL_DIRECTOR,
      taskTemplate: 'hr.license-renewal-urgent',
    },
    slaHours: 2,
    cooldownHours: 0,
    autoResolve: { type: 'condition_cleared', afterHours: null },
    owner: ROLES.HR_MANAGER,
    compliance: ['SCFHS', 'CBAHI 8.7'],
    kpiLinks: ['hr.credentials.compliance.pct'],
  },
  {
    id: 'operational.therapist.mandatory_cert.expired',
    nameEn: 'Therapist mandatory certification (CPR/First Aid) — expired',
    nameAr: 'شهادة إلزامية للمعالج (CPR/الإسعافات الأولية) — منتهية الصلاحية',
    domain: 'operational',
    severity: 'critical',
    category: 'absence',
    trigger: {
      source: {
        service: 'hrCredentialService',
        method: 'expiredMandatoryCertsForBeneficiary',
        path: 'count',
      },
      condition: {
        operator: '>=',
    ...
    response: {
      blocking: true,
      notify: [ROLES.HR_MANAGER, ROLES.CLINICAL_DIRECTOR, ROLES.QUALITY_COORDINATOR],
      escalateTo: ROLES.CLINICAL_DIRECTOR,
      taskTemplate: 'hr.mandatory-cert-renewal',
    },
    slaHours: 4,
    cooldownHours: 0,
    autoResolve: { type: 'condition_cleared', afterHours: null },
    owner: ROLES.HR_MANAGER,
    compliance: ['CBAHI 8.7', 'SCFHS'],
    kpiLinks: ['hr.credentials.compliance.pct'],
  },
  {
    id: 'operational.employment_contract.expiring.45d',
    nameEn: 'Therapist employment contract — expires in 45 days or less',
    nameAr: 'عقد عمل المعالج — ينتهي خلال 45 يوم أو أقل',
    domain: 'operational',
    severity: 'warning',
    category: 'absence',
    trigger: {
      source: {
        service: 'hrCredentialService',
        method: 'contractsExpiringForBeneficiary',
        path: 'count',
      },
      condition: {
        operator: '>=',
    ...
    response: {
      blocking: false,
      notify: [ROLES.HR_MANAGER, ROLES.CLINICAL_DIRECTOR],
      escalateTo: ROLES.HR_MANAGER,
      taskTemplate: 'hr.contract-renewal-reminder',
    },
    slaHours: 168,
    cooldownHours: 720,
    autoResolve: { type: 'condition_cleared', afterHours: null },
  ...
  {
    id: 'operational.leave_balance.overflow_45d',
    nameEn: 'Therapist leave balance — carryable annual exceeds 45 days',
    nameAr: 'رصيد إجازة المعالج — المتراكم يتجاوز 45 يوماً',
    domain: 'operational',
    severity: 'warning',
    category: 'threshold',
    trigger: {
      source: {
        service: 'hrWorkforceService',
        method: 'leaveBalanceOverflowForBeneficiary',
        path: 'count',
      },
      condition: {
        operator: '>=',
    ...
    response: {
      ...
      notify: [ROLES.HR_MANAGER, ROLES.THERAPY_SUPERVISOR],
      escalateTo: ROLES.HR_MANAGER,
      taskTemplate: 'hr.leave-plan-outreach',
    },
    slaHours: 336,
    cooldownHours: 720,
    autoResolve: { type: 'condition_cleared', afterHours: null },
    owner: ROLES.HR_MANAGER,
    compliance: ['Saudi Labor Law Art. 111'],
    kpiLinks: [],
  },
  {
    id: 'operational.performance_review.overdue_365d',
    nameEn: 'Therapist annual performance review — overdue by 365 days',
    nameAr: 'تقييم الأداء السنوي للمعالج — متأخر أكثر من 365 يوم',
    domain: 'operational',
    severity: 'info',
    category: 'absence',
    trigger: {
      source: {
        service: 'hrWorkforceService',
        method: 'overdueReviewsForBeneficiary',
        path: 'count',
      },
      condition: {
        operator: '>=',
    ...
    response: {
      ...
      notify: [ROLES.HR_MANAGER, ROLES.THERAPY_SUPERVISOR],
      escalateTo: ROLES.HR_MANAGER,
      taskTemplate: 'hr.performance-review-schedule',
    },
    slaHours: 720,
    cooldownHours: 720,
    autoResolve: { type: 'condition_cleared', afterHours: null },
  ...
  {
    id: 'operational.probation_review.overdue_7d',
    nameEn: 'Therapist probation review — not recorded within 7 days of probation end',
    nameAr: 'تقييم فترة تجربة المعالج — غير موثّق خلال 7 أيام من انتهاء الفترة',
    domain: 'operational',
    severity: 'warning',
    category: 'absence',
    trigger: {
      source: {
        service: 'hrWorkforceService',
        method: 'overdueProbationReviewsForBeneficiary',
        path: 'count',
      },
      condition: {
        operator: '>=',
    ...
    response: {
      ...
      notify: [ROLES.HR_MANAGER, ROLES.CLINICAL_DIRECTOR],
      escalateTo: ROLES.HR_MANAGER,
      taskTemplate: 'hr.probation-review-urgent',
    },
    slaHours: 168,
    cooldownHours: 336,
    autoResolve: { type: 'condition_cleared', afterHours: null },
    owner: ROLES.HR_MANAGER,
    compliance: ['Saudi Labor Law Art. 53'],
    kpiLinks: [],
  },
  {
    id: 'operational.shift.unassigned_therapist',
    nameEn: 'Therapist treating beneficiary is not on any active shift template',
    nameAr: 'المعالج يقدم جلسات دون إدراج في أي وردية عمل نشطة',
    domain: 'operational',
    severity: 'info',
    category: 'absence',
    trigger: {
      source: {
        service: 'hrWorkforceService',
        method: 'therapistsNotOnShiftForBeneficiary',
        path: 'count',
      },
      condition: {
        operator: '>=',
    ...
    response: {
      blocking: false,
      notify: [ROLES.HR_MANAGER, ROLES.MANAGER],
      escalateTo: ROLES.HR_MANAGER,
      taskTemplate: 'hr.shift-roster-reconcile',
    },
    slaHours: 720,
    cooldownHours: 720,
    autoResolve: { type: 'condition_cleared', afterHours: null },
