...
router.post('/', async (req, res) => {
  try {
    ...
    const recommendations = recommendationsEngine.generateRecommendations(tenantId, userId, {
      limit,
      cacheMaxAge,
    });
...
router.post('/tenant/personalize', async (req, res) => {
  try {
    ...
    const personalized = recommendationsEngine.personalizeForTenant(tenantId, users, options);
...
router.get('/:userId/history', async (req, res) => {
  try {
    ...
    const history = recommendationsEngine.getRecommendationHistory(tenantId, req.params.userId, {
  ...
  try {
    ...
    if (!userId || !recommendationId || !rating) {
      return res.status(400).json({
        success: false,
        message:
          'معرف المستخدم والتوصية والتقييم مطلوبة | User ID, recommendation ID, and rating required',
      });
    }

    const feedback = recommendationsEngine.recordFeedback(tenantId, userId, recommendationId, {
      rating,
      helpful,
      comment,
      action,
