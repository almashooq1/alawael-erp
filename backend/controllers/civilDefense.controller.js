...
class CivilDefenseController {
  ...
  async getCertificatesForFacility(req, res) {
    try {
      ...
      const certificates = await SafetyCertificate.find({ facilityId }).sort({
        submissionDate: -1,
      });
  ...
  async resolveViolation(req, res) {
    try {
      const { facilityId } = req.params;
      const { violationId, _actions, completionDate } = req.body;
    ...
    try {
      ...
      const report = {
        ...
        summary: {
          ...
          requiresImprovement:
            compliance.compliancePercentage < 90 && compliance.compliancePercentage >= 70,
