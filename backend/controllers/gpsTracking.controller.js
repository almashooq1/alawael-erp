...
class GPSTrackingController {
  ...
  static async getRouteMap(req, res) {
    try {
      ...
      logger.error('خطأ في جلب المسار:', error);
      res.status(500).json({
        success: false,
        message: 'حدث خطأ داخلي',
      });
  ...
  static async acknowledgeAlert(req, res) {
    try {
      ...
      logger.error('خطأ في تأكيد التنبيه:', error);
      res.status(500).json({
        success: false,
        message: 'حدث خطأ داخلي',
      });
    ...
    try {
      ...
      if (format === 'csv') {
        ...
        const csvData = locations
          .map(
            loc =>
              `${loc.timestamp},${loc.lat},${loc.lon},${loc.speed},${loc.heading},${loc.accuracy}`
          )
          .join('\n');
      ...
      logger.error('خطأ في تصدير البيانات:', error);
      res.status(500).json({
        success: false,
        message: 'حدث خطأ داخلي',
      });
