  ...
  try {
    ...
    res.status(200).json({
      ...
      data: {
        backupCodes,
        message:
          'Save these backup codes in a safe place. You can use them to regain access if you lose your authenticator.',
      },
    });
  } catch (error) {
    res.status(500).json({
...
exports.verifyAndEnableEmailOTP = async (req, res) => {
  try {
    ...
    const otpLog = await OTPLog.findOne({ userId, method: 'email' }).sort({ createdAt: -1 });
...
exports.verifyAndEnableSMSOTP = async (req, res) => {
  try {
    ...
    // Find the most recent OTP log
    const otpLog = await OTPLog.findOne({ userId, method: 'sms' }).sort({ createdAt: -1 });
