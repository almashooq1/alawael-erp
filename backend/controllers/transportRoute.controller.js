...
class TransportRouteController {
  ...
  static async getAllRoutes(req, res) {
    try {
      ...
      if (search) {
        query.$or = [
          { routeName: { $regex: escapeRegex(search), $options: 'i' } },
          { routeCode: { $regex: escapeRegex(search), $options: 'i' } },
        ];
      }
  ...
  static async updateRoute(req, res) {
    try {
      const { name, description, stops, driver, vehicle, schedule, status, notes } = req.body;
      const route = await TransportRoute.findByIdAndUpdate(
        req.params.id,
        { name, description, stops, driver, vehicle, schedule, status, notes },
        {
          new: true,
          runValidators: true,
        }
