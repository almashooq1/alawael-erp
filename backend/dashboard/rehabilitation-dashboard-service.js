...
class RehabilitationDashboardService extends EventEmitter {
  ...
  async initialize(connection, services = {}) {
    ...
    console.log('✅ Rehabilitation Dashboard Service initialized');
  }
  ...
  async acknowledgeAlert(alertId) {
    if (!this.Alert) return null;
    return this.Alert.findOneAndUpdate(
      { alertId },
      { status: 'acknowledged' },
      { new: true }
    );
  ...
  async getKPIs(centerId) {
    return [
      { key: 'attendance_rate', label: 'نسبة الحضور', value: 92, target: 90, unit: '%', trend: 'up' },
      { key: 'satisfaction_rate', label: 'نسبة الرضا', value: 87, target: 85, unit: '%', trend: 'up' },
      { key: 'active_beneficiaries', label: 'المستفيدون النشطون', value: 128, unit: '', trend: 'up' },
      { key: 'sessions_completed', label: 'الجلسات المكتملة', value: 350, unit: '', trend: 'up' },
      { key: 'staff_attendance', label: 'حضور الموظفين', value: 93, target: 95, unit: '%', trend: 'stable' },
      { key: 'transport_efficiency', label: 'كفاءة النقل', value: 94, target: 90, unit: '%', trend: 'up' },
    ];
  }
