...
const ReviewSchema = new mongoose.Schema(
  {
    ...
    status: {
      type: String,
      enum: Object.keys(collaborationConfig.reviewStatuses),
      default: 'pending',
    },
...
class DocumentCollaborationService extends EventEmitter {
  ...
  async addComment(documentId, content, options = {}) {
    ...
    if (options.parentId) {
      await this.Comment.findByIdAndUpdate(options.parentId, {
        $push: { replies: { $each: [comment._id], $slice: -500 } },
      });
    }
  ...
  async replyToComment(commentId, content, options = {}) {
    return this.addComment((await this.Comment.findById(commentId)).documentId, content, {
      ...options,
      parentId: commentId,
      type: 'general',
    });
