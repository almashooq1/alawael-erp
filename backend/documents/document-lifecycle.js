...
class DocumentLifecycleService extends EventEmitter {
  ...
  async initialize(connection) {
    ...
    logger.info('✅ Document Lifecycle Service initialized');
  }
  ...
  async createDocument(data) {
    ...
    const retentionPolicy =
      lifecycleConfig.retention[data.retentionPolicy] || lifecycleConfig.retention.default;
  ...
  async getVersions(documentId) {
    return this.DocumentVersion.find({ documentId }).sort({ version: -1 });
  }
  ...
  async getStatistics(tenantId) {
    ...
    const [total, byStatus, byCategory, recent] = await Promise.all([
      ...
      this.Document.find(filter).sort({ createdAt: -1 }).limit(5),
    ]);
