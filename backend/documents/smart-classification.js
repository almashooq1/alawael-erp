...
class SmartClassificationService {
  ...
  async initialize(connection) {
    ...
    this.ClassificationResult = connection.model(
    ...
    logger.info('✅ Smart Classification Service initialized');
  }
  ...
  extractEntities(content) {
    ...
    const amountPatterns = [/(\d+(?:,\d{3})*(?:\.\d{2})?)\s*(ريال|ر\.س|SAR|دولار|USD|درهم|AED)/gi];
