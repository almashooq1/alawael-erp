...
class SaudiHRService {
  ...
  async calculatePayroll(employeeId, month, year) {
    ...
    const workedDays = attendance.filter(
      a => a.status === 'present' || a.status === 'late' || a.status === 'early_departure'
    ).length;
    ...
    const totalEarnings =
      basicSalary + housingAllowance + transportAllowance + otherAllowances + overtime;
  ...
  async processMonthlyPayroll(month, year, departmentId = null) {
    ...
    for (const employee of employees) {
      try {
        ...
        results.push({ employee: employee._id, error: 'حدث خطأ داخلي', success: false });
      }
    }
