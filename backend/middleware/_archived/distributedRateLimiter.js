...
class DistributedRateLimiter {
  ...
  async checkLimit(userId, category = 'global', limit = null, windowSize = null) {
    ...
    try {
      ...
      logger.error('[RateLimiter] Redis error:', { error: error.message });
  ...
  async getUserQuota(userId) {
    ...
    if (this.redis) {
      try {
        ...
        logger.error('[RateLimiter] Error getting quota:', { error: error.message });
      }
    }
  ...
  async setUserQuota(userId, limit, windowSize = null) {
    ...
    if (this.redis) {
      try {
        ...
        logger.error('[RateLimiter] Error setting quota:', { error: error.message });
      }
    }
  }
  ...
  async resetLimit(userId, category = 'global') {
    ...
    if (this.redis) {
      try {
        ...
        logger.error('[RateLimiter] Error resetting limit:', { error: error.message });
      }
    }
