  ...
  async invalidate(pattern) {
    ...
    if (this.redisEnabled) {
      try {
        ...
        logger.error('[Cache] Redis invalidate error:', error.message);
      }
    }
  ...
  async processWarmingQueue() {
    ...
    while (this.warmingQueue.length > 0) {
      ...
      try {
        ...
        logger.error(`[Cache] Warming failed for ${key}:`, error.message);
      }
    ...
    logger.info(`[Cache] Warmed ${warmed} entries in ${duration}ms`);
  }
