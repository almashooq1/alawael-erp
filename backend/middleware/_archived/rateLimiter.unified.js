...
try {
  ...
  if (redisModule && RedisStore) {
    redisAvailable = true;
    logger.info('Redis rate limiter modules loaded');
  }
} catch (e) {
...
const initRedis = async () => {
  if (!redisModule || !RedisStore) {
    logger.warn('Redis module not available, using memory store');
    return false;
  }
  try {
...
const wsRateLimiter = () => {
  ...
  return (socket, next) => {
    ...
    // التحقق من الحد
    const ipConnections = Array.from(connections.entries()).filter(([key]) =>
      key.startsWith(ip)
    ).length;
