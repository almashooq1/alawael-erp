    ...
    try {
      ...
      switch (strategy) {
        case 'any':
          ...
          hasAccess = permissions.some(perm => rbacSystem.hasPermission(userId, perm, context));
          break;

        case 'weighted': {
          // User needs majority of permissions
          const weight =
            permissions.filter(perm => rbacSystem.hasPermission(userId, perm, context)).length /
            permissions.length;
      ...
      logger.error('RBAC Authorization Error:', { error: _error.message, stack: _error.stack });
      res.status(500).json({
        success: false,
...
function rbacAuthorizeRole(requiredRoles) {
  return (req, res, next) => {
    try {
      ...
      logger.error('RBAC Role Authorization Error:', { error: error.message, stack: error.stack });
      res.status(500).json({
        success: false,
        message: 'Authorization check failed',
        error: error.message,
 ...
 */
function rbacRateLimit(_options = {}) {
  return (req, res, next) => {
    try {
      const userId = req.user?.id || req.headers['x-user-id'];
      ...
      const _limit = limits[userRole?.id] || limits.user;
      ...
      logger.error('Rate Limit Error:', _error);
      next();
    }
  };
}
