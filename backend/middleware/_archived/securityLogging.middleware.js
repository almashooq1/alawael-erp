...
class SecurityLogger {
  ...
  logToConsole(event) {
    ...
    winstonLogger.info(`${color}[SECURITY] ${event.severity} - ${event.eventType}${COLORS.reset}`);
    winstonLogger.info(`  User: ${event.username || event.userId} | IP: ${event.ip}`);
    winstonLogger.info(`  Action: ${event.action} on ${event.resource}`);
    winstonLogger.info(`  Details: ${JSON.stringify(event.details)}`);
  }
  ...
  flushToDisk() {
    ...
    const logData = this.eventQueue.map(event => JSON.stringify(event)).join('\n') + '\n';

    fs.appendFileSync(filename, logData, 'utf8');
    winstonLogger.info(
      `${COLORS.green}[LOG] Flushed ${this.eventQueue.length} events to disk${COLORS.reset}`
    );
  ...
  triggerAlert(event) {
    ...
    if (event.severity === 'CRITICAL') {
      ...
      winstonLogger.info(`${COLORS.red}🚨 CRITICAL SECURITY EVENT - ALERT${COLORS.reset}`);

      // @todo [P2] Integrate with alerting service (Slack, PagerDuty, SIEM)
  ...
  cleanupOldLogs(days = 30) {
    ...
    winstonLogger.info(`${COLORS.green}Deleted ${deletedCount} old security logs${COLORS.reset}`);
  }
...
const isSecurityRelevantRequest = req => {
  const sensitiveRoutes = ['/auth', '/admin', '/users', '/roles', '/permissions', '/export'];
 ...
 */
const logAuthAttempt = (success, userId, username, ip, userAgentStr = 'unknown') => {
  logger.logEvent(success ? EVENT_TYPES.LOGIN_SUCCESS : EVENT_TYPES.LOGIN_FAILED, {
    userId,
    username,
    ip,
