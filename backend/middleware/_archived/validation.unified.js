...
const passwordRules = (minLength = 8) => [
  body('password')
    ...
    .withMessage(`كلمة المرور يجب أن تكون ${minLength} أحرف على الأقل`)
    .matches(/\d/)
...
const phoneRules = () => [
  body('phone')
    ...
    .matches(/^(\+966|0)?5\d{8}$/)
    .withMessage('رقم هاتف سعودي غير صالح'),
];
...
const nationalIdRules = () => [
  body('nationalId')
    ...
    .matches(/^[12]\d{9}$/)
    .withMessage('رقم هوية سعودي غير صالح'),
];
...
const idRules = (paramName = 'id') => [
  param(paramName).notEmpty().withMessage('المعرف مطلوب').isMongoId().withMessage('معرف غير صالح'),
];
...
const paginationRules = () => [
  query('page').optional().isInt({ min: 1 }).withMessage('رقم الصفحة يجب أن يكون رقم صحيح موجب'),
  query('limit')
    .optional()
    .isInt({ min: 1, max: 100 })
...
const updateUserRules = () => [
  param('id').notEmpty().withMessage('المعرف مطلوب').isMongoId().withMessage('معرف غير صالح'),
  body('name')
    .optional()
    .trim()
    .isLength({ min: 2, max: 100 })
    .withMessage('الاسم يجب أن يكون بين 2 و 100 حرف'),
  body('email').optional().trim().isEmail().withMessage('بريد إلكتروني غير صالح'),
  body('phone')
    .optional()
    .matches(/^(\+966|0)?5\d{8}$/)
    .withMessage('رقم هاتف سعودي غير صالح'),
...
const employeeRules = () => [
  ...
  body('salary').optional().isFloat({ min: 0 }).withMessage('الراتب يجب أن يكون رقم موجب'),
  body('joinDate').optional().isISO8601().withMessage('تاريخ غير صالح'),
];
...
const beneficiaryRules = () => [
  ...
  body('gender')
    ...
    .isIn(['male', 'female'])
    .withMessage('جنس غير صالح'),
  body('disabilityType').optional().trim(),
];
 ...
 */
const invoiceRules = () => [
  body('invoiceNumber').trim().notEmpty().withMessage('رقم الفاتورة مطلوب'),
  body('amount')
    .notEmpty()
    .withMessage('المبلغ مطلوب')
    .isFloat({ min: 0.01 })
