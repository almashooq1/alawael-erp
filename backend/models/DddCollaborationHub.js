...
const DDDCollabMessage = mongoose.models.DDDCollabMessage || mongoose.model('DDDCollabMessage', dddMessageSchema);
...
async function sendMessage(channelId, senderId, content, options = {}) {
  ...
  const message = await DDDCollabMessage.create({
    messageId,
    channelId,
    sender: senderId,
    content,
...
async function getChannelMessages(channelId, options = {}) {
  ...
  return DDDCollabMessage.find(query).sort({ createdAt: -1 }).limit(limit).lean();
}

async function markAsRead(channelId, userId) {
  await DDDCollabMessage.updateMany(
...
async function addReaction(messageId, userId, emoji) {
  const msg = await DDDCollabMessage.findOne({ messageId });
...
async function searchMessages(query, options = {}) {
  ...
  return DDDCollabMessage.find(filter)
    .sort({ createdAt: -1 })
...
async function getUnreadCount(userId) {
  ...
  for (const ch of channels) {
    ...
    const unread = await DDDCollabMessage.countDocuments({
...
async function getCollaborationDashboard() {
  const [channelCount, messageCount, onlineCount] = await Promise.all([
    ...
    DDDCollabMessage.countDocuments({ isDeleted: false }),
