...
DevelopmentPlanSchema.methods.calculateProgress = function () {
  ...
  if (totalGoals > 0) {
    const goalsCompletion =
      this.developmentGoals.reduce((sum, goal) => sum + (goal.completionPercentage || 0), 0) /
      totalGoals;
  ...
  if (totalTrainings > 0) {
    const trainingsCompletion =
      (this.plannedTrainings.filter(t => t.status === 'completed').length / totalTrainings) * 100;
    totalCompletion += trainingsCompletion * 0.35;
    totalItems += 0.35;
  }

  if (totalResponsibilities > 0) {
    const responsibilitiesCompletion =
      this.expandedResponsibilities.reduce((sum, r) => sum + (r.completionPercentage || 0), 0) /
