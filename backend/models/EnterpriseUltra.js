...
const CourtHearingSchema = new Schema(
  {
    ...
    hearingType: {
      type: String,
      enum: ['initial', 'discovery', 'trial', 'appeal', 'mediation', 'settlement', 'procedural'],
      required: true,
    },
...
const PowerOfAttorneySchema = new Schema(
  {
    ...
    poaType: {
      type: String,
      enum: ['general', 'special', 'limited', 'durable', 'financial', 'healthcare', 'litigation'],
      required: true,
    },
    ...
    status: {
      type: String,
      enum: ['active', 'expired', 'revoked', 'pending_notarization', 'draft'],
      default: 'draft',
    },
    issueDate: { type: Date },
...
const LegalOpinionSchema = new Schema(
  {
    ...
    status: {
      type: String,
      enum: ['requested', 'in_review', 'draft_ready', 'approved', 'delivered', 'archived'],
      default: 'requested',
    },
...
const BoardMeetingSchema = new Schema(
  {
    ...
    meetingType: {
      type: String,
      enum: ['regular', 'extraordinary', 'annual_general', 'special', 'committee'],
      required: true,
    },
    status: {
      type: String,
      enum: ['draft', 'scheduled', 'in_session', 'adjourned', 'completed', 'cancelled'],
      default: 'draft',
    },
    ...
    agenda: [
      {
        ...
        status: {
          type: String,
          enum: ['pending', 'discussed', 'deferred', 'approved', 'rejected'],
        },
      },
    ],
    attendees: [
    ...
    decisions: [
      {
        ...
        votesAgainst: Number,
        abstentions: Number,
        result: { type: String, enum: ['approved', 'rejected', 'deferred'] },
      },
    ],
    ...
    members: [
      {
        ...
        name: String,
        role: { type: String, enum: ['chairman', 'member', 'secretary', 'advisor'] },
        joinDate: Date,
        termEnd: Date,
        status: { type: String, enum: ['active', 'rotated', 'resigned'] },
      },
    ],
    meetingFrequency: {
      type: String,
      enum: ['weekly', 'bi_weekly', 'monthly', 'quarterly', 'semi_annual', 'annual', 'as_needed'],
    },
...
const BoardResolutionSchema = new Schema(
  {
    ...
    resolutionType: {
      type: String,
      enum: ['ordinary', 'special', 'circular', 'unanimous', 'emergency'],
      required: true,
    },
    category: {
      type: String,
      enum: ['financial', 'strategic', 'operational', 'governance', 'hr', 'legal', 'compliance'],
    },
    status: {
      type: String,
      enum: [
...
const GovernancePolicySchema = new Schema(
  {
    ...
    acknowledgments: [
      { user: { type: Schema.Types.ObjectId, ref: 'User' }, date: Date, acknowledged: Boolean },
    ],
    revisionHistory: [
      {
        version: String,
...
const BCPPlanSchema = new Schema(
  {
    ...
    status: {
      type: String,
      enum: ['draft', 'under_review', 'approved', 'active', 'outdated', 'archived'],
      default: 'draft',
    },
    version: { type: String, default: '1.0' },
    owner: { type: Schema.Types.ObjectId, ref: 'User' },
    ...
    recoveryStrategies: [
      { process: String, strategy: String, resources: [String], estimatedTime: Number },
    ],
    contactList: [
      { name: String, role: String, phone: String, email: String, alternatePhone: String },
    ],
...
const BusinessImpactAnalysisSchema = new Schema(
  {
    ...
    status: {
      type: String,
      enum: ['draft', 'in_assessment', 'reviewed', 'approved'],
      default: 'draft',
    },
    criticality: {
      type: String,
      enum: ['mission_critical', 'essential', 'important', 'non_critical'],
      required: true,
    },
    ...
    dependencies: {
      internal: [{ process: String, criticality: String }],
      external: [{ vendor: String, service: String, criticality: String }],
      technology: [{ system: String, criticality: String }],
    },
...
const CrisisIncidentSchema = new Schema(
  {
    ...
    severity: {
      type: String,
      enum: ['level_1_critical', 'level_2_major', 'level_3_moderate', 'level_4_minor'],
      required: true,
    },
    status: {
      type: String,
    ...
    crisisTeam: [
      { member: { type: Schema.Types.ObjectId, ref: 'User' }, role: String, contactMethod: String },
    ],
...
const BCDrillSchema = new Schema(
  {
    ...
    findings: [
      { finding: String, severity: String, corrective: String, dueDate: Date, assignee: String },
    ],
...
const DisasterRecoveryPlanSchema = new Schema(
  {
    ...
    tier: {
      type: String,
      enum: ['tier_1_critical', 'tier_2_essential', 'tier_3_important', 'tier_4_non_critical'],
      required: true,
    },
    status: {
      type: String,
      enum: ['draft', 'approved', 'active', 'testing', 'outdated'],
      default: 'draft',
    },
    rto: { type: Number },
    rpo: { type: Number },
    owner: { type: Schema.Types.ObjectId, ref: 'User' },
    infrastructure: {
      servers: [{ name: String, type: String, location: String }],
      databases: [{ name: String, size: String, backupFrequency: String }],
      networks: [{ name: String, type: String }],
    },
    backupStrategy: {
      method: { type: String, enum: ['full', 'incremental', 'differential', 'continuous'] },
      frequency: String,
      location: String,
    ...
    failoverConfig: {
      type: { type: String, enum: ['hot', 'warm', 'cold', 'cloud'] },
      target: String,
...
const CXSurveySchema = new Schema(
  {
    ...
    surveyType: {
      type: String,
      enum: ['nps', 'csat', 'ces', 'custom', 'post_interaction', 'periodic', 'event_based'],
      required: true,
    },
    status: {
      type: String,
    ...
    channel: {
      type: String,
      enum: ['email', 'sms', 'web', 'app', 'whatsapp', 'kiosk', 'qr_code'],
      default: 'web',
    },
    questions: [
      {
    ...
    schedule: {
      startDate: Date,
      endDate: Date,
      frequency: { type: String, enum: ['one_time', 'daily', 'weekly', 'monthly', 'quarterly'] },
    },
...
const CXFeedbackSchema = new Schema(
  {
    ...
    source: {
      type: String,
      enum: ['survey', 'social_media', 'email', 'phone', 'walk_in', 'app', 'website', 'whatsapp'],
      required: true,
    },
...
const CXComplaintSchema = new Schema(
  {
    ...
    channel: {
      type: String,
      enum: ['phone', 'email', 'web', 'app', 'walk_in', 'social_media', 'whatsapp'],
      required: true,
    },
    category: {
      type: String,
...
const CustomerJourneySchema = new Schema(
  {
    ...
    touchpoints: [
      {
        ...
        metrics: { satisfaction: Number, dropOffRate: Number, avgDuration: Number },
      },
    ],
    ...
    improvementActions: [
      { action: String, touchpoint: String, priority: String, status: String, owner: String },
    ],
    organization: { type: Schema.Types.ObjectId, ref: 'Organization' },
    createdBy: { type: Schema.Types.ObjectId, ref: 'User' },
  },
...
const EnergyReadingSchema = new Schema(
  {
    ...
    energyType: {
      type: String,
      enum: ['electricity', 'natural_gas', 'diesel', 'solar', 'water', 'steam', 'chilled_water'],
      required: true,
    },
    ...
    unit: {
      type: String,
      enum: ['kWh', 'MWh', 'm3', 'gallons', 'liters', 'BTU', 'therms'],
      default: 'kWh',
    },
...
const CarbonFootprintSchema = new Schema(
  {
    ...
    status: {
      type: String,
      enum: ['draft', 'calculated', 'verified', 'published'],
      default: 'draft',
    },
    scope1: {
      total: Number,
    ...
    scope3: {
      total: Number,
      categories: [
        { category: String, description: String, emissions: Number, dataQuality: String },
      ],
    },
...
const ESGReportSchema = new Schema(
  {
    ...
    framework: {
      type: String,
      enum: ['gri', 'sasb', 'tcfd', 'cdp', 'ungc', 'sdg', 'custom', 'saudi_vision_2030'],
      required: true,
    },
    period: { from: Date, to: Date },
    status: {
      type: String,
      enum: ['draft', 'data_collection', 'review', 'approved', 'published'],
      default: 'draft',
    },
    environmental: {
    ...
    disclosures: [
      { indicator: String, description: String, value: Schema.Types.Mixed, unit: String },
    ],
    materialTopics: [
      {
        topic: String,
...
const SustainabilityGoalSchema = new Schema(
  {
    ...
    vision2030Pillar: {
      type: String,
      enum: ['vibrant_society', 'thriving_economy', 'ambitious_nation'],
    },
    category: {
      type: String,
      enum: [
...
const MaturityAssessmentSchema = new Schema(
  {
    ...
    framework: {
      type: String,
      enum: ['cmmi', 'custom', 'gartner', 'mckinsey', 'mit_sloan', 'deloitte'],
      default: 'custom',
    },
    status: {
      type: String,
...
const InnovationProjectSchema = new Schema(
  {
    ...
    stage: {
      type: String,
      enum: ['ideation', 'validation', 'development', 'testing', 'launch', 'scale'],
      default: 'ideation',
    },
...
const TechRadarEntrySchema = new Schema(
  {
    ...
    scorecard: {
      maturity: { type: Number, min: 1, max: 5 },
      scalability: { type: Number, min: 1, max: 5 },
      communitySupport: { type: Number, min: 1, max: 5 },
      costEffectiveness: { type: Number, min: 1, max: 5 },
      securityPosture: { type: Number, min: 1, max: 5 },
      overallScore: Number,
    },
...
const TransformationKPISchema = new Schema(
  {
    ...
    unit: {
      type: String,
      enum: ['percentage', 'count', 'hours', 'days', 'currency', 'ratio', 'score'],
      default: 'percentage',
    },
...
module.exports = {
  ...
  PowerOfAttorney:
    mongoose.models.PowerOfAttorney || mongoose.model('PowerOfAttorney', PowerOfAttorneySchema),
  LegalOpinion: mongoose.models.LegalOpinion || mongoose.model('LegalOpinion', LegalOpinionSchema),
  RegulatoryFiling:
    mongoose.models.RegulatoryFiling || mongoose.model('RegulatoryFiling', RegulatoryFilingSchema),

  // 2. Governance
  BoardMeeting: mongoose.models.BoardMeeting || mongoose.model('BoardMeeting', BoardMeetingSchema),
  BoardCommittee:
    mongoose.models.BoardCommittee || mongoose.model('BoardCommittee', BoardCommitteeSchema),
  BoardResolution:
    mongoose.models.BoardResolution || mongoose.model('BoardResolution', BoardResolutionSchema),
  GovernancePolicy:
    mongoose.models.GovernancePolicy || mongoose.model('GovernancePolicy', GovernancePolicySchema),
  GovernanceReport:
    mongoose.models.GovernanceReport || mongoose.model('GovernanceReport', GovernanceReportSchema),
  ...
  CustomerJourney:
    mongoose.models.CustomerJourney || mongoose.model('CustomerJourney', CustomerJourneySchema),
  ServiceBenchmark:
    mongoose.models.ServiceBenchmark || mongoose.model('ServiceBenchmark', ServiceBenchmarkSchema),

  // 5. Sustainability
  EnergyReading:
    mongoose.models.EnergyReading || mongoose.model('EnergyReading', EnergyReadingSchema),
  CarbonFootprint:
    mongoose.models.CarbonFootprint || mongoose.model('CarbonFootprint', CarbonFootprintSchema),
