...
licenseAuditLogSchema.index({ timestamp: 1 }, { expireAfterSeconds: 365 * 24 * 60 * 60 });
...
licenseAuditLogSchema.statics.findByLicense = function (licenseId, limit = 100) {
  return this.find({ licenseId }).sort({ timestamp: -1 }).limit(limit);
};
 ...
 */
licenseAuditLogSchema.statics.getDepartmentStatistics = async function (
  department,
  dateRange = {}
) {
  const query = {
