...
const _EvaluationCriteriaSchema = new mongoose.Schema({
  category: {
    type: String,
    enum: [
...
PerformanceEvaluationSchema.methods.calculateOverallScore = function () {
  ...
  if (this.evaluations.peerEvaluations && this.evaluations.peerEvaluations.length > 0) {
    const peerAverage =
      this.evaluations.peerEvaluations.reduce(
        (sum, evaluationItem) => sum + evaluationItem.score,
        0
      ) / this.evaluations.peerEvaluations.length;
  ...
  if (this.evaluations.recipientEvaluations && this.evaluations.recipientEvaluations.length > 0) {
    const recipientAverage =
      this.evaluations.recipientEvaluations.reduce(
        (sum, evaluationItem) => sum + evaluationItem.score,
        0
      ) / this.evaluations.recipientEvaluations.length;
