...
SupplierSchema.methods = {
  ...
  calculateOverallRating() {
    ...
    this.performance.overallRating =
      (qualityRating + deliveryRating + serviceRating + priceCompetitiveness) / 4;
  ...
  updateOnTimePercentage() {
    if (this.performance.totalOrders > 0) {
      this.performance.onTimePercentage =
        (this.performance.onTimeOrders / this.performance.totalOrders) * 100;
    }
  ...
  isTrustworthy() {
    return (
      this.performance.overallRating >= 4 &&
...
SupplierSchema.statics = {
  ...
  async rankSuppliersByPerformance(limit = 10) {
    return this.find({ status: 'ACTIVE' }).sort({ 'performance.overallRating': -1 }).limit(limit);
  },
