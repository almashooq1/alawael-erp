    ...
    violations: [
      {
        ...
        licenseAction: {
          type: String,
          enum: ['none', 'suspension', 'revocation', 'warning', 'mandatory_training'],
          default: 'none',
        },
      },
    ],
...
trafficAccidentReportSchema.virtual('totalInjured').get(function () {
  return (this.people?.injuries?.length || 0) + (this.people?.pedestrians?.length || 0);
});
...
trafficAccidentReportSchema.virtual('daysSinceAccident').get(function () {
  return Math.floor((Date.now() - this.accidentInfo.accidentDateTime) / (1000 * 60 * 60 * 24));
});
...
trafficAccidentReportSchema.methods.markAsUnderInvestigation = function (investigatingOfficerId) {
...
trafficAccidentReportSchema.methods.addComment = function (userId, userName, comment) {
  this.comments.push({
    userId,
    userName,
    comment,
...
// Statics
// ========================================
trafficAccidentReportSchema.statics.getStatistics = async function (filters = {}) {
  const matchStage = { archived: false };

  if (filters.startDate || filters.endDate) {
    matchStage['accidentInfo.accidentDateTime'] = {};
    if (filters.startDate) {
      matchStage['accidentInfo.accidentDateTime'].$gte = new Date(filters.startDate);
    }
    if (filters.endDate) {
      matchStage['accidentInfo.accidentDateTime'].$lte = new Date(filters.endDate);
    }
  }
...
trafficAccidentReportSchema.statics.getStatusDistribution = async function (filters = {}) {
  const matchStage = { archived: false };
...
trafficAccidentReportSchema.statics.getSeverityDistribution = async function (filters = {}) {
  const matchStage = { archived: false };
