...
class SmartAccessControlService {
  ...
  async initialize(connection) {
    ...
    logger.info('✅ Smart Access Control Service initialized');
  }
  ...
  async getUserBehaviorProfile(userId) {
    ...
    // Get from sessions
    const sessions = await this.AccessSession.find({ userId }).sort({ createdAt: -1 }).limit(30);
  ...
  isSensitiveAction(action, resource) {
    ...
    return sensitiveActions.includes(action) || sensitiveResources.some(r => resource?.includes(r));
  }
