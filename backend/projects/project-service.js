...
class ProjectManagementService extends EventEmitter {
  ...
  async getProject(projectId) {
    return this.Project.findById(projectId).populate('branch').populate('parentProject');
  }
  ...
  async getTask(taskId) {
    return this.Task.findById(taskId).populate('projectId').populate('parentTask');
  }
  ...
  async logTime(taskId, hours, userId) {
    ...
    task.timeTracking.remaining = Math.max(
      0,
      (task.timeTracking.estimated || 0) - task.timeTracking.logged
    );
  ...
  async getDashboardStatistics(tenantId) {
    ...
    const [totalProjects, activeProjects, totalTasks, tasksByStatus] = await Promise.all([
      this.Project.countDocuments(filter),
      this.Project.countDocuments({ ...filter, status: 'active' }),
      this.Task.countDocuments(filter),
      this.Task.aggregate([{ $match: filter }, { $group: { _id: '$status', count: { $sum: 1 } } }]),
    ]);
