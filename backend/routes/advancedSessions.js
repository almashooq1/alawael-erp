...
// Protect all routes — require valid JWT
 ...
 * إنشاء جلسة جديدة
 */
router.post('/', authorize(['admin', 'manager', 'specialist']), async (req, res) => {
  try {
    const {
      beneficiaryId,
      programId,
 ...
 * تحديث جلسة
 */
router.put('/:id', authorize(['admin', 'manager', 'specialist']), async (req, res) => {
  try {
 ...
 * بدء جلسة
 */
router.post('/:id/start', authorize(['admin', 'manager', 'specialist']), async (req, res) => {
  try {
    const session = await AdvancedSession.model.findById(req.params.id);
 ...
 */
router.post('/:id/complete', authorize(['admin', 'manager', 'specialist']), async (req, res) => {
  try {
    const session = await AdvancedSession.model.findById(req.params.id);
 ...
 * إلغاء جلسة
 */
router.post('/:id/cancel', authorize(['admin', 'manager', 'specialist']), async (req, res) => {
  try {
    const session = await AdvancedSession.model.findById(req.params.id);
 ...
 * إعادة جدولة جلسة
 */
router.post('/:id/reschedule', authorize(['admin', 'manager', 'specialist']), async (req, res) => {
  try {
    const session = await AdvancedSession.model.findById(req.params.id);
