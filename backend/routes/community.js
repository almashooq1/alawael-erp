...
router.get('/content/stats', (req, res) => {
  try {
    const stats = {
      ...
      averageRating:
        communityData.content.reduce((acc, c) => acc + c.rating, 0) /
        Math.max(communityData.content.length, 1),
      contentByCategory: {},
      contentByType: {},
    };

    communityData.content.forEach(c => {
      stats.contentByCategory[c.disabilityCategory] =
        (stats.contentByCategory[c.disabilityCategory] || 0) + 1;
...
router.put('/content/:id', authenticate, (req, res) => {
  try {
    ...
    if (disabilityCategory) content.disabilityCategory = disabilityCategory;
    if (contentUrl) content.contentUrl = contentUrl;
    if (level) content.level = level;
    if (tags) content.tags = tags;
    if (accessibilityFeatures) content.accessibilityFeatures = accessibilityFeatures;
...
router.get('/sessions/:id', (req, res) => {
  try {
    ...
    const session = communityData.sessions.find(s => {
      return typeof sessionId === 'number' ? s._id === sessionId : String(s._id) === sessionId;
    });
...
router.put('/sessions/:id', authenticate, (req, res) => {
  try {
    ...
    if (targetDisabilityCategory) session.targetDisabilityCategory = targetDisabilityCategory;
...
router.put('/library/:id', authenticate, (req, res) => {
  try {
    ...
    const { title, author, resourceType, fileUrl, category, description, language, tags, license } =
      req.body;
...
router.get('/sessions/stats', (req, res) => {
  try {
    const stats = {
      ...
      upcomingSessions: communityData.sessions.filter(s => new Date(s.scheduledDate) > new Date())
        .length,
    };
...
router.get('/library/stats', (req, res) => {
  try {
    const stats = {
      ...
      averageRating:
        communityData.library.reduce((acc, r) => acc + r.rating, 0) /
        Math.max(communityData.library.length, 1),
      resourcesByType: {},
      resourcesByCategory: {},
    };
...
router.get('/subscriptions/stats', (req, res) => {
  try {
    const stats = {
      totalSubscriptions:
        communityData.subscriptions.filter(s => s.status === 'active' && !s.features).length || 0,
      activeSubscriptions:
        communityData.subscriptions.filter(s => s.status === 'active' && !s.features).length || 0,
      subscriptionByPlan: {},
      revenue: 0,
...
router.get('/subscriptions/:userId', (req, res) => {
  try {
    const subscription = communityData.subscriptions.find(
