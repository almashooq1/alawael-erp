...
router.put('/lessons/:id', authenticate, authorize(['admin', 'instructor']), async (req, res) => {
  try {
    ...
    const lesson = await Lesson.findByIdAndUpdate(
      req.params.id,
      { title, content, type, videoUrl, duration, order, isPublished, resources },
      {
        new: true,
        runValidators: true,
      }
 ...
 * @access  Private
 */
router.post('/enrollment/:courseId/complete-lesson/:lessonId', authenticate, async (req, res) => {
  try {
    const userId = req.user.id;
 ...
 * @access  Private
 */
router.post('/quiz/:quizId/submit', authenticate, async (req, res) => {
  try {
    ...
    const quizResult = enrollment.quizResults.find(qr => qr.quiz.toString() === quizId);
 ...
 * @access  Private (Instructor/Admin)
 */
router.post('/media', authenticate, authorize(['admin', 'instructor']), async (req, res) => {
  try {
    const mediaData = {
      ...req.body,
 ...
 * @access  Private
 */
router.post('/courses/:courseId/rate', authenticate, async (req, res) => {
  try {
