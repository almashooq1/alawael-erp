...
router.get('/search/profiles', authMiddleware, async (req, res) => {
  try {
    ...
    if (keyword) {
      filter.$or = [
        { 'personalInfo.firstName': new RegExp(escapeRegex(keyword), 'i') },
        { 'personalInfo.lastName': new RegExp(escapeRegex(keyword), 'i') },
        { 'personalInfo.email': new RegExp(escapeRegex(keyword), 'i') },
      ];
    }
...
router.get('/:employeeId', authMiddleware, async (req, res) => {
  try {
    const profile = await EmployeeProfile.findOne({ userId: req.params.employeeId }).populate(
      'userId',
      'email name role'
    );
