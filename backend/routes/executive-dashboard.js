...
router.get('/', requireAuth, (req, res) => {
  try {
    ...
    res
      .status(500)
      .json(responseFormatter.serverError('Failed to fetch executive dashboard', error));
  }
});
...
router.get('/kpis', requireAuth, (req, res) => {
  try {
    ...
    res.json(
      responseFormatter.list(kpis, 'KPIs', {
        total: kpis.length,
...
router.post('/kpis', requireAdmin, (req, res) => {
  try {
    const { name, name_ar, category, target, current, unit, frequency, owner, description } =
      req.body;
...
router.get('/departments', requireAuth, (req, res) => {
  try {
    ...
    res
      .status(500)
      .json(responseFormatter.serverError('Failed to fetch department comparison', error));
  }
});
...
router.get('/data-quality', requireAuth, (req, res) => {
  try {
    ...
    res
      .status(500)
      .json(responseFormatter.serverError('Failed to fetch data quality metrics', error));
  }
});
