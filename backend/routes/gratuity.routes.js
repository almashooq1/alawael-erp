    ...
    try {
      ...
      if (adjustmentFactor !== 1) {
        result.summary.netSettlement =
          Math.round(result.summary.netSettlement * adjustmentFactor * 100) / 100;
      }
...
router.post(
  ...
  async (req, res) => {
    try {
      ...
      const result = await GratuityService.approveGratuity(gratuityId, req.user.userId, remarks);
...
router.post(
  ...
  async (req, res) => {
    try {
      ...
      const result = await GratuityService.processPayment(gratuityId, paymentMethod, bankDetails);
...
router.post(
  ...
  async (req, res) => {
    try {
      ...
      const result = await GratuityService.completePayment(gratuityId, paymentReference);
  ...
  async (req, res) => {
    try {
      ...
      gratuities.forEach(g => {
        ...
        summary.byScenario[g.terminationScenario] =
          (summary.byScenario[g.terminationScenario] || 0) + 1;
