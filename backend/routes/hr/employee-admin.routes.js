...
// W1586 — server-derived / identity-generated fields that a create payload must
// NEVER set. `employee_number` is auto-generated by the Employee pre-save hook;
// the audit/system columns are stamped by the server. Spreading the raw request
// body into Employee.create() would let a caller forge the employee number or
// overwrite audit provenance, so these are stripped from the incoming body.
const EMPLOYEE_CREATE_PROTECTED = Object.freeze([
  'employee_number',
  '_id',
  '__v',
  'created_by',
...
function createEmployeeAdminRouter({
  ...
  //
  // W1586 — SECURITY: the router mounts behind `authenticate` only (app.js), so
  // before this gate ANY authenticated principal could create an Employee with
  // an arbitrary salary / national_id / IBAN and even forge the employee_number
  // (mass-assignment via `...body`). Creating staff records is inherently an
  // HR-admin function → gate to HR roles, and strip server-derived fields from
  // the payload so even an HR caller cannot overwrite generated/audit columns.
  router.post('/employees', requireRole('hr', 'admin', 'hr_manager'), async (req, res) => {
    try {
      if (!req.user) return res.status(401).json({ error: 'auth required' });
      const ctx = resolveCallerContext(req);
      const Employee = require('../../models/HR/Employee');
      const body = camelKeysToSnake(req.body || {});
      for (const k of EMPLOYEE_CREATE_PROTECTED) delete body[k];
