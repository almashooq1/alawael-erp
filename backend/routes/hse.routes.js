...
// W1614 — SafetyInspection identity fields (branchId derives from the inspector's User.branchId
...
router.get('/dashboard', authenticate, requireBranchAccess, async (req, res) => {
  try {
    ...
    const [
      ...
      SafetyInspection.countDocuments({ ...bf }), // W1614 — SafetyInspection now has branchId
...
router.get('/inspections', authenticate, requireBranchAccess, async (req, res) => {
  try {
    ...
    const filter = { ...branchFilter(req) }; // W1614 — restricted → own branch only
    if (status) filter.status = status;
...
router.post('/inspections', authenticate, requireBranchAccess, async (req, res) => {
  try {
    // W1614 — branchId derives from the inspector's User.branchId (pre-save); strip spoof/identity.
...
router.put('/inspections/:id', authenticate, requireBranchAccess, async (req, res) => {
  try {
    // W1614 — scoped update (foreign branch → not-found) + strip identity fields.
...
router.delete(
  ...
  async (req, res) => {
    try {
      // W1614 — scoped delete: no cross-branch inspection deletion.
