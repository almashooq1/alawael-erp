...
// Health check — Integration Hub (scaffolded, controller not yet implemented)
router.get('/health', (_req, res) => {
  res.json({
    status: 'planned',
    message: 'Integration Hub module — controller not yet implemented',
  });
});

// All routes below require IntegrationHubController (not yet created)
// Returning 501 Not Implemented until the controller is built
router.all('*', (_req, res) => {
  res.status(501).json({
    success: false,
    message:
      'Integration Hub — not yet implemented. Create controllers/IntegrationHubController.js to activate.',
    status: 'NOT_IMPLEMENTED',
  });
});

// NOTE: The route definitions below are preserved for future implementation.
// Once IntegrationHubController is created, move the catch-all to the end of file.
