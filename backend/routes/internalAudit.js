...
router.get('/audit-plans/:planId', authenticate, requireBranchAccess, async (req, res) => {
  try {
...
router.delete(
  ...
  async (req, res) => {
    try {
      const plan = await AnnualAuditPlan.findOneAndDelete({ planId: req.params.planId });
      if (!plan) {
        return res.status(404).json({ success: false, message: 'خطة التدقيق غير موجودة' });
      }
...
router.get('/surprise-audits', authenticate, requireBranchAccess, async (req, res) => {
  try {
...
router.get('/surprise-audits/:auditId', authenticate, requireBranchAccess, async (req, res) => {
  try {
...
router.delete(
  ...
  async (req, res) => {
    try {
      const audit = await SurpriseAudit.findOneAndDelete({ auditId: req.params.auditId });
      if (!audit) {
        return res.status(404).json({ success: false, message: 'عملية التدقيق غير موجودة' });
      }
...
router.post('/non-conformance-reports', authenticate, requireBranchAccess, async (req, res) => {
  try {
...
router.get('/non-conformance-reports', authenticate, requireBranchAccess, async (req, res) => {
  try {
...
// إنشاء متابعة إغلاق
router.post('/closure-followups', authenticate, requireBranchAccess, async (req, res) => {
  try {
...
router.get('/closure-followups', authenticate, requireBranchAccess, async (req, res) => {
  try {
...
router.delete(
  ...
  async (req, res) => {
    try {
      const report = await NonConformanceReport.findOneAndDelete({ ncrId: req.params.ncrId });
      if (!report) {
        return res.status(404).json({ success: false, message: 'التقرير غير موجود' });
      }
...
router.delete(
  ...
  async (req, res) => {
    try {
      const action = await CorrectivePreventiveAction.findOneAndDelete({
        actionId: req.params.actionId,
      });
      if (!action) {
        return res.status(404).json({ success: false, message: 'الإجراء غير موجود' });
...
router.delete(
  ...
  async (req, res) => {
    try {
      const followUp = await ClosureFollowUp.findOneAndDelete({
        followUpId: req.params.followUpId,
      });
      if (!followUp) {
        return res.status(404).json({ success: false, message: 'المتابعة غير موجودة' });
...
router.get('/internal-audit-dashboard', authenticate, requireBranchAccess, async (req, res) => {
  try {
