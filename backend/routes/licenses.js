...
router.get(
  ...
  asyncHandler(async (req, res) => {
    const license = await licenseService.getLicenseByNumber(req.params.licenseNumber);
...
router.put(
  ...
  authorize(['admin', 'license_manager']),
  asyncHandler(async (req, res) => {
    const license = await licenseService.updateLicense(req.params.id, req.body, req.user.id);
...
router.post(
  ...
  asyncHandler(async (req, res) => {
    const results = await licenseService.verifyLicense(req.params.id, req.user.id);
...
router.post(
  ...
  authorize(['admin', 'license_manager']),
  asyncHandler(async (req, res) => {
    const license = await licenseService.renewLicense(req.params.id, req.body, req.user.id);
...
router.post(
  ...
  asyncHandler(async (req, res) => {
    ...
    for (const id of licenseIds) {
      try {
        const license = await licenseService.renewLicense(id, renewalData, req.user.id);
        results.push({ id, success: true, license });
      } catch (error) {
        results.push({ id, success: false, error: 'حدث خطأ في المعالجة' });
      }
    }
...
router.post(
  ...
  asyncHandler(async (req, res) => {
    ...
    for (const id of licenseIds) {
      try {
        const verificationResults = await licenseService.verifyLicense(id, req.user.id);
        results.push({ id, success: true, verificationResults });
      } catch (error) {
        results.push({ id, success: false, error: 'حدث خطأ في التحقق' });
      }
    }
...
router.use((err, _req, res, _next) => {
  ...
  res.status(status).json({
    success: false,
    message: 'حدث خطأ في معالجة التراخيص',
  });
});
