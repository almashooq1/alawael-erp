...
router.get('/overview/:beneficiaryId',
  auth,
  async (req, res) => {
    try {
...
// Progress timeline
router.get('/progress/:beneficiaryId',
  auth,
  async (req, res) => {
    try {
      const { beneficiaryId } = req.params;
...
// Home programs
router.get('/home-programs/:beneficiaryId',
  auth,
  async (req, res) => {
    try {
      const { beneficiaryId } = req.params;
  ...
  async (req, res) => {
    try {
      ...
      if (!beneficiaryId || !message) {
        return res.status(400).json({ success: false, message: 'beneficiaryId and message are required' });
      }
...
// Notifications
router.get('/notifications/:beneficiaryId',
  auth,
  async (req, res) => {
    try {
      const { beneficiaryId } = req.params;
...
// Available reports
router.get('/reports/:beneficiaryId',
  auth,
  async (req, res) => {
    try {
      const { beneficiaryId } = req.params;
