...
/**
 * GET /api/qiwa/employees
 * List all employees registered in Qiwa
 */
router.get(
  '/employees',
  handleAsyncError(async (req, res) => {
    ...
    res.json({
      ...
      message: 'قائمة الموظفين المسجلين في قوى',
 ...
 * Generic employee verification (delegates to iqama or national-id)
 */
router.post(
  '/employees/verify',
  handleAsyncError(async (req, res) => {
    ...
    // Delegate to specific verify method based on ID type
    const isIqama = idType === 'iqama' || (idNumber && idNumber.startsWith('2'));
    const result = isIqama
      ? await qiwaService.verifyByIqama(idNumber)
      : await qiwaService.verifyByNationalId(idNumber);
