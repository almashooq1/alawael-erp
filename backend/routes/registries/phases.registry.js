...
module.exports = function registerPhaseRoutes(
  ...
  safeMount(app, ['/api/campaigns', '/api/v1/campaigns'], '../routes/campaigns.routes');
  safeMount(app, ['/api/donors', '/api/v1/donors'], '../routes/donors.routes');
  safeMount(app, ['/api/donations', '/api/v1/donations'], '../routes/donations.routes');
  logger.info('Donations system mounted (3 modules: campaigns, donors, donations)');

  // ── Vendor Management — إدارة الموردين ──────────────────────────────
  safeMount(app, ['/api/vendors', '/api/v1/vendors'], '../routes/vendors.routes');
  safeMount(
    app,
    ['/api/vendor-evaluations', '/api/v1/vendor-evaluations'],
    '../routes/vendor-evaluations.routes'
  );
