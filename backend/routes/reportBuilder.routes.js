...
router.get('/data-sources/:id/fields', authenticate, [
  param('id').notEmpty(),
], async (req, res) => {
  try {
...
router.get('/reports/:id', authenticate, [
  param('id').notEmpty(),
], async (req, res) => {
  try {
    if (handleValidation(req, res)) return;
...
router.post('/reports', authenticate, authorize(['admin', 'manager', 'report_manager']), [
...
router.put('/reports/:id', authenticate, authorize(['admin', 'manager', 'report_manager']), [
  param('id').notEmpty(),
], async (req, res) => {
  try {
    if (handleValidation(req, res)) return;
    const report = reportBuilder.updateReport(req.params.id, { ...req.body, updatedBy: req.user?.id });
...
router.delete('/reports/:id', authenticate, authorize(['admin', 'manager']), [
  param('id').notEmpty(),
], async (req, res) => {
  try {
    if (handleValidation(req, res)) return;
...
router.post('/reports/:id/duplicate', authenticate, authorize(['admin', 'manager', 'report_manager']), [
  param('id').notEmpty(),
], async (req, res) => {
  try {
    if (handleValidation(req, res)) return;
...
router.post('/reports/:id/columns', authenticate, authorize(['admin', 'manager', 'report_manager']), [
  param('id').notEmpty(),
...
// Remove column
router.delete('/reports/:id/columns/:fieldId', authenticate, authorize(['admin', 'manager', 'report_manager']), async (req, res) => {
  try {
...
router.put('/reports/:id/columns/reorder', authenticate, authorize(['admin', 'manager', 'report_manager']), [
  param('id').notEmpty(),
  body('orderedFieldIds').isArray().withMessage('ترتيب الحقول مطلوب'),
], async (req, res) => {
  try {
    if (handleValidation(req, res)) return;
...
// Add filter
router.post('/reports/:id/filters', authenticate, authorize(['admin', 'manager', 'report_manager']), [
  param('id').notEmpty(),
  body('fieldId').notEmpty().withMessage('معرف الحقل مطلوب'),
...
// Update filter
router.put('/reports/:id/filters/:filterId', authenticate, authorize(['admin', 'manager', 'report_manager']), async (req, res) => {
  try {
...
// Set sorting
router.put('/reports/:id/sorting', authenticate, authorize(['admin', 'manager', 'report_manager']), [
  param('id').notEmpty(),
], async (req, res) => {
  try {
    if (handleValidation(req, res)) return;
...
// Set grouping
router.put('/reports/:id/group-by', authenticate, authorize(['admin', 'manager', 'report_manager']), [
  param('id').notEmpty(),
], async (req, res) => {
  try {
    if (handleValidation(req, res)) return;
...
router.post('/reports/:id/calculated-fields', authenticate, authorize(['admin', 'manager', 'report_manager']), [
  param('id').notEmpty(),
...
router.put('/reports/:id/chart', authenticate, authorize(['admin', 'manager', 'report_manager']), [
  param('id').notEmpty(),
], async (req, res) => {
  try {
    if (handleValidation(req, res)) return;
...
router.post('/reports/:id/execute', authenticate, [
  param('id').notEmpty(),
], async (req, res) => {
  try {
    if (handleValidation(req, res)) return;
    const result = reportBuilder.executeReport(req.params.id, { ...req.query, ...req.body, userId: req.user?.id });
...
router.get('/reports/:id/executions', authenticate, [
  param('id').notEmpty(),
], async (req, res) => {
  try {
    if (handleValidation(req, res)) return;
...
router.post('/templates/:id/create-report', authenticate, authorize(['admin', 'manager', 'report_manager']), [
  param('id').notEmpty(),
], async (req, res) => {
  try {
    if (handleValidation(req, res)) return;
...
router.post('/reports/:id/save-as-template', authenticate, authorize(['admin', 'manager']), [
  param('id').notEmpty(),
], async (req, res) => {
  try {
    if (handleValidation(req, res)) return;
    const tmpl = reportBuilder.saveAsTemplate(req.params.id, { ...req.body, createdBy: req.user?.id });
...
router.post('/reports/:id/export', authenticate, [
  param('id').notEmpty(),
...
router.post('/schedules', authenticate, authorize(['admin', 'manager', 'report_manager']), [
  body('reportId').notEmpty().withMessage('معرف التقرير مطلوب'),
  body('frequency').isIn(['daily', 'weekly', 'monthly', 'quarterly']).withMessage('التكرار غير مدعوم'),
], async (req, res) => {
  try {
    if (handleValidation(req, res)) return;
...
router.put('/schedules/:id', authenticate, authorize(['admin', 'manager', 'report_manager']), [
  param('id').notEmpty(),
], async (req, res) => {
  try {
    if (handleValidation(req, res)) return;
...
router.delete('/schedules/:id', authenticate, authorize(['admin', 'manager']), [
  param('id').notEmpty(),
], async (req, res) => {
  try {
    if (handleValidation(req, res)) return;
...
router.post('/reports/:id/share', authenticate, authorize(['admin', 'manager']), [
  param('id').notEmpty(),
], async (req, res) => {
  try {
    if (handleValidation(req, res)) return;
    const share = reportBuilder.shareReport(req.params.id, { ...req.body, sharedBy: req.user?.id });
...
router.get('/reports/:id/shares', authenticate, [
  param('id').notEmpty(),
], async (req, res) => {
  try {
    if (handleValidation(req, res)) return;
...
router.post('/reports/:id/favorite', authenticate, [
  param('id').notEmpty(),
], async (req, res) => {
  try {
    if (handleValidation(req, res)) return;
...
router.get('/reports/:id/versions', authenticate, [
  param('id').notEmpty(),
], async (req, res) => {
  try {
    if (handleValidation(req, res)) return;
