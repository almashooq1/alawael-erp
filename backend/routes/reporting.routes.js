...
router.get('/latest', authenticate, async (req, res) => {
  try {
    ...
    const report = await FinancialReport.getLatestReport(req.user.organizationId, reportType);
...
router.post(
  '/',
  authenticate,
  authorize(['finance_manager', 'accountant', 'admin']),
  async (req, res) => {
    try {
      const {
        reportType,
...
router.post(
  '/:reportId/approve',
  authenticate,
  authorize(['finance_director', 'cfo', 'admin']),
  async (req, res) => {
    try {
      const report = await FinancialReport.findOneAndUpdate(
        { reportId: req.params.reportId, organizationId: req.user.organizationId },
