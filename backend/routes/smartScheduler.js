...
// Authentication required for all scheduler routes
 ...
 */
router.post('/create-schedule', authorize(['admin', 'manager']), async (req, res) => {
  try {
    const {
      beneficiaryId,
      programId,
 ...
 */
router.post('/:id/approve-schedule', authorize(['admin', 'manager']), async (req, res) => {
  try {
    const scheduler = await SmartScheduler.model.findById(req.params.id);
 ...
 * تفعيل الجدولة
 */
router.post('/:id/activate-schedule', authorize(['admin', 'manager']), async (req, res) => {
  try {
    const scheduler = await SmartScheduler.model.findById(req.params.id);
