  ...
  try {
    ...
    const stats = {
      ...
      summary: {
        ...
        totalEarlyLeave: records.filter(r => r.attendanceStatus === 'early_departure').length,
...
router.get(
  ...
  async (req, res) => {
    try {
      ...
      const records = await SmartAttendance.find(query)
...
router.get(
  ...
  async (req, res) => {
    try {
      const leaves = await SmartLeave.find({ overallStatus: 'pending', isDeleted: false })
        .populate('employeeId', 'name email department')
        .populate('coverage.alternateEmployeeId', 'name')
        .sort({ createdAt: -1 });
...
router.get(
  ...
  async (req, res) => {
    try {
      ...
      let attendanceRecords = await SmartAttendance.find(query)
        .populate('employeeId', 'name department')
        .sort({ date: -1 });

      // تصفية حسب الإدارة إذا لزم الأمر
      if (department) {
        attendanceRecords = attendanceRecords.filter(r => r.employeeId.department === department);
      }

      // حساب الإحصائيات الشاملة
      const summary = {
        totalRecords: attendanceRecords.length,
        presentCount: attendanceRecords.filter(r => r.attendanceStatus === 'present').length,
        lateCount: attendanceRecords.filter(r => r.lateness.isLate).length,
        absentCount: attendanceRecords.filter(r => r.attendanceStatus === 'absent').length,
