...
// ─── GET variants (frontend calls GET with query params) ─────────────────────
router.get('/email', optionalAuth, (req, res) => {
  try {
    const { email } = req.query;
    if (!email) return res.status(400).json(responseFormatter.validationError('email is required'));
    const valid = validator.isValidEmail(email);
    res.json(
      responseFormatter.success({ email, valid }, valid ? 'Email is valid' : 'Email is invalid')
    );
...
router.get('/phone', optionalAuth, (req, res) => {
  try {
    const { phone } = req.query;
    if (!phone) return res.status(400).json(responseFormatter.validationError('phone is required'));
    const valid = validator.isValidPhone(phone);
    res.json(
      responseFormatter.success({ phone, valid }, valid ? 'Phone is valid' : 'Phone is invalid')
    );
...
router.get('/url', optionalAuth, (req, res) => {
  try {
    const { url } = req.query;
    if (!url) return res.status(400).json(responseFormatter.validationError('url is required'));
    const valid = validator.isValidUrl(url);
    res.json(responseFormatter.success({ url, valid }, valid ? 'URL is valid' : 'URL is invalid'));
  } catch (error) {
    res.status(500).json(responseFormatter.serverError('URL validation failed', error));
