...
JobHistorySchema.index(
  { createdAt: 1 },
  { expireAfterSeconds: schedulerConfig.historyRetention * 86400 }
);
...
class SchedulerService {
  ...
  async executeJob(jobId) {
    ...
    try {
      ...
      const result = await this.executeWithTimeout(handler(job.config.params), job.config.timeout);
  ...
  async resumeJob(jobId) {
    const job = await this.Job.findByIdAndUpdate(
      jobId,
      { status: 'active' },
  ...
  async getJobHistory(jobId, limit = 100) {
    return this.JobHistory.find({ jobId }).sort({ startedAt: -1 }).limit(limit);
  }
  ...
  startCleanup() {
    cron.schedule(schedulerConfig.cleanupInterval, async () => {
      logger.info('🧹 Cleaning up old job history...');
