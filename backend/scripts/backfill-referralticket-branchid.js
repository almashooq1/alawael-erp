...
/**
 * backfill-referralticket-branchid.js — W633 (R4 denormalization).
 * ════════════════════════════════════════════════════════════════════
 * Backfills `branchId` on existing ReferralTicket docs from the (required)
 * beneficiary's branch. Dry-run unless `--commit`; only SETS a missing
 * branchId, never overwrites.
 *
 * Usage:
 *   MONGODB_URI=... node scripts/backfill-referralticket-branchid.js          # dry-run
 *   MONGODB_URI=... node scripts/backfill-referralticket-branchid.js --commit
 *   ... --json
 */
...
(async () => {
  ...
  const { ReferralTicket } = require('../models/MDTCoordination');
  ...
  const missing = await ReferralTicket.find({
  ...
  for (const t of missing) {
    let branchId = null;
    if (Beneficiary && t.beneficiary) {
      const b = await Beneficiary.findById(t.beneficiary).select('branchId').lean();
    ...
    resolved++;
    if (COMMIT) await ReferralTicket.updateOne({ _id: t._id }, { $set: { branchId } });
  }
  ...
  if (JSON_OUT) {
    ...
    log('');
    log(`W633 ReferralTicket.branchId backfill — ${summary.mode}`);
    log('────────────────────────────────────────────────');
  ...
  console.error('backfill-referralticket-branchid failed:', err.message);
  process.exit(2);
});
