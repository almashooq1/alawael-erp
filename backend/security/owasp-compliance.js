...
const accessControlMiddleware = (options = {}) => {
  ...
  return (req, res, next) => {
    ...
    const isPublic = allowPublicRoutes.some(
      route => req.path.startsWith(route) || req.path === route
    );
    ...
    if (
      ...
      !req.user.permissions?.includes(req.requiredPermission)
    ) {
...
const securityLogger = {
  ...
  async sendToWebhook(entry) {
    try {
      ...
      logger.error('Failed to send security event to webhook:', error.message);
    }
  },
...
const ssrfProtection = {
  blockedIPs: new Set(['127.0.0.1', '0.0.0.0', 'localhost', '::1']),
