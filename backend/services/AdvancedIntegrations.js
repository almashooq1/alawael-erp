...
class GoogleMapsService {
  ...
  async calculateDistanceMatrix(origins, destinations) {
    try {
      ...
      logger.error('❌ Distance matrix calculation failed:', error);
      return { success: false, error: 'حدث خطأ داخلي' };
    }
  }
...
class PaymentService {
  ...
  async payDriver(driverId, amount, bankAccount) {
    try {
      ...
      logger.error('❌ Driver payment failed:', error);
      return { success: false, error: 'حدث خطأ داخلي' };
    }
  }
  ...
  async getPaymentHistory(customerId, limit = 10) {
    try {
      ...
      logger.error('❌ Payment history retrieval failed:', error);
      return { success: false, error: 'حدث خطأ داخلي' };
    }
  }
...
class IntegrationOrchestrator {
  ...
  async assignTrip(tripData, driverData) {
    try {
      ...
      logger.error('❌ Trip assignment flow failed:', error);
      return { success: false, error: 'حدث خطأ داخلي' };
    }
  }
  ...
  async completeTrip(tripId, driverData, earnings) {
    try {
      ...
      logger.error('❌ Trip completion flow failed:', error);
      return { success: false, error: 'حدث خطأ داخلي' };
    }
  }
  ...
  async handleSafetyViolation(driverData, violationType) {
    try {
      ...
      logger.error('❌ Safety violation alert failed:', error);
      return { success: false, error: 'حدث خطأ داخلي' };
    }
  }
}
