...
class EnhancedSmartMeasurementEngine {
  ...
  async analyzeAndLinkMeasurement(measurementResult, beneficiary) {
    try {
      ...
      const rankedPrograms = scoredPrograms.sort((a, b) => b.totalScore - a.totalScore).slice(0, 8); // أفضل 8 برامج
      ...
      logger.error('❌ خطأ في التحليل:', error);
      throw error;
    }
  }
  ...
  calculateSeverityMatch(measurementResult, program) {
    ...
    const intensityMultiplier =
      (program.intensity || 'STANDARD') === 'INTENSIVE'
        ? severityScore >= 4
          ? 1.5
          : 0.8
        : severityScore <= 2
  ...
  calculateDemographicMatch(beneficiary, program) {
    ...
    if (program.ageRange) {
      ...
      if (age >= program.ageRange.minAge && age <= program.ageRange.maxAge) {
        ...
        Math.abs(age - program.ageRange.minAge) < 5 ||
  ...
  calculateEvidenceScore(measurementType, program) {
    ...
    if (measurementType.isStandardized && measurementType.normSource && program.evidenceBase) {
      score += 40;
    }
  ...
  generateComprehensiveRecommendations(rankedPrograms, beneficiary) {
    return {
      ...
      shortTerm: rankedPrograms.filter(p => p.recommendationLevel === 'HIGH').slice(0, 3),

      longTerm: rankedPrograms.filter(p => p.recommendationLevel === 'MEDIUM'),
  ...
  generateFamilyRecommendations(programs) {
    return {
      ...
      weeklyMeetings: true,
      resources: ['كتيبات إرشادية', 'فيديوهات تدريبية', 'اجتماعات استشارية'],
    };
  }
  ...
  identifyAreas(programs) {
    return ['مجالات تحتاج تركيز مكثف', 'مهارات أساسية تحتاج تطوير', 'سلوكيات تحتاج تعديل'];
  }
