...
class LicenseService {
  ...
  async getLicenseById(licenseId) {
    try {
      const license = await License.findById(licenseId).populate('documents').lean();
  ...
  async getLicenseByNumber(licenseNumber) {
    try {
      return await License.findOne({ licenseNumber }).populate('documents').lean();
    } catch (error) {
    ...
    try {
      ...
      if (search) {
        query.$or = [
          ...
          { 'entity.name': { $regex: escapeRegex(search), $options: 'i' } },
          { 'entity.idNumber': { $regex: escapeRegex(search), $options: 'i' } },
        ];
      }
      ...
      const licenses = await License.find(query).sort(sortObject).skip(skip).limit(limit).lean();
  ...
  async verifyRequiredDocuments(licenseId) {
    try {
      ...
      const missing = requiredDocs.filter(req => !submittedDocs.includes(req.name));
  ...
  async checkCompliance(licenseId) {
    try {
      ...
      return {
        ...
        criticalViolations: license.compliance.violations.filter(v => v.severity === 'critical')
          .length,
