...
class MLService {
  ...
  async predictOrderDemand(historicalOrders, daysAhead = 30) {
    try {
      ...
      logger.error('Order demand prediction error:', error);
  ...
  async predictCustomerChurn(customers) {
    try {
      const riskAssessment = customers.map(customer => {
        ...
        const spendingScore =
          customer.avgOrderValue > 0 ? Math.max(1 - customer.avgOrderValue / 1000, 0) : 0;
      ...
      const averageRisk =
        riskAssessment.reduce((sum, r) => sum + r.churnRisk, 0) / riskAssessment.length;
    ...
    try {
      const recommendations = products.map(product => {
        ...
        const stdDev = Math.sqrt(
          product.demandHistory.reduce((sum, d) => sum + Math.pow(d - avgDemand, 2), 0) /
  ...
  detectSeasonality(monthlyRevenues) {
    ...
    const allRevenues = Array.from(monthlyData.values()).flat();
