...
const { MeasurementResult } = require('../models/MeasurementModels');
const { RehabilitationProgram, ProgramProgress } = require('../models/RehabilitationProgramModels');
...
class SmartMeasurementProgramEngine {
  ...
  async analyzeMeasurementAndActivatePrograms(measurementResultId) {
    try {
      ...
      logger.error('خطأ في محرك الربط الذكي:', error);
      throw error;
    }
  }
  ...
  async activateProgramsForBeneficiary(beneficiaryId, rankedPrograms, measurementResultId) {
    ...
    for (const programData of rankedPrograms) {
      try {
        ...
        logger.error(`خطأ في تفعيل البرنامج ${programData.programId}:`, error);
      }
    }
  ...
  async generateComprehensiveRecommendations(
    ...
    if (
      measurementResult.overallLevel === 'PROFOUND' ||
  ...
  generateMatchReasoning(program, measurementResult) {
    return [
      ...
      `البرنامج يوفر: ${program.objectives
        .slice(0, 2)
        .map(o => o.description)
        .join(', ')}`,
    ].join(' | ');
  ...
  async generateBeneficiaryReport(beneficiaryId) {
    try {
      ...
      const { IndividualRehabPlan } = require('../models/MeasurementModels');
  ...
  calculateOverallEffectiveness(progress) {
    ...
    const objectiveRate =
      (progress.objectiveProgress.filter(obj => obj.status === 'MET' || obj.status === 'EXCEEDED')
        .length /
        progress.objectiveProgress.length) *
      100;
