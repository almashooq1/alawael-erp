...
class QiwaTestSuite {
  ...
  async runVerificationTests() {
    ...
    await this.testCase('Labor Record Retrieval', async () => {
      this.service.getEmployeeLaborRecord = async iqama => ({
        ...
        data: {
          iqama,
          contracts: [{ contractId: 'CNT001', status: 'active' }],
        },
      });
  ...
  async runPerformanceTests() {
    ...
    await this.testCase('Metrics Collection', async () => {
      const metrics = this.service.getMetrics();
      assert(Object.prototype.hasOwnProperty.call(metrics, 'totalRequests'));
      assert(Object.prototype.hasOwnProperty.call(metrics, 'successfulRequests'));
      assert(Object.prototype.hasOwnProperty.call(metrics, 'failedRequests'));
    });
  }
