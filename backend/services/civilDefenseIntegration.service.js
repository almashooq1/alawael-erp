...
class CivilDefenseIntegrationService {
  ...
  async requestSafetyCertificate(buildingData) {
    try {
      ...
      const response = await axios.post(`${this.apiBaseUrl}/certificates/request`, requestPayload, {
      ...
      logger.error('Error requesting safety certificate:', error);
  ...
  async scheduleFireSafetyInspection(inspectionData) {
    try {
      ...
      logger.error('Error scheduling fire safety inspection:', error);
      throw new Error(error.message);
    }
  }
  ...
  async scheduleEmergencyDrill(drillData) {
    try {
      ...
      logger.error('Error scheduling emergency drill:', error);
      throw new Error(error.message);
    }
  }
  ...
  async uploadSafetyDocuments(facilityId, documents) {
    try {
      ...
      const response = await axios.post(`${this.apiBaseUrl}/documents/upload`, formData, {
        headers: {
          ...this.getAuthHeaders(),
  ...
  async sendAuditConfirmationEmail(auditData, requestData) {
    try {
      ...
      logger.error('Error sending audit confirmation email:', error);
  ...
  async sendEmergencyDrillNotification(drillData, requestData) {
    try {
      ...
      logger.error('Error sending drill notification:', error);
      return { success: false };
    }
  }
