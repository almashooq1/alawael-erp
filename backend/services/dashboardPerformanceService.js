...
class DashboardPerformanceService {
  ...
  evictOldest() {
    ...
    this.cache.forEach((entry, key) => {
      const score =
        entry.accessCount > 0 ? entry.lastAccessed.getTime() : entry.createdAt.getTime();
  ...
  optimizeDataset(data, options = {}) {
    const { maxItems = 100, summarizeAfter = 50, deepCopy = false } = options;
    ...
    // Summarize if needed
    if (Array.isArray(optimized) && optimized.length > summarizeAfter && optimized[0]?.history) {
