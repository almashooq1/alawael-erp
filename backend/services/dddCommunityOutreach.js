'use strict';
/**
 * DDD Community Outreach Service
 * ────────────────────────────────
 * Phase 36 – Community Engagement & Outreach (Module 2/4)
 *
 * Manages community programs, awareness campaigns, partnership networks,
 * outreach events, needs assessments, and impact tracking.
 */
...
class CommunityOutreach {
  async createProgram(data) {
    return DDDOutreachProgram.create(data);
  }
  async listPrograms(filter = {}, page = 1, limit = 20) {
    return DDDOutreachProgram.find(filter)
      .sort({ createdAt: -1 })
      .skip((page - 1) * limit)
      .limit(limit)
      .lean();
  ...
  async updateProgram(id, data) {
    return DDDOutreachProgram.findByIdAndUpdate(id, data, { new: true }).lean();
  }

  async createPartner(data) {
    return DDDCommunityPartner.create(data);
  }
  async listPartners(filter = {}, page = 1, limit = 20) {
    return DDDCommunityPartner.find(filter)
  ...
  async listEvents(filter = {}, page = 1, limit = 20) {
    return DDDOutreachEvent.find(filter)
      .sort({ eventDate: -1 })
      .skip((page - 1) * limit)
      .limit(limit)
      .lean();
  ...
  async createImpactReport(data) {
    return DDDImpactReport.create(data);
  }
  async listImpactReports(filter = {}, page = 1, limit = 20) {
    return DDDImpactReport.find(filter)
      .sort({ reportDate: -1 })
      .skip((page - 1) * limit)
      .limit(limit)
      .lean();
  ...
  async healthCheck() {
    const [programs, partners, events, reports] = await Promise.all([
      DDDOutreachProgram.countDocuments(),
      DDDCommunityPartner.countDocuments(),
      DDDOutreachEvent.countDocuments(),
      DDDImpactReport.countDocuments(),
    ]);
    return {
      status: 'ok',
      module: 'CommunityOutreach',
      counts: { programs, partners, events, reports },
    };
  }
}

/* ═══════════════════ Router Factory ═══════════════════ */
function createCommunityOutreachRouter() {
  const { Router } = require('express');
  const router = Router();
  const svc = new CommunityOutreach();

  router.get('/community-outreach/health', async (_req, res) => {
    try {
  ...
  router.post('/community-outreach/programs', async (req, res) => {
    try {
  ...
  router.get('/community-outreach/programs', async (req, res) => {
    try {
      const { page = 1, limit = 20, ...f } = req.query;
      res.json(await svc.listPrograms(f, +page, +limit));
    } catch (e) {
      res.status(500).json({ error: e.message });
    }
  });
  router.put('/community-outreach/programs/:id', async (req, res) => {
    try {
      res.json(await svc.updateProgram(req.params.id, req.body));
    } catch (e) {
      res.status(500).json({ error: e.message });
    }
  });
  router.post('/community-outreach/partners', async (req, res) => {
    try {
      res.status(201).json(await svc.createPartner(req.body));
    } catch (e) {
      res.status(500).json({ error: e.message });
    }
  });
  router.get('/community-outreach/partners', async (req, res) => {
    try {
      const { page = 1, limit = 20, ...f } = req.query;
      res.json(await svc.listPartners(f, +page, +limit));
    } catch (e) {
      res.status(500).json({ error: e.message });
    }
  });
  router.post('/community-outreach/events', async (req, res) => {
    try {
  ...
  router.get('/community-outreach/events', async (req, res) => {
    try {
      const { page = 1, limit = 20, ...f } = req.query;
  ...
  router.post('/community-outreach/impact-reports', async (req, res) => {
    try {
      res.status(201).json(await svc.createImpactReport(req.body));
    } catch (e) {
      res.status(500).json({ error: e.message });
    }
  });
  router.get('/community-outreach/impact-reports', async (req, res) => {
    try {
      const { page = 1, limit = 20, ...f } = req.query;
      res.json(await svc.listImpactReports(f, +page, +limit));
    } catch (e) {
      res.status(500).json({ error: e.message });
    }
  });
  router.get('/community-outreach/stats', async (_req, res) => {
    try {
      res.json(await svc.getOutreachStats());
    } catch (e) {
      res.status(500).json({ error: e.message });
    }
  });
