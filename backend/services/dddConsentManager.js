...
/* ═══════════════════════════════════════════════════════════════════════
   ...
   ═══════════════════════════════════════════════════════════════════════ */
function anonymizeField(value, type = 'string') {
  if (!value) return value;
  switch (type) {
    case 'name':
      return 'ANONYMIZED';
    case 'email':
      return `anon-${crypto.randomBytes(4).toString('hex')}@redacted.local`;
    case 'phone':
      return '+00-REDACTED';
    case 'nationalId':
...
function pseudonymize(data, salt = 'ddd-platform') {
  if (!data) return data;
  const str = typeof data === 'string' ? data : JSON.stringify(data);
  return crypto.createHmac('sha256', salt).update(str).digest('hex').substring(0, 16);
}

/* ═══════════════════════════════════════════════════════════════════════
   9. Retention Policy Functions
   ═══════════════════════════════════════════════════════════════════════ */
async function seedRetentionPolicies() {
  for (const policy of DEFAULT_RETENTION_POLICIES) {
    await DDDRetentionPolicy.findOneAndUpdate(
      { domain: policy.domain },
      { $setOnInsert: policy },
      { upsert: true, new: true }
    );
  }
  return DDDRetentionPolicy.find({}).lean();
}

async function getRetentionPolicies() {
  return DDDRetentionPolicy.find({ isActive: true }).sort({ domain: 1 }).lean();
}

async function updateRetentionPolicy(domain, updates) {
  const policy = await DDDRetentionPolicy.findOneAndUpdate(
    { domain },
    { $set: updates },
    { new: true }
  );
  if (!policy) throw new Error(`Retention policy not found: ${domain}`);
  return policy.toObject();
}

/* ═══════════════════════════════════════════════════════════════════════
   10. Consent Dashboard
   ═══════════════════════════════════════════════════════════════════════ */
async function getConsentDashboard(branchId) {
  const match = { isDeleted: { $ne: true } };
  if (branchId) match.branchId = new mongoose.Types.ObjectId(branchId);

  const [byPurpose, byStatus, expiringSoon] = await Promise.all([
    DDDConsent.aggregate([
      { $match: match },
      { $sort: { beneficiaryId: 1, purpose: 1, updatedAt: -1 } },
      { $group: { _id: { b: '$beneficiaryId', p: '$purpose' }, latest: { $first: '$$ROOT' } } },
      {
        $group: {
          _id: '$latest.purpose',
          granted: { $sum: { $cond: [{ $eq: ['$latest.status', 'granted'] }, 1, 0] } },
          denied: { $sum: { $cond: [{ $eq: ['$latest.status', 'denied'] }, 1, 0] } },
          withdrawn: { $sum: { $cond: [{ $eq: ['$latest.status', 'withdrawn'] }, 1, 0] } },
          total: { $sum: 1 },
        },
      },
      { $sort: { _id: 1 } },
    ...
    DDDConsent.countDocuments({
      ...match,
      status: 'granted',
      expiresAt: { $gt: new Date(), $lt: new Date(Date.now() + 30 * 86400000) },
    }),
  ]);

  return {
    byPurpose: byPurpose.reduce((o, p) => {
      o[p._id] = { granted: p.granted, denied: p.denied, withdrawn: p.withdrawn, total: p.total };
      return o;
    }, {}),
    byStatus: byStatus.reduce((o, s) => {
      o[s._id] = s.count;
...
function createConsentRouter() {
  ...
  /* Grant consent */
  router.post('/consent/grant/:beneficiaryId', async (req, res) => {
    try {
  ...
  /* Withdraw consent */
  router.post('/consent/withdraw/:beneficiaryId', async (req, res) => {
    try {
  ...
  /* Get consent status for beneficiary */
  router.get('/consent/status/:beneficiaryId', async (req, res) => {
    try {
  ...
  router.get('/consent/check/:beneficiaryId/:purpose', async (req, res) => {
    try {
  ...
  router.get('/consent/purposes', (_req, res) => {
    res.json({ success: true, purposes: CONSENT_PURPOSES });
  });

  /* Consent dashboard */
  router.get('/consent/dashboard', async (req, res) => {
    try {
  ...
  /* Process DSAR - access */
  router.post('/dsar/:id/process-access', async (req, res) => {
    try {
  ...
  /* Process DSAR - erasure */
  router.post('/dsar/:id/process-erasure', async (req, res) => {
    try {
  ...
  /* DSAR dashboard */
  router.get('/dsar/dashboard', async (req, res) => {
    try {
  ...
  router.post('/retention-policies/seed', async (_req, res) => {
    try {
  ...
  router.put('/retention-policies/:domain', async (req, res) => {
    try {
