...
async function checkReferentialIntegrity(modelName, def) {
  ...
  for (const ref of def.references) {
    const RefModel = model(ref.model);
...
/* ═══════════════════════════════════════════════════════════════════════
   10. Express Router
   ═══════════════════════════════════════════════════════════════════════ */
function createDataQualityRouter() {
  const router = Router();

  /* Assess single model */
  router.post('/data-quality/assess/:modelName', async (req, res) => {
    try {
  ...
  /* Global assessment */
  router.post('/data-quality/assess-global', async (_req, res) => {
    try {
  ...
  /* Latest report for model */
  router.get('/data-quality/latest/:modelName', async (req, res) => {
    try {
  ...
  /* Global dashboard (latest) */
  router.get('/data-quality/dashboard', async (_req, res) => {
    try {
      const report = await DDDDataQualityReport.findOne({
        scope: 'global',
        isDeleted: { $ne: true },
  ...
  /* Quality trend */
  router.get('/data-quality/trend', async (req, res) => {
    try {
  ...
  router.get('/data-quality/models', (_req, res) => {
    const models = Object.entries(MODEL_QUALITY_DEFS).map(([name, def]) => ({
      modelName: name,
      domain: def.domain,
      requiredFields: def.requiredFields.length,
      references: def.references?.length || 0,
      stalenessThresholdDays: def.stalenessThresholdDays,
    }));
