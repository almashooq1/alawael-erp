'use strict';
/**
 * DDD Donor Relations Service
 * ────────────────────────────
 * Phase 36 – Community Engagement & Outreach (Module 3/4)
 *
 * Manages donor profiles, donation tracking, fundraising campaigns,
 * pledge management, gift acknowledgment, and donor analytics.
 */
...
/* ═══════════════════ Domain Class ═══════════════════ */
class DonorRelations {
  async createDonor(data) {
    return DDDDonorProfile.create(data);
  }
  async listDonors(filter = {}, page = 1, limit = 20) {
    return DDDDonorProfile.find(filter)
      .sort({ lifetimeGiving: -1 })
      .skip((page - 1) * limit)
      .limit(limit)
      .lean();
  }
  async updateDonor(id, data) {
    return DDDDonorProfile.findByIdAndUpdate(id, data, { new: true }).lean();
  }

  async recordDonation(data) {
    return DDDDonationRecord.create(data);
  }
  async listDonations(filter = {}, page = 1, limit = 20) {
    return DDDDonationRecord.find(filter)
      .sort({ donatedAt: -1 })
      .skip((page - 1) * limit)
      .limit(limit)
      .lean();
  ...
  async createCampaign(data) {
    return DDDFundraisingCampaign.create(data);
  }
  async listCampaigns(filter = {}, page = 1, limit = 20) {
    return DDDFundraisingCampaign.find(filter)
      .sort({ startDate: -1 })
      .skip((page - 1) * limit)
      .limit(limit)
      .lean();
  ...
  async logStewardship(data) {
    return DDDStewardshipLog.create(data);
  }
  async listStewardship(filter = {}, page = 1, limit = 20) {
    return DDDStewardshipLog.find(filter)
      .sort({ performedAt: -1 })
      .skip((page - 1) * limit)
      .limit(limit)
      .lean();
  ...
  async healthCheck() {
    const [donors, donations, campaigns, stewardship] = await Promise.all([
      DDDDonorProfile.countDocuments(),
      DDDDonationRecord.countDocuments(),
      DDDFundraisingCampaign.countDocuments(),
      DDDStewardshipLog.countDocuments(),
    ]);
    return {
      status: 'ok',
      module: 'DonorRelations',
      counts: { donors, donations, campaigns, stewardship },
    };
  }
}

/* ═══════════════════ Router Factory ═══════════════════ */
function createDonorRelationsRouter() {
  const { Router } = require('express');
  const router = Router();
  const svc = new DonorRelations();

  router.get('/donor-relations/health', async (_req, res) => {
    try {
  ...
  router.post('/donor-relations/donors', async (req, res) => {
    try {
      res.status(201).json(await svc.createDonor(req.body));
    } catch (e) {
      res.status(500).json({ error: e.message });
    }
  });
  router.get('/donor-relations/donors', async (req, res) => {
    try {
      const { page = 1, limit = 20, ...f } = req.query;
      res.json(await svc.listDonors(f, +page, +limit));
    } catch (e) {
      res.status(500).json({ error: e.message });
    }
  });
  router.put('/donor-relations/donors/:id', async (req, res) => {
    try {
      res.json(await svc.updateDonor(req.params.id, req.body));
    } catch (e) {
      res.status(500).json({ error: e.message });
    }
  });
  router.post('/donor-relations/donations', async (req, res) => {
    try {
      res.status(201).json(await svc.recordDonation(req.body));
    } catch (e) {
      res.status(500).json({ error: e.message });
    }
  });
  router.get('/donor-relations/donations', async (req, res) => {
    try {
      const { page = 1, limit = 20, ...f } = req.query;
      res.json(await svc.listDonations(f, +page, +limit));
    } catch (e) {
      res.status(500).json({ error: e.message });
    }
  });
  router.post('/donor-relations/campaigns', async (req, res) => {
    try {
  ...
  router.get('/donor-relations/campaigns', async (req, res) => {
    try {
      const { page = 1, limit = 20, ...f } = req.query;
  ...
  router.post('/donor-relations/stewardship', async (req, res) => {
    try {
      res.status(201).json(await svc.logStewardship(req.body));
    } catch (e) {
      res.status(500).json({ error: e.message });
    }
  });
  router.get('/donor-relations/stewardship', async (req, res) => {
    try {
      const { page = 1, limit = 20, ...f } = req.query;
      res.json(await svc.listStewardship(f, +page, +limit));
    } catch (e) {
      res.status(500).json({ error: e.message });
    }
  });
  router.get('/donor-relations/stats', async (_req, res) => {
    try {
      res.json(await svc.getDonorStats());
    } catch (e) {
      res.status(500).json({ error: e.message });
    }
  });
