...
/* ═══════════════════════════════════════════════════════════════════════
   ...
   ═══════════════════════════════════════════════════════════════════════ */
function localeMiddleware(options = {}) {
  const {
  ...
  return (req, _res, next) => {
    let locale = req.query[queryParam] || req.headers['x-locale'] || null;

    if (!locale && req.headers[headerName]) {
      const accept = req.headers[headerName];
    ...
    req.locale = SUPPORTED_LOCALES.includes(locale) ? locale : defaultLocale;
    req.dir = LOCALE_META[req.locale]?.dir || 'rtl';
    next();
  };
}

/* ═══════════════════════════════════════════════════════════════════════
   7. Export / Import for professional translation
   ═══════════════════════════════════════════════════════════════════════ */
async function exportForTranslation(namespace, locale = 'en') {
  await refreshCache();
  const ns = namespace ? [namespace] : Object.keys(translationCache);
  const entries = [];

  for (const n of ns) {
    if (!translationCache[n]) continue;
    for (const [key, trans] of Object.entries(translationCache[n])) {
      entries.push({
        key,
        namespace: n,
        source: trans.en || key,
        target: trans[locale] || '',
        translated: !!trans[locale],
      });
    }
  }
...
async function importTranslations(entries, locale) {
  let imported = 0;
  for (const entry of entries) {
    if (!entry.key || !entry.target) continue;
    const existing = await DDDTranslation.findOne({
      key: entry.key,
      namespace: entry.namespace || 'common',
    });
    if (existing) {
      existing.translations[locale] = entry.target;
      await existing.save();
    } else {
      await DDDTranslation.create({
        key: entry.key,
        namespace: entry.namespace || 'common',
        translations: { [locale]: entry.target, en: entry.source },
        isCustom: true,
      });
    }
    imported++;
...
/* ═══════════════════════════════════════════════════════════════════════
   8. Express Router
   ═══════════════════════════════════════════════════════════════════════ */
function createLocalizationRouter() {
  const router = Router();
  ...
  router.get('/i18n/translate/:key(*)', async (req, res) => {
    try {
  ...
  /* Coverage */
  router.get('/i18n/stats/coverage', async (_req, res) => {
    try {
  ...
  /* Supported locales */
  router.get('/i18n/locales', (_req, res) => {
    res.json({
      success: true,
  ...
  /* Namespaces */
  router.get('/i18n/namespaces', async (_req, res) => {
    await refreshCache();
    const namespaces = Object.keys(translationCache).map(ns => ({
      name: ns,
      keyCount: Object.keys(translationCache[ns]).length,
    }));
    res.json({ success: true, namespaces });
  });

  /* CRUD custom translations */
  router.post('/i18n/translations', async (req, res) => {
    try {
  ...
  router.get('/i18n/translations/custom', async (req, res) => {
    try {
  ...
  router.delete('/i18n/translations/:key(*)', async (req, res) => {
    try {
  ...
  /* Seed builtins */
  router.post('/i18n/seed', async (_req, res) => {
    try {
  ...
  /* Export */
  router.get('/i18n/export', async (req, res) => {
    try {
  ...
  /* Import */
  router.post('/i18n/import', async (req, res) => {
    try {
