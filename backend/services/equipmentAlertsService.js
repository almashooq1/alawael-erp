...
class EquipmentAlertsService {
  ...
  static async checkWarrantyAlerts() {
    ...
    try {
      ...
      logger.error('Error checking warranty alerts:', error);
    }
    ...
    try {
      ...
      for (const maintenance of overdueMaintenances) {
        const daysOverdue = Math.ceil(
          (now - new Date(maintenance.preventiveSchedule.nextScheduledDate)) / (1000 * 60 * 60 * 24)
        );
      ...
      logger.error('Error checking overdue maintenances:', error);
    }
    ...
    try {
      ...
      for (const maintenance of upcomingMaintenances) {
        const daysUntil = Math.ceil(
          (new Date(maintenance.preventiveSchedule.nextScheduledDate) - now) / (1000 * 60 * 60 * 24)
        );
  ...
  static async checkOverdueLendings() {
    ...
    try {
      ...
      logger.error('Error checking overdue lendings:', error);
    }
  ...
  static async checkCriticalFaults() {
    ...
    try {
      ...
      for (const fault of criticalFaults) {
        const timeOpen = Math.ceil((new Date() - new Date(fault.detectedDate)) / (1000 * 60 * 60));
      ...
      logger.error('Error checking critical faults:', error);
    }
  ...
  static async checkUpcomingCalibrations() {
    ...
    try {
      ...
      logger.error('Error checking upcoming calibrations:', error);
    }
  ...
  static async checkHighUtilizationEquipment() {
    ...
    try {
      ...
      logger.error('Error checking high utilization:', error);
    }
