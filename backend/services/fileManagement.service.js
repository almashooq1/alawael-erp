...
class FileManagementService {
  ...
  searchFiles(userId, query) {
    ...
    return files.filter(f => f.filename.toLowerCase().includes(lowerQuery));
  }
  ...
  deleteMultipleFiles(userId, filenames) {
    ...
    filenames.forEach(filename => {
      ...
      try {
        ...
        failed.push({ filename, error: 'حدث خطأ داخلي' });
      }
    });
  ...
  getFileStatistics(userId) {
    ...
    const stats = {
      ...
      averageFileSizeFormatted:
        files.length > 0 ? this.formatBytes(usage.used / files.length) : '0 B',
      largestFile: files.length > 0 ? files.reduce((a, b) => (a.size > b.size ? a : b)) : null,
