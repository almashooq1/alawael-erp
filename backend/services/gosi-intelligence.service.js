...
class GOSIIntelligenceService extends EventEmitter {
    ...
    try {
      ...
      if (monthsEmployed >= 1) {
        ...
        predictions.recommendations.push(
          'Schedule GOSI registration after 1-month work anniversary'
        );
      }
      ...
      predictions.estimatedBenefits = {
        ...
        expectedBenefits: isSaudi
          ...
          : ['Work injury coverage', 'Basic benefits'],
      };
    ...
    try {
      ...
      if (daysSinceGOSIUpdate > 90) {
        ...
        riskAssessment.alerts.push(
          `GOSI data should be refreshed (${Math.floor(daysSinceGOSIUpdate)} days old)`
        );
      }
      ...
      if (medicalInsuranceExpiry) {
        const daysUntilExpiry =
          (new Date(medicalInsuranceExpiry) - Date.now()) / (1000 * 60 * 60 * 24);
        if (daysUntilExpiry < 0) {
          ...
          riskAssessment.alerts.push(
            `Medical insurance expires in ${Math.floor(daysUntilExpiry)} days`
          );
          riskAssessment.urgentActions.push(
            `📅 Schedule medical insurance renewal (${Math.floor(daysUntilExpiry)} days remaining)`
          );
        }
      }
      ...
      if (salaryHistory.length > 0) {
        ...
        if (salaryDifference > 0.3) {
          ...
          riskAssessment.alerts.push(
            `Salary change of ${(salaryDifference * 100).toFixed(1)}% detected`
          );
      ...
      if (contractEndDate) {
        ...
        if (daysUntilEnd < 30 && daysUntilEnd > 0) {
          ...
          riskAssessment.urgentActions.push(
            `📋 Prepare contract renewal or exit procedures (${Math.floor(daysUntilEnd)} days remaining)`
          );
        }
      }
  ...
  forecastFinancialImpact(employeeData, forecastMonths = 12) {
    try {
      ...
      // Generate insights
      forecast.insights.push(
        `💰 Total 12-month salary cost: ${this._formatCurrency(forecast.summary.totalSalary)}`
      );
      forecast.insights.push(
        `💼 Total employer contribution: ${this._formatCurrency(forecast.summary.totalEmployerContribution)}`
      );
      forecast.insights.push(
        `👤 Total employee contribution: ${this._formatCurrency(forecast.summary.totalEmployeeContribution)}`
      );

      const costPerMonth =
        Math.round((forecast.summary.totalCostOfEmployment / forecastMonths) * 100) / 100;
    ...
    try {
      ...
      if (salary < 5000) {
        recommendations.insights.push(
          '💡 Consider performance-based salary increases to improve employee retention'
        );
      }

      recommendations.insights.push(
        '📊 Regular compliance audits can reduce legal risks by up to 70%'
      );
      recommendations.insights.push(
        '🎯 Automated GOSI updates reduce administrative errors by 95%'
      );
