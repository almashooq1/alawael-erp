...
class GPSSecurityService {
  ...
  static getEncryptionKey(vehicleId) {
    const { gpsEncryptionKey } = require('../config/secrets');
    const baseKey = gpsEncryptionKey;
  ...
  static maskSensitiveLocationData(locationData, userRole) {
    ...
    if (userRole === 'public') {
      ...
      masked.address = masked.address.split(',')[0]; // المدينة فقط
    }
  ...
  static detectAnomalousAccess(accessLog, history) {
    ...
    const recentAccesses = history.filter(
      h => new Date() - new Date(h.timestamp) < 60000 // آخر دقيقة
    ).length;
    ...
    if (accessLog.action === 'export_data' && accessLog.userRole !== 'fleet_manager') {
      anomalies.push({
