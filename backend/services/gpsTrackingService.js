...
class GPSTrackingService {
  ...
  static async recordLocation(driverId, locationData) {
    try {
      ...
      logger.error('خطأ في تسجيل الموقع:', error);
      throw error;
    }
  }
  ...
  static checkViolations(location) {
    ...
    if (location.acceleration) {
      const totalAccel = Math.sqrt(
        location.acceleration.x ** 2 + location.acceleration.y ** 2 + location.acceleration.z ** 2
      );
    ...
    try {
      ...
      return {
        ...
        stats: {
          ...
          duration:
            (locations[locations.length - 1].timestamp - locations[0].timestamp) / 1000 / 60,
          pointCount: locations.length,
        },
      };
    } catch (error) {
      logger.error('خطأ في جلب خريطة المسار:', error);
      throw error;
    }
  }
  ...
  static async getBehaviorReport(driverId, startTime, endTime) {
    try {
      ...
      logger.error('خطأ في جلب تقرير السلوك:', error);
      throw error;
    }
  }
  ...
  static async findNearbyDrivers(longitude, latitude, maxDistance = 1000) {
    try {
      ...
      logger.error('خطأ في البحث عن السائقين:', error);
      throw error;
    }
  }
  ...
  static async getFleetStatistics(driverId = null, timeRange = 'today') {
    try {
      ...
      const query = { timestamp: { $gte: startTime, $lte: now } };
      if (driverId) {
      ...
      return {
        ...
        totalDistance: locations
          .reduce((acc, loc) => acc + (loc.distanceDriven || 0), 0)
          .toFixed(2),
        timeRange,
      };
    } catch (error) {
      logger.error('خطأ في جلب إحصائيات الأسطول:', error);
      throw error;
    }
  }
}
