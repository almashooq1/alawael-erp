...
class GuardianService {
  ...
  static async getPerformanceComparison(guardianId) {
    ...
    return {
      ...
      averageScore: (comparison.reduce((sum, b) => sum + b.score, 0) / comparison.length).toFixed(
        2
      ),
    };
  }
  ...
  static async getAdvancedAnalytics(guardianId) {
    ...
    const analytics = {
      academic: {
        ...
        trend:
          progressData.length >= 2
            ? progressData[progressData.length - 1].academicScore - progressData[0].academicScore
            : 0,
        distribution: {
          ...
          satisfactory: progressData.filter(p => p.academicScore >= 60 && p.academicScore < 70)
            .length,
      ...
      activities: {
        averageCompletion: (
          progressData.reduce((sum, p) => sum + p.activityCompletionRate, 0) / progressData.length
        ).toFixed(2),
      },
    };
  ...
  static async getFinancialForecast(guardianId) {
    ...
    const nextMonth = payments
      .filter(p => {
        ...
        return (
          paymentDate > now && paymentDate <= new Date(now.getTime() + 30 * 24 * 60 * 60 * 1000)
        );
      })
      .reduce((sum, p) => sum + (p.amount - (p.amountPaid || 0)), 0);
    ...
    const nextThreeMonths = payments
      .filter(p => {
        const paymentDate = new Date(p.dueDate);
        return (
          paymentDate > now && paymentDate <= new Date(now.getTime() + 90 * 24 * 60 * 60 * 1000)
        );
      })
      .reduce((sum, p) => sum + (p.amount - (p.amountPaid || 0)), 0);
