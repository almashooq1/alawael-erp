...
class SmartAttendanceService {
  ...
  static async recordSmartCheckIn(employeeId, checkInData) {
    try {
      ...
      // تحليل التأخير
      const lateAnalysis = await this.analyzeLateness(attendance.checkInTime, applicableRules);
      ...
      const intelligenceAnalysis = await this.analyzeAnomalies(employeeId, attendance);
  ...
  static async analyzeLateness(checkInTime, rules) {
    ...
    const [requiredHours, requiredMinutes] = mandatoryCheckInTime.split(':').map(Number);
    ...
    if (delayMinutes > gracePeriod) {
      const severity = delayMinutes > rules.latePolicies.maxLateMinutes ? 'critical' : 'warning';
  ...
  static async analyzeEarlyLeave(checkInTime, checkOutTime, rules) {
    ...
    const [checkOutHours, checkOutMinutes] = mandatoryCheckOutTime.split(':').map(Number);
  ...
  static async analyzeAnomalies(employeeId, currentAttendance) {
    try {
      ...
      const stats = {
        ...
        earlyLeaveCount: recentAttendance.filter(a => a.earlyLeave.isEarlyLeave).length,
        absenceCount: recentAttendance.filter(a => a.attendanceStatus === 'absent').length,
        averageWorkHours:
      ...
      const riskLevel = stats.lateCount >= 5 ? 'high' : stats.lateCount >= 3 ? 'medium' : 'low';
  ...
  static async updateEmployeeStats(employeeId) {
    try {
      ...
      // تحديث في النموذج
      employee.attendance = {
        totalPresent: thisMonthAttendance.filter(a => a.attendanceStatus === 'present').length,
        totalLate: thisMonthAttendance.filter(a => a.attendanceStatus === 'late_arrival').length,
        totalAbsent: thisMonthAttendance.filter(a => a.attendanceStatus === 'absent').length,
        totalEarlyLeave: thisMonthAttendance.filter(a => a.attendanceStatus === 'early_departure')
          .length,
      ...
      logger.error('خطأ في تحديث إحصائيات الموظف:', error);
    }
  }
  ...
  static async generateMonthlyReport(employeeId, month, year) {
    ...
    const summary = {
      ...
      attendancePercentage: (
        (records.filter(r => r.attendanceStatus === 'present').length / records.length) *
        100
      ).toFixed(2),
    };
