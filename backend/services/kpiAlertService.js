...
class KPIAlertService {
  ...
  evaluateKPI(kpiId, currentValue, targetValue) {
    const rules = Array.from(this.alertRules.values()).filter(r => r.kpiId === kpiId && r.enabled);
   ...
   */
  checkCondition(currentValue, rule, targetValue = 100) {
  ...
  getActiveAlerts(kpiId = null, severity = null) {
    let activeAlerts = Array.from(this.alerts.values()).filter(a => a.status === 'active');
