...
```javascript
const { MigrationManager, CSVProcessor } = require('./services/migration');
...
for (const table of tables) {
  const result = await manager.migrateFromCSV(`./data/${table}.csv`, table);
...
for (const table of tables) {
  await manager.exportTableToCSV(table, `./backups/${table}_${Date.now()}.csv`);
}
...
const validation = await csvProcessor.validateCSVStructure('./file.csv', ['expected', 'columns']);
