...
const results = await migration.migrateTables(['users', 'products', 'orders'], {
  continueOnError: true,
});

// Migrate with transformation
const transformed = await migration.migrateTableWithTransform('users', row => ({
  ...row,
...
const plan = manager.createMigrationPlan(['users', 'products', 'orders'], {
  preValidation: true,
  postValidation: true,
  skipDuplicates: true,
});
...
// Import from CSV
const csvResult = await manager.migrateFromCSV('./data.csv', 'users', {
  transform: {
...
```javascript
const result = await manager.migrateFromCSV('./imports/users.csv', 'users', {
  transform: {
...
describe('Migration System', () => {
  ...
  it('should validate CSV structure', async () => {
    const result = await csvProcessor.validateCSVStructure('./test-data.csv', ['id', 'name']);
