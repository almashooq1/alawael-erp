...
class MigrationService extends EventEmitter {
    ...
    try {
      ...
      if (this.options.detectDuplicates && this.duplicateDetector) {
        ...
        if (duplicateResult.duplicates.length > 0) {
          ...
          this.log(`Found ${duplicateResult.count} duplicate records`, 'warning', migrationId);
        }
      }
      ...
      for (const record of processedData) {
        try {
          ...
          this.log('حدث خطأ داخلي', 'error', migrationId);
        }
      }
  ...
  log(message, level = 'info', migrationId = null) {
    ...
    try {
      ...
      fs.appendFileSync(logFile, `[${level.toUpperCase()}] ${logEntry.timestamp} - ${message}\n`);
    } catch (error) {
