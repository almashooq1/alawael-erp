...
```javascript
const verification = await manager.dbMigration.verifyAllMigrations(['users', 'products', 'orders']);
```
...
```javascript
const plan = manager.createMigrationPlan(['users', 'products', 'customers', 'orders', 'invoices'], {
  preValidation: true,
  postValidation: true,
});
```
...
```javascript
await manager.migrateFromCSV('./imports/customers.csv', 'customers', {
