...
class NotificationSystem extends EventEmitter {
  ...
  async sendNotification(notification, channels = ['inApp', 'email']) {
    try {
      ...
      logger.error('Notification system error:', error);
      throw error;
    }
  }
   ...
   */
  async retryFailed() {
    const failed = this.notificationQueue.filter(
      n => n.status === 'failed' && n.attempts < n.maxAttempts
    );
...
class EmailChannel {
  async send(notification) {
    ...
    return new Promise((resolve, reject) => {
      logger.info(
        `📧 Sending email to ${notification.recipient ? notification.recipient.replace(/(.{2}).*(@.*)/, '$1***$2') : '<unknown>'}:`,
...
class SMSChannel {
  async send(notification) {
    ...
    return new Promise(resolve => {
      logger.info(
        `📱 Sending SMS to ${notification.phoneNumber ? '***' + notification.phoneNumber.slice(-4) : '<unknown>'}:`,
        notification.body
      );
...
class InAppChannel {
  ...
  async send(notification) {
    ...
    logger.info('💬 In-app notification created:', notification.title);
