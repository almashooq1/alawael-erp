...
class PolicyEngine extends EventEmitter {
  ...
  _initializeDefaultPolicies() {
    const defaultPolicies = [
      ...
      },
      {
        ...
        rules: [
          {
            conditions: [{ type: 'role', value: 'user' }],
            actions: ['read', 'update'],
      ...
      },
      {
        ...
        rules: [
          {
            conditions: [{ type: 'time', value: { start: '18:00', end: '08:00' } }],
  ...
  evaluatePolicies(context) {
    try {
      const { userId, action, resource, userContext = {} } = context;
    ...
    try {
      ...
      const resourceMatch = rule.resources.some(
        resource =>
          ...
          context.resource.startsWith(resource + ':')
      );

      if (!resourceMatch) return false;

      // Check if action matches
      const actionMatch = rule.actions.some(action => action === '*' || action === context.action);
  ...
  _evaluateCustomCondition(condition, context) {
    try {
      ...
      // Safe expression evaluation - only allow comparison operators on context properties
      // Block dangerous patterns (RCE prevention)
      const dangerousPatterns =
        /\b(require|import|eval|Function|constructor|prototype|__proto__|process|global|module|exports|this|fetch|XMLHttpRequest|child_process)\b/;
      if (dangerousPatterns.test(expression)) {
        this.logger.warn(`Blocked dangerous expression: ${expression.substring(0, 50)}`);
        return false;
      }
      // Only allow safe comparison expressions: context.prop op value
      const safePattern = /^[a-zA-Z0-9_.\s+\-*/<>=!&|'",%[\]?:()]+$/;
      if (!safePattern.test(expression)) {
        this.logger.warn(`Blocked unsafe expression pattern: ${expression.substring(0, 50)}`);
        return false;
      }
      // Build safe evaluation by replacing context references with actual values
