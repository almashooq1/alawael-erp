...
class RealtimeCollaborationService extends EventEmitter {
   ...
   */
  applyChange(change) {
    const { sessionId, userId, documentId, operation, position, content, timestamp } = change;
   ...
   */
  addComment(comment) {
    const { sessionId, userId, documentId, position, content, resolved = false } = comment;
