...
class RealtimeDashboardService {
  ...
  getConnectionStats() {
    return {
      ...
      activeSources: Array.from(this.dataConnections.values()).filter(s => s.status === 'active')
        .length,
      erroredSources: Array.from(this.dataConnections.values()).filter(s => s.status === 'error')
        .length,
  ...
  async refreshAllData() {
    ...
    for (const sourceId of this.dataConnections.keys()) {
      try {
        ...
        results[sourceId] = { success: false, error: 'حدث خطأ داخلي' };
      }
    }
