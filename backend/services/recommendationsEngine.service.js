...
class RecommendationsEngineService extends EventEmitter {
  ...
  generateRecommendations(tenantId, userId, params = {}) {
    try {
      ...
      recommendations.push(
        ...cfRecs.map(r => ({ ...r, strategy: 'collaborative_filtering', score: r.score * 0.35 }))
      );

      // Content-based filtering
      const cbRecs = this._contentBasedFiltering(tenantId, userId, userContext);
      recommendations.push(
        ...cbRecs.map(r => ({ ...r, strategy: 'content_based', score: r.score * 0.3 }))
      );

      // Hybrid approach
      const hybridRecs = this._hybridRecommendation(tenantId, userId, userContext);
      recommendations.push(
        ...hybridRecs.map(r => ({ ...r, strategy: 'hybrid', score: r.score * 0.25 }))
      );

      // Context-aware
      const contextRecs = this._contextAwareRecommendation(tenantId, userId, userContext);
      recommendations.push(
        ...contextRecs.map(r => ({ ...r, strategy: 'context_aware', score: r.score * 0.1 }))
      );
  ...
  updateUserPreferences(tenantId, userId, preferences) {
    try {
      const key = `${tenantId}:${userId}`;
      const prefs =
        this.userPreferences.get(key) || this._createDefaultPreferences(tenantId, userId);
      ...
      if (preferences.maxRecommendations !== undefined)
        prefs.maxRecommendations = preferences.maxRecommendations;
  ...
  recordABTestEvent(testId, variant, eventType) {
    try {
      ...
      results.conversionRate =
        results.clicks > 0 ? (results.conversions / results.clicks) * 100 : 0;
