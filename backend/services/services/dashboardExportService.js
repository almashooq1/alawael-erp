...
class DashboardExportService {
  ...
  async exportToPDF(dashboardData, options = {}) {
    try {
      ...
      doc.fontSize(10).text(`تاريخ التقرير: ${new Date().toLocaleString('ar-SA')}`, { align: 'center' });
      doc.moveDown();
      ...
      if (dashboardData.executive && dashboardData.executive.kpis) {
        doc.fontSize(14).font('Courier-Bold').text('المؤشرات الرئيسية | Key Performance Indicators');
        doc.fontSize(10);
      ...
      for (let i = 1; i <= pages; i++) {
        ...
        doc.fontSize(9).text(`صفحة ${i} من ${pages}`, 50, doc.page.height - 50, { align: 'center' });
      }
  ...
  generateEmailBody(dashboardData) {
    ...
    الحالة العامة: ${
      summary.kpisCritical > 0 ? 'حرجة | Critical' :
      summary.kpisAtRisk > 0 ? 'محفوف بالمخاطر | At Risk' :
      'صحي | Healthy'
    }
