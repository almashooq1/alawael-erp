...
class SmartFleetDashboardService {
  ...
  static async calculateFleetAnalytics(vehicles, trips) {
    ...
    analytics.distance.weekly = weekTrips.reduce(
      (sum, t) => sum + (t.statistics?.totalDistance || 0),
      0
    );

    // معدل الاستهلاك
    const totalFuel = vehicles.reduce(
      (sum, v) => sum + (v.maintenance?.fuelConsumption?.currentFuelLevel || 0),
      0
    );
    analytics.fuel.averageConsumption = totalFuel / vehicles.length;

    // معدل سلامة الأسطول
    const violations = trips.reduce((sum, t) => sum + (t.incidents?.length || 0), 0);
    ...
    if (trips.length > 0) {
      const avgDuration =
        trips.reduce((sum, t) => sum + (t.statistics?.totalDuration || 0), 0) / trips.length;
  ...
  static async getDriverPerformanceReport(driverId, days = 30) {
    try {
      ...
      metrics.fuelEfficiency =
        trips.length > 0
          ? trips.reduce((sum, t) => sum + (t.fuelData?.efficiency || 0), 0) / trips.length
          : 0;
  ...
  static async getFleetKPIs(timeframe = 'daily') {
    try {
      ...
      kpis.operational = {
        ...
        averageTripDistance:
          trips.length > 0
            ? trips.reduce((sum, t) => sum + (t.statistics?.totalDistance || 0), 0) / trips.length
            : 0,
        utilizationRate: (trips.length / (vehicles.length * 8)) * 100, // حسب ساعات العمل
        onTimePercentage:
          trips.length > 0 ? (trips.filter(t => t.arrivedOnTime).length / trips.length) * 100 : 0,
      };
      ...
      kpis.financial = {
        ...
        maintenanceCost,
        costPerKM:
          kpis.operational.totalDistance > 0 ? totalCost / kpis.operational.totalDistance : 0,
        revenuePerTrip: trips.length > 0 ? (kpis.operational.totalDistance * 10) / trips.length : 0,
      };
      ...
      kpis.environmental = {
        ...
        emissionsPerKM:
          kpis.operational.totalDistance > 0 ? co2Emissions / kpis.operational.totalDistance : 0,
      };
