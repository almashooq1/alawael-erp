...
const _Driver = require('../models/Driver');
const logger = require('../utils/logger');
const _notificationService = require('./notificationService');
...
class SmartGPSTrackingService {
  ...
  static enrichLocationData(data) {
    ...
    if (previousLocation && previousTimestamp) {
      ...
      // حساب تغيير الاتجاه
      bearingChange = this.calculateBearingChange(previousLocation.bearing, bearing);
    }
  ...
  static generateSafetyAlerts(vehicle, enrichedData, trip) {
    ...
    if (speed > speedLimit) {
      const severity =
        speed > speedLimit + 30 ? 'critical' : speed > speedLimit + 10 ? 'high' : 'medium';
      alerts.push({
        type: 'speeding',
        severity,
  ...
  static generateMaintenanceAlerts(vehicle) {
    ...
    const daysUntilMaintenance = nextMaintenanceDate
      ? Math.floor((nextMaintenanceDate - new Date()) / (1000 * 60 * 60 * 24))
      : null;
  ...
  static async predictETA(vehicle, destination, route = null) {
    try {
      ...
      // درجة الثقة
      const confidence = this.calculateETAConfidence(vehicle, effectiveSpeed, remainingDistance);
  ...
  static addToLocationHistory(vehicle, enrichedData) {
    ...
    if (vehicle.gpsTracking.locationHistory.length > 10000) {
      vehicle.gpsTracking.locationHistory = vehicle.gpsTracking.locationHistory.slice(-10000);
    }
  }
  ...
  static getSpeedLimitByZone(enrichedData) {
    ...
    const { latitude: _latitude, longitude: _longitude } = enrichedData;
   ...
   * عامل حركة المرور
   */
  static async getTrafficFactor(_start, _end) {
   ...
   * عامل الطقس
   */
  static async getWeatherFactor(_location) {
  ...
  static findOptimalSequence(start, pickupPoints, dropoffPoints) {
    ...
    const _allPoints = [start, ...pickupPoints, ...dropoffPoints];
    ...
    while (remaining.length > 0) {
      ...
      let minDist = this.calculateDistanceHaversine(current[1], current[0], nearest[1], nearest[0]);

      remaining.forEach((point, idx) => {
        const dist = this.calculateDistanceHaversine(current[1], current[0], point[1], point[0]);
        if (dist < minDist) {
          minDist = dist;
          nearest = point;
        }
