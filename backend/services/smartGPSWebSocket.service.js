...
class SmartGPSWebSocketService {
  ...
  static sendToDriverNotification(driverId, notification) {
    ...
    const driverClients = Array.from(this.connectedClients.values()).filter(
      client => client.userId === driverId && client.isActive
    );
  ...
  static alertFleetManager(alert) {
    const managers = Array.from(this.connectedClients.values()).filter(
      client => client.userType === 'fleet_manager' && client.isActive
    );
   ...
   */
  static cleanupIdleSessions(maxIdleTime = 30 * 60 * 1000) {
    // 30 دقيقة
    const now = new Date();
  ...
  static getClientsByType() {
    ...
    this.connectedClients.forEach(client => {
      if (Object.prototype.hasOwnProperty.call(breakdown, client.userType)) {
  ...
  static getConnectionStatistics() {
    return {
      ...
      avgSubscriptionsPerClient:
        Array.from(this.connectedClients.values()).reduce(
          (sum, client) => sum + client.subscribedVehicles.size,
          0
