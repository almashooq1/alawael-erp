...
class SmartNotificationsService extends EventEmitter {
  ...
  getUserNotifications(userId, options = {}) {
    const { status = null, limit = 50, offset = 0, sortBy = 'createdAt' } = options;
  ...
  getPerformanceReport(userId = null) {
    ...
    const report = {
      ...
      deliveredRate:
        (notifications.filter(n => n.engagement.delivered).length / notifications.length) * 100 ||
        0,
      readRate:
        (notifications.filter(n => n.engagement.read).length / notifications.length) * 100 || 0,
      clickRate:
        (notifications.filter(n => n.engagement.clicked).length / notifications.length) * 100 || 0,
      averageScore:
        notifications.reduce((sum, n) => sum + n.aiScore, 0) / notifications.length || 0,
      channelDistribution: {},
      typeDistribution: {},
    };
