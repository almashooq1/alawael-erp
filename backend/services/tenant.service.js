...
class TenantService extends EventEmitter {
  ...
  createTenant(tenantData) {
    try {
      const { name, slug, email, planType = 'starter', subdomain, metadata = {} } = tenantData;
  ...
  getStatistics() {
    return {
      ...
      suspendedTenants: Array.from(this.tenants.values()).filter(t => t.status === 'suspended')
        .length,
      archivedTenants: Array.from(this.tenants.values()).filter(t => t.status === 'archived')
        .length,
