      ...
      const performance = await TrafficAccidentReport.aggregate([
        ...
        },
        {
          $group: {
            ...
            inProgressCount: {
              $sum: {
                $cond: [{ $eq: ['$investigation.status', 'in_progress'] }, 1, 0],
              },
            },
            avgResolutionTime: {
              $avg: {
                $cond: [
                  { $gte: ['$investigation.completedDate', null] },
                  {
                    $divide: [
                      {
                        $subtract: ['$investigation.completedDate', '$investigation.startDate'],
                      },
  ...
  async extractKeyInsights(filters = {}) {
    try {
      ...
      if (stats.criticalSeverity > 0) {
        const criticalPercentage = ((stats.criticalSeverity / stats.totalReports) * 100).toFixed(1);
        insights.push({
          type: 'CRITICAL_CASES',
