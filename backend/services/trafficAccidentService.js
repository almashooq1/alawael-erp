...
class TrafficAccidentService {
  ...
  async completeInvestigation(reportId, investigationResults, userId) {
    try {
      ...
      const { findings, rootCause, contributingFactors, recommendations, primaryCause } =
  ...
  async generatePDFReport(reportId, includeAttachments = false) {
    try {
      ...
      // Header
      doc.fontSize(20).font('Helvetica-Bold').text('تقرير الحادثة المرورية', { align: 'center' });
      doc.moveDown();
      ...
      doc
        .fontSize(11)
        .font('Helvetica')
        .text(`إجمالي الخسائر: ${report.financialImpact.totalLoss} SAR`);
  ...
  async getStatistics(filters = {}) {
    try {
      ...
      const [statistics, statusDistribution, severityDistribution] = await Promise.all([
