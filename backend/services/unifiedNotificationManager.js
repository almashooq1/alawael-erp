...
try {
  ...
  // logger used after it's defined below — deferred to module load
}
const logger = require('../utils/logger');
// Log Twilio availability after logger is loaded
if (!twilio) {
  logger.warn('Twilio module not installed. SMS via Twilio will be unavailable.');
}
...
class UnifiedNotificationManager extends EventEmitter {
  ...
  async sendNotification(userId, notificationData) {
    try {
      ...
      this.emit('notificationError', { userId, error: 'حدث خطأ داخلي' });
      throw error;
    }
  }
    ...
    try {
      ...
      for (const userId of userIds) {
        try {
          ...
          results.push({ userId, status: 'failed', error: 'حدث خطأ داخلي' });
        }
      }
