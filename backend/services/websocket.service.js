...
class WebSocketService {
  ...
  registerTripHandlers(socket) {
    ...
    socket.on('subscribe:trip', async tripId => {
      try {
        ...
        logger.info(`📡 Socket ${socket.id} subscribed to trip ${tripId}`);
      } catch (error) {
  ...
  emitVehicleUpdate(vehicleId, updateData) {
    ...
    logger.info(`📤 Emitted vehicle update for ${vehicleId}`);
  }
  ...
  emitGPSUpdate(vehicleId, locationData) {
    ...
    logger.info(`📍 Emitted GPS update for vehicle ${vehicleId}`);
  }
  ...
  emitTripUpdate(tripId, updateData) {
    ...
    logger.info(`📤 Emitted trip update for ${tripId}`);
  }
  ...
  emitEmergencyAlert(vehicleId, alertData) {
    ...
    logger.info(`🚨 Emergency alert emitted for vehicle ${vehicleId}`);
  }
  ...
  emitLowFuelWarning(vehicleId, fuelLevel) {
    ...
    logger.info(`⚠️ Low fuel warning emitted for vehicle ${vehicleId}`);
  }
  ...
  registerNotificationHandlers(socket) {
    ...
    logger.info(`📢 Socket ${socket.id} joined notification room for user ${socket.userId}`);
