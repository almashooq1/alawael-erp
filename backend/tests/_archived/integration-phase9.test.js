...
describe('Integration Service (Phase 9)', () => {
  ...
  test('configureIntegration should create new integration', async () => {
    ...
    const result = await integrationService.configureIntegration('Slack', 'WEBHOOK', {
      url: 'http://test.com',
    });
  ...
  test('configureIntegration should update existing integration', async () => {
    ...
    const _result = await integrationService.configureIntegration('Slack', 'WEBHOOK', {
  ...
  test('triggerWebhook should fail if integration is inactive', async () => {
    ...
    await expect(integrationService.triggerWebhook('TestHook', {})).rejects.toThrow(
