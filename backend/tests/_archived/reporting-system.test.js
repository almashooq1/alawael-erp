...
describe('Advanced Reporting System', () => {
  ...
  describe('ReportBuilder', () => {
    it('should build custom template fluently', () => {
      const _customTemplate = reportingService
        .builder('customSales')
      ...
      expect(_customTemplate.name).toBe('customSales');
      expect(_customTemplate.title).toBe('Custom Sales Report');
      expect(_customTemplate.fields.length).toBe(3);
      expect(_customTemplate.options.includeSummary).toBe(true);
    });
    ...
    it('should create valid template from builder', () => {
      ...
      const validation = customTemplate.validateData([{ id: '1', name: 'Test' }]);
  ...
  describe('ReportScheduler', () => {
    ...
    it('should filter scheduled reports by enabled status', () => {
      const _schedule1 = reportingService.scheduleReport('active', {
        templateName: 'sales',
        frequency: 'daily',
        time: '09:00',
      });
  ...
  describe('ReportingService', () => {
    it('should register and retrieve templates', () => {
      const newTemplate = new ReportTemplate('test', 'custom', 'Test Report', 'Test Description', [
        { key: 'id', label: 'ID' },
      ]);
    ...
    it('should generate PDF report', async () => {
      const result = await reportingService.generateReport('sales', mockSalesData, 'pdf');
    ...
    it('should generate Excel report', async () => {
      const result = await reportingService.generateReport('sales', mockSalesData, 'excel');
    ...
    it('should generate CSV report', async () => {
      const result = await reportingService.generateReport('sales', mockSalesData, 'csv');
    ...
    it('should reject invalid template name', async () => {
      try {
        await reportingService.generateReport('nonexistent', mockSalesData, 'pdf');
        throw new Error('Should throw error');
      } catch (error) {
  ...
  describe('Integration Tests', () => {
    it('should generate multiple format reports', async () => {
      const pdfResult = await reportingService.generateReport('sales', mockSalesData, 'pdf');
      const excelResult = await reportingService.generateReport('sales', mockSalesData, 'excel');
      const csvResult = await reportingService.generateReport('sales', mockSalesData, 'csv');
    ...
    it('should handle multiple data types in reports', async () => {
      const result = await reportingService.generateReport('inventory', mockInventoryData, 'xlsx');
