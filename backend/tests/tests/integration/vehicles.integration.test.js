  ...
  describe('GET /api/vehicles/:id - Get Vehicle Details', () => {
    ...
    test('should get vehicle details with populated driver', async () => {
      try {
        ...
        // Driver might not be populated, so check before accessing
        if (response.body.data.driver) {
          expect(response.body.data.driver.name).toBeDefined();
        } else {
          // Driver not populated is acceptable
