/**
 * Auto-generated unit tests for models/BranchTarget.js
...
mockModelFn.modelName = 'BranchTarget';
...
let Model;
try { Model = require('../../models/BranchTarget'); } catch(e) { /* load fail */ }

describe('models/BranchTarget', () => {
  ...
  test('static "getMonthlyTargets" attached to schema', () => {
    if (!Model) return;
    // Check if static was assigned during module load
    const fn = mockSchema.statics['getMonthlyTargets'];
    if (fn) expect(typeof fn).toBe('function');
    else expect(true).toBe(true); // may not have been assigned in mocked env
  });

  test('method "getAchievement" attached to schema', () => {
    if (!Model) return;
    const fn = mockSchema.methods['getAchievement'];
    if (fn) expect(typeof fn).toBe('function');
