...
// Auto-generated unit test for middleware/advancedAuth.js
...
let mw;
try { mw = require('../../middleware/advancedAuth'); } catch (e) { mw = null; }

describe('middleware/advancedAuth.js', () => {
  ...
  test('action exists and is callable', async () => {
    if (!mw || typeof mw.action !== 'function') return;
    const req = mockReq();
    const res = mockRes();
    const next = jest.fn();
    try {
      const result = mw.action(req, res, next);
      if (result && typeof result === 'function') {
        // Factory pattern — invoke returned middleware
        try { await result(req, res, next); } catch (e) { /* expected */ }
      } else if (result && typeof result.then === 'function') {
