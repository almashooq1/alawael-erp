...
jest.mock('../../models/LeaveRequest', () => ({
  LeaveRequest: Object.assign(
    jest.fn().mockImplementation(() => ({ save: jest.fn().mockResolvedValue({ _id: 'id1' }) })),
    mockLeaveRequestChain
  ),
  LeaveBalance: Object.assign(
    jest.fn().mockImplementation(() => ({ save: jest.fn().mockResolvedValue({ _id: 'id1' }) })),
    mockLeaveRequestChain
  ),
...
describe('leaveManagement.service service', () => {
  ...
  test('initializeYearlyBalances is callable', async () => {
    ...
    let r;
    try {
      r = await svc.initializeYearlyBalances({});
    } catch (e) {
      r = e;
    }
    expect(r).toBeDefined();
  ...
  test('calculateWorkingDays is callable', async () => {
    ...
    let r;
    try {
      r = await svc.calculateWorkingDays({});
    } catch (e) {
      r = e;
    }
    expect(r).toBeDefined();
