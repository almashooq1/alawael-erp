...
jest.mock(
  '../../services/mobileDeviceManager.service',
  () => new Proxy({}, { get: () => jest.fn().mockResolvedValue({}) })
);
jest.mock(
  '../../services/offlineSyncManager.service',
  () => new Proxy({}, { get: () => jest.fn().mockResolvedValue({}) })
);
jest.mock(
  '../../services/pushNotificationOptimizer.service',
  () => new Proxy({}, { get: () => jest.fn().mockResolvedValue({}) })
);
jest.mock(
  '../../services/mobileAnalytics.service',
  () => new Proxy({}, { get: () => jest.fn().mockResolvedValue({}) })
);
...
let ctrl;
try {
  ctrl = require('../../controllers/mobileApp.controller');
} catch {
  ctrl = null;
}
