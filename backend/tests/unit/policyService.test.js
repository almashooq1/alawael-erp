...
jest.mock('../../models/Policy', () => ({
  ...
  PolicyAcknowledgement: Object.assign(
    jest.fn().mockImplementation(() => ({ save: jest.fn().mockResolvedValue({ _id: 'id1' }) })),
    mockPolicyChain
  ),
}));
...
jest.mock('../../models/PolicyAcknowledgement', () => ({
  Policy: Object.assign(
    jest.fn().mockImplementation(() => ({ save: jest.fn().mockResolvedValue({ _id: 'id1' }) })),
    mockPolicyAcknowledgementChain
  ),
  PolicyAcknowledgement: Object.assign(
    jest.fn().mockImplementation(() => ({ save: jest.fn().mockResolvedValue({ _id: 'id1' }) })),
    mockPolicyAcknowledgementChain
...
describe('policyService service', () => {
  ...
  test('sendForAcknowledgement is callable', async () => {
    ...
    let r;
    try {
      r = await svc.sendForAcknowledgement({});
    } catch (e) {
      r = e;
    }
    expect(r).toBeDefined();
  ...
  test('acknowledgePolicies is callable', async () => {
    ...
    let r;
    try {
      r = await svc.acknowledgePolicies({});
    } catch (e) {
      r = e;
    }
    expect(r).toBeDefined();
  ...
  test('getPendingAcknowledgements is callable', async () => {
    ...
    let r;
    try {
      r = await svc.getPendingAcknowledgements({});
    } catch (e) {
      r = e;
    }
    expect(r).toBeDefined();
  ...
  test('getAcknowledgementReports is callable', async () => {
    ...
    let r;
    try {
      r = await svc.getAcknowledgementReports({});
    } catch (e) {
      r = e;
    }
    expect(r).toBeDefined();
