...
class VehicleManagementService extends EventEmitter {
  ...
  async getVehicle(vehicleId) {
    return this.Vehicle.findById(vehicleId).populate('assignment.branch');
  }
   ...
   * Update vehicle
   */
  async updateVehicle(vehicleId, updates, _userId) {
   ...
   */
  async updateVehicleStatus(vehicleId, status, _userId, reason = '') {
    const vehicle = await this.Vehicle.findByIdAndUpdate(
      vehicleId,
  ...
  async getAvailableVehicles(options = {}) {
    ...
    return this.Vehicle.find(filter).populate('assignment.branch');
  }

  /**
   * Assign driver to vehicle
   */
  async assignDriver(vehicleId, driverData, _userId) {
    const vehicle = await this.Vehicle.findByIdAndUpdate(
      vehicleId,
      {
   ...
   */
  async releaseDriver(vehicleId, _userId) {
    const vehicle = await this.Vehicle.findByIdAndUpdate(
      vehicleId,
      {
...
module.exports = VehicleManagementService;
