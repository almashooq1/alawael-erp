    ...
    steps: [
      {
        ...
        assignment: {
          type: {
            type: String,
            enum: ['user', 'role', 'group', 'manager', 'previous_assignee', 'formula'],
          },
          assignee: String,
...
class IntelligentWorkflowEngine {
  ...
  async createTask(instance, step, definition) {
    // Determine assignee
    const assignee = await this.resolveAssignee(instance, step, definition);
   ...
   */
  async resolveAssignee(instance, step, _definition) {
  ...
  async getUserTasks(userId, status = null) {
    ...
    return TaskInstance.find(query).populate('workflowInstance').sort({ createdAt: -1 });
  }
  ...
  async executeIntegration(instance, step, definition) {
    try {
      ...
      if (config.type === 'api' || config.type === 'webhook') {
        ...
        const _body = JSON.stringify(config.body || {}).replace(/\{\{(\w+)\}\}/g, (match, key) => {
      ...
      if (task) {
        task.retries += 1;
        task.lastError = 'حدث خطأ داخلي';
        await task.save();

        if (task.retries < task.maxRetries) {
          // Retry after delay
          setTimeout(
            () => this.executeIntegration(instance, step, definition),
            step.integration.retryDelay
          );
        }
      }
    }
  ...
  async findUserByRole(_roleId) {
    return null;
  }
  async findUserFromGroup(_groupId) {
    return null;
  }
  async getUserManager(_userId) {
    return null;
  }
  async evaluateFormula(_instance, _formula) {
    return null;
  }
  async notifyAssignee(_task, _step) {
    return true;
  }
  async sendNotifications(_instance, _step) {
    return true;
  }
  async sendCompletionNotification(_instance) {
    return true;
  }
  async sendEscalationNotification(_task) {
    return true;
  }
}
