    ...
    steps: [
      {
        ...
        type: {
          type: String,
          enum: ['task', 'decision', 'parallel', 'delay', 'notification'],
          required: true,
        },
...
class WorkflowEngine {
  ...
  registerDefaultActions() {
    ...
    // Database action
    this.registerAction('dbUpdate', async (params, _context) => {
      const { model: _model, query: _query, update: _update } = params;
      // Perform database update
      return { updated: true };
    });

    // API action
    this.registerAction('apiCall', async (params, _context) => {
    ...
    // Log action
    this.registerAction('log', async (params, _context) => {
      logger.info(`[Workflow] ${params.message}`, params.data || '');
      return { logged: true };
    });
  }
  ...
  async executeStep(instance, step) {
    ...
    try {
      ...
      historyEntry.completedAt = new Date();
      historyEntry.error = 'حدث خطأ داخلي';
      instance.history.push(historyEntry);
   ...
   */
  async executeParallel(step, _context) {
  ...
  purchaseOrder: {
    ...
    steps: [
      ...
      },
      {
        ...
        type: 'task',
        action: 'notify',
        nextStep: 'wait_manager',
      },
      {
        id: 'wait_manager',
        name: 'انتظار موافقة المدير',
        type: 'decision',
        conditions: [
