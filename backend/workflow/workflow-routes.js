...
router.post(
  ...
  authMiddleware,
  checkPermission('workflow.publish'),
  async (req, res) => {
    try {
...
router.get('/instances', authMiddleware, checkPermission('workflow.view'), async (req, res) => {
  try {
    ...
    if (!req.user.isAdmin) {
      query.$or = [{ requester: req.user.id }, { currentAssignee: req.user.id }];
    }
...
router.get('/statistics', authMiddleware, checkPermission('workflow.view'), async (req, res) => {
  try {
    ...
    const workflowStats = await workflowEngine.getWorkflowStatistics(
      workflowId,
      startDate,
      endDate
    );
