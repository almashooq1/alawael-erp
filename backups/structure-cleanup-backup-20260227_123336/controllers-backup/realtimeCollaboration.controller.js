...
class RealtimeCollaborationController {
  ...
  static async joinSession(req, res) {
    try {
      ...
      const session = RealtimeCollaborationService.addUserToSession(sessionId, userId, { color });
  ...
  static async exportChangeHistory(req, res) {
    try {
      ...
      const history = RealtimeCollaborationService.exportChangeHistory(sessionId, { userId });
