...
const ApprovalWorkflow = ({ communicationId }) => {
  ...
  const canApprove = stage => {
    ...
    return (
      stageData.approvers.some(a => a.userId === currentUser.id) && stageData.status === 'pending'
    );
  };
              ...
              <Step key={stage.id}>
                  ...
                  optional={
                    <Typography variant="caption">
                      {stage.status === 'completed' &&
                        `تمت الموافقة في ${formatDate(stage.completedAt)}`}
                      {stage.status === 'pending' && 'قيد المراجعة'}
                      {stage.status === 'rejected' &&
                        `تم الرفض في ${formatDate(stage.completedAt)}`}
                    </Typography>
                  }
                  StepIconProps={{
                    icon:
                ...
                >
                  <Box sx={{ display: 'flex', alignItems: 'center', gap: 1 }}>
                    ...
                    {stage.required && <Chip label="إلزامي" size="small" color="error" />}
                  </Box>
                </StepLabel>
                <StepContent>
                  <Box sx={{ mb: 2 }}>
                      ...
                      {stage.approvers.map(approver => (
                        <ListItem key={approver.id}>
                          <ListItemAvatar>
                            ...
                            >
                              <Avatar src={approver.avatar}>{approver.name?.charAt(0)}</Avatar>
                            </Badge>
                          </ListItemAvatar>
                          <ListItemText
