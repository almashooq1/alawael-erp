...
const BarcodeStatistics = () => {
  ...
  if (!stats) {
    return <Alert severity="info">No statistics available. Generate some barcodes first.</Alert>;
  }
  ...
  return (
    <Box sx={{ width: '100%' }}>
      ...
      <Grid container spacing={2} sx={{ mb: 3 }}>
        ...
        <Grid item xs={12} sm={6} md={3}>
          <StatCard title="Active Barcodes" value={stats.activeBarcodes || 0} color="success" />
        </Grid>
        <Grid item xs={12} sm={6} md={3}>
          <StatCard
            title="Total Scans"
        ...
        <Grid item xs={12} sm={6} md={3}>
          <StatCard title="Inactive Barcodes" value={stats.inactiveBarcodes || 0} color="warning" />
        </Grid>
      </Grid>
                ...
                <Stack spacing={1}>
                  {stats.typeDistribution.map(item => (
                    ...
                    >
                      ...
                      <Chip
                        label={`${item.count} (${Math.round((item.count / stats.totalBarcodes) * 100)}%)`}
                        size="small"
                      />
                    </Box>
                  ))}
