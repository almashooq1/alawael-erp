...
const CompensationStructureManagement = () => {
  ...
  const loadStructures = async () => {
    ...
    try {
      ...
      const response = await axios.get('/api/payroll/compensation/structures', {
  ...
  const handleSubmit = async e => {
    ...
    try {
      ...
      if (editingId) {
        await axios.put(`/api/payroll/compensation/structures/${editingId}`, formData, {
          headers: { Authorization: `Bearer ${token}` },
        });
        alert('تم تحديث الهيكل بنجاح');
      } else {
        await axios.post('/api/payroll/compensation/structures', formData, {
          headers: { Authorization: `Bearer ${token}` },
        });
  ...
  const handleDelete = async id => {
    if (window.confirm('هل تريد حذف هذا الهيكل؟')) {
      try {
        const token = localStorage.getItem('token');
        await axios.delete(`/api/payroll/compensation/structures/${id}`, {
          headers: { Authorization: `Bearer ${token}` },
        });
            ...
            <div key={structure._id} className="structure-card">
              ...
              <div className="card-body">
                ...
                <div className="info-section">
                  ...
                  <ul>
                    <li>
                      ضمان اجتماعي: {structure.mandatoryDeductions?.socialSecurity?.percentage}%
                    </li>
                    <li>
                      تأمين صحي: {structure.mandatoryDeductions?.healthInsurance?.percentage}%
                    </li>
