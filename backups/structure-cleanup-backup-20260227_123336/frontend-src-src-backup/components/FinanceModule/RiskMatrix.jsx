...
const RiskMatrix = ({ organizationId }) => {
  ...
  const handleUpdateRisk = async (riskId, updates) => {
    try {
      const response = await fetch(`/api/finance/risks/${riskId}`, {
        method: 'PUT',
        headers: {
          'Content-Type': 'application/json',
          Authorization: `Bearer ${localStorage.getItem('token')}`,
  ...
  const handleDeleteRisk = async riskId => {
    ...
    try {
      await fetch(`/api/finance/risks/${riskId}`, {
        method: 'DELETE',
        headers: {
          Authorization: `Bearer ${localStorage.getItem('token')}`,
        },
  ...
  const criticalRisksCount = risks.filter(
    r => getRiskSeverity(r.probability, r.impact) === 'critical'
  ).length;

  const highRisksCount = risks.filter(
    r => getRiskSeverity(r.probability, r.impact) === 'high'
  ).length;
            ...
            return (
              <circle
                ...
                style={{
                  cursor: 'pointer',
                  filter:
                    selectedRisk?.id === risk.id ? 'drop-shadow(0 0 8px rgba(0,0,0,0.3))' : 'none',
                }}
              >
    ...
    return (
      <div className="detail-panel">
        <div className="detail-header">
          ...
          <button className="btn-close" onClick={() => setSelectedRisk(null)}>
            ×
          </button>
        </div>
        ...
        <div className="detail-content">
          ...
          <div className="detail-actions">
            ...
            <button className="btn btn-delete" onClick={() => handleDeleteRisk(selectedRisk.id)}>
              🗑️ حذف
            </button>
          </div>
        </div>
        ...
        <div className="mitigation-detail">
          ...
          <div className="mitigation-grid">
            ...
            <div className="mitigation-card">
              ...
              <p>
                {selectedRisk.budget
                  ? selectedRisk.budget.toLocaleString('ar-SA') + ' ر.س'
                  : 'لم تحدد'}
              </p>
            </div>
          </div>
      ...
      <div className="indicators-grid">
        ...
        <div className="indicator-card">
          ...
          <div className="indicator-value">
            {risks.length > 0
              ? Math.round(
                  (risks.filter(r => r.mitigationSteps && r.mitigationSteps.length > 0).length /
                    risks.length) *
                    100
                )
              : 0}
    ...
    <div className="risk-matrix">
      <div className="matrix-header-section">
        ...
        <div className="header-controls">
          ...
          <button className="btn btn-primary" onClick={() => setShowAddRiskModal(true)}>
            ➕ إضافة مخاطر جديد
          </button>
