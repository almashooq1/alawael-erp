        ...
        <div className="types-grid">
          {types.map(type => (
            <div key={type.code} className="type-card">
              ...
              <button className="select-btn">اختر هذا المقياس</button>
            </div>
          ))}
        </div>
      )}
...
export const MeasurementResultForm = ({ beneficiaryId, measurementTypeCode }) => {
  ...
  const handleSubmit = async e => {
    ...
    try {
      const response = await axios.post(`${API_URL}/measurements/results/${beneficiaryId}`, {
        measurementTypeCode,
        ...formData,
      });
    ...
    <div className="measurement-form-container">
      ...
      <form onSubmit={handleSubmit} className="measurement-form">
        ...
        <div className="form-group">
          ...
          <select name="performanceLevel" value={formData.performanceLevel} onChange={handleChange}>
  ...
  const fetchStats = async () => {
    try {
      ...
      const response = await axios.get(
        `${API_URL}/measurements/comprehensive-report/${beneficiaryId}`
      );
      setStats({
