...
export default function PayrollDashboard() {
  ...
  const loadMonthlyPayroll = async () => {
    ...
    try {
      ...
      const response = await axios.get(`/api/payroll/monthly/${month}/${year}`, {
        headers: { Authorization: `Bearer ${token}` },
      });
  ...
  const loadStatistics = async () => {
    try {
      ...
      const response = await axios.get(`/api/payroll/stats/${month}/${year}`, {
        headers: { Authorization: `Bearer ${token}` },
      });
    ...
    <div className="payroll-dashboard">
      ...
      <div className="statistics-grid">
        <div className="stat-card">
          ...
          <strong style={{ color: '#28a745' }}>{formatCurrency(stats.totalGross)}</strong>
        </div>
        <div className="stat-card">
          <h3>الخصومات</h3>
          <strong style={{ color: '#dc3545' }}>{formatCurrency(stats.totalDeductions)}</strong>
        </div>
        <div className="stat-card">
          <h3>الرواتب الصافية</h3>
          <strong style={{ color: '#007bff' }}>{formatCurrency(stats.totalNet)}</strong>
        </div>
        <div className="stat-card">
          <h3>عدد الموظفين</h3>
          ...
          <tbody>
            {monthData.length === 0 ? (
              ...
              monthData.map(payroll => (
                <PayrollRow key={payroll._id} payroll={payroll} onApprove={loadMonthlyPayroll} />
              ))
            )}
          </tbody>
        </table>
              ...
              <tbody>
                {departmentStats.map((dept, idx) => (
                  <tr key={idx}>
                    ...
                    <td style={{ textAlign: 'right' }}>{formatCurrency(dept.avgSalary)}</td>
                  </tr>
                ))}
              </tbody>
            </table>
