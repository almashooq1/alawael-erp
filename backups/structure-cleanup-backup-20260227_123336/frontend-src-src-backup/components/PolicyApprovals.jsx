...
export const PolicyApprovals = ({ userId, userName, userRole }) => {
  ...
  const handleApprovalSubmit = async () => {
    ...
    try {
      ...
      await axios.post(`${API_BASE}/policies/${selectedPolicy.policyId}/${endpoint}`, payload);

      toast.success(approvalAction === 'approve' ? 'تم الموافقة على السياسة' : 'تم رفض السياسة');
  ...
  const isCurrentUserApprover = policy => {
    return policy.approvals.some(a => a.approverRole === userRole && a.status === 'PENDING');
  };
                    ...
                    {pendingPolicies.map(policy => (
                      <tr key={policy._id}>
                        <td>
                          ...
                          <br />
                          <small className="text-muted">{policy.policyNameAr}</small>
                        </td>
                        <td>{policy.policyType}</td>
                        <td>{new Date(policy.createdAt).toLocaleDateString('ar')}</td>
                        <td>
                          <ApprovalStatusChart approvals={policy.approvals} />
                        </td>
          ...
          {selectedPolicy && (
            <>
                    ...
                    <Accordion.Item eventKey={idx.toString()} key={idx}>
                      ...
                      <Accordion.Body>
                        {approval.status === 'PENDING' ? (
                          ...
                          <>
                            <p>
                              <strong>الموافق الفعلي:</strong> {approval.approverName}
                            </p>
                            <p>
                              <strong>التاريخ:</strong>{' '}
                              {new Date(approval.approvalDate).toLocaleDateString('ar')}
                            </p>
                            {approval.comments && (
                              <p>
                                <strong>الملاحظات:</strong> {approval.comments}
                              </p>
                            )}
                          </>
                        )}
              ...
              <Form.Group className="mb-3">
                <Form.Label>{approvalAction === 'approve' ? 'ملاحظاتك:' : 'سبب الرفض:'}</Form.Label>
                <Form.Control
                  as="textarea"
                  rows={3}
                  value={comments}
