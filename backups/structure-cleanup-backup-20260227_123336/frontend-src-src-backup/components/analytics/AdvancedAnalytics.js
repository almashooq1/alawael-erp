              ...
              <Stack spacing={1}>
                {revenueData.slice(-3).map((item, idx) => {
                  ...
                  return (
                    <Box
                      key={idx}
                      sx={{ pb: 2, borderBottom: idx < 2 ? '1px solid #e0e0e0' : 'none' }}
                    >
                      <Box sx={{ display: 'flex', justifyContent: 'space-between', mb: 1 }}>
                        <Typography variant="body2">{item.month}</Typography>
                        <Chip
                          label={`${performance}%`}
                          color={performance >= 100 ? 'success' : 'warning'}
                          size="small"
                        />
                      </Box>
                      <LinearProgress
                        variant="determinate"
                        value={Math.min(performance, 110)}
                  ...
                  <Box key={idx}>
                    <Box sx={{ display: 'flex', justifyContent: 'space-between', mb: 0.5 }}>
                      ...
                      <Chip
                        ...
                        color={item.score >= item.target ? 'success' : 'warning'}
                        size="small"
                      />
                    </Box>
                    <LinearProgress
