...
const WorkflowAutomation = () => {
  ...
  const [automationRules, setAutomationRules] = useState([
    {
      id: 'RULE001',
      trigger: 'عند إنشاء فاتورة',
      condition: 'المبلغ < 5000',
      action: 'اعتماد تلقائي',
      status: 'نشط',
    },
    {
      id: 'RULE002',
      trigger: 'في نهاية الشهر',
      condition: 'دائما',
      action: 'إرسال تقرير',
      status: 'نشط',
    },
    {
      id: 'RULE003',
      trigger: 'عند تعيين مهمة',
      condition: 'الأولوية = عالي',
      action: 'إرسال إشعار',
      status: 'نشط',
    },
  ]);
              ...
              {executions.map(exec => (
                <TableRow key={exec.id} hover>
                  ...
                  <TableCell>
                    <Chip
                      label={exec.status}
                      color={exec.status === 'مكتمل' ? 'success' : 'warning'}
                      size="small"
                    />
                  </TableCell>
                  <TableCell>
                    <Chip
                      label={exec.result}
                      color={exec.result === 'نجح' ? 'success' : 'error'}
                      size="small"
                    />
                  </TableCell>
                </TableRow>
