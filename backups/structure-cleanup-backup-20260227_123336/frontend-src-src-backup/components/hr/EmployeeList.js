...
const EmployeeList = ({ employees, onRefresh, onEdit }) => {
  ...
  const handleDelete = async emp => {
    if (
      window.confirm(
        `هل أنت متأكد من حذف الموظف ${emp.firstName} ${emp.lastName}؟\n\n⚠️ يمكنك استرجاعه لاحقاً.`
      )
    ) {
      try {
        setLoading(true);
  ...
  const activeFilterCount =
    (departmentFilter !== 'الكل' ? 1 : 0) + (statusFilter !== 'الكل' ? 1 : 0);
  ...
  return (
    <>
            ...
            <TableRow>
              <TableCell padding="checkbox" sx={{ color: 'white' }}>
                <Checkbox
                  ...
                  checked={
                    paginatedEmployees.length > 0 && selected.length === paginatedEmployees.length
                  }
                ...
                return (
                  ...
                  >
                    ...
                    <TableCell align="center">
                      <Rating
                        value={emp.performance?.currentRating || 0}
                        readOnly
                        size="small"
                        precision={0.5}
                      />
      ...
      >
        <MenuItem
          onClick={() => handleShowDetails(selectedEmployee)}
          ...
          {selectedEmployee && (
            <Stack spacing={3}>
              ...
              <Paper elevation={0} sx={{ p: 3, backgroundColor: '#fff9f0', borderRadius: 2 }}>
                ...
                >
                  <Rating
                    value={selectedEmployee.performance?.currentRating || 0}
                    readOnly
                    size="large"
                    precision={0.5}
                    sx={{ mb: 1 }}
