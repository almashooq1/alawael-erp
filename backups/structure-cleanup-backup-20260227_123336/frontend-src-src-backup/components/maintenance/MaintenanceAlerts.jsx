    ...
    <div className={`alert-item alert-severity-${alert.severity}`}>
      ...
      <div className="alert-content">
        ...
        <div className="alert-details">
          {alert.vehicleId && (
            <span className="detail-item">🚗 {getVehicleDisplay(alert.vehicleId, vehicles)}</span>
          )}
          {alert.dueDate && (
          ...
          {alert.estimatedCost && (
            <span className="detail-item">💰 {alert.estimatedCost} ريال</span>
          )}
        </div>
      </div>
  ...
  return (
    <div className="maintenance-alerts">
      ...
      {loading ? (
        ...
        <div className="alert alert-success">✅ لا توجد تنبيهات في هذه الفئة</div>
      )}
      ...
      {alerts.length > 0 && (
        <div className="alerts-tips">
          <h3>💡 النصائح</h3>
          <ul>
            <li>✓ ركز على التنبيهات الحرجة والعالية أولاً</li>
            <li>✓ قم بعمل الصيانة الوقائية قبل تحويل المشاكل لحرجة</li>
            <li>✓ استخدم التنبؤات الذكية لتجنب الأعطال المفاجئة</li>
            <li>✓ راقب المخزون الحرج وأعد الطلبات مسبقاً</li>
          </ul>
        </div>
      )}
    </div>
