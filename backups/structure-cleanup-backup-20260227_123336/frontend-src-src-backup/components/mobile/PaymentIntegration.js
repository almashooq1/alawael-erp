    ...
    <Box sx={{ p: 3 }}>
      ...
      <Grid container spacing={2} sx={{ mb: 3 }}>
        {[
          {
            label: 'الرصيد الكلي',
            value: `₪${stats.balance.toLocaleString('ar')}`,
            icon: '💰',
            color: '#4caf50',
          },
          {
            label: 'الإنفاق الكلي',
            value: `₪${stats.totalSpent.toLocaleString('ar')}`,
            icon: '💸',
            color: '#f44336',
          },
          {
            label: 'الإيرادات',
            value: `₪${stats.totalReceived.toLocaleString('ar')}`,
            icon: '📈',
            color: '#2196f3',
          },
          {
            label: 'المتوسط الشهري',
            value: `₪${stats.monthlyAvg.toLocaleString('ar')}`,
            icon: '📊',
            color: '#ff9800',
          },
        ].map((stat, idx) => (
      ...
      <Grid container spacing={2} sx={{ mb: 3 }}>
        {paymentMethods.map(method => (
          <Grid item xs={12} sm={6} key={method.id}>
            <Card
              sx={{ borderRadius: 2, borderTop: method.isDefault ? '4px solid #667eea' : 'none' }}
            >
              <CardContent>
                <Box
                  sx={{
                ...
                >
                  <Box sx={{ display: 'flex', gap: 1.5, alignItems: 'flex-start' }}>
                    <Box
                      sx={{ fontSize: 28, color: method.type === 'card' ? '#667eea' : '#4caf50' }}
                    >
                      {method.type === 'card' ? '🏧' : '👛'}
                    </Box>
                    <Box>
                      ...
                      {method.type === 'card' ? (
                        ...
                        <>
                          ...
                          <Typography
                            variant="caption"
                            sx={{ fontWeight: 600, color: '#4caf50', display: 'block' }}
                          >
            ...
            <Paper sx={{ p: 2.5, borderRadius: 2 }}>
              <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                ...
                <Box sx={{ textAlign: 'right' }}>
                  ...
                  <Chip
                    label={payment.frequency === 'monthly' ? 'شهري' : 'سنوي'}
                    size="small"
                    variant="outlined"
                    sx={{ mt: 0.5 }}
                  />
        ...
        <Table>
          ...
          <TableBody>
            {transactions.map(trans => (
              <TableRow
                key={trans.id}
                sx={{ '&:hover': { backgroundColor: '#f8f9ff' }, cursor: 'pointer' }}
              >
                <TableCell sx={{ fontWeight: 600 }}>{trans.date}</TableCell>
                <TableCell>{trans.description}</TableCell>
                <TableCell
                  sx={{ fontWeight: 700, color: trans.amount > 0 ? '#4caf50' : '#f44336' }}
                >
                  {trans.amount > 0 ? '+' : ''}
                  {trans.amount} ₪
                </TableCell>
