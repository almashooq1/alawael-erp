...
const ThreatDetection = () => {
  ...
  const vulnerabilities = [
    ...
    {
      id: '2',
      name: 'SSL Certificate قريب من الانتهاء',
      severity: 'medium',
      cves: 'N/A',
      fixed: false,
      found: '2026-01-15',
    ...
    {
      id: '3',
      name: 'قاعدة البيانات بدون نسخة احتياطية',
      severity: 'critical',
      cves: 'N/A',
      fixed: false,
      found: '2026-01-01',
    ...
    <Box sx={{ p: 3 }}>
      ...
      {tabValue === 0 && (
        <Stack spacing={2}>
          <Alert severity="warning" sx={{ borderRadius: 2 }}>
            ⚠️ <strong>تحذير:</strong> تم كشف {threats.length} تهديداً في آخر 24 ساعة.{' '}
            {threats.filter(t => t.status === 'blocked').length} منها تم حظره بنجاح.
          </Alert>
      ...
      {tabValue === 1 && (
        <Stack spacing={2}>
          ...
          {vulnerabilities.map(vuln => (
            <Card
              key={vuln.id}
              sx={{ borderRadius: 2, borderLeft: `4px solid ${getSeverityColor(vuln.severity)}` }}
            >
              <CardContent>
                <Box
                  sx={{
      ...
      <Dialog open={openDialog} onClose={() => setOpenDialog(false)} maxWidth="sm" fullWidth>
        ...
        <DialogContent sx={{ mt: 2 }}>
          <Stack spacing={2}>
            <Alert severity="info">
              سيتم فحص جميع الملفات والإعدادات للتأكد من عدم وجود ثغرات أمنية.
            </Alert>
            <Typography variant="body2">
              هذا الفحص قد يستغرق 15-30 دقيقة. يمكنك متابعة التقدم من لوحة المعلومات.
            </Typography>
          </Stack>
        </DialogContent>
