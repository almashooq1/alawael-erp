...
export default function PerformanceDashboard() {
  ...
  const { clinicMetrics, therapistMetrics, monthlyTrend, documentationStats, noShowTrend } =
    metrics;
  ...
  return (
    <Box sx={{ p: 3 }}>
                ...
                {therapistMetrics.slice(0, 10).map((therapist, index) => (
                  <TableRow key={therapist._id}>
                    ...
                    <TableCell align="right">
                      {(therapist.completionRate || 0).toFixed(1)}%
                    </TableCell>
      ...
      {/* Action Section */}
      <Box sx={{ mt: 3 }}>
        <Button variant="contained" color="primary" onClick={fetchMetrics} sx={{ mr: 1 }}>
          Refresh Data
        </Button>
