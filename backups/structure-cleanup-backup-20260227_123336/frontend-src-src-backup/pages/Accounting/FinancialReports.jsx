...
const FinancialReports = () => {
  ...
  const fetchReport = async () => {
    ...
    try {
      ...
      const response = await axios.get(`${API_URL}/api/accounting/reports/${reportType}`, {
  ...
  const downloadPDF = async () => {
    try {
      const token = localStorage.getItem('token');
      const response = await axios.get(`${API_URL}/api/accounting/reports/${reportType}/pdf`, {
        headers: { Authorization: `Bearer ${token}` },
        params: { startDate, endDate },
        responseType: 'blob',
      });
  ...
  const downloadExcel = async () => {
    try {
      const token = localStorage.getItem('token');
      const response = await axios.get(`${API_URL}/api/accounting/reports/${reportType}/excel`, {
        headers: { Authorization: `Bearer ${token}` },
        params: { startDate, endDate },
        responseType: 'blob',
      });
          ...
          <Table>
            ...
            <TableBody>
              ...
              <TableRow sx={{ bgcolor: 'grey.100', fontWeight: 'bold' }}>
                ...
                <TableCell align="right">
                  <strong>{totalDebit.toLocaleString('ar-SA')}</strong>
                </TableCell>
                <TableCell align="right">
                  <strong>{totalCredit.toLocaleString('ar-SA')}</strong>
                </TableCell>
              </TableRow>
            </TableBody>
          </Table>
      ...
      <Paper sx={{ p: 3 }}>
        ...
        {reportData && (
          <Box sx={{ display: 'flex', gap: 2, mb: 3 }}>
            <Button variant="outlined" startIcon={<PdfIcon />} onClick={downloadPDF}>
              تحميل PDF
            </Button>
            <Button variant="outlined" startIcon={<ExcelIcon />} onClick={downloadExcel}>
              تحميل Excel
            </Button>
          </Box>
        )}
        ...
        {loading ? (
          ...
          <>
            ...
            {['cash-flow', 'general-ledger', 'aged-receivables', 'aged-payables'].includes(
              reportType
            ) && (
