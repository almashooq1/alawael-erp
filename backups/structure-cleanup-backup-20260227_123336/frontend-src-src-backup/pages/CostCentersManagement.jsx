...
const CostCentersManagement = () => {
  ...
  const BudgetBar = ({ center }) => {
    ...
    const color = utilization > 90 ? 'error' : utilization > 75 ? 'warning' : 'success';
  ...
  const renderDialogContent = () => {
    ...
    if (dialogType === 'cost') {
      return (
        <Grid container spacing={2}>
          <Grid item xs={12}>
            <Alert severity="info">تسجيل تكلفة لمركز: {selectedCenter?.name}</Alert>
          </Grid>
          <Grid item xs={12} md={6}>
            <TextField
              fullWidth
    ...
    if (dialogType === 'revenue') {
      return (
        <Grid container spacing={2}>
          <Grid item xs={12}>
            <Alert severity="success">تسجيل إيراد لمركز: {selectedCenter?.name}</Alert>
          </Grid>
          <Grid item xs={12} md={6}>
            <TextField
              fullWidth
    ...
    if (dialogType === 'kpi') {
      return (
        <Grid container spacing={2}>
          <Grid item xs={12}>
            <Alert severity="info">تحديث مؤشر أداء لمركز: {selectedCenter?.name}</Alert>
          </Grid>
          <Grid item xs={12}>
            <TextField
              fullWidth
          ...
          <TableBody>
            {costCenters.map(center => (
              <TableRow key={center._id}>
                ...
                <TableCell align="center">
                  <Tooltip title="عرض">
                    <IconButton size="small" onClick={() => handleOpenDialog('view', center)}>
                      <Visibility />
                    </IconButton>
                  </Tooltip>
                  <Tooltip title="تعديل">
                    <IconButton size="small" onClick={() => handleOpenDialog('edit', center)}>
                      <Edit />
                    </IconButton>
                  </Tooltip>
                  <Tooltip title="تسجيل تكلفة">
                    <IconButton size="small" onClick={() => handleOpenDialog('cost', center)}>
                      <TrendingDown />
                    </IconButton>
                  </Tooltip>
                  {(center.type === 'revenue' || center.type === 'profit') && (
                    <Tooltip title="تسجيل إيراد">
                      <IconButton size="small" onClick={() => handleOpenDialog('revenue', center)}>
                        <TrendingUp />
                      </IconButton>
                    </Tooltip>
                  )}
