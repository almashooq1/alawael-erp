...
class SCFHSVerificationService {
  ...
  validateLicenseInput(licenseData) {
    ...
    if (
      !licenseData.professionalFirstName ||
      typeof licenseData.professionalFirstName !== 'string'
    ) {
  ...
  verifyLicenseFormat(licenseData) {
    ...
    if (
      licenseData.professionalFirstName.length < 2 ||
      licenseData.professionalFirstName.length > 50
    ) {
      errors.push({
    ...
    if (
      licenseData.specializationCode &&
      !/^[A-Z0-9]{3,10}$/i.test(licenseData.specializationCode)
    ) {
      errors.push({
  ...
  async checkCompliance(licenseData) {
    try {
      ...
      const response = await axios.get(
        `${this.verificationEndpoint}/compliance/${licenseData.licenseNumber}`
      );
  ...
  performFraudDetection(licenseData, databaseResult) {
    ...
    const expiryDate = new Date(licenseData.licenseExpiryDate);
    const validityYears = (expiryDate - issueDate) / (1000 * 60 * 60 * 24 * 365);
    ...
    for (const [, category] of Object.entries(specializations)) {
      if (category.categories) {
        for (const subCat of category.categories) {
          if (
            subCat.id === licenseData.specialization ||
            subCat.name === licenseData.specialization
          ) {
            foundSpec = true;
            specData = subCat;
            break;
