...
router.post('/forecast/orders', async (req, res) => {
  try {
    ...
    const forecast = await MLService.predictOrderDemand(historicalOrders, days);
...
router.post('/churn/predict', async (req, res) => {
  try {
    ...
    const customersWithMetrics = customers.map(customer => {
      const lastOrderDate = customer.lastOrderDate ? new Date(customer.lastOrderDate) : new Date(0);
      const daysInactive = Math.floor((new Date() - lastOrderDate) / (1000 * 60 * 60 * 24));
    ...
    const prediction = await MLService.predictCustomerChurn(customersWithMetrics);

    // Sort by risk descending
    const sorted = {
      ...prediction,
      riskAssessment: prediction.riskAssessment.sort((a, b) => b.churnRisk - a.churnRisk),
    };
...
router.post('/recommendations/products', async (req, res) => {
  try {
    ...
    const allProducts = await Product.find().select('_id category price name').lean();
...
router.post('/inventory/optimize', async (req, res) => {
  try {
    ...
    const products = await Product.find().select('_id currentStock unitCost').lean();
    ...
    const productsWithDemand = await Promise.all(
      products.map(async product => {
        ...
        const dailyDemand = Array(90).fill(0);
        orders.forEach(order => {
    ...
    const optimization = await MLService.optimizeInventory(productsWithDemand);
...
router.get('/insights/summary', async (req, res) => {
  try {
    ...
    const [ordersData, customersData, productsData] = await Promise.all([
      Order.find({ createdAt: { $gte: thirtyDaysAgo } }).select('createdAt totalAmount'),
