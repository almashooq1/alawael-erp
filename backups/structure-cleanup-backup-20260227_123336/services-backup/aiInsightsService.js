...
class AIInsightsService {
  ...
  generateInsights(kpiId, kpiData, dataPoints = []) {
    ...
    if (dataPoints.length > 5) {
      ...
      const recentVariance =
        recentValues.reduce((a, b) => a + Math.pow(b - recentMean, 2)) / recentValues.length;
  ...
  predictFutureValue(dataPoints = [], periods = 4) {
    ...
    return {
      ...
      predictions,
      averagePrediction: Math.round(
        predictions.reduce((a, b) => a + b.predictedValue, 0) / predictions.length
      ),
    };
  }
  ...
  detectPatterns(dataPoints = [], periodLength = 7) {
    ...
    patterns.push({
      ...
      variation: (
        ((maxDay.averageValue - minDay.averageValue) / minDay.averageValue) *
        100
      ).toFixed(1),
    });
      ...
      switch (insight.type) {
        ...
        case 'volatility':
          recommendations.push({
            ...
            estimatedImpact:
              'Reducing volatility could improve predictability and target achievement',
          });
          break;
  ...
  generateOutcomePrediction(kpis = []) {
    ...
    return {
      ...
      recommendation:
        totalConfidence / predictions.length > 75
          ? 'Trust predictions for planning'
          : 'Gather more data for reliable predictions',
    };
  }
