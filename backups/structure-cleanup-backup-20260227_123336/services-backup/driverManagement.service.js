...
class DriverManagementService {
  ...
  static async calculateReliabilityScore(driverId) {
    try {
      ...
      const reliabilityScore =
        ...
        (driver.performance.attendanceScore * 0.1 || 0);
      ...
      vehicles.forEach(vehicle => {
        if (vehicle.maintenanceHistory && vehicle.maintenanceHistory.length > 0) {
          const lastMaintenanceDate =
            vehicle.maintenanceHistory[vehicle.maintenanceHistory.length - 1].maintenanceDate;
  ...
  static async predictPerformanceTrend(driverId) {
    try {
      ...
      const recentTrips = await Trip.find({ driver: driverId }).sort({ createdAt: -1 }).limit(10);
    ...
    try {
      ...
      const report = {
        ...
        violations: {
          ...
          other:
            driver.violations.totalViolations -
            driver.violations.speedingIncidents -
            driver.violations.accidents,
        },
