...
class GratuityService {
  ...
  static async approveGratuity(gratuityId, approvedBy, remarks = '') {
    try {
      ...
      await this.createAuditLog(gratuityId, gratuity.employeeId, 'APPROVED', {
        approvedBy,
        remarks,
      });
  ...
  static async getAuditTrail(gratuityId) {
    try {
      const auditLogs = await GratuityAudit.find({ gratuityId }).sort({ timestamp: -1 });
  ...
  static isEligibleForGratuity(employee, scenario = 'RESIGNATION') {
    ...
    if (
      scenario === 'DEATH' ||
      scenario === 'RETIREMENT' ||
      scenario === 'DISMISSAL_WITHOUT_CAUSE'
    ) {
