...
class WebhookSubscription {
  ...
  getRetryDelay(attemptNumber) {
    const baseDelay = Math.min(
      this.retryPolicy.initialDelay * Math.pow(this.retryPolicy.backoffMultiplier, attemptNumber),
...
class APIIntegrator {
  ...
  async makeRequest(method, url, data = null, endpointName = null) {
    ...
    const resultEndpoint = endpointName || url.split('/').pop();
    return {
      success: true,
      status: 200,
  ...
  getIntegrationHealth() {
    const health = {
      ...
      apis: {
        ...
        endpoints: this.apis.reduce(
          (sum, api) => sum + (api.endpoints?.size || api.endpoints?.length || 0),
          0
        ),
    ...
    health.healthPercentage =
      totalItems > 0 ? ((totalItems - totalErrors) / totalItems) * 100 : 100;
