...
class RBACManager extends EventEmitter {
  ...
  _assignDefaultPermissions() {
    ...
    const managerPermIds = allPermIds.filter(
      pid => pid.includes('user') || pid.includes('resource') || pid.includes('report')
    );
    ...
    const guestPermIds = allPermIds.filter(
      pid => pid === 'perm-resource:read' || pid === 'perm-report:read'
    );
  ...
  getStatistics() {
    try {
      return {
        ...
        roleAssignments: Array.from(this.userRoles.values()).reduce(
