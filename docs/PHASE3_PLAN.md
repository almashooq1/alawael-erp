...
> آخر مزامنة: 2026-05-19 (بعد Wave 123).
...
| **P3.6** | Parent Chatbot            | ✅ **Phase 1+2a+2b مُسلَّم (Waves 120+122+123)** | Wave 120 foundation + Wave 122 context resolver + Wave 123 [parent-chatbot-llm.service.js](../backend/intelligence/parent-chatbot-llm.service.js) LLM classifier (Claude Haiku 4.5، JSON-only output، LRU+TTL cache 5min/256، 1 retry على 5xx، 5s timeout). Hybrid: LLM-PRIMARY → rule-based fallback. 90/90 tests pass. Env-gated `ANTHROPIC_API_KEY`. **classifierSource** على response (llm/llm-cache/rule). | Phase 3: Parent Portal UI                                                          |
