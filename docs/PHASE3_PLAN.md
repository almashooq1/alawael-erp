...
> آخر مزامنة: 2026-05-19 (بعد Wave 122).
...
| **P3.6** | Parent Chatbot            | ⚠️ **Phase 1+2a مُسلَّم (Waves 120+122)** | Wave 120 foundation (classifier + templates + session model + 2 routes) + Wave 122 [parent-chatbot-context.service.js](../backend/intelligence/parent-chatbot-context.service.js) لإملاء placeholders من DB (10 intent resolvers على Appointment/Invoice/Beneficiary/CarePlanVersion/Branch/Employee). 57/57 tests pass. **Forbidden-token guard على قيم runtime** (يرفض dirty token values كذلك). | Phase 2b: LLM-backed classifier (يستبدل rule-based). Phase 3: Parent Portal UI     |
