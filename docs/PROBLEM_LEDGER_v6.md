...
> Updated: 2026-06-21T19:39:00+03:00  
...
| P2-4 | P2       | `[llm-anomaly-history] save failed` in production logs               | `backend/intelligence/llm-anomaly-history.service.js`, `backend/__tests__/wave144-llm-anomaly-history.test.js`        | `error1.log`: `[llm-anomaly-history] save failed:` recurring every ~10 min                                                                                                              | Save path failed silently (logged only message); likely symptom of P0-1/P0-2 DB timeouts | prod-silent-failure              | **DIAGNOSTICS ADDED W1436; pending post-deploy verification** |
...
- P2-4 structured diagnostics added in W1436. Root cause still likely P0-1/P0-2 DB timeouts; verification pending after production DB index build.
...
| `prod-logs`            | ✅ MongoDB timeout root cause identified as missing compound indexes; indexes added in W1426. P2-4 LLM save failure now logs structured diagnostics (W1436); verification pending after P0 DB fix. |
...
| P2-4  | P2       | `[llm-anomaly-history] save failed:` in production logs — likely symptom of P0-1/P0-2 DB timeouts.                                                                  | Diagnostics added in W1436. After P0 DB fix, re-check `error1.log`; new log line includes error name/code, source, total, recordedAt. If persists, inspect `LlmAnomalySnapshot` validation/schema. |
