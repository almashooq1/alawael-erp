...
| `ENABLE_SPEECH_RETENTION_CRON=true`     | Anytime — sweeper safe to enable (W284d)                                         | For full PDPL compliance: `npm install @aws-sdk/client-s3` + set `AWS_REGION`. **Verify**: `cd backend && npm run check:speech-s3-ready` (exits 0 if ready, 1 with per-gap fix instructions). Without these, sweeper falls back to log-only with boot-time WARN. |
