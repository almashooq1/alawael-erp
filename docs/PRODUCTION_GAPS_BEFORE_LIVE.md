...
| **Speech S3 retention purge** | log-only sweeper (W284c)         | `s3Client.send(DeleteObjectCommand)` not called; sweep logs intent only | Storage cost grows unbounded over time. PDPL: data retention policy violated (audio kept past expiry).                          | **PDPL compliance risk** — but only after months without intervention.             | ✗ No marker guard (sweep is data-deletion, not data-emission). Recommended fix: replace log placeholder with real S3 SDK call before pilot Week 2. |
...
| **Speech S3 deletion**            | log-only purge in sweeper                                                                  | Replace placeholder with real `S3Client.DeleteObjectCommand` before pilot Week 2 (PDPL retention)                                                                                                                                      |
...
| `ENABLE_SPEECH_RETENTION_CRON=true` | After replacing log-placeholder S3 purger    | Wire `s3Client.send(DeleteObjectCommand)`                                         |
