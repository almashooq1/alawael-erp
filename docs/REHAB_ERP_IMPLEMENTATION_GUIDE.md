...
describe('Beneficiary API Integration Tests', () => {
  ...
  describe('POST /api/v3/beneficiaries', () => {
    it('يجب تسجيل مستفيد جديد بنجاح', async () => {
      ...
      const res = await request(app).post('/api/v3/beneficiaries').set('Authorization', `Bearer ${authToken}`).send(payload).expect(201);
    ...
    it('يجب رفض الهوية الوطنية المكررة', async () => {
      ...
      await request(app).post('/api/v3/beneficiaries').set('Authorization', `Bearer ${authToken}`).send(payload).expect(409);
    });

    it('يجب رفض الطلب بدون توكن', async () => {
      await request(app).post('/api/v3/beneficiaries').send({ name_ar: 'أي اسم' }).expect(401);
    });
    ...
    it('يجب رفض المعالج الوصول لفرع آخر', async () => {
      ...
      await request(app).post('/api/v3/beneficiaries').set('Authorization', `Bearer ${therapistToken}`).send(payload).expect(403);
    });
  });
...
_وُثِّق بواسطة: ALAWAEL Technology Team | الإصدار 1.0 | شعبان 1447هـ / مارس 2026م_
