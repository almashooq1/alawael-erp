...
| #   | Item                                                                                                                                                                   | Wave                      | Mode     | Why no-regrets                                                                                                                  |
| --- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------- | -------- | ------------------------------------------------------------------------------------------------------------------------------- |
| 1   | **MFA tier retrofit on `iep.routes.js`** — wrap mutating endpoints (sign, status transition) with `requireMfaTier(2)` matching CarePlanVersion's analogous transitions | 🤖 Claude-side autonomous | 1 wave   | Closes ADR-019 layer-2 gap on IEP. Required by Approach B; included free in Approach A or C.                                    |
| 2   | **PDPL TTL declaration** on IndividualEducationPlan + IFSP collections (30-day minimum per W14 baseline)                                                               | 🤖 Claude-side autonomous | 1 wave   | Closes ADR-007 gap. Required by all 3 approaches.                                                                               |
| 3   | **Cross-link FKs** (`CarePlanVersion.linkedIepId`, `IndividualEducationPlan.linkedCarePlanVersionId`) — even Approach A benefits during the migration window           | 🤝 small build            | 1 wave   | Approach B requires this; Approach A's migration script can leverage it; Approach C needs it for the adapter's read-projection. |
| 4   | **Document the 2 frontend IEP paths** (`/iep/` vs `/iep-plan/`) — note which is canonical so future frontend work picks correctly                                      | 🤖 Claude-side autonomous | 0.5 wave | Unrelated to A/B/C but should not be left ambiguous.                                                                            |

**Recommendation**: ship items 1, 2, 4 immediately (Claude-side, MED-LOW risk). Hold item 3 until the meeting decides direction.
