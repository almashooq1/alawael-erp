...
const assessment = await metricsService.administerMetric(
  'FMS-2024',
  {
    id: 'BEN-001',
    age: 7,
...
```javascript
const adlAssessment = await metricsService.administerMetric('ADL-2024', beneficiaryData, {
  'تناول الطعام': 8,
  الاستحمام: 6,
  'ارتداء الملابس': 7,
  التنقل: 5,
...
```javascript
const vinelandAssessment = await metricsService.administerMetric('VABS-2024', beneficiaryData, {
  // التواصل
  الاستقبالية: 25,
  التعبيرية: 22,
  الكتابية: 18,
...
```javascript
const qolAssessment = await metricsService.administerMetric('DQOLS-2024', beneficiaryData, {
  // الاستقلالية
  'القرارات اليومية': 20,
  التنقل: 15,
  'العناية الذاتية': 18,
...
// تخصيص وإضافة هدف
const goalResult = planService.customizeGoalFromBank(planId, 'GM-02', {
  accuracy: 70,
...
const session = planService.recordServiceSession(planId, serviceId, {
  ...
  present: true,
  late: false,
  activities: ['تمارين التوازن على الكرة', 'تدريب المشي على الشريط', 'تمارين تقوية العضلات'],
...
const earlyInterventionProgram = {
  components: ['العلاج الطبيعي', 'العلاج الوظيفي', 'علاج التخاطب', 'التدخل السلوكي', 'دعم الأسرة'],
...
const inclusionProgram = {
  accommodations: ['تكييف المنهج الدراسي', 'وقت إضافي للاختبارات', 'وسائل تعليمية بديلة', 'مساعد تربوي متخصص'],
  supportServices: ['غرفة مصادر', 'خدمات التخاطب', 'العلاج الوظيفي', 'الإرشاد النفسي'],
};
```
...
const hrsdIntegration = {
  services: ['تسجيل ذوي الإعاقة', 'إصدار بطاقات الإعاقة', 'صرف الإعانات', 'برامج التأهيل المهني'],
  endpoints: {
...
const mohIntegration = {
  services: ['استيراد التقارير الطبية', 'التاريخ الصحي الموحد', 'وصفات العلاج', 'التكامل مع نظام صحة'],
};
```
...
_تم إعداد هذا الدليل كجزء من نظام الألايل لتأهيل ذوي الإعاقة_
