...
| 6   | **Stored XSS عبر رفع SVG**: `routes/uploads.routes.js` يسمح بـ `image/svg+xml` في `ALLOWED_MIMES` ولا يستدعي `validateUploadedFile`، والملفات تُخدَم علناً عبر nginx من نفس الأصل (`/uploads/*`) — SVG قد يحمل `<script>` يُنفَّذ في أصل التطبيق. (مسار admin-only لكنه يرفع حقن-محتوى-المدير إلى XSS كامل الأصل) | عالية    | قرار منتج: هل يُحتاج رفع SVG فعلاً؟ المعالجة تغيّر سلوكاً مدفوعاً ⇒ مؤجَّلة |

> **الفجوة #6 (W1355):** المسارَان الشقيقان `medicalFiles.js` + `media.routes.js`
> يطبّقان `validateUploadedFile` (الذي يحظر `image/svg+xml` ضمن `BLOCKED_MIMES`
> ويتحقّق من magic-bytes)، بينما `routes/uploads.routes.js` (رفع المدير العام،
> يُخدَم علناً عبر nginx) يتخطّاه ويسمح بـ SVG صراحةً. المعالجة المتّسقة مع
> النمط القائم: استدعاء `validateUploadedFile` بعد `upload.single('file')` +
> إسقاط `image/svg+xml` من `ALLOWED_MIMES` ليطابق الشقيقَين. هذا يغيّر سلوكاً
> مدفوعاً (يرفض رفع SVG كان يعمل) ⇒ مؤجَّل كقرار منتج وليس إصلاحاً آلياً.
> دفاع-في-العمق بديل غير-مُسقِط لـ SVG: خدمة الملفات المرفوعة بترويسة
> `Content-Disposition: attachment` + `X-Content-Type-Options: nosniff` على
> `/uploads/*` في nginx (تغيير بنية تحتية).
