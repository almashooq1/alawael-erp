...
function AdvancedLoginComponent({ onLoginSuccess }) {
  ...
  // المصادقة الثنائية
  const [_twoFADialog, _setTwoFADialog] = useState(false);
  const [_twoFAToken, _setTwoFAToken] = useState('');
   ...
   */
  const _checkPasswordStrength = async pwd => {
    try {
