...
import {
  ...
  Alert, Badge, Modal, ListGroup, Accordion, Spinner, Navbar
...
export const IntelligentLMSDashboard = ({ token, userId }) => {
  ...
  const [_enrollments, _setEnrollments] = useState([]);
...
const CourseDetailsModal = ({ course, show, onHide, _token }) => {
...
const MyLearningTab = ({ token, _userId }) => {
  const [enrollments, _setEnrollments] = useState([]);
...
const EnrollmentCard = ({ enrollment, onSelect, _token }) => {
  return (
...
const EnrollmentDetailsPanel = ({ enrollment, token }) => {
  ...
  const [_activeLesson, _setActiveLesson] = useState(null);
  const [loading, setLoading] = useState(false);
...
const LessonContent = ({ lesson, token }) => {
  const [_content, _setContent] = useState(null);
  const [loading, setLoading] = useState(false);
...
const QuizComponent = ({ lesson, _token, onClose }) => {
...
const AnalyticsTab = ({ _token, selectedCourse }) => {
  const [metrics, _setMetrics] = useState(null);
  const [loading, setLoading] = useState(false);
...
const RecommendationsTab = ({ token, _userId }) => {
...
const ProgressTab = ({ token, _userId }) => {
  const [progressData, _setProgressData] = useState(null);
  const [loading, setLoading] = useState(false);
