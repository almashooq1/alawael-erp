...
describe('ReportsAnalyticsIntegrated Component', () => {
  ...
  describe('Tab Navigation', () => {
    ...
    test('should switch to payroll tab when clicked', async () => {
      ...
      const matches = screen.getAllByText(/إجمالي الرواتب|total payroll/i);
    ...
    test('should switch to export tab when clicked', async () => {
      ...
      const matches = screen.getAllByText(/Excel|PDF/);
      expect(matches.length).toBeGreaterThan(0);
    });
  });
  ...
  describe('Overview Tab', () => {
    ...
    test('should display summary statistics', async () => {
      ...
      await waitFor(() => {
        const matches = screen.getAllByText(/متوسط الحضور|متوسط الراتب/i);
        expect(matches.length).toBeGreaterThan(0);
      });
    });
  });
  ...
  describe('Payroll Tab', () => {
    ...
    test('should display payroll metrics', async () => {
      ...
      await waitFor(() => {
        // Numbers are formatted with Arabic locale (toLocaleString('ar-EG'))
        // 225000 => ٢٢٥٬٠٠٠ and 217500 => ٢١٧٬٥٠٠
        const matches = screen.getAllByText(/٢٢٥|٢١٧|إجمالي الرواتب|صافي الرواتب/i);
        expect(matches.length).toBeGreaterThan(0);
      });
    });
  ...
  describe('Performance Tab', () => {
    ...
    test('should display performance metrics', async () => {
      ...
      await waitFor(() => {
        const matches = screen.getAllByText(/3.8|متفوق|إنتاجية/i);
        expect(matches.length).toBeGreaterThan(0);
      });
    });
    ...
    test('should display performance breakdown', async () => {
      ...
      await waitFor(() => {
        const matches = screen.getAllByText(/متفوق|جيد|مرضي/i);
        expect(matches.length).toBeGreaterThan(0);
      });
    });
  });
  ...
  describe('Export Tab', () => {
    test('should display export options', async () => {
      ...
      const matches = screen.getAllByText(/Excel|CSV/);
      expect(matches.length).toBeGreaterThan(0);
    });
    ...
    test('should export to Excel when button clicked', async () => {
      ...
      await waitFor(() => {
        const exportTab = screen.getByText(/التصدير والأرشفة/).closest('button');
        if (exportTab) {
          fireEvent.click(exportTab);
        }
      });

      // Click the "تصدير الآن" button inside the export tab
      const excelButton = screen.getByText('تصدير الآن');
  ...
  describe('Data Fetching', () => {
    ...
    test('should handle API errors gracefully', async () => {
      ...
      // Component catches individual API errors and falls back to mock data
      await waitFor(() => {
        expect(screen.getByText(/التقارير والتحليلات/i)).toBeInTheDocument();
      });
    });
    ...
    test('should use mock data when API fails', async () => {
      ...
      await waitFor(() => {
        // Component falls back to mock data silently
        const matches = screen.getAllByText(/التقارير والتحليلات|موظف/i);
        expect(matches.length).toBeGreaterThan(0);
      });
    });
  });
  ...
  describe('Refresh Functionality', () => {
    ...
    test('should refresh reports when refresh button is clicked', async () => {
      ...
      // Refresh button is icon-only (RefreshCw SVG), find it by its class
      await waitFor(() => {
        expect(screen.getByText(/التقارير والتحليلات/)).toBeInTheDocument();
      });
      const refreshButton = document.querySelector('button .lucide-refresh-cw')?.closest('button');
  ...
  describe('Responsive Design', () => {
    test('should be responsive on mobile', async () => {
      ...
      expect(await screen.findByText(/التقارير والتحليلات/i)).toBeInTheDocument();
    });
  ...
  describe('Accessibility', () => {
    ...
    test('should have accessible tabs', async () => {
      render(<ReportsAnalyticsIntegrated />);
    ...
    test('should have accessible input fields', async () => {
      render(<ReportsAnalyticsIntegrated />);
