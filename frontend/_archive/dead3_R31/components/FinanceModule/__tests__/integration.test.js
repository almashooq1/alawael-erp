...
describe('Integration - سير العمل عبر المكونات', () => {
  ...
  test('يجب أن يتدفق البيانات من المخاطر إلى الاحتياطيات', async () => {
    ...
    await waitFor(() => expect(screen.getAllByText(/مصفوفة/i).length).toBeGreaterThan(0));
  });
...
describe('Integration - اتساق البيانات', () => {
  ...
  test('يجب أن يكون إجمالي الأصول = الخصوم + رأس المال', async () => {
    fetch.mockResolvedValue({
      ...
      json: async () => ({
        assets: {
          current: [{ name: 'نقد', value: 400000 }],
          fixed: [{ name: 'مباني', value: 350000 }],
        },
        liabilities: {
          current: [{ name: 'ذمم', value: 150000 }],
          longTerm: [{ name: 'قروض', value: 150000 }],
        },
        equity: [
          { name: 'رأس المال', value: 350000 },
          { name: 'أرباح', value: 100000 },
        ],
      }),
    });
    render(<FinancialReports organizationId="org-1" />);
  ...
  test('يجب أن تكون أرقام الامتثال متسقة عبر الآراء', async () => {
    ...
    const mockViolations = Array.from({ length: violationCount }, (_, i) => ({
      severity: 'medium',
      type: 'financial',
    ...
    fetch.mockImplementation(async url => {
      if (url.includes('violations'))
        return { ok: true, json: async () => ({ violations: mockViolations }) };
      if (url.includes('audit'))
        return {
          ok: true,
          json: async () => ({ entries: Array(violationCount).fill({ action: 'test' }) }),
        };
      return { ok: true, json: async () => ({}) };
    });
...
describe('Integration - تكامل API', () => {
  ...
  test('يجب أن يعيد المحاولة عند فشل الاتصال', async () => {
    ...
    // ValidationDashboard shows error + manual retry button
    await waitFor(() => expect(screen.queryByText(/خطأ/)).toBeInTheDocument());
  });
...
describe('Integration - الأداء', () => {
  ...
  test('يجب أن تحافظ على استجابة البيانات الضخمة', async () => {
    ...
    const { _rerender } = render(<FinancialReports organizationId="org-1" />);
    await waitFor(() => expect(fetch).toHaveBeenCalled());
  });
  ...
  test('يجب أن يوفر تقارير الامتثال المطلوبة', async () => {
    fetch.mockResolvedValue({
      ...
      json: async () => ({
        complianceRate: 0.95,
        violations: [
          { severity: 'low', type: 'documentation', description: 'test compliance violation' },
        ],
      }),
    });
