...
const FaceRecognitionService = ({ _camera, onClose, _onDetectFace }) => {
  ...
  const simulateFaceDetection = useCallback(async () => {
    ...
    const _recognized = mockDetectedFaces.filter(f => f.status === 'recognized');
  ...
  const handleConfirmFace = useCallback((faceId, _name) => {
