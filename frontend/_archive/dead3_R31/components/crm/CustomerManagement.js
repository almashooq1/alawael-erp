    ...
    <Box sx={{ p: 3 }}>
      ...
      <Grid container spacing={2} sx={{ mb: 3 }}>
        {[
          ...
          { label: 'إجمالي القيمة', value: `${(stats.totalValue / 1000).toFixed(0)}K`, icon: '💰', color: '#2196f3' },
        ...
        <Stack spacing={2}>
          <Box sx={{ display: 'flex', gap: 2, flexWrap: 'wrap' }}>
            ...
            <FormControl size="small" sx={{ minWidth: 150 }}>
              ...
              <Select value={filterSegment} onChange={e => setFilterSegment(e.target.value)} label="القطاع">
        ...
        <DialogContent sx={{ mt: 2 }}>
          <Stack spacing={2}>
            ...
            <FormControl fullWidth>
              <InputLabel>القطاع</InputLabel>
              <Select value={formData.segment || ''} onChange={e => setFormData({ ...formData, segment: e.target.value })} label="القطاع">
      ...
      {selectedCustomers.length > 0 && (
        <SpeedDial ariaLabel="إجراءات جماعية" sx={{ position: 'fixed', bottom: 24, left: 24 }} icon={<SpeedDialIcon />}>
          <SpeedDialAction
            icon={<MailIcon />}
            tooltipTitle="إرسال بريد"
            onClick={() =>
              setSnackbar({ open: true, message: `✅ سيتم إرسال بريد ل ${selectedCustomers.length} عميل`, severity: 'success' })
            }
          />
          <SpeedDialAction
            icon={<ShareIcon />}
            tooltipTitle="مشاركة"
            onClick={() => setSnackbar({ open: true, message: `✅ تم التشارك مع ${selectedCustomers.length} عميل`, severity: 'success' })}
          />
        </SpeedDial>
      )}
    </Box>
