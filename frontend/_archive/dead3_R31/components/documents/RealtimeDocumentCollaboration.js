...
const RealtimeDocumentCollaboration = ({
  ...
  const [activeUsers, _setActiveUsers] = useState([
    currentUser,
  ...
  const [_userCursors, _setUserCursors] = useState({});
  ...
  const _handleAddChange = useCallback(
    (changeType, content) => {
      const newChange = {
        id: `change_${Date.now()}`,
    ...
    >
      ...
      >
        ...
        <Box sx={{ display: 'flex', alignItems: 'center', gap: 1 }}>
          ...
          {syncStatus === 'syncing' && (
            <Chip label="جاري التزامن..." size="small" variant="outlined" icon={<ScheduleIcon />} />
          )}
