...
const LegalManagement = () => {
  ...
  // Legal Consultant Management
  const [consultants, _setConsultants] = useState([
    {
      id: 1,
  ...
  // Dialog Management
  const [_openRiskDialog, setOpenRiskDialog] = useState(false);
  const [_openConsultantDialog, setOpenConsultantDialog] = useState(false);
  const [_openDetailView, setOpenDetailView] = useState(false);
    ...
    <Box sx={{ p: 3, backgroundColor: '#f5f7fa', minHeight: '100vh' }}>
                    ...
                    >
                      ...
                      <TableCell>
                        <Box>
                          ...
                          {isExpiring && <Chip label={`ينتهي في ${daysLeft} يوم`} size="small" color="warning" sx={{ mt: 0.5 }} />}
                        </Box>
                      </TableCell>
      ...
      {activeTab === 1 && (
        <Box>
          ...
          <Grid container spacing={2}>
            {complianceItems.map((item, _index) => (
        ...
        <Box>
          ...
          <Grid container spacing={2}>
            {risks.map((risk, _index) => {
              const severityColor = risk.severity === 'عالي' ? 'error' : risk.severity === 'متوسط' ? 'warning' : 'info';
              return (
      ...
      {activeTab === 3 && (
        <Box>
          ...
          <Grid container spacing={2}>
            {consultants.map((consultant, _index) => (
              <Grid item xs={12} md={6} key={consultant.id}>
                <Card sx={{ height: '100%' }}>
                  <CardHeader
                    ...
                    action={<Chip label={consultant.status} color={consultant.status === 'نشط' ? 'success' : 'default'} size="small" />}
                  />
                  <CardContent>
                    <Stack spacing={2}>
