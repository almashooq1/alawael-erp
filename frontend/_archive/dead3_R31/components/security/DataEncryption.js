...
const DataEncryption = () => {
  const [encryptionKeys, _setEncryptionKeys] = useState([
    {
      id: 1,
      name: 'Master Key 1',
      algorithm: 'AES-256',
  ...
  const [dataEncryption, _setDataEncryption] = useState([
    {
      id: 1,
  ...
  const [certificates, _setCertificates] = useState([
    {
      id: 1,
  ...
  const [encryptionPolicies, _setEncryptionPolicies] = useState([
    { id: 1, name: 'تشفير بيانات العملاء', enabled: true, algorithm: 'AES-256', enforcement: 'strict', compliance: 'GDPR' },
    { id: 2, name: 'تشفير الاتصالات', enabled: true, algorithm: 'TLS 1.3', enforcement: 'strict', compliance: 'NIST' },
    { id: 3, name: 'تشفير النسخ الاحتياطية', enabled: true, algorithm: 'AES-256', enforcement: 'moderate', compliance: 'ISO 27001' },
  ]);

  const [_openDialog, _setOpenDialog] = useState(false);
  const [_selectedKey, _setSelectedKey] = useState(null);
    ...
    <Box sx={{ p: 3 }}>
      ...
      <Grid container spacing={2} sx={{ mb: 3 }}>
        {[
          ...
          { label: 'بيانات مشفرة', value: `${stats.encryptedData}/${dataEncryption.length}`, icon: '🔒', color: '#4caf50' },
        ...
        {encryptionPolicies.map((policy, idx) => (
          <Box key={policy.id}>
            <ListItem sx={{ py: 2 }}>
              ...
              <ListItemText primary={policy.name} secondary={`${policy.algorithm} • ${policy.compliance}`} />
