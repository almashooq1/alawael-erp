...
const Expenses = () => {
  ...
  const fetchExpenses = async () => {
    try {
      setLoading(true);
      const response = await axios.get('http://localhost:5000/api/accounting/expenses');
  ...
  const fetchStatistics = async () => {
    try {
      const response = await axios.get('http://localhost:5000/api/accounting/expenses/stats');
  ...
  const handleSubmit = async () => {
    try {
      ...
      if (selectedExpense) {
        await axios.put(
          `http://localhost:5000/api/accounting/expenses/${selectedExpense._id}`,
          expenseData
        );
      } else {
        await axios.post('http://localhost:5000/api/accounting/expenses', expenseData);
      }
  ...
  const handleApprove = async (expenseId) => {
    if (window.confirm('هل أنت متأكد من الموافقة على هذا المصروف؟')) {
      try {
        await axios.post(`http://localhost:5000/api/accounting/expenses/${expenseId}/approve`, {
  ...
  const handleReject = async (expenseId) => {
    ...
    if (reason) {
      try {
        await axios.post(`http://localhost:5000/api/accounting/expenses/${expenseId}/reject`, {
  ...
  const handleDeleteExpense = async (expenseId) => {
    if (window.confirm('هل أنت متأكد من حذف هذا المصروف؟')) {
      try {
        await axios.delete(`http://localhost:5000/api/accounting/expenses/${expenseId}`);
        fetchExpenses();
        fetchStatistics();
