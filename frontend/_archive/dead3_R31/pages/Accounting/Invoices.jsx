...
const Invoices = () => {
  ...
  const fetchInvoices = async () => {
    try {
      setLoading(true);
      const response = await axios.get('http://localhost:5000/api/accounting/invoices');
  ...
  const fetchStatistics = async () => {
    try {
      const response = await axios.get('http://localhost:5000/api/accounting/invoices/stats');
  ...
  const handleSubmit = async () => {
    try {
      ...
      if (selectedInvoice) {
        await axios.put(
          `http://localhost:5000/api/accounting/invoices/${selectedInvoice._id}`,
          invoiceData
        );
      } else {
        await axios.post('http://localhost:5000/api/accounting/invoices', invoiceData);
      }
  ...
  const handleRecordPayment = async () => {
    try {
      await axios.post(
        `http://localhost:5000/api/accounting/invoices/${selectedInvoice._id}/payment`,
        paymentData
      );
  ...
  const handleSendInvoice = async (invoiceId) => {
    try {
      await axios.post(`http://localhost:5000/api/accounting/invoices/${invoiceId}/send`);
      fetchInvoices();
  ...
  const handleDeleteInvoice = async (invoiceId) => {
    if (window.confirm('هل أنت متأكد من حذف هذه الفاتورة؟')) {
      try {
        await axios.delete(`http://localhost:5000/api/accounting/invoices/${invoiceId}`);
        fetchInvoices();
        fetchStatistics();
      } catch (error) {
