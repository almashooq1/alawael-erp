...
const Payments = () => {
  ...
  const fetchPayments = async () => {
    try {
      setLoading(true);
      const response = await axios.get('http://localhost:5000/api/accounting/payments');
  ...
  const fetchInvoices = async () => {
    try {
      const response = await axios.get('http://localhost:5000/api/accounting/invoices?status=sent,partial');
  ...
  const fetchStatistics = async () => {
    try {
      const response = await axios.get('http://localhost:5000/api/accounting/payments/stats');
  ...
  const handleSubmit = async () => {
    try {
      ...
      if (selectedPayment) {
        await axios.put(
          `http://localhost:5000/api/accounting/payments/${selectedPayment._id}`,
          paymentData
        );
      } else {
        await axios.post('http://localhost:5000/api/accounting/payments', paymentData);
      }
  ...
  const handleDeletePayment = async (paymentId) => {
    if (window.confirm('هل أنت متأكد من حذف هذه الدفعة؟')) {
      try {
        await axios.delete(`http://localhost:5000/api/accounting/payments/${paymentId}`);
        fetchPayments();
        fetchInvoices();
        fetchStatistics();
  ...
  const handlePrintReceipt = async (paymentId) => {
    try {
      const response = await axios.get(
        `http://localhost:5000/api/accounting/payments/${paymentId}/receipt`,
        { responseType: 'blob' }
      );
