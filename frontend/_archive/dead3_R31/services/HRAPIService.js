...
import {
  ...
  setToken as saveStoredToken,
  removeToken as clearStoredToken,
} from '../utils/tokenStorage';

const API_BASE_URL = process.env.REACT_APP_API_URL || 'http://localhost:5000/api/hr';
...
class HRAPIService {
  ...
  // جلب تقرير الحضور حسب الفترة
  static async getAttendanceReport(startDate, endDate) {
    try {
      const response = await fetch(
        `${API_BASE_URL}/attendance?startDate=${startDate}&endDate=${endDate}`,
        {
          headers: this.getHeaders(),
        }
      );
      ...
      throw new Error(`خطأ في جلب تقرير الحضور: ${error.message}`);
    }
  }
