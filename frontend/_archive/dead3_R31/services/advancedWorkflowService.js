...
import { getToken as getStoredToken, setToken as saveStoredToken } from '../utils/tokenStorage';
...
class AdvancedWorkflowService {
  ...
  async getWorkflows(filters = {}) {
    try {
      ...
      const response = await axios.get(`${this.apiBaseUrl}/workflows?${params.toString()}`, {
        headers: this.getAuthHeaders(),
      });
  ...
  async getWorkflowById(workflowId) {
    try {
      const response = await axios.get(`${this.apiBaseUrl}/workflows/${workflowId}`, {
        headers: this.getAuthHeaders(),
      });
  ...
  async getWorkflowAnalytics(filters = {}) {
    try {
      const params = new URLSearchParams(filters);
      const response = await axios.get(`${this.apiBaseUrl}/analytics?${params.toString()}`, {
        headers: this.getAuthHeaders(),
      });
  ...
  async getAuditLog(filters = {}) {
    try {
      const params = new URLSearchParams(filters);
      const response = await axios.get(`${this.apiBaseUrl}/audit-log?${params.toString()}`, {
        headers: this.getAuthHeaders(),
      });
