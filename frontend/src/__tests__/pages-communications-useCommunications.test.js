/**
 * Auto-generated tests for pages/communications/useCommunications.js
...
const SRC = path.resolve(__dirname, '../pages/communications/useCommunications.js');

describe('pages/communications/useCommunications.js', () => {
  let source;
  beforeAll(() => {
  ...
  test('uses React hooks (useCommunications, useState, useEffect, useCallback, useConfirmDialog)', () => {
    const hookPattern = /use[A-Z]\w+/g;
    const matches = source.match(hookPattern) || [];
    expect(matches.length).toBeGreaterThan(0);
  });

  test('has default export (useCommunications)', () => {
    expect(source).toMatch(/export\s+default/);
    expect(source).toMatch(/useCommunications/);
  });
