  ...
  >
    <DialogTitle sx={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
      <Box display="flex" alignItems="center" gap={1}>
        <ICONS.StructureIcon color="primary" /> {editingId ? 'تحرير الهيكل' : 'إضافة هيكل جديد'}
      </Box>
              ...
              onChange={e =>
                setFormData({
                  ...
                  applicableTo: {
                    ...
                    salaryRange: { ...formData.applicableTo.salaryRange, min: +e.target.value },
                  },
                })
              }
            />
            ...
            <TextField
              ...
              onChange={e =>
                setFormData({
                  ...
                  applicableTo: {
                    ...formData.applicableTo,
                    salaryRange: { ...formData.applicableTo.salaryRange, max: +e.target.value },
                  },
                })
              }
            />
          ...
          {formData.fixedAllowances.map((al, idx) => (
            <Box key={idx} display="flex" gap={2} alignItems="center" sx={{ mb: 1.5 }}>
              ...
              {formData.fixedAllowances.length > 1 && (
                <IconButton color="error" onClick={() => removeAllowance(idx)}>
                  <DeleteIcon />
                </IconButton>
              )}
            </Box>
          ))}
          <Button startIcon={<AddIcon />} onClick={addAllowance} sx={{ mt: 1 }}>
            إضافة مزية
          </Button>
        </Box>
      )}
              ...
              <Paper variant="outlined" sx={{ p: 2, borderRadius: 2 }}>
                ...
                {item.extra && (
                  <TextField
                    ...
                    fullWidth
                    label={
                      item.extra === 'minScore' ? 'الحد الأدنى للدرجة' : 'سنوات الخدمة المطلوبة'
                    }
                    type="number"
          ...
          <Grid item xs={6}>
            <TextField
              ...
              onChange={e =>
                setFormData({
                  ...formData,
                  paidLeave: { ...formData.paidLeave, annualDays: +e.target.value },
                })
              }
            />
          </Grid>
          <Grid item xs={6}>
            <TextField
              ...
              onChange={e =>
                setFormData({
                  ...formData,
                  paidLeave: { ...formData.paidLeave, accruedPerMonth: +e.target.value },
                })
              }
            />
          </Grid>
    ...
    <DialogActions sx={{ p: 2, gap: 1, justifyContent: 'space-between' }}>
      <Box display="flex" gap={1}>
        {formSection > 0 && <Button onClick={() => setFormSection(s => s - 1)}>السابق</Button>}
      </Box>
      <Box display="flex" gap={1}>
        <Button onClick={onClose}>إلغاء</Button>
        {formSection < FORM_SECTIONS.length - 1 ? (
          <Button variant="contained" onClick={() => setFormSection(s => s + 1)}>
            التالي
          </Button>
        ) : (
          <Button
