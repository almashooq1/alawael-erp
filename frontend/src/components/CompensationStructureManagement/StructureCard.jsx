...
const StructureCard = ({ structure, expanded, onToggle, onEdit, onDuplicate, onDelete }) => {
    ...
    >
      <CardContent>
        ...
        <Box display="flex" justifyContent="space-between" alignItems="flex-start" mb={2}>
          ...
          <Chip
            label={structure.isActive ? 'نشط' : 'غير نشط'}
        ...
        <Box display="flex" gap={1} flexWrap="wrap" mb={1}>
          <Chip
            size="small"
            label={`النطاق: ${SCOPE_LABELS[structure.applicableTo?.scope] || 'الكل'}`}
            variant="outlined"
          />
        ...
        <Collapse in={expanded}>
          ...
          <Typography variant="subtitle2" fontWeight={700} gutterBottom>
            <ICONS.AllowanceIcon fontSize="small" sx={{ verticalAlign: 'middle', mr: 0.5 }} />{' '}
            المزايا الثابتة
          </Typography>
          <Box sx={{ mb: 2 }}>
            {structure.fixedAllowances?.map((al, idx) => (
              ...
              >
                ...
                <Chip
                  label={`${al.amount.toLocaleString('ar-SA')} ر.س`}
                  size="small"
                  color="primary"
                  variant="outlined"
                />
          ...
          {/* Deductions */}
          <Typography variant="subtitle2" fontWeight={700} gutterBottom>
            <DeductionIcon fontSize="small" sx={{ verticalAlign: 'middle', mr: 0.5 }} /> الخصومات
            الإلزامية
          </Typography>
          <Box sx={{ mb: 2 }}>
            <Box display="flex" justifyContent="space-between" sx={{ py: 0.5 }}>
              ...
              <Typography variant="body2" fontWeight={600}>
                {structure.mandatoryDeductions?.socialSecurity?.percentage}%
              </Typography>
            </Box>
            <Box display="flex" justifyContent="space-between" sx={{ py: 0.5 }}>
              <Typography variant="body2">التأمين الصحي</Typography>
              <Typography variant="body2" fontWeight={600}>
                {structure.mandatoryDeductions?.healthInsurance?.percentage}%
              </Typography>
            </Box>
            <Box display="flex" justifyContent="space-between" sx={{ py: 0.5 }}>
              <Typography variant="body2">GOSI</Typography>
              <Typography variant="body2" fontWeight={600}>
                {structure.mandatoryDeductions?.GOSI?.percentage}%
              </Typography>
            </Box>
          </Box>
          ...
          {/* Incentives */}
          <Typography variant="subtitle2" fontWeight={700} gutterBottom>
            <ICONS.MoneyIcon fontSize="small" sx={{ verticalAlign: 'middle', mr: 0.5 }} /> هيكل
            الحوافز
          </Typography>
          <Box sx={{ mb: 1 }}>
            {Object.entries(structure.incentiveStructure || {}).map(([key, val]) => (
              <Box key={key} display="flex" justifyContent="space-between" sx={{ py: 0.3 }}>
                ...
                <Typography variant="body2" fontWeight={600}>
                  {val.percentage
                    ? `${val.percentage}%`
                    : `${val.amount?.toLocaleString('ar-SA')} ر.س`}
                </Typography>
              </Box>
            ))}
          </Box>
        ...
        <Box display="flex" justifyContent="space-between" alignItems="center" mt={1}>
          ...
          <Box display="flex" gap={0.5}>
            <Tooltip title="نسخ">
              <IconButton size="small" onClick={() => onDuplicate(structure)}>
                <CopyIcon fontSize="small" />
              </IconButton>
            </Tooltip>
            <Tooltip title="تحرير">
              <IconButton size="small" color="primary" onClick={() => onEdit(structure)}>
                <EditIcon fontSize="small" />
              </IconButton>
            </Tooltip>
            <Tooltip title="حذف">
              <IconButton size="small" color="error" onClick={() => onDelete(structure._id)}>
