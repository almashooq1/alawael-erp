/**
 * CompensationStructureManagement — Custom Hook
 */
import { useState, useEffect, useCallback, useMemo } from 'react';
import apiClient from 'services/api.client';
import { useSnackbar } from 'contexts/SnackbarContext';
import { DEMO_STRUCTURES, INITIAL_FORM } from './constants';
...
const useCompensation = () => {
  ...
  const updateAllowance = (index, field, value) => {
    const updated = [...formData.fixedAllowances];
    updated[index] = {
      ...updated[index],
      [field]: field === 'amount' ? parseFloat(value) || 0 : value,
    };
    setFormData({ ...formData, fixedAllowances: updated });
  };
  ...
  const removeAllowance = index =>
    setFormData({
      ...formData,
      fixedAllowances: formData.fixedAllowances.filter((_, i) => i !== index),
    });
