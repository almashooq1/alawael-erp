...
const DEMO_INCENTIVES = [
  {
    ...
    month: '2026-02',
    year: 2026,
    reason: 'تميز في إنجاز مشروع النظام الجديد',
    status: 'approved',
  ...
  },
  {
    ...
    month: '2026-02',
    year: 2026,
    reason: 'حضور كامل خلال الربع الأول',
    status: 'paid',
  ...
  },
  {
    ...
    department: 'المالية',
    incentiveType: 'project',
    amount: 2500,
    month: '2026-03',
    year: 2026,
    reason: 'إنجاز مشروع التدقيق السنوي',
    status: 'pending',
  ...
  },
  {
    ...
    month: '2026-01',
    year: 2026,
    reason: 'تقدير خاص من الإدارة',
    status: 'paid',
    createdAt: '2026-01-15',
    transactionRef: 'TXN-20260115-001',
  },
  {
    _id: 'i5',
    employeeId: 'EMP-2505',
    ...
    month: '2026-02',
    year: 2026,
    reason: 'التزام بمعايير السلامة طوال الفترة',
    status: 'approved',
  ...
  },
  {
    ...
    department: 'التأهيل',
    incentiveType: 'seasonal',
    amount: 1200,
    month: '2026-03',
    year: 2026,
    reason: 'مكافأة شهر رمضان',
    status: 'draft',
  ...
  },
  {
    _id: 'i8',
    employeeId: 'EMP-2508',
    employeeName: 'هند الشمري',
    department: 'خدمة العملاء',
    incentiveType: 'performance',
    amount: 1800,
    month: '2026-02',
    year: 2026,
    reason: 'أعلى تقييم رضا عملاء',
    status: 'approved',
...
const IncentivesManagement = () => {
  ...
  const loadIncentives = useCallback(async () => {
    ...
    try {
      const url =
        filter !== 'all' ? `/compensation/incentives?status=${filter}` : '/compensation/incentives';
  ...
  const handleMarkPaid = useCallback(async () => {
    if (!transactionRef) {
      showSnackbar('يرجى إدخال رقم المرجع', 'warning');
      return;
    }
    ...
    try {
      ...
      setIncentives(prev =>
        prev.map(i => (i._id === id ? { ...i, status: 'paid', transactionRef } : i))
      );
  ...
  const handleExportCSV = useCallback(() => {
    ...
    a.href = url;
    a.download = 'incentives-export.csv';
    a.click();
  ...
  const FILTER_TABS = [
    ...
    {
      value: 'pending',
      label: `قيد الموافقة (${incentives.filter(i => i.status === 'pending').length})`,
    },
    {
      value: 'approved',
      label: `معتمد (${incentives.filter(i => i.status === 'approved').length})`,
    },
  ...
  return (
    <Container maxWidth="xl" sx={{ py: 3 }}>
      ...
      >
        ...
        >
          ...
          <Box display="flex" gap={1}>
            <Tooltip title="تحديث">
              <IconButton sx={{ color: 'white' }} onClick={loadIncentives}>
                <RefreshIcon />
              </IconButton>
            </Tooltip>
            ...
            <TableBody>
              {filtered.length === 0 ? (
                ...
                filtered.slice(page * rowsPerPage, page * rowsPerPage + rowsPerPage).map(inc => {
                  const typeInfo = INCENTIVE_TYPES[inc.incentiveType] || {
                    label: inc.incentiveType,
                    color: neutralColors.textMuted,
                  };
                  const statusInfo = STATUS_CONFIG[inc.status] || {
                    label: inc.status,
                    color: 'default',
                  };
                                ...
                                <IconButton
                                  ...
                                  onClick={() =>
                                    setConfirmDialog({
                                      ...
                                      action: 'approve',
                                      id: inc._id,
                                      title: 'هل تريد الموافقة على هذا الحافز؟',
                                    })
                                  }
                                >
      ...
      <Dialog
        ...
        onClose={() => setConfirmDialog({ open: false, action: null, id: null, title: '' })}
        PaperProps={{ sx: { borderRadius: 3 } }}
      >
        <DialogTitle>{confirmDialog.title}</DialogTitle>
        <DialogActions sx={{ p: 2 }}>
          <Button
            onClick={() => setConfirmDialog({ open: false, action: null, id: null, title: '' })}
          >
            إلغاء
          </Button>
          <Button
      ...
      >
        ...
        <DialogContent sx={{ pt: 2 }}>
          <TextField
            ...
            placeholder="مثال: TXN-20260301-001"
            sx={{ mt: 1 }}
          />
        </DialogContent>
        <DialogActions sx={{ p: 2 }}>
