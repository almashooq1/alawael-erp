...
const PIE_COLORS = [
  ...chartColors.category.slice(0, 6),
...
const PayrollAnalyticsDashboard = () => {
  ...
  const deptChartData = useMemo(
    () =>
      departmentStats.map(d => ({
        ...d,
        color: DEPT_COLORS[d.name] || neutralColors.textSecondary,
      })),
    [departmentStats]
  );
  ...
  const handleExportCSV = useCallback(() => {
    ...
    a.href = url;
    a.download = 'payroll-analytics.csv';
    a.click();
  ...
  return (
    <Container maxWidth="xl" sx={{ py: 3 }}>
      ...
      >
        ...
        >
          ...
          <Box display="flex" gap={1}>
            <Tooltip title="تحديث">
              <IconButton sx={{ color: 'white' }} onClick={loadAnalytics}>
                <RefreshIcon />
              </IconButton>
            </Tooltip>
            <Tooltip title="تصدير CSV">
              <IconButton sx={{ color: 'white' }} onClick={handleExportCSV}>
                <DownloadIcon />
              </IconButton>
            </Tooltip>
            <Tooltip title="طباعة">
      ...
      >
        ...
        <Button variant="outlined" size="small" onClick={loadAnalytics} startIcon={<RefreshIcon />}>
          تطبيق
        </Button>
      </Paper>
      ...
      <Grid container spacing={2} sx={{ mb: 3 }}>
        {[
          ...
          },
          {
            ...
            value: `${fmt(summary.totalPayroll)} ر.س`,
            sub: 'الشهر الحالي',
            icon: <MoneyIcon />,
            color: chartColors.category[2],
          },
          {
            label: 'الحوافز',
            value: `${fmt(incentiveStats.total)} ر.س`,
            sub: `${incentiveStats.count || 0} حافز`,
            icon: <TrophyIcon />,
            color: statusColors.warning,
          },
          {
            label: 'الخصومات',
            value: `${fmt(deductionStats.total)} ر.س`,
            sub: `${deductionStats.types?.length || 0} نوع`,
            icon: <DownIcon />,
            color: chartColors.category[9],
          },
          {
            label: 'متوسط الراتب',
            value: `${fmt(summary.averageSalary)} ر.س`,
            sub: 'للموظف الواحد',
            icon: <TrendIcon />,
            color: chartColors.category[4],
          },
          {
            label: 'معدل الموافقة',
            value: `${payrollStats.approvalRate || 0}%`,
            sub: 'الرواتب الموافق عليها',
            icon: <PercentIcon />,
          ...
          <Grid item xs={12} sm={6} md={4} lg={2} key={idx}>
            <Card
              sx={{ borderRadius: 3, boxShadow: '0 2px 12px rgba(0,0,0,0.08)', height: '100%' }}
            >
              <CardContent
                sx={{
                  display: 'flex',
              ...
              >
                <Avatar sx={{ bgcolor: `${m.color}15`, color: m.color, width: 48, height: 48 }}>
                  {m.icon}
                </Avatar>
                  ...
                  <Box display="flex" alignItems="center" gap={1}>
                    <Box
                      sx={{
                        ...
                        height: 10,
                        borderRadius: '50%',
                        bgcolor: t.color || PIE_COLORS[idx],
                      }}
                    />
      ...
      {trends.length > 0 && (
        <Paper sx={{ p: 3, mb: 3, borderRadius: 3 }}>
          ...
          <Box display="flex" justifyContent="center" gap={3} mt={1}>
            {[
              { label: 'الرواتب', color: statusColors.primaryBlue },
              { label: 'الحوافز', color: statusColors.success },
              { label: 'الخصومات', color: chartColors.category[9] },
            ].map(l => (
                ...
                <TableBody>
                  {salaryDistribution.map((range, idx) => (
                    <TableRow key={idx} hover>
                      ...
                      <TableCell>
                        <Chip label={range.count} size="small" color="primary" variant="outlined" />
                      </TableCell>
            ...
            {topPerformers.slice(0, 5).map((p, idx) => {
              ...
              return (
                <Box
                  ...
                  sx={{
                    py: 1.5,
                    borderBottom: idx < 4 ? `1px solid ${surfaceColors.softGray}` : 'none',
                  }}
                >
                  <Avatar
                    sx={{
                      ...
                      color: idx < 3 ? 'white' : neutralColors.textSecondary,
                      width: 36,
                      height: 36,
                      fontSize: 14,
                      fontWeight: 700,
      ...
      <Paper sx={{ p: 3, borderRadius: 3 }}>
        ...
        <Grid container spacing={2}>
          {[
            {
              label: 'أعلى راتب',
              value: `${fmt(summary.maxSalary)} ر.س`,
              color: statusColors.success,
            },
            {
              label: 'أقل راتب',
              value: `${fmt(summary.minSalary)} ر.س`,
              color: chartColors.category[7],
            },
            {
            ...
            {
              label: 'متوسط الحافز',
              value: `${fmt(incentiveStats.average)} ر.س`,
              color: statusColors.info,
            },
            {
              label: 'متوسط الخصم',
              value: `${fmt(deductionStats.average)} ر.س`,
              color: statusColors.purple,
            },
          ].map((s, idx) => (
