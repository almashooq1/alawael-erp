...
const PayrollDashboard = () => {
  ...
  const stats = useMemo(() => {
    ...
    const totalDeductions = filteredData.reduce(
      (s, p) => s + (p.gosi || 0) + (p.deductions || 0),
      0
    );
    const processed = filteredData.filter(
      p => p.status === 'مُعالج' || p.status === 'processed'
    ).length;
              ...
              <Typography variant="body2" sx={{ opacity: 0.9 }}>
                ...
                {isDemo && (
                  <Chip
                    ...
                    size="small"
                    sx={{ ml: 1, bgcolor: 'rgba(255,255,255,0.25)', color: 'white', height: 20 }}
                  />
                )}
              </Typography>
            </Box>
        ...
        {[
          {
            ...
            val: `${fmt(stats.totalNet)} ر.س`,
            icon: <WalletIcon />,
            color: statusColors.successDeep,
            sub: `${stats.count} موظف`,
          },
          {
            label: 'إجمالي الخصومات',
            val: `${fmt(stats.totalDeductions)} ر.س`,
            icon: <TrendIcon />,
          ...
          {
            label: 'حالة المعالجة',
            val: `${stats.processed}/${stats.count}`,
            icon: <ApprovedIcon />,
            color: statusColors.warningDarker,
            sub: stats.pending > 0 ? `${stats.pending} معلق` : 'الكل مُعالج',
          },
        ].map((s, i) => (
              ...
              <Box sx={{ display: 'flex', alignItems: 'center', gap: 1 }}>
                <Box
                  sx={{
                    ...
                    height: 12,
                    borderRadius: '50%',
                    bgcolor: DEPT_COLORS[d] || neutralColors.textMuted,
                  }}
                />
                {d}
              </Box>
                  ...
                  <TableRow key={p._id} hover>
                    ...
                    <TableCell>
                      <Chip
                        label={p.department}
                        size="small"
                        sx={{ bgcolor: `${deptColor}15`, color: deptColor, fontWeight: 'bold' }}
                      />
                    </TableCell>
                ...
                <Grid item xs={12} sm={6} md={4} key={dept}>
                  ...
                  >
                    ...
                    >
                      ...
                      <Chip
                        label={`${info.count} موظف`}
                        size="small"
                        sx={{ bgcolor: `${color}15`, color }}
                      />
                    </Box>
                    <Typography variant="h6" fontWeight="bold">
                      {fmt(info.totalNet)} ر.س
                    </Typography>
                    <LinearProgress
                      variant="determinate"
          ...
          <DialogContent dividers>
            ...
            <Grid container spacing={2}>
              {[
                ...
                {
                  label: 'بدل النقل',
                  val: selectedEmployee.transportAllowance,
                  color: 'info.main',
                },
                {
                  label: 'بدلات أخرى',
                  val: selectedEmployee.otherAllowance || 0,
                  color: 'info.main',
                },
              ].map((item, i) => (
              ...
              <Grid item xs={6}>
                ...
                <Typography variant="h6" color="warning.main">
                  -{fmt(selectedEmployee.gosi || 0)} ر.س
                </Typography>
              </Grid>
              <Grid item xs={6}>
                <Typography variant="caption" color="text.secondary">
                ...
                <Typography variant="h6" color="error.main">
                  -{fmt(selectedEmployee.deductions || 0)} ر.س
                </Typography>
              </Grid>
              <Grid item xs={12}>
                <Divider />
              ...
              <Grid item xs={12}>
                <Paper
                  sx={{
                    p: 2,
                    bgcolor: surfaceColors.successLight,
                    borderRadius: 2,
                    textAlign: 'center',
                  }}
                >
