  ...
  return (
    <>
      ...
      >
        ...
        <DialogContent sx={{ minHeight: 400 }}>{children}</DialogContent>
      </Dialog>
    </>
  );
};
        ...
        <PieChart>
          ...
          >
            {(data.length > 0 ? data : [{ name: 'لا توجد بيانات', value: 1 }]).map((_, i) => (
              <Cell
                key={i}
                fill={data.length > 0 ? COLORS[i % COLORS.length] : surfaceColors.divider}
              />
            ))}
          </Pie>
          <RechartsTooltip content={<ChartTooltip />} />
        ...
        <PieChart>
          ...
          >
            {(data.length > 0 ? data : [{ name: 'لا توجد بيانات', value: 1 }]).map((_, i) => (
              <Cell
                key={i}
                fill={data.length > 0 ? COLORS[(i + 2) % COLORS.length] : surfaceColors.divider}
              />
            ))}
          </Pie>
          <RechartsTooltip content={<ChartTooltip />} />
