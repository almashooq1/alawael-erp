...
const useAnimatedValue = (end, duration = 1000) => {
  ...
  useEffect(() => {
    ...
    const numEnd =
      typeof end === 'number' ? end : parseFloat(String(end).replace(/[^0-9.-]/g, '')) || 0;
    if (numEnd === 0) {
      setVal(0);
      return;
    }
...
const FinanceOverview = ({ finance = {}, charts = {}, delay = 0 }) => {
  ...
  const _hasFinanceCards =
    finance.monthlyRevenue > 0 ||
    finance.monthExpenses > 0 ||
    finance.monthNetIncome > 0 ||
    finance.pendingInvoices > 0;
