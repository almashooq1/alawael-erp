...
const ACTIONS = [
  {
    ...
    fields: [
      ...
      { key: 'template', label: 'القالب', type: 'text' },
    ],
  },
  {
    ...
    color: '#ff9800',
    fields: [
      { key: 'message', label: 'الرسالة', type: 'text' },
      { key: 'priority', label: 'الأولوية', type: 'select', options: ['high', 'medium', 'low'] },
    ],
  },
  {
    value: 'add_tag',
  ...
  },
  {
    ...
    fields: [
      ...
      { key: 'method', label: 'الطريقة', type: 'select', options: ['POST', 'PUT', 'PATCH'] },
    ],
  },
];
...
export default function AutomationBuilder({ open, onClose, onSave, rule }) {
  ...
  const setTriggerConfig = (key, val) => {
    setForm(p => ({
      ...p,
      trigger: { ...p.trigger, config: { ...p.trigger.config, [key]: val } },
    }));
  };
  ...
  const updateActionConfig = (idx, key, val) => {
    setForm(p => ({
      ...
      actions: p.actions.map((a, i) =>
        i === idx ? { ...a, config: { ...a.config, [key]: val } } : a
      ),
    }));
  };
        ...
        <Stepper activeStep={activeStep} orientation="vertical">
                  ...
                  <Grid item xs={4}>
                    <TextField
                      ...
                      onChange={e =>
                        setForm(p => ({
                          ...p,
                          priority: Math.min(10, Math.max(1, +e.target.value)),
                        }))
                      }
                  ...
                  <FormControlLabel
                    control={
                      <Switch
                        ...
                        onChange={e => setForm(p => ({ ...p, stopOnError: e.target.checked }))}
                      />
                    }
            ...
            <StepContent>
              ...
              {form.trigger.event === 'status_change' && (
                <Stack spacing={1}>
                  <TextField
                    size="small"
                    label="الحالة المصدر"
                    value={form.trigger.config.fromStatus || ''}
                    onChange={e => setTriggerConfig('fromStatus', e.target.value)}
                  />
                  <TextField
                    size="small"
                    label="الحالة الهدف"
                    value={form.trigger.config.toStatus || ''}
                    onChange={e => setTriggerConfig('toStatus', e.target.value)}
                  />
                </Stack>
              )}
              {form.trigger.event === 'keyword' && (
                <TextField
                  ...
                  value={form.trigger.config.keywords || ''}
                  onChange={e => setTriggerConfig('keywords', e.target.value)}
                />
              )}
              {form.trigger.event === 'schedule' && (
                <TextField
                  ...
                  fullWidth
                  label="تعبير Cron"
                  value={form.trigger.config.cron || ''}
                  onChange={e => setTriggerConfig('cron', e.target.value)}
                  placeholder="0 9 * * *"
                  helperText="مثال: 0 9 * * * (كل يوم الساعة 9)"
                />
              )}
              {form.trigger.event === 'webhook' && (
                <TextField
                  ...
                  onChange={e => setTriggerConfig('secret', e.target.value)}
                />
              )}
              {form.trigger.event === 'date' && (
                <Stack spacing={1}>
                  <TextField
                    size="small"
                    label="حقل التاريخ"
                    value={form.trigger.config.dateField || ''}
                    onChange={e => setTriggerConfig('dateField', e.target.value)}
                  ...
                  <TextField
                    ...
                    label="قبل بكم يوم"
                    type="number"
                    value={form.trigger.config.daysBefore || 0}
                    onChange={e => setTriggerConfig('daysBefore', +e.target.value)}
                  />
                </Stack>
              )}
          ...
          <Step>
            ...
            <StepContent>
              ...
              <Button startIcon={<AddIcon />} size="small" onClick={addCondition} sx={{ mb: 2 }}>
                إضافة شرط
              </Button>
              ...
              <Stack direction="row" spacing={1}>
                ...
                <Button variant="contained" size="small" onClick={() => setActiveStep(3)}>
                  التالي
                </Button>
              </Stack>
            </StepContent>
              ...
              {form.actions.map((a, idx) => {
                ...
                return (
                  <Accordion
                    ...
                    defaultExpanded
                    variant="outlined"
                    sx={{ mb: 1, borderRadius: '8px !important' }}
                  >
                    <AccordionSummary expandIcon={<ExpandMoreIcon />}>
                      <Stack direction="row" spacing={1} alignItems="center">
                        ...
                        {a.delay > 0 && (
                          <Chip label={`تأخير ${a.delay}ث`} size="small" variant="outlined" />
                        )}
                      </Stack>
                    </AccordionSummary>
                    <AccordionDetails>
          ...
          <Paper sx={{ p: 2, mt: 3, borderRadius: 2, bgcolor: '#f0f4ff' }}>
            ...
            <Stack direction="row" spacing={1} alignItems="center" flexWrap="wrap">
              ...
              {form.conditions.length > 0 && (
                <Chip
                  label={`${form.conditions.length} شرط`}
                  size="small"
                  variant="outlined"
                  icon={<RuleIcon />}
                />
              ...
              {form.actions.map((a, i) => {
                ...
                return (
                  <Chip
                    ...
                    label={def?.label}
                    size="small"
                    sx={{ bgcolor: def?.color + '22', color: def?.color }}
                  />
                );
              })}
            </Stack>
