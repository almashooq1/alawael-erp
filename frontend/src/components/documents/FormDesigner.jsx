...
function FieldEditor({
  ...
  const updateValidation = (key, val) =>
    onChange(index, { ...field, validation: { ...field.validation, [key]: val } });
  ...
  return (
    <Paper
      ...
      sx={{
        ...
        mb: 1,
        borderRadius: 2,
        borderRight: `4px solid ${field.required ? '#f44336' : '#1976d2'}`,
    ...
    >
      <Stack direction="row" alignItems="center" spacing={1} mb={1}>
        ...
        <Chip
          label={FIELD_TYPES.find(t => t.type === field.type)?.label || field.type}
          size="small"
          color="primary"
          icon={FIELD_TYPES.find(t => t.type === field.type)?.icon}
        />
        <Typography fontWeight="bold" flex={1}>
          {field.labelAr || 'حقل جديد'}
        </Typography>
          ...
          <Grid container spacing={1}>
            <Grid item xs={6}>
              <FormControl fullWidth size="small">
                ...
                <Select
                  ...
                  label="يعتمد على حقل"
                  onChange={e => update('conditionalOn', e.target.value)}
                >
                  <MenuItem value="">بلا</MenuItem>
                  {allFields
                    .filter((_, fi) => fi !== index)
                    .map((af, fi) => (
                      <MenuItem key={fi} value={af.name}>
                        {af.labelAr || af.name || `حقل ${fi + 1}`}
                      </MenuItem>
                    ))}
                </Select>
              </FormControl>
            ...
            <Grid item xs={6}>
              <TextField
                ...
                onChange={e => update('conditionalValue', e.target.value)}
              />
            </Grid>
          </Grid>
        </AccordionDetails>
      ...
      <Grid container spacing={2}>
        {fields.map((f, i) => (
          <Grid item xs={12} md={f.type === 'textarea' ? 12 : 6} key={i}>
            ...
            {f.type === 'number' && (
              <TextField fullWidth type="number" label={f.labelAr} required={f.required} />
            )}
            {f.type === 'email' && (
              <TextField fullWidth type="email" label={f.labelAr} required={f.required} />
            )}
            {f.type === 'phone' && (
              <TextField fullWidth type="tel" label={f.labelAr} required={f.required} />
            )}
            {f.type === 'url' && (
              <TextField fullWidth type="url" label={f.labelAr} required={f.required} />
            )}
            {f.type === 'date' && (
              <TextField
                fullWidth
            ...
            {f.type === 'radio' && (
              <Box>
                ...
                {(f.options || []).map((o, oi) => (
                  <Chip key={oi} label={o} variant="outlined" sx={{ mr: 0.5, mb: 0.5 }} />
                ))}
              </Box>
            )}
            {f.type === 'file' && (
            ...
            {f.type === 'signature' && (
              <Paper variant="outlined" sx={{ p: 2, textAlign: 'center', borderStyle: 'dashed' }}>
                <SignatureIcon sx={{ fontSize: 40, color: 'text.secondary' }} />
                <Typography variant="body2" color="text.secondary">
                  {f.labelAr || 'منطقة التوقيع'}
                </Typography>
              </Paper>
            )}
            {f.type === 'location' && (
            ...
            {f.type === 'rating' && (
              <Stack direction="row">
                {[1, 2, 3, 4, 5].map(s => (
                  <RatingIcon key={s} sx={{ color: s <= 3 ? '#ffc107' : '#e0e0e0' }} />
                ))}
              </Stack>
            )}
          </Grid>
...
export default function FormDesigner({ open, onClose, onSave, template }) {
  ...
  const addField = type =>
    setFields(prev => [...prev, { ...DEFAULT_FIELD, type, name: `field_${prev.length + 1}` }]);
  ...
  const cloneField = idx =>
    setFields(prev => [
      ...prev.slice(0, idx + 1),
      { ...prev[idx], name: `${prev[idx].name}_copy` },
      ...prev.slice(idx + 1),
    ]);
  ...
  const handleSave = () => {
    onSave({
      ...
      category,
      color,
      fields: fields.map(({ conditionalOn, conditionalValue, ...f }) => ({
        ...f,
      ...
      <DialogTitle>
        <Stack direction="row" justifyContent="space-between" alignItems="center">
          ...
          <Stack direction="row" spacing={1}>
            <Button
              size="small"
              startIcon={previewMode ? <SettingsIcon /> : <PreviewIcon />}
          ...
          <>
            ...
            {fields.length === 0 && (
              <Alert severity="info" sx={{ mb: 2 }}>
                ابدأ بإضافة حقول من شريط الأدوات أعلاه
              </Alert>
            )}
            {fields.map((f, i) => (
              <FieldEditor
