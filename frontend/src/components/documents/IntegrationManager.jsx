                    ...
                    secondaryAction={
                      <Stack direction="row" spacing={0.5}>
                        ...
                        <Tooltip title="السجلات">
                          <IconButton size="small" onClick={() => handleViewLogs(intg._id)}>
                            <LogsIcon />
                          </IconButton>
                        </Tooltip>
                        <Tooltip title="حذف">
                  ...
                  >
                    <ListItemAvatar>
                      <Badge
                        variant="dot"
                        sx={{
                          '& .MuiBadge-badge': { bgcolor: STATUS_COLORS[intg.status] || '#94a3b8' },
                        }}
                        overlap="circular"
                      >
      ...
      <Dialog open={createOpen} onClose={() => setCreateOpen(false)} maxWidth="sm" fullWidth>
        ...
        <DialogContent>
          <Stack spacing={2} mt={1}>
            ...
            <TextField
              ...
              label="URL"
              size="small"
              value={newIntg.config?.url || ''}
              onChange={e =>
                setNewIntg({ ...newIntg, config: { ...newIntg.config, url: e.target.value } })
              }
        ...
        <DialogActions>
          ...
          <Button
            ...
            variant="contained"
            disabled={!newIntg.name || !newIntg.config?.url}
          >
            إنشاء
          </Button>
        </DialogActions>
              ...
              {logs.map(log => (
                <ListItem key={log._id}>
                  <ListItemAvatar>
                    <Avatar
                      sx={{
                        bgcolor: log.status === 'success' ? '#d1fae5' : '#fef2f2',
                        width: 32,
                        height: 32,
                      }}
                    >
