    ...
    <Box dir="rtl">
      ...
      <Stack direction="row" justifyContent="space-between" alignItems="center" mb={2}>
        <Stack direction="row" spacing={1} alignItems="center">
          ...
          {graph && (
            <Chip
              label={`${graph.nodes?.length ?? 0} عقدة • ${graph.edges?.length ?? 0} رابط`}
              size="small"
              color="primary"
              variant="outlined"
            />
        ...
        <Stack direction="row" spacing={0.5}>
          ...
          <IconButton size="small" onClick={() => setZoom(Math.max(zoom - 0.2, 0.4))}>
            <ZoomOut />
          </IconButton>
      ...
      {graph && graph.nodes && graph.nodes.length > 0 && (
        ...
        >
          ...
          <svg
            width="100%"
            height={Math.max(300, graph.nodes.length * 80)}
            viewBox={`0 0 600 ${Math.max(300, graph.nodes.length * 80)}`}
          >
            {/* Edges */}
              ...
              return (
                <g key={`node-${node.id}`}>
                  <circle
                    ...
                    cy={cy}
                    r={isRoot ? 30 : 22}
                    fill={isRoot ? '#3b82f6' : '#e2e8f0'}
                    stroke={isRoot ? '#1e40af' : '#94a3b8'}
                    strokeWidth={2}
                  />
                  <text
                    x={cx}
                    y={cy + 4}
                    fill={isRoot ? '#fff' : '#374151'}
        ...
        <Stack direction="row" spacing={1} flexWrap="wrap" useFlexGap>
          {linkTypes.map(type => (
            <Chip
              ...
              size="small"
              sx={{
                bgcolor: (LINK_COLORS[type.key] || '#999') + '20',
                color: LINK_COLORS[type.key] || '#999',
              }}
            />
          ))}
        </Stack>
        ...
        <CardContent>
          ...
          {links.length === 0 ? (
            ...
            <List dense>
              {links.map((link, i) => {
                const doc =
                  link.relDirection === 'outbound' ? link.targetDocument : link.sourceDocument;
                return (
                  <React.Fragment key={link._id || i}>
                    <ListItem>
                      <ListItemAvatar>
                        <Avatar
                          sx={{
                            bgcolor: (LINK_COLORS[link.linkType] || '#999') + '30',
                            width: 36,
                            height: 36,
                          }}
                        >
                          <LinkIcon
                            sx={{ color: LINK_COLORS[link.linkType] || '#999', fontSize: 18 }}
                          />
                        </Avatar>
                      </ListItemAvatar>
                      <ListItemText
                        ...
                        secondary={
                          <Stack direction="row" spacing={1} component="span" alignItems="center">
                            <Chip
                              label={link.typeInfo?.labelAr || link.linkType}
                              size="small"
                              sx={{
                                bgcolor: (LINK_COLORS[link.linkType] || '#999') + '20',
                                height: 20,
                              }}
                            />
                            <span>
                              {link.relDirection === 'outbound' ? '← يشير إلى' : '→ يُشار من'}
                            </span>
                          </Stack>
                        }
                      />
