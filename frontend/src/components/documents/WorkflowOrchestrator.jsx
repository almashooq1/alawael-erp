...
function NodeEditor({
  ...
  const updateConfig = (key, val) =>
    onChange(index, { ...node, config: { ...node.config, [key]: val } });
  ...
  const _incomingEdges = edges.filter(e => e.targetNodeId === node.nodeId);

  return (
    <Paper
      dir="rtl"
      sx={{ p: 2, mb: 1.5, borderRadius: 2, borderRight: `4px solid ${typeInfo.color}` }}
    >
      <Stack direction="row" alignItems="center" spacing={1} mb={1.5}>
        ...
        <Chip
          ...
          size="small"
          sx={{ bgcolor: `${typeInfo.color}15`, color: typeInfo.color }}
        />
        <Typography fontWeight="bold" flex={1}>
          {node.nameAr || node.name || node.nodeId}
        </Typography>
        <Tooltip title="أعلى">
          <span>
        ...
        <Grid container spacing={1.5} mt={0.5}>
          ...
          <Grid item xs={6} md={3}>
            <FormControl fullWidth size="small">
              ...
              <Select
                ...
                label="نوع المسند"
                onChange={e => updateConfig('assigneeType', e.target.value)}
              >
                <MenuItem value="user">مستخدم</MenuItem>
                <MenuItem value="role">دور</MenuItem>
                <MenuItem value="department">قسم</MenuItem>
          ...
          <Grid item xs={6} md={3}>
            <TextField
              ...
              type="number"
              label="المهلة (ساعات)"
              value={node.config?.dueHours || ''}
              onChange={e => updateConfig('dueHours', parseInt(e.target.value))}
            />
          </Grid>
          <Grid item xs={6} md={3}>
            <FormControl fullWidth size="small">
      ...
      {node.type === 'timer' && (
        <TextField
          fullWidth
          size="small"
          label="المدة (ISO 8601)"
          value={node.config?.timerDuration || ''}
          onChange={e => updateConfig('timerDuration', e.target.value)}
          sx={{ mt: 1 }}
          placeholder="PT1H30M"
        />
      )}
      ...
      {node.type === 'signal' && (
        <TextField
          ...
          size="small"
          label="اسم الإشارة"
          value={node.config?.signalName || ''}
          onChange={e => updateConfig('signalName', e.target.value)}
          sx={{ mt: 1 }}
        />
      )}
      ...
      <Box mt={1.5}>
        <Stack direction="row" spacing={2} alignItems="center" mb={0.5}>
          ...
          <Typography variant="caption" color="text.secondary">
            الاتصالات الصادرة ({outgoingEdges.length})
          </Typography>
          <FormControl size="small" sx={{ minWidth: 150 }}>
            <Select
              ...
              value=""
              onChange={e => {
                if (e.target.value) onAddEdge(node.nodeId, e.target.value);
              }}
              renderValue={v => v || 'إضافة اتصال...'}
            >
              {allNodes
                .filter(
                  n =>
                    n.nodeId !== node.nodeId &&
                    !outgoingEdges.some(oe => oe.targetNodeId === n.nodeId)
                )
                .map(n => (
                  <MenuItem key={n.nodeId} value={n.nodeId}>
                    {n.nameAr || n.name || n.nodeId}
                  </MenuItem>
                ))}
            </Select>
          </FormControl>
        ...
        <Stack direction="row" spacing={0.5} flexWrap="wrap">
          {outgoingEdges.map((e, i) => {
            ...
            return (
              <Chip
                key={i}
                label={`→ ${target?.nameAr || target?.name || e.targetNodeId}`}
                size="small"
                onDelete={() => onRemoveEdge(e.edgeId)}
                variant="outlined"
              />
            );
          })}
        ...
        {Object.entries(nodesByType).map(([type, count]) => {
          ...
          return (
            <Chip
              key={type}
              icon={info.icon}
              label={`${info.label || type}: ${count}`}
              variant="outlined"
            />
          );
        })}
        <Chip
          label={`${definition.edges?.length || 0} اتصال`}
          icon={<LinkIcon />}
          variant="outlined"
        />
      </Stack>
      ...
      {definition.nodes.map((node, i) => {
        ...
        return (
          <Card
            key={i}
            variant="outlined"
            sx={{ mb: 1, borderRadius: 2, borderRight: `3px solid ${info.color || '#999'}` }}
          >
            <CardContent sx={{ py: 1, '&:last-child': { pb: 1 } }}>
              <Stack direction="row" spacing={1} alignItems="center">
                <Avatar sx={{ width: 28, height: 28, bgcolor: info.color || '#999' }}>
                  {info.icon || <TaskIcon />}
                </Avatar>
...
export default function WorkflowOrchestrator({ open, onClose, definition, onSave }) {
  ...
  const addNode = type => {
    ...
    setNodes(prev => [
      ...prev,
      {
        nodeId: id,
        type,
        nameAr: NODE_TYPES.find(t => t.type === type)?.label || '',
        name: '',
        config: {},
      },
    ]);
  ...
  const cloneNode = idx => {
    ...
    setNodes(prev => [
      ...prev.slice(0, idx + 1),
      { ...src, nodeId: newId, nameAr: `${src.nameAr} (نسخة)` },
      ...prev.slice(idx + 1),
    ]);
  };
  ...
  const validate = () => {
    ...
    if (starts.length !== 1)
      issues.push({
        severity: 'error',
        message: `يجب أن يحتوي على نقطة بداية واحدة (وُجد ${starts.length})`,
      });
    if (ends.length < 1)
    ...
    for (const e of edges) {
      if (!nodeIds.has(e.sourceNodeId))
        issues.push({
          severity: 'error',
          message: `الحافة ${e.edgeId}: المصدر ${e.sourceNodeId} غير موجود`,
        });
      if (!nodeIds.has(e.targetNodeId))
        issues.push({
          severity: 'error',
          message: `الحافة ${e.edgeId}: الهدف ${e.targetNodeId} غير موجود`,
        });
    }

    for (const n of nodes) {
      if (n.type === 'start') continue;
      if (!edges.some(e => e.targetNodeId === n.nodeId))
        issues.push({
          severity: 'warning',
          message: `العقدة "${n.nameAr || n.nodeId}" لا يوجد اتصال وارد`,
        });
    }
        ...
        <Stack direction="row" justifyContent="space-between" alignItems="center">
          <Stack direction="row" spacing={2} alignItems="center">
            ...
            <Typography variant="h6" fontWeight="bold">
              {definition?._id ? 'تعريف سير العمل' : 'إنشاء تعريف جديد'}
            </Typography>
            {definition?.status && (
              <Chip
                label={definition.status === 'active' ? 'نشط' : 'مسودة'}
                color={definition.status === 'active' ? 'success' : 'default'}
                size="small"
              />
            )}
          </Stack>
        ...
        <Grid container spacing={2} mb={2}>
          ...
          <Grid item xs={12} md={2}>
            <FormControl fullWidth size="small" disabled={!editMode}>
              ...
              <Select value={category} label="التصنيف" onChange={e => setCategory(e.target.value)}>
                {['عام', 'مشتريات', 'عقود', 'مالية', 'موارد بشرية', 'تقنية'].map(c => (
          ...
          <Grid item xs={6} md={2}>
            <TextField
              ...
              type="number"
              label="مدة SLA (ساعات)"
              value={sla.maxDurationHours || ''}
              onChange={e => setSla(p => ({ ...p, maxDurationHours: e.target.value }))}
              disabled={!editMode}
            />
          </Grid>
          <Grid item xs={6} md={2}>
        ...
        {validation && (
          <Box mb={2}>
            <Alert
              ...
              icon={validation.valid ? <ValidIcon /> : <ErrorIcon />}
            >
        ...
        {editMode ? (
          <>
              ...
              <Stack direction="row" flexWrap="wrap" gap={0.5}>
                {NODE_TYPES.map(nt => (
                  <Chip
                    ...
                    clickable
                    size="small"
                    sx={{ borderColor: nt.color, color: nt.color }}
                  />
                ))}
              </Stack>
            </Paper>
            ...
            <Typography variant="subtitle1" fontWeight="bold" gutterBottom>
              العقد ({nodes.length}) | الاتصالات ({edges.length})
            </Typography>
