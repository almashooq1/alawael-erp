...
function TrendIndicator({ trend, gradient }) {
  ...
  return (
    <Box
      sx={{ display: 'flex', alignItems: 'center', justifyContent: 'center', mt: 0.5, gap: 0.3 }}
    >
      ...
      {stats.map((stat, i) => {
        ...
        return (
          <Grid item xs={6} sm={4} md={gridSize} key={stat.label || i}>
            <Card
              ...
              sx={{
                ...
                border: isOutlined
                  ...
                  : !stat.gradient
                    ? `1px solid ${alpha(statColor, isDark ? 0.15 : 0.12)}`
                    : 'none',
                '&:hover':
                  stat.onClick || stat.path
                    ? {
            ...
            >
              <CardContent
                sx={{ p: isCompact ? '10px !important' : '14px !important', textAlign: 'center' }}
              >
                {stat.icon && (
                  <Avatar
                    sx={{
