...
import { Box, Typography, Grid, Skeleton, Chip, Tooltip } from '@mui/material';
...
const DEMO = {
  ...
  scatter: [
    { x: 45, y: 87, color: '#6366f1', size: 8 },
    { x: 78, y: 92, color: '#22c55e', size: 10 },
    { x: 32, y: 65, color: '#f59e0b', size: 7 },
    { x: 91, y: 78, color: '#ef4444', size: 9 },
    { x: 55, y: 95, color: '#06b6d4', size: 12 },
    { x: 67, y: 84, color: '#8b5cf6', size: 8 },
    { x: 23, y: 72, color: '#ec4899', size: 6 },
    { x: 84, y: 88, color: '#10b981', size: 11 },
    { x: 40, y: 61, color: '#f97316', size: 7 },
    { x: 72, y: 79, color: '#0ea5e9', size: 9 },
  ],
  hourly: [
    12, 18, 25, 42, 67, 89, 134, 178, 213, 245, 267, 289, 312, 298, 276, 241, 198, 167, 134, 98, 72,
    54, 38, 21,
  ],
  geographic: [
  ...
  insights: [
    {
      icon: '📈',
      title: 'نمو قياسي',
      text: 'زيادة 22% في الإيرادات هذا الربع مدفوعة بارتفاع استخدام التقارير التلقائية والذكاء الاصطناعي.',
      color: '#22c55e',
    },
    {
      icon: '🕐',
      title: 'ذروة الاستخدام',
      text: 'أعلى نشاط بين 10:00 و14:00. يُنصح بجدولة الصيانة خارج هذه الفترة.',
      color: '#6366f1',
    },
    {
      icon: '📱',
      title: 'الجوال في تصاعد',
      text: 'ارتفع استخدام الجوال من 24% إلى 32% خلال 6 أشهر. يُنصح بتحسين تجربة الجوال.',
      color: '#f59e0b',
    },
    {
      icon: '🎯',
      title: 'صفحة الجدول الأعلى',
      text: 'صفحة الجدول الزمني تحقق أعلى معدل تفاعل (8.9 دق/جلسة) مقارنة بباقي الصفحات.',
      color: '#06b6d4',
    },
  ],
};
        ...
        const kpis = DEMO.kpis.map((k, i) => {
          if (i === 0)
            return {
              ...k,
              value:
                s.kpis?.sessions?.total != null
                  ? s.kpis.sessions.total.toLocaleString('ar-SA')
                  : k.value,
            };
          if (i === 1)
            return {
              ...k,
              value:
                s.kpis?.users?.active != null
                  ? s.kpis.users.active.toLocaleString('ar-SA')
                  : k.value,
            };
          if (i === 4)
            return {
            ...
            return {
              ...
              value:
                s.clinical?.feedback?.average != null
                  ? `${Number(s.clinical.feedback.average).toFixed(1)}/5`
                  : k.value,
            };
          return k;
        });
            ...
            {['هذا الأسبوع', 'هذا الشهر', 'هذا الربع', 'هذا العام'].map(p => (
              <motion.button
                ...
                style={{
                  ...
                  cursor: 'pointer',
                  color:
                    period === p ? '#fff' : isDark ? 'rgba(255,255,255,0.65)' : 'rgba(0,0,0,0.6)',
                ...
                <Glass sx={{ p: 3, height: '100%' }}>
                  ...
                  {(data?.deviceDist || []).map((d, i) => (
                    <Box key={i} sx={{ mb: 2 }}>
                      <Box sx={{ display: 'flex', justifyContent: 'space-between', mb: 0.5 }}>
                        ...
                        <Typography variant="caption" fontWeight={800} sx={{ color: d.color }}>
                          {d.pct}%
                        </Typography>
                      </Box>
                      <Box
                  ...
                  >
                    {[
                      ...
                      <Box
                        key={i}
                        sx={{ display: 'flex', justifyContent: 'space-between', mb: i < 2 ? 1 : 0 }}
                      >
                        <Typography
                          variant="caption"
                          sx={{ color: isDark ? 'rgba(255,255,255,0.5)' : 'rgba(0,0,0,0.5)' }}
              ...
              <Grid item xs={12} md={6}>
                <Glass sx={{ p: 3 }}>
                  <Typography
                    ...
                    sx={{ color: isDark ? 'rgba(255,255,255,0.9)' : 'rgba(0,0,0,0.85)' }}
                  >
                    🕐 نشاط الاستخدام (24 ساعة)
                  </Typography>
                  <Box
                    sx={{ display: 'flex', alignItems: 'flex-end', gap: '2px', height: 80, mt: 2 }}
                  >
                    {(data?.hourly || []).map((v, i) => {
            ...
            <Grid container spacing={3}>
              <Grid item xs={12}>
                <Glass sx={{ p: 3 }}>
                  ...
                  >
                    {[
                      ['المستخدمون النشطون', '#6366f1', '+18%'],
                      ['إجمالي الزيارات', '#22c55e', '+24%'],
                      ['التحويلات', '#f59e0b', '+11%'],
                    ].map(([l, c, p], i) => (
                      <Box
                        key={i}
              ...
              <Grid item xs={12} md={6}>
                <Glass sx={{ p: 3 }}>
                  ...
                  <Typography
                    ...
                    sx={{ color: isDark ? 'rgba(255,255,255,0.45)' : 'rgba(0,0,0,0.45)' }}
                  >
                    محور X: الزيارات • محور Y: التحويلات • الحجم: الإيرادات
                  </Typography>
                      ...
                      >
                        ...
                        <Box sx={{ textAlign: 'center' }}>
                          <Typography
                            ...
                            sx={{
                              color:
                                p.score >= 8.5 ? '#22c55e' : p.score >= 7 ? '#f59e0b' : '#ef4444',
                            }}
                          >
                            {p.score}
                          </Typography>
                        ...
                        <Box sx={{ flex: 1 }}>
                          <Box sx={{ display: 'flex', justifyContent: 'space-between', mb: 0.5 }}>
                            ...
                            <Typography variant="caption" fontWeight={700} sx={{ color: g.color }}>
                              {g.users.toLocaleString()} مستخدم ({g.pct}%)
                            </Typography>
                          </Box>
                          <Box
                            sx={{
                        ...
                        >
                          ...
                          >
                            ...
                            <Typography
                              ...
                              sx={{ color: isDark ? 'rgba(255,255,255,0.45)' : 'rgba(0,0,0,0.45)' }}
                            >
                              {b.sessions.toLocaleString()} جلسة
                            </Typography>
                            <Chip
                              label={b.growth}
                              size="small"
                ...
                <Glass sx={{ p: 3 }}>
                  ...
                  >
                    ...
                    {[
                      ...
                      {
                        from: 'الدخول → استخدام الميزات',
                        loss: '42%',
                        reason: 'بحاجة لتوجيه أفضل',
                      },
                    ].map((d, i) => (
              ...
              <Grid item xs={12} md={5}>
                <Glass sx={{ p: 3 }}>
                  ...
                  {[
                    ...
                    {
                      label: 'تحويل الزيارة → تسجيل',
                      value: '38.7%',
                      color: '#6366f1',
                      prev: '34.2%',
                    },
                    {
                      label: 'تحويل التسجيل → استخدام',
                      value: '58.3%',
                      color: '#f59e0b',
                      prev: '52.1%',
                    },
                    {
                      label: 'معدل الاحتفاظ (30 يوم)',
                      value: '72.4%',
                      color: '#06b6d4',
                      prev: '68.9%',
                    },
                  ...
                  <Box sx={{ display: 'flex', alignItems: 'center', gap: 2, mb: 3 }}>
                    ...
                    <Box>
                      ...
                      <Typography
                        ...
                        sx={{ color: isDark ? 'rgba(255,255,255,0.45)' : 'rgba(0,0,0,0.45)' }}
                      >
                        تحليل عميق للبيانات بالذكاء الاصطناعي
                      </Typography>
                    </Box>
                  </Box>
                  <Grid container spacing={2}>
