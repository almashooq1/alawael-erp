...
const ActionChip = ({ action }) => {
  const map = {
    ...
    LOGIN: 'primary',
    LOGOUT: 'default',
    ZATCA_SUBMIT: 'warning',
    NPHIES_CLAIM: 'secondary',
    MUQEEM_QUERY: 'info',
    GOSI_SYNC: 'primary',
  };
...
const IntegrationBadges = ({ log }) => (
  <Box display="flex" gap={0.5}>
    {log.zatcaRelated && (
      <Tooltip title="ZATCA">
        <ZatcaIcon fontSize="small" color="warning" />
      </Tooltip>
    )}
    {log.nphiesRelated && (
      <Tooltip title="NPHIES">
        <NphiesIcon fontSize="small" color="secondary" />
      </Tooltip>
    )}
    {log.muqeemRelated && (
      <Tooltip title="مقيم">
        <MuqeemIcon fontSize="small" color="info" />
      </Tooltip>
    )}
    {log.gosiRelated && (
      <Tooltip title="GOSI">
        <GosiIcon fontSize="small" color="primary" />
      </Tooltip>
    )}
  </Box>
);
        ...
        <Grid container spacing={2} mb={3}>
          ...
          <Grid item xs={6} sm={3}>
            <StatCard
              ...
              value={stats.integrationEvents?.toLocaleString()}
              icon={<SecurityIcon />}
              color="warning.main"
            />
          </Grid>
      ...
      {stats && (
        <Grid container spacing={2} mb={3}>
          {[
            {
              label: 'ZATCA',
              value: stats.zatcaEvents,
              icon: <ZatcaIcon />,
              color: 'warning.main',
            },
            {
              label: 'NPHIES',
              value: stats.nphiesEvents,
              icon: <NphiesIcon />,
            ...
            <Grid container spacing={2}>
              ...
              <Grid item xs={12} sm={6} md={4}>
                <TextField
                  fullWidth
                  size="small"
                  label="بحث نصي (endpoint / resource)"
                  ...
                  {logs.map((log, idx) => (
                    <React.Fragment key={log._id || idx}>
                      <TableRow
                        ...
                        sx={{
                          cursor: 'pointer',
                          bgcolor: log.success === false ? 'error.light' : 'inherit',
                        }}
