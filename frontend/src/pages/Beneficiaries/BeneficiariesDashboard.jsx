...
export default function BeneficiariesDashboard() {
  ...
  const [byCategory, setByCategory] = useState(DATA_BY_CATEGORY);
  const [statusDist, setStatusDist] = useState(DATA_STATUS_DIST);
  const [monthlyReg, setMonthlyReg] = useState(DATA_MONTHLY_REG);
  const [progressDist, setProgressDist] = useState(DATA_PROGRESS_DIST);
  const [ageDist, setAgeDist] = useState(DATA_AGE_DIST);
  ...
  const loadDashboard = useCallback(async () => {
    ...
    try {
      ...
      if (sData) {
        ...
        setStats({
          total: sData.total ?? DATA_STATS.total,
          active: sData.byStatus?.active ?? sData.active ?? DATA_STATS.active,
          pending: sData.byStatus?.pending ?? sData.pending ?? DATA_STATS.pending,
          inactive: sData.byStatus?.inactive ?? sData.inactive ?? DATA_STATS.inactive,
          newThisMonth: sData.newThisMonth ?? DATA_STATS.newThisMonth,
          avgProgress: sData.avgProgress ?? DATA_STATS.avgProgress,
          avgSessions: sData.avgSessions ?? DATA_STATS.avgSessions,
          completionRate: sData.completionRate ?? DATA_STATS.completionRate,
        });

        /* by category */
        if (sData.byCategory && sData.byCategory.length > 0) {
          setByCategory(
            sData.byCategory.map(c => ({
        ...
        if (sData.monthlyRegistrations && sData.monthlyRegistrations.length > 0) {
          ...
          setMonthlyReg(
            sData.monthlyRegistrations.map(r => ({
        ...
        if (sData.progressDistribution && sData.progressDistribution.length > 0) {
          ...
          setProgressDist(
            sData.progressDistribution.map((p, i) => ({
        ...
        if (sData.ageDistribution && sData.ageDistribution.length > 0) {
          ...
          setAgeDist(
            sData.ageDistribution.map(a => ({
              range:
                ageLabels[a._id?.min ?? a.min] ||
                a.range ||
                `${a._id?.min || 0}-${a._id?.max || 99}`,
              count: a.count || 0,
            }))
          );
        }
        ...
        if (Array.isArray(beneficiaries) && beneficiaries.length > 0) {
          const { active, pending, inactive } = computeStatusCounts(beneficiaries, 'status', [
            'active',
            'pending',
            'inactive',
          ]);
          ...
          const avgProg = Math.round(
            beneficiaries.reduce((s, b) => s + (b.progress || 0), 0) / beneficiaries.length
          );
          const avgSess = (
            beneficiaries.reduce((s, b) => s + (b.sessions || 0), 0) / beneficiaries.length
          ).toFixed(1);
          ...
          setStats({
            ...
            completionRate: Math.round(
              (beneficiaries.filter(b => (b.progress || 0) >= 80).length / beneficiaries.length) *
                100
            ),
          });
        }
          ...
          <Box sx={{ display: 'flex', alignItems: 'center', gap: 2 }}>
            ...
            <Box>
              ...
              <Typography variant="body2" sx={{ opacity: 0.9 }}>
                تحليل وإحصائيات المستفيدين من خدمات المركز
              </Typography>
            </Box>
          </Box>
          <Button
            variant="contained"
            color="inherit"
            sx={{ color: brandColors.primaryStart, fontWeight: 600 }}
        ...
        <Grid container spacing={2} sx={{ mb: 4 }}>
          ...
          <Grid item xs={12} sm={6} md={3} lg={2}>
            <ModuleKPICard
              title="نشطون"
              value={stats.active}
              subtitle={`${Math.round((stats.active / Math.max(stats.total, 1)) * 100)}% من الإجمالي`}
              icon={<ActiveIcon />}
              color="success"
            />
          </Grid>
            ...
            >
              ...
              {monthlyReg.length > 0 ? (
                <ResponsiveContainer
                  ...
                  height={280}
                  role="img"
                  aria-label="رسم بياني للتسجيل الشهري"
                >
                  <AreaChart data={monthlyReg}>
                    <defs>
                      <linearGradient id="benReg" x1="0" y1="0" x2="0" y2="1">
                        <stop offset="5%" stopColor={brandColors.primaryStart} stopOpacity={0.2} />
                        <stop
                          offset="95%"
                          stopColor={brandColors.primaryStart}
                          stopOpacity={0.01}
                        />
                      </linearGradient>
                      <linearGradient id="benActive" x1="0" y1="0" x2="0" y2="1">
                        <stop offset="5%" stopColor={statusColors.success} stopOpacity={0.15} />
                        <stop offset="95%" stopColor={statusColors.success} stopOpacity={0.01} />
                      </linearGradient>
                    </defs>
                    ...
                    <Area
                      ...
                      strokeWidth={2.5}
                      name="التسجيلات"
                      dot={{ r: 4, fill: brandColors.primaryStart }}
                      activeDot={{ r: 6 }}
                    />
                    <Area
                      type="monotone"
                      ...
                      strokeWidth={2}
                      name="نشطون"
                      dot={{ r: 3, fill: statusColors.success }}
                    />
                  </AreaChart>
                </ResponsiveContainer>
              ) : (
            ...
            >
              ...
              {statusDist.length > 0 ? (
                <ResponsiveContainer
                  ...
                  height={280}
                  role="img"
                  aria-label="رسم بياني لحالة المستفيدين"
                >
                  <PieChart>
                    <Pie
                      data={statusDist}
            ...
            >
              ...
              {progressDist.length > 0 ? (
                <ResponsiveContainer
                  ...
                  height={260}
                  role="img"
                  aria-label="رسم بياني لتوزيع التقدم"
                >
            ...
            >
              ...
              {ageDist.length > 0 ? (
                <ResponsiveContainer
                  ...
                  height={260}
                  role="img"
                  aria-label="رسم بياني للفئات العمرية"
                >
                  <BarChart data={ageDist} layout="vertical" barSize={20}>
                    <defs>
                      <linearGradient id="benAgeGrad" x1="0" y1="0" x2="1" y2="0">
              ...
              <TableBody>
                {recentList.map(b => (
                  <TableRow
                    ...
                    onClick={() => navigate(`/beneficiary-portal/${b.id}`)}
                  >
