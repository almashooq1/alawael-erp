...
import { useBranchAnalytics, useBranchKPIs, useBranchTargets } from '../hooks/useBranchData';
...
const ScoreGauge = memo(({ score = 0, grade = 'B', size = 110 }) => {
  ...
  return (
    <Box sx={{ position: 'relative', width: size, height: size }}>
      ...
      >
        <Typography sx={{ fontSize: '1.3rem', fontWeight: 900, color, lineHeight: 1 }}>
          {score}
        </Typography>
        <Typography sx={{ fontSize: '0.65rem', fontWeight: 700, color, opacity: 0.7 }}>
          {grade}
        </Typography>
      </Box>
    </Box>
...
const MetricPill = memo(({ label, value, active, onClick, color = '#3b82f6' }) => (
  ...
  >
    {label}
    {value != null && (
      <Typography component="span" sx={{ ml: 0.8, fontSize: '0.68rem', opacity: 0.7 }}>
        {value}
      </Typography>
    )}
  </Box>
...
const RecCard = memo(({ rec, idx }) => {
  ...
  const pMap = {
    ...
    medium: {
      border: '#eab308',
      bg: isDark ? 'rgba(234,179,8,0.08)' : '#fffbeb',
      text: '#92400e',
      badge: '#eab308',
      label: 'متوسط',
    },
          ...
          <Box sx={{ flex: 1 }}>
            ...
            {rec.action_ar && (
              <Box
                sx={{
                  ...
                  py: 0.6,
                  borderRadius: 1.5,
                  background: alpha(s.border, 0.08),
                  fontSize: '0.75rem',
                  fontWeight: 600,
                  color: 'text.primary',
                }}
            ...
            {rec.expected_impact && (
              <Typography sx={{ fontSize: '0.68rem', color: 'text.secondary', mt: 0.6 }}>
                التأثير المتوقع:{' '}
                <Box component="strong" sx={{ color: s.border }}>
                  {rec.expected_impact}
                </Box>
              </Typography>
            )}
...
const AnomalyRow = memo(({ anomaly, idx }) => {
  ...
  return (
    ...
    >
      ...
      >
        <Box>
          <Typography sx={{ fontSize: '0.82rem', fontWeight: 700, color: '#92400e' }}>
            {anomaly.metric}
          </Typography>
          <Typography sx={{ fontSize: '0.72rem', color: 'text.secondary', mt: 0.2 }}>
            القيمة: {anomaly.value?.toFixed(1)} · المعتاد: {anomaly.mean?.toFixed(1)} ±{' '}
            {anomaly.std?.toFixed(1)}
          </Typography>
        </Box>
        <Box
          sx={{
...
const MetricMiniCard = memo(({ metric, trend, onClick, selected, idx }) => {
  const dirColor =
    trend?.direction === 'up' ? '#10b981' : trend?.direction === 'down' ? '#ef4444' : '#f59e0b';
  const dirIcon =
    trend?.direction === 'up' ? (
...
export default function BranchAnalytics({ branchCode }) {
  ...
  } = useBranchAnalytics(branchCode, { days, forecastMetric: selectedMetric, forecastDays: 7 });
  const { kpis, loading: kpisLoading } = useBranchKPIs(branchCode);
  const { targets, loading: _targetsLoading } = useBranchTargets(
    branchCode,
      ...
      >
        ...
        >
          ...
          <Box sx={{ display: 'flex', alignItems: 'center', gap: 0.8 }}>
            <Typography sx={{ fontSize: '0.75rem', color: 'text.secondary', mr: 0.5 }}>
              نطاق:
            </Typography>
            {[7, 14, 30, 60, 90].map(d => (
              <Box
        ...
        >
          ...
          >
            ...
            <Box sx={{ display: 'flex', alignItems: 'center', gap: 2.5 }}>
              <ScoreGauge
                score={
                  metricTrend?.last_value ? Math.min(100, Math.round(metricTrend.last_value)) : 0
                }
                grade="B+"
                size={110}
              />
              <Box sx={{ color: '#fff' }}>
                <Typography sx={{ fontSize: '0.8rem', opacity: 0.75, mb: 0.5 }}>
                  آخر قيمة
                </Typography>
                ...
                {metricTrend?.slope != null && (
                  <Box sx={{ display: 'flex', alignItems: 'center', gap: 0.5, mt: 0.8 }}>
                    ...
                    <Typography
                      sx={{
                        fontSize: '0.72rem',
                        color: metricTrend.slope >= 0 ? '#86efac' : '#fca5a5',
                        fontWeight: 700,
                      }}
                    >
                      {metricTrend.slope >= 0 ? '+' : ''}
                      {metricTrend.slope.toFixed(2)}/يوم
                    </Typography>
                  </Box>
                )}
              </Box>
            ...
            >
              {METRICS.slice(0, 4).map(m => {
                ...
                return (
                  ...
                  >
                    <Typography
                      sx={{ fontSize: '0.68rem', color: 'rgba(255,255,255,0.75)', mb: 0.3 }}
                    >
                      {m.label_ar}
                    </Typography>
                    ...
                    <Typography sx={{ fontSize: '0.65rem', color: dirColor, fontWeight: 700 }}>
                      ...
                      {t?.slope != null
                        ? ` ${t.slope > 0 ? '+' : ''}${t.slope.toFixed(2)}/يوم`
                        : ''}
                    </Typography>
                  </Box>
                );
                ...
                <Box sx={{ display: 'flex', gap: 0.8, flexWrap: 'wrap', mb: 2 }}>
                  {METRICS.map(m => (
                    <MetricPill
                      key={m.value}
                      label={m.label_ar}
                      value={trends?.[m.value]?.last_value?.toFixed(1)}
                  ...
                  {metricTrend ? (
                    <Box sx={{ p: 3 }}>
                      ...
                      >
                        <StatMini
                          label="آخر قيمة"
                          value={metricTrend.last_value?.toFixed(2)}
                          color="#3b82f6"
                        />
                        <StatMini
                          label="المتوسط"
                          value={metricTrend.avg?.toFixed(2)}
                          color="#8b5cf6"
                        />
                        <StatMini
                          label="الميل/يوم"
                          value={metricTrend.slope?.toFixed(3)}
                          color={metricTrend.slope >= 0 ? '#10b981' : '#ef4444'}
                        />
                        <StatMini
                          label="دقة R²"
                          value={metricTrend.r2?.toFixed(3)}
                          color="#f59e0b"
                        />
                      </Box>
                      <Box sx={{ overflowX: 'auto', pb: 0.5 }}>
                        <LineChart
                          ...
                          color="#3b82f6"
                          height={150}
                          width={Math.max(400, metricHistory.length * 14)}
                  ...
                  {forecast ? (
                      ...
                      >
                        <StatMini
                          ...
                          value={
                            forecast.predictions
                              ? (
                                  forecast.predictions.reduce((s, p) => s + (p.predicted ?? 0), 0) /
                        ...
                        <StatMini
                          ...
                          value={
                            forecast.predictions
                              ? Math.max(
                                  ...forecast.predictions.map(p => p.predicted ?? 0)
                                ).toFixed(1)
                              : '—'
                          }
                        />
                      ...
                      {forecast.model_quality && (
                        <Box
                          sx={{
                            ...
                            p: 1.5,
                            borderRadius: 2,
                            background: isDark ? 'rgba(255,255,255,0.04)' : alpha('#8b5cf6', 0.05),
                            display: 'flex',
                            gap: 3,
                            flexWrap: 'wrap',
                          }}
                        >
                          {[
                            {
                              label: 'دقة النموذج',
                              value: `${(forecast.model_quality?.r2 * 100)?.toFixed(1)}%`,
                            },
                    ...
                    <Box sx={{ p: 5, textAlign: 'center', color: 'text.secondary' }}>
                      ...
                      <Typography sx={{ fontWeight: 600, mb: 0.5 }}>
                        لا تتوفر بيانات كافية للتوقع
                      </Typography>
                      <Typography sx={{ fontSize: '0.8rem', opacity: 0.7 }}>
                        يتطلب النموذج 14 يوم على الأقل من البيانات
                      </Typography>
                    </Box>
                  )}
                </Glass>
                ...
                <Box sx={{ p: 3 }}>
                  {anomalies?.length > 0 ? (
                    <>
                      ...
                      >
                        ...
                        <Typography sx={{ fontSize: '0.82rem', color: '#92400e', fontWeight: 600 }}>
                          تم رصد <Box component="strong">{anomalies.length} شذوذ</Box> في آخر 14
                          يوم. Z &gt; 2 تحذير · Z &gt; 3 حالة حرجة
                        </Typography>
                      </Box>
                      {anomalies.map((a, i) => (
                    ...
                    <Box sx={{ textAlign: 'center', py: 5 }}>
                      ...
                      <Typography
                        sx={{ fontWeight: 700, color: '#10b981', fontSize: '1rem', mb: 0.5 }}
                      >
                        لم يُرصد أي شذوذ
                      </Typography>
                      <Typography sx={{ color: 'text.secondary', fontSize: '0.85rem' }}>
                        جميع المقاييس ضمن النطاق الطبيعي خلال آخر 14 يوم
                      </Typography>
                    </Box>
                  )}
                </Box>
              ...
              <Box>
                  ...
                  >
                    ...
                    <Box>
                      ...
                      <Typography sx={{ color: 'rgba(255,255,255,0.75)', fontSize: '0.78rem' }}>
                        {recommendations?.length ?? 0} توصية مخصصة بناءً على تحليل بيانات آخر {days}{' '}
                        يوم
                      </Typography>
                    </Box>
                  </Box>
                ...
                {recommendations?.length > 0 ? (
                  ...
                  <Glass sx={{ p: 4, textAlign: 'center' }}>
                    ...
                    <Typography
                      sx={{ fontWeight: 700, color: '#10b981', fontSize: '1.05rem', mb: 0.5 }}
                    >
                      أداء ممتاز!
                    </Typography>
                    <Typography sx={{ color: 'text.secondary', fontSize: '0.85rem' }}>
                      لا توجد توصيات تحسين في الوقت الحالي
                    </Typography>
                  </Glass>
                )}
              </Box>
                  ...
                  <Box sx={{ p: 3 }}>
                    {targets && kpis ? (
                      ...
                      <Box sx={{ textAlign: 'center', py: 4, color: 'text.secondary' }}>
                        ...
                        <Typography sx={{ fontSize: '0.85rem' }}>
                          لم يتم تعيين الأهداف بعد لهذا الشهر
                        </Typography>
                        ...
                        {Object.entries(kpis).map(([key, stat]) => {
                          ...
                          return (
                            ...
                            >
                              ...
                              >
                                <Typography
                                  sx={{ fontSize: '0.68rem', color: 'text.secondary', mb: 0.5 }}
                                >
                                  {key.replace(/_/g, ' ')}
                                </Typography>
                                <Typography sx={{ fontSize: '1.3rem', fontWeight: 800, color }}>
                                  ...
                                  <Typography
                                    component="span"
                                    sx={{ fontSize: '0.68rem', fontWeight: 400, opacity: 0.7 }}
                                  >
                                    {' '}
                                    {stat.unit}
                                  </Typography>
                                ...
                                <Typography
                                  sx={{ fontSize: '0.68rem', color: 'text.disabled', mt: 0.3 }}
                                >
                                  HQ: {stat.hq_avg}
                                </Typography>
                                <Box
                                ...
                                >
                                  {diff >= 0 ? (
                                    ...
                                    <ArrowDownward sx={{ fontSize: 12, color }} />
                                  )}
