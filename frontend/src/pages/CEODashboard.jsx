    ...
    >
      ...
      {count !== undefined && (
        <Box
          component="span"
          sx={{
            background: active ? 'rgba(255,255,255,.25)' : alpha(theme.palette.primary.main, 0.15),
            borderRadius: 10,
            px: 0.8,
            py: 0.1,
            fontSize: 11,
...
const AlertRow = memo(({ type, msg, branch, time }) => {
  ...
  return (
    ...
    >
      ...
      <Chip
        size="small"
        label={type === 'critical' ? 'حرج' : type === 'warning' ? 'تحذير' : 'معلومة'}
...
const BranchRow = memo(({ name, revenue, patients, score, trend }) => {
  ...
  return (
    ...
    >
      ...
      <TableCell>
        <Typography variant="body2">{patients.toLocaleString()}</Typography>
      </TableCell>
      <TableCell>
        <Box sx={{ display: 'flex', alignItems: 'center', gap: 1 }}>
          <LinearProgress
...
const DEMO = {
  ...
  alerts: [
    {
      type: 'critical',
      msg: 'انخفاض حاد في إيرادات فرع المدينة (-18%)',
      branch: 'المدينة المنورة',
      time: 'منذ ساعة',
    },
    {
      type: 'warning',
      msg: 'نسبة غياب الموظفين فوق المعدل (12%)',
      branch: 'جدة',
      time: 'منذ 3 ساعات',
    },
    {
      type: 'warning',
      msg: 'معدل استيعاب منخفض في فرع مكة',
      branch: 'مكة المكرمة',
      time: 'منذ 5 ساعات',
    },
  ...
  aiInsights: [
    {
      icon: '📈',
      text: 'من المتوقع نمو إيرادات فرع جدة بنسبة 15% خلال الربع القادم بناءً على الاتجاه الحالي.',
    },
    {
      icon: '⚠️',
      text: 'فرع المدينة المنورة يحتاج تدخلاً عاجلاً — توقع استمرار الانخفاض دون إجراء.',
    },
    {
      icon: '🎯',
      text: 'الهدف السنوي البالغ 55M ر.س قابل للتحقيق إذا حافظنا على معدل النمو الحالي (4.8M/شهر).',
    },
    {
      icon: '💡',
      text: 'زيادة كادر العلاج الطبيعي في الرياض بنسبة 10% قد يرفع الطاقة الاستيعابية إلى 95%.',
    },
  ],
};
...
export default function CEODashboard() {
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      ...
      const fmt = n => (n != null ? Number(n).toLocaleString('ar-SA') : null);
      const pct = v => (v != null ? `${Math.round(v)}%` : null);
      const kpis = [
        {
          ...D.kpis[0],
          value:
            s.finance?.monthlyRevenue != null
              ? `${(s.finance.monthlyRevenue / 1e6).toFixed(1)}M`
              : D.kpis[0].value,
        },
        { ...D.kpis[1], value: fmt(s.kpis?.beneficiaries?.total) || D.kpis[1].value },
        { ...D.kpis[2], value: fmt(s.kpis?.employees?.total) || D.kpis[2].value },
        { ...D.kpis[3], value: pct(s.clinical?.goals?.progress) || D.kpis[3].value },
        {
          ...D.kpis[4],
          value:
            s.clinical?.feedback?.average != null
              ? `${(s.clinical.feedback.average * 20).toFixed(0)}%`
              : D.kpis[4].value,
        },
        { ...D.kpis[5] },
      ];
          ...
          <Box sx={{ display: 'flex', gap: 3, mt: 2.5, flexWrap: 'wrap' }}>
            {[
              { label: 'الفروع النشطة', value: '5' },
              {
                ...
                value: D.alerts.filter(a => a.type === 'critical').length,
              },
      ...
      <Glass sx={{ p: 1.5, mb: 3, display: 'flex', gap: 1, flexWrap: 'wrap' }}>
        {TABS.map((t, i) => (
          <TabBtn
            ...
            active={tab === i}
            onClick={() => setTab(i)}
            count={i === 3 ? D.alerts.length : undefined}
          />
        ))}
      </Glass>
                ...
                <Glass sx={{ p: 3, height: '100%' }}>
                  ...
                  >
                    <Box>
                      ...
                      <Typography variant="caption" color="text.secondary">
                        الأشهر السبعة الماضية (مليون ر.س)
                      </Typography>
                    </Box>
                    <Chip
                      label="نمو +12.4%"
          ...
          {tab === 2 && (
            <Grid container spacing={3}>
              ...
              <Grid item xs={12} md={7}>
                <Glass sx={{ p: 3 }}>
                  <Typography variant="h6" fontWeight={700} mb={2}>
                    توزيع الإيرادات حسب الفرع
                  </Typography>
                  {D.branches.map((b, i) => {
                    const pct = (b.revenue / D.branches.reduce((s, x) => s + x.revenue, 0)) * 100;
                    return (
                      <Box key={i} sx={{ mb: 2 }}>
                        <Box sx={{ display: 'flex', justifyContent: 'space-between', mb: 0.5 }}>
                          <Typography variant="body2" fontWeight={600}>
                          ...
                          <Typography variant="body2" fontWeight={700} color="primary.main">
                            {(b.revenue / 1000000).toFixed(2)}M ({pct.toFixed(1)}%)
                          </Typography>
                        </Box>
                        <motion.div
                          initial={{ scaleX: 0 }}
              ...
              <Grid item xs={12} md={5}>
                <Glass sx={{ p: 3 }}>
                  ...
                  <Sparkline data={D.patients} color="#06b6d4" height={80} />
                  <Box sx={{ mt: 2 }}>
                    {[
              ...
              <Grid item xs={12} md={8}>
                <Glass sx={{ p: 3 }}>
                  <Box sx={{ display: 'flex', alignItems: 'center', gap: 1, mb: 3 }}>
                    ...
                    <Chip label={`${D.alerts.length} تنبيه`} size="small" color="warning" />
                  </Box>
                  {D.alerts.map((a, i) => (
                    <motion.div
                      key={i}
              ...
              <Grid item xs={12}>
                <Glass
                  sx={{
                    ...
                    background: isDark
                      ? 'linear-gradient(135deg,rgba(99,102,241,.2),rgba(139,92,246,.15))'
                      : 'linear-gradient(135deg,rgba(99,102,241,.1),rgba(139,92,246,.07))',
                  }}
                >
                  <Box sx={{ display: 'flex', alignItems: 'center', gap: 2, mb: 3 }}>
                    ...
                    <Box>
                      ...
                      <Typography variant="caption" color="text.secondary">
                        تحليل متقدم وتوقعات استراتيجية
                      </Typography>
                    </Box>
                    <Chip
                      label="مدعوم بـ AI"
                        ...
                        <Box key={i} sx={{ mb: 1 }}>
                          ...
                          <LinearProgress
                            ...
                            sx={{
                              ...
                              borderRadius: 3,
                              '& .MuiLinearProgress-bar': {
                                background: `linear-gradient(90deg,${s.c},${s.c}aa)`,
                              },
                            }}
                          />
                        </Box>
              ...
              <Grid item xs={12} md={6}>
                <Glass sx={{ p: 3 }}>
                  ...
                  {[
                    ...
                    {
                      label: 'المرضى المتوقعون',
                      value: '13,500',
                      trend: '+5.1%',
                      color: '#06b6d4',
                    },
                    {
                      label: 'معدل الأداء المتوقع',
                      value: '89%',
                      trend: '+2.3%',
                      color: '#6366f1',
                    },
