        ...
        <Grid container spacing={2} sx={{ mb: 4 }}>
          ...
          <Grid item xs={12}>
            <Card elevation={2}>
              <CardContent>
                <Typography variant="h6" gutterBottom>
                  عدد التقييمات لكل مقياس
                </Typography>
                <Grid container spacing={1}>
                  {analytics.byScale &&
                    Object.entries(analytics.byScale).map(([scaleKey, count]) => {
                        ...
                        <Grid item xs={6} sm={4} md={3} key={scaleKey}>
                          <Paper
                            ...
                            sx={{
                              p: 1.5,
                              textAlign: 'center',
                              borderTop: `3px solid ${scaleDef?.color || '#666'}`,
                            }}
                          >
                            <Avatar
                              sx={{
                                bgcolor: scaleDef?.color || '#666',
                                mx: 'auto',
                                mb: 0.5,
                                width: 36,
                                height: 36,
          ...
          {comparison.map(item => {
            ...
            return (
              <Grid item xs={12} sm={6} md={4} key={item.scaleKey}>
                <Card elevation={2} sx={{ borderTop: `4px solid ${scaleDef?.color || '#666'}` }}>
                  <CardContent>
                      ...
                      {profile.strengths.map((s, i) => (
                        <li key={i}>
                          <Typography variant="body2">
                            {s.domain || s}: {s.score !== null ? `${s.score}%` : ''}
                          </Typography>
                        </li>
                      ))}
                    </Box>
            ...
            <Grid item xs={12} md={6}>
              <Card elevation={2} sx={{ borderTop: '4px solid #d32f2f' }}>
                <CardContent>
                  <Typography
                    variant="h6"
                    sx={{ color: '#d32f2f', display: 'flex', alignItems: 'center', gap: 1 }}
                  >
                  ...
                  {profile.weaknesses?.length > 0 ? (
                    <Box component="ul" sx={{ pl: 2, mt: 1 }}>
                      {profile.weaknesses.map((w, i) => (
                        <li key={i}>
                          <Typography variant="body2">
                            {w.domain || w}: {w.score !== null ? `${w.score}%` : ''}
                          </Typography>
                        </li>
                      ))}
                    </Box>
