...
const EmployeeProfileView = ({ form: f, onClose, onEdit, onPrint }) => {
  ...
  const maritalLabel =
    MARITAL_STATUS.find(m => m.value === f.maritalStatus)?.label || f.maritalStatus || '—';
  const totalSalary =
    (+f.basicSalary || 0) +
    (+f.housingAllowance || 0) +
  ...
  return (
    <>
      <DialogTitle sx={{ p: 0 }}>
        <Box
          sx={{
            background: `linear-gradient(135deg, ${deptColor}dd 0%, ${deptColor}99 100%)`,
            color: 'white',
            p: 3,
            position: 'relative',
          }}
        >
          <Box sx={{ position: 'absolute', top: 12, left: 12, display: 'flex', gap: 1 }}>
            <Tooltip title="طباعة">
              <IconButton size="small" sx={{ color: 'white' }} onClick={onPrint}>
                <PrintIcon fontSize="small" />
              </IconButton>
            </Tooltip>
            <Tooltip title="تعديل">
              <IconButton size="small" sx={{ color: 'white' }} onClick={onEdit}>
          ...
          <Box sx={{ display: 'flex', alignItems: 'center', gap: 2.5, mt: 1 }}>
            <Avatar
              sx={{ width: 72, height: 72, bgcolor: '#ffffff30', fontSize: 28, fontWeight: 700 }}
            >
              {(f.firstName || '?')[0]}
              {(f.lastName || '')[0]}
            </Avatar>
            <Box>
              ...
              <Box sx={{ display: 'flex', gap: 1, mt: 0.5 }}>
                <Chip
                  ...
                  size="small"
                  sx={{ bgcolor: '#ffffff30', color: 'white' }}
                />
      ...
      <DialogContent sx={{ py: 3 }}>
        <Grid container spacing={3}>
          <Grid item xs={12} md={6}>
            ...
            />
            <InfoRow
              icon={<BirthIcon sx={{ fontSize: 16 }} />}
              label="تاريخ الميلاد"
              value={f.dateOfBirth}
            />
            <InfoRow
              icon={<NatIcon sx={{ fontSize: 16 }} />}
              label="الجنسية"
              value={f.nationality}
            />
            <InfoRow
              icon={<IdIcon sx={{ fontSize: 16 }} />}
              label="رقم الهوية"
              value={f.idNumber}
            />
            <InfoRow
          ...
          <Grid item xs={12} md={6}>
            ...
            <InfoRow
              ...
              value={f.basicSalary ? `${Number(f.basicSalary).toLocaleString('ar-SA')} ر.س` : '—'}
            />
            {totalSalary > 0 && (
              <InfoRow
                icon={<BankIcon sx={{ fontSize: 16 }} />}
                label="إجمالي الراتب"
                value={`${totalSalary.toLocaleString('ar-SA')} ر.س`}
              />
            )}
            {f.bankName && (
              <InfoRow icon={<BankIcon sx={{ fontSize: 16 }} />} label="البنك" value={f.bankName} />
            )}
            {f.iban && (
              <InfoRow icon={<IdIcon sx={{ fontSize: 16 }} />} label="IBAN" value={f.iban} />
            )}
          </Grid>
        </Grid>
      </DialogContent>
