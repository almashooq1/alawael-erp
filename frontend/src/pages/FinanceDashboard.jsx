    ...
    >
      ...
      {count !== undefined && (
        <Box
          component="span"
          sx={{
            background: active ? 'rgba(255,255,255,.25)' : alpha('#22c55e', 0.15),
            borderRadius: 10,
            px: 0.8,
            py: 0.1,
            fontSize: 11,
...
const LineChart = memo(
  ({ data = [], color = '#22c55e', height = 100, width = 300, labels = [] }) => {
    ...
    const pts = data.map(
      (v, i) => `${i * step},${height - ((v - min) / range) * (height - 20) - 10}`
    );
  ...
  return (
    <Box sx={{ mb: 2 }}>
      <Box sx={{ display: 'flex', justifyContent: 'space-between', mb: 0.5 }}>
        ...
        <Box sx={{ display: 'flex', alignItems: 'center', gap: 1 }}>
          <Typography variant="body2" fontWeight={700} sx={{ color: over ? '#ef4444' : '#22c55e' }}>
...
const DEMO = {
  ...
  alerts: [
    {
      type: 'critical',
      msg: 'تجاوز ميزانية المعدات الطبية بنسبة 12%',
      dept: 'المشتريات',
      time: 'اليوم',
    },
    {
      type: 'warning',
      msg: '18 فاتورة متأخرة عن الدفع (أكثر من 30 يوم)',
      dept: 'الفوترة',
      time: 'منذ ساعتين',
    },
    ...
    {
      type: 'success',
      msg: 'تحقيق هدف الإيرادات الشهري بنسبة 106%',
      dept: 'المالية',
      time: 'اليوم',
    },
    {
      type: 'info',
      msg: 'موعد إغلاق الميزانية الربعية في 5 أبريل',
      dept: 'المحاسبة',
      time: 'تذكير',
    },
  ],
  aiInsights: [
    {
      icon: '📈',
      text: 'الإيرادات في تصاعد مستمر — النمو المتوقع للربع الثاني هو +14% بناءً على النمط الحالي.',
    },
    {
      icon: '⚠️',
      text: 'إذا استمر الإنفاق على المعدات بهذا المعدل، سيتجاوز الميزانية السنوية بـ 280,000 ر.س بنهاية العام.',
    },
    {
      icon: '💡',
      text: 'تحسين عمليات تحصيل الفواتير المتأخرة يمكن أن يحرر تدفقاً نقدياً فورياً بـ 380,000 ر.س.',
    },
    {
      icon: '🎯',
      text: 'الهدف السنوي البالغ 55M ر.س قابل للتحقيق بنسبة 94% إذا حافظنا على المعدل الحالي.',
    },
  ],
  branchRevenue: [
...
export default function FinanceDashboard() {
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      ...
      const rev = s.finance?.monthlyRevenue;
      const exp = s.finance?.totalExpenses;
      const kpis = [
        { ...D.kpis[0], value: rev != null ? `${(rev / 1000).toFixed(0)}K` : D.kpis[0].value },
        { ...D.kpis[1], value: exp != null ? `${(exp / 1000).toFixed(0)}K` : D.kpis[1].value },
        {
          ...D.kpis[2],
          value:
            rev != null && exp != null ? `${((rev - exp) / 1000).toFixed(0)}K` : D.kpis[2].value,
        },
        { ...D.kpis[3] },
        { ...D.kpis[4] },
        { ...D.kpis[5] },
        ...
        <Glass
          sx={{
            ...
            background: isDark
              ...
              : 'linear-gradient(135deg,rgba(34,197,94,.1),rgba(16,185,129,.07))',
          }}
        >
          <Box
            sx={{
            ...
            <Box sx={{ display: 'flex', alignItems: 'center', gap: 2 }}>
              ...
              <Box>
                ...
                <Typography variant="body2" color="text.secondary">
                  تتبع شامل للإيرادات والمصروفات · مارس 2026 · آخر تحديث:{' '}
            ...
            <Box sx={{ display: 'flex', gap: 1 }}>
              ...
              <Tooltip title="تحديث">
                <IconButton
                  onClick={refresh}
                  size="small"
                  sx={{ background: alpha('#22c55e', 0.1) }}
                >
          ...
          {tab === 0 && (
            <Grid container spacing={3}>
                ...
                <Glass
                  sx={{
                    ...
                    background: isDark
                      ...
                      : 'linear-gradient(135deg,rgba(34,197,94,.08),rgba(16,185,129,.05))',
                  }}
                >
                  <Typography variant="h6" fontWeight={700} mb={2}>
                    الملخص المالي — مارس 2026
                  </Typography>
                ...
                <Glass sx={{ p: 3 }}>
                  ...
                  >
                    <Box>
                      ...
                      <Typography variant="caption" color="text.secondary">
                        مليون ر.س — آخر 7 أشهر
                      </Typography>
                    </Box>
                    <Box sx={{ display: 'flex', gap: 2 }}>
                      {[
                        { label: 'الإيرادات', color: '#22c55e' },
                        { label: 'المصروفات', color: '#ef4444' },
                        { label: 'الربح', color: '#6366f1' },
                      ].map((l, i) => (
                        <Box key={i} sx={{ display: 'flex', alignItems: 'center', gap: 0.5 }}>
                          <Box
                            sx={{ width: 12, height: 3, borderRadius: 2, background: l.color }}
                          />
                          <Typography variant="caption" color="text.secondary">
                            {l.label}
                          </Typography>
              ...
              <Grid item xs={12} md={4}>
                <Glass sx={{ p: 3 }}>
                  <Typography variant="h6" fontWeight={700} mb={2}>
                    إيرادات الفروع Q1 vs Q2
                  </Typography>
                  {D.branchRevenue.map((b, i) => (
                    <Box key={i} sx={{ mb: 2 }}>
                      <Box sx={{ display: 'flex', justifyContent: 'space-between', mb: 0.4 }}>
                        ...
                        <Box sx={{ display: 'flex', alignItems: 'center', gap: 0.5 }}>
                          <Typography variant="caption" fontWeight={700} sx={{ color: b.color }}>
                            {b.q2}K
                          </Typography>
                          {b.q2 >= b.q1 ? (
                ...
                <Glass sx={{ p: 3 }}>
                  ...
                  >
                    <Box>
                      ...
                      <Typography variant="caption" color="text.secondary">
                        مليون ر.س — يناير إلى يوليو 2026
                      </Typography>
                    </Box>
                    <Chip
                      label="↑ 12.4%"
                      icon={<TrendingUpIcon />}
                      sx={{ background: alpha('#22c55e', 0.15), color: '#16a34a', fontWeight: 700 }}
                    />
                  </Box>
                  {/* Bar Chart */}
                  <Box sx={{ display: 'flex', alignItems: 'flex-end', gap: 1.5, height: 160 }}>
                    {D.revenue.map((v, i) => {
                      const pct = v / Math.max(...D.revenue);
                      return (
                        <Box
                          key={i}
                          sx={{
                        ...
                        >
                          ...
                          <motion.div
                            ...
                            style={{
                              ...
                              height: `${pct * 130}px`,
                              background:
                                i === D.revenue.length - 1
                                  ? 'linear-gradient(180deg,#22c55e,#16a34a)'
                                  : `rgba(34,197,94,${0.3 + pct * 0.5})`,
                            }}
                          />
                          <Typography
                            variant="caption"
                      ...
                      <Grid item xs={12} sm={6} md={4} lg={2.4} key={i}>
                        ...
                        >
                          ...
                          >
                            ...
                            <Typography variant="h6" fontWeight={800} sx={{ color: b.color }}>
                              {b.q2}K
                            </Typography>
                            <Box
                              sx={{
                            ...
                            >
                              ...
                              <Typography
                                variant="caption"
                                sx={{
                                  color: b.q2 >= b.q1 ? '#22c55e' : '#ef4444',
                                  fontWeight: 700,
                                }}
                              >
                                {b.q2 >= b.q1 ? '+' : ''}
                                {(((b.q2 - b.q1) / b.q1) * 100).toFixed(1)}%
                              </Typography>
                            </Box>
                          </Box>
                        </motion.div>
                ...
                <Glass sx={{ p: 3 }}>
                  ...
                  >
                    ...
                    <Chip
                      size="small"
                      label="2,160,000 ر.س"
                      sx={{ background: alpha('#ef4444', 0.12), color: '#ef4444', fontWeight: 700 }}
                    />
                  </Box>
                  {D.expenses_breakdown.map((e, i) => (
                    <motion.div
                      key={i}
              ...
              <Grid item xs={12} md={6}>
                <Glass sx={{ p: 3 }}>
                  ...
                  <Box sx={{ display: 'flex', alignItems: 'flex-end', gap: 1, height: 140 }}>
                    {D.expenses.map((v, i) => {
                      const pct = v / Math.max(...D.expenses);
                      return (
                        <Box
                          key={i}
                          sx={{
                        ...
                        >
                          ...
                          <motion.div
                            ...
                            style={{
                              ...
                              height: `${pct * 110}px`,
                              background: `rgba(239,68,68,${0.4 + pct * 0.5})`,
                            }}
                          />
                  ...
                  <Typography variant="subtitle2" fontWeight={700} mb={1.5} color="text.secondary">
                    بنود تجاوزت الميزانية
                  </Typography>
                  {D.expenses_breakdown
                    .filter(e => e.pct > 100)
                    .map((e, i) => (
                      <Box
                        key={i}
                      ...
                      >
                        ...
                        <Box sx={{ display: 'flex', alignItems: 'center', gap: 1 }}>
                          ...
                          <Chip
                            size="small"
                            label={`+${e.amount - e.budget} ر.س زيادة`}
                            sx={{
                              background: alpha('#ef4444', 0.12),
                              color: '#ef4444',
                              fontSize: 9,
              ...
              >
                ...
                <Box sx={{ display: 'flex', gap: 1.5 }}>
                  {[
                    {
                      label: 'مدفوع',
                      count: D.invoices.filter(i => i.status === 'مدفوع').length,
                      color: '#22c55e',
                    },
                    {
                      label: 'معلق',
                      count: D.invoices.filter(i => i.status === 'معلق').length,
                      color: '#f59e0b',
                    },
                    {
                      label: 'متأخر',
                      count: D.invoices.filter(i => i.status === 'متأخر').length,
                      color: '#ef4444',
                    },
                  ].map((s, i) => (
                    <Chip
                      ...
                      sx={{ background: alpha(s.color, 0.12), color: s.color, fontWeight: 700 }}
                    />
                  ))}
                </Box>
              </Box>
              ...
              <Grid item xs={12}>
                ...
                >
                  <Box sx={{ display: 'flex', alignItems: 'center', gap: 2, mb: 3 }}>
                    ...
                    <Box>
                      <Typography variant="h6" fontWeight={700}>
                        توصيات ذكاء اصطناعي — المالية
                      </Typography>
                      <Typography variant="caption" color="text.secondary">
                        تحليل مالي متقدم وتوقعات استراتيجية
                      </Typography>
                    </Box>
                    <Chip
                      label="AI-Powered"
                  ...
                  {[
                    ...
                    >
                      ...
                      >
                        ...
                        <Box sx={{ display: 'flex', alignItems: 'center', gap: 1 }}>
                          <Typography variant="body2" fontWeight={700} sx={{ color: f.color }}>
                            {f.value}
                          </Typography>
                          <Chip
                            size="small"
