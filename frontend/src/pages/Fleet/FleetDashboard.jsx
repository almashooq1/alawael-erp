...
const DATA_RECENT_TRIPS = [
  {
    id: 1,
    route: 'المركز ← مدرسة النور',
    driver: 'سعيد الحربي',
    vehicle: 'حافلة #3',
    time: '07:30',
    students: 18,
...
export default function FleetDashboard() {
  ...
  const [vehicleStatus, setVehicleStatus] = useState(DATA_VEHICLE_STATUS);
  const [fuelTrend, _setFuelTrend] = useState(DATA_FUEL_TREND);
  const [vehicleTypes, setVehicleTypes] = useState(DATA_VEHICLE_TYPES);
  const [topDrivers, setTopDrivers] = useState(DATA_DRIVERS);
  const [recentTrips, setRecentTrips] = useState(DATA_RECENT_TRIPS);

  const loadDashboard = useCallback(async () => {
    setLoading(true);
    try {
      const [vehiclesRes, driversRes, tripsRes, routesRes] = await Promise.all([
        fleetService.getVehicles().catch(err => {
          logger.warn('Fleet: vehicles fetch', err);
          return null;
        }),
        fleetService.getDrivers().catch(err => {
          logger.warn('Fleet: drivers fetch', err);
          return null;
        }),
        fleetService.getTrips().catch(err => {
          logger.warn('Fleet: trips fetch', err);
          return null;
        }),
        fleetService.getRoutes().catch(err => {
          logger.warn('Fleet: routes fetch', err);
          return null;
        }),
      ]);
      ...
      if (Array.isArray(vehicles) && vehicles.length > 0) {
        const { active, maintenance: inMaint } = computeStatusCounts(vehicles, 'status', [
          'active',
          'maintenance',
        ]);
        ...
        setStats(prev => ({
          ...
          activeRoutes: Array.isArray(routes)
            ? routes.filter(r => r.status === 'active').length
            : prev.activeRoutes,
          tripsToday: Array.isArray(trips)
            ? trips.filter(t => t.date?.slice(0, 10) === new Date().toISOString().slice(0, 10))
                .length
            : prev.tripsToday,
          ...
          <Box sx={{ display: 'flex', alignItems: 'center', gap: 2 }}>
            ...
            <Box>
              <Typography variant="h4" fontWeight="bold">
                لوحة تحكم الأسطول والنقل
              </Typography>
              <Typography variant="body2" sx={{ opacity: 0.9 }}>
                إدارة المركبات والسائقين والرحلات
              </Typography>
            </Box>
          </Box>
          <Button
            variant="contained"
            color="inherit"
            sx={{ color: statusColors.pinkDark, fontWeight: 600 }}
        ...
        <Grid container spacing={2} sx={{ mb: 4 }}>
          <Grid item xs={12} sm={6} md={3} lg={2}>
            <ModuleKPICard
              ...
              subtitle={`${stats.activeVehicles} نشطة`}
              icon={<VehicleIcon />}
              color="primary"
            />
          </Grid>
          ...
          <Grid item xs={12} sm={6} md={3} lg={2}>
            <ModuleKPICard
              title="تكلفة الوقود"
              value={`${(stats.fuelCostMonth / 1000).toFixed(1)}K`}
            ...
            <Paper sx={{ p: 3, borderRadius: 3, height: '100%' }}>
              ...
              {fuelTrend.length > 0 ? (
                <ResponsiveContainer
                  ...
                  height={280}
                  role="img"
                  aria-label="رسم بياني لتكلفة الوقود والرحلات الشهرية"
                >
                  <BarChart data={fuelTrend}>
                    ...
                    <Line
                      ...
                      type="monotone"
                      dataKey="trips"
                      stroke={statusColors.primaryBlue}
                      strokeWidth={2}
                      name="عدد الرحلات"
                      dot={{ r: 4 }}
                    />
            ...
            <Paper sx={{ p: 3, borderRadius: 3, height: '100%' }}>
              ...
              {vehicleStatus.length > 0 ? (
                <ResponsiveContainer
                  ...
                  height={200}
                  role="img"
                  aria-label="رسم بياني لحالة المركبات"
                >
                  <PieChart>
                    <Pie
                      data={vehicleStatus}
              ...
              {vehicleTypes.length > 0 ? (
                <ResponsiveContainer
                  ...
                  height={200}
                  role="img"
                  aria-label="رسم بياني لأنواع المركبات"
                >
                  <PieChart>
                    <Pie
                      data={vehicleTypes}
                    ...
                    {topDrivers.map(d => (
                      <TableRow key={d.id} hover>
                        ...
                        <TableCell align="center">
                          <Chip label={d.trips} size="small" variant="outlined" />
                        </TableCell>
                        <TableCell>
                          <Chip
                            label={STATUS_MAP[d.status]?.label || d.status}
                            color={STATUS_MAP[d.status]?.color || 'default'}
                            size="small"
                          />
                        </TableCell>
                      </TableRow>
          ...
          <Grid item xs={12} md={7}>
            <Paper sx={{ p: 3, borderRadius: 3 }}>
              ...
              >
                ...
                <Button size="small" onClick={() => navigate('/fleet')}>
                  عرض الكل
                </Button>
              </Box>
              <TableContainer>
                  ...
                  <TableBody>
                    {recentTrips.map(t => (
                      <TableRow key={t.id} hover>
                        ...
                        <TableCell>
                          <Chip
                            label={STATUS_MAP[t.status]?.label || t.status}
                            color={STATUS_MAP[t.status]?.color || 'default'}
                            size="small"
                          />
                        </TableCell>
                      </TableRow>
