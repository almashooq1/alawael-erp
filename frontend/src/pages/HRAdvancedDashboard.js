...
const HRAdvancedDashboard = () => {
  ...
  return (
    <Container maxWidth="xl" sx={{ mt: 4, mb: 4 }}>
      <Typography variant="h4" gutterBottom>Advanced HR System</Typography>
      ...
      {tabInfo.value === 0 && (
        <Grid container spacing={3}>
            <Grid item xs={12}>
                <Box mb={2}>
                    <Button variant="contained" color="success" onClick={handleCheckIn} sx={{ mr: 2 }}>Check In</Button>
                        ...
                        <TableBody>
                            {attendance.map((row) => (
                                <TableRow key={row._id}>
                                    <TableCell>{row.employeeId?.firstName} {row.employeeId?.lastName}</TableCell>
                                    <TableCell>{new Date(row.checkIn).toLocaleTimeString()}</TableCell>
                                    <TableCell><Chip label={row.status} color="primary" size="small"/></TableCell>
                                </TableRow>
                            ))}
                        </TableBody>
                    </Table>
        ...
        <TableContainer component={Paper}>
            <Table>
                ...
                <TableBody>
                    ...
                    {payroll.length === 0 && <TableRow><TableCell colSpan={5}>No Records</TableCell></TableRow>}
                </TableBody>
            </Table>
        </TableContainer>
      )}
