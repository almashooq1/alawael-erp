    ...
    >
      ...
      {count !== undefined && (
        <Box
          component="span"
          sx={{
            background: active ? 'rgba(255,255,255,.25)' : alpha(color, 0.15),
            borderRadius: 10,
            px: 0.8,
            py: 0.1,
            fontSize: 11,
...
const TrainingCard = memo(
  ({ title, instructor, enrolled, capacity, progress, category, delay = 0 }) => {
          ...
          <Box>
            ...
            <LinearProgress
              ...
              sx={{
                height: 6,
                borderRadius: 3,
                '& .MuiLinearProgress-bar': { background: `linear-gradient(90deg,${c},${c}aa)` },
              }}
            />
          </Box>
        </Glass>
...
const DEMO = {
  ...
  trainings: [
    ...
    {
      title: 'بروتوكولات السلامة المهنية',
      instructor: 'م. سعد الغامدي',
      enrolled: 25,
      capacity: 25,
      progress: 100,
      category: 'سلامة',
    ...
    {
      title: 'مهارات التواصل مع المرضى',
      instructor: 'د. هدى الزهراني',
      enrolled: 12,
      capacity: 15,
      progress: 40,
      category: 'سلوكي',
    ...
    {
      title: 'تقنيات التأهيل الحديثة',
      instructor: 'د. فارس النمر',
      enrolled: 8,
      capacity: 10,
      progress: 80,
      category: 'طبي',
    ...
    {
      title: 'الذكاء الاصطناعي في الرعاية',
      instructor: 'د. وليد المالكي',
      enrolled: 14,
      capacity: 15,
      progress: 30,
      category: 'تقني',
  ...
  alerts: [
    {
      type: 'warning',
      msg: 'انتهاء عقود 3 موظفين خلال 30 يوماً',
      dept: 'الموارد البشرية',
      time: 'اليوم',
    },
    {
      type: 'critical',
      msg: 'نسبة غياب مرتفعة في قسم التأهيل (12%)',
      dept: 'التأهيل',
      time: 'منذ ساعتين',
    },
    ...
    {
      type: 'info',
      msg: 'موعد مراجعة الرواتب الربعية في 15 أبريل',
      dept: 'المالية',
      time: 'تذكير',
    },
    {
      type: 'warning',
      msg: 'طلب إجازة مفاجئة من 4 موظفين في نفس اليوم',
      dept: 'التمريض',
      time: 'منذ 4 ساعات',
    },
  ],
  ...
  aiInsights: [
    {
      icon: '🎯',
      text: 'يُتوقع ارتفاع معدل الغياب في فصل الصيف — يُنصح بمراجعة سياسة الإجازات مسبقاً.',
    },
    {
      icon: '📊',
      text: 'موظفو قسم التأهيل الذين أتموا البرامج التدريبية أظهروا أداءً أعلى بنسبة 23%.',
    },
...
export default function HRAdvancedDashboard() {
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      const [dashRes, empRes] = await Promise.allSettled([
        apiClient.get('/employee-affairs/dashboard'),
        apiClient.get('/employee-affairs?limit=8'),
      ]);
      const d =
        dashRes.status === 'fulfilled' ? dashRes.value.data?.data || dashRes.value.data || {} : {};
      const empList = empRes.status === 'fulfilled' ? empRes.value.data?.employees || [] : [];
      const kpis = [
        {
          ...D.kpis[0],
          value:
            d.overview?.totalEmployees != null
              ? d.overview.totalEmployees.toLocaleString('ar-SA')
              : D.kpis[0].value,
        },
        { ...D.kpis[1] },
        {
          ...D.kpis[2],
          value: d.kpis?.turnoverRate != null ? `${d.kpis.turnoverRate}%` : D.kpis[2].value,
        },
        { ...D.kpis[3] },
        { ...D.kpis[4] },
        {
          ...D.kpis[5],
          value:
            d.overview?.terminatedThisMonth != null
              ? d.overview.terminatedThisMonth.toString()
              : D.kpis[5].value,
        },
      ];
      const employees =
        empList.length > 0
          ? empList.map(e => ({
              name: `${e.firstName || ''} ${e.lastName || ''}`.trim() || D.employees[0].name,
  ...
  const filteredEmp = D.employees.filter(
    e =>
            ...
            <Box sx={{ display: 'flex', alignItems: 'center', gap: 2 }}>
              ...
              <Box>
                ...
                <Typography variant="body2" color="text.secondary">
                  إدارة شاملة وذكية لكادر المنظمة · آخر تحديث:{' '}
            ...
            <Grid container spacing={3}>
              ...
              <Grid item xs={12} md={6}>
                <Glass sx={{ p: 3 }}>
                  ...
                  {D.deptDist.map((d, i) => {
                    const _pct = (d.count / D.kpis[0].value.replace(',', '')) * 100;
                    return (
                      <Box key={i} sx={{ mb: 1.5 }}>
            ...
            <Box>
              ...
              >
                <Box>
                  ...
                  <Typography variant="caption" color="text.secondary">
                    14 برنامج نشط | 132 موظف مسجل
                  </Typography>
                </Box>
                <Chip
                  label="نسبة الإتمام: 62%"
                  icon={<SchoolIcon />}
                  sx={{ background: alpha('#06b6d4', 0.15), color: '#06b6d4', fontWeight: 700 }}
                />
              </Box>
              <Grid container spacing={2.5}>
                {D.trainings.map((t, i) => (
                  <Grid item xs={12} sm={6} md={4} key={i}>
                    <TrainingCard {...t} delay={i * 0.07} />
                  </Grid>
                ))}
              </Grid>
              ...
              <Glass sx={{ p: 3, mt: 3 }}>
                ...
                <Grid container spacing={2}>
                  {[
                    {
                      label: 'ساعات التدريب الإجمالية',
                      value: '1,240 ساعة',
                      color: '#6366f1',
                      icon: <AccessTimeIcon />,
                    },
                    {
                      label: 'متوسط ساعات/موظف',
                      value: '3.6 ساعة',
                      color: '#22c55e',
                      icon: <WorkIcon />,
                    },
            ...
            <Grid container spacing={3}>
                ...
                <Glass
                  sx={{
                    ...
                    background: isDark
                      ...
                      : 'linear-gradient(135deg,rgba(245,158,11,.08),rgba(249,115,22,.05))',
                  }}
                >
                  <Typography variant="h6" fontWeight={700} mb={2}>
                    مراجعة الأداء الربعية — Q1 2026
                  </Typography>
                  <Grid container spacing={2}>
                    {[
              ...
              <Grid item xs={12} md={5}>
                <Glass sx={{ p: 3, height: '100%' }}>
                  ...
                  <Box sx={{ mt: 2, p: 2, background: alpha('#22c55e', 0.08), borderRadius: 2 }}>
                    <Box sx={{ display: 'flex', alignItems: 'center', gap: 1 }}>
          ...
          {tab === 5 && (
            <Grid container spacing={3}>
              <Grid item xs={12}>
                ...
                >
                  <Box sx={{ display: 'flex', alignItems: 'center', gap: 2, mb: 3 }}>
                    ...
                    <Box>
                      <Typography variant="h6" fontWeight={700}>
                        توصيات ذكاء اصطناعي — HR
                      </Typography>
                      <Typography variant="caption" color="text.secondary">
                        تحليل بيانات الموظفين وتوقعات مستقبلية
                      </Typography>
                    </Box>
                    <Chip
                      label="AI-Powered"
              ...
              <Grid item xs={12} md={6}>
                <Glass sx={{ p: 3 }}>
                  <Typography variant="h6" fontWeight={700} mb={2}>
                    توقعات الاحتياجات — الربع القادم
                  </Typography>
                  {[
                    {
                    ...
                    >
                      <Box
                        sx={{
                          ...
                          borderRadius: 2,
                          border: `1px solid ${alpha(n.color, 0.15)}`,
                        }}
                      >
                        <Box>
                          <Typography variant="body2" fontWeight={600}>
              ...
              <Grid item xs={12} md={6}>
                <Glass sx={{ p: 3 }}>
                  ...
                  <Typography variant="caption" color="text.secondary" display="block" mb={2}>
                    بناءً على نمط الأداء والرضا الوظيفي
                  </Typography>
                  {[
                    ...
                    <Box key={i} sx={{ mb: 2 }}>
                      <Box sx={{ display: 'flex', justifyContent: 'space-between', mb: 0.5 }}>
                        <Box sx={{ display: 'flex', alignItems: 'center', gap: 1 }}>
                          <Avatar
                            sx={{
                              ...
                              fontSize: 12,
                              background:
                                r.risk > 60 ? '#ef4444' : r.risk > 40 ? '#f59e0b' : '#22c55e',
                            }}
                          >
                            {r.name.charAt(0)}
                          </Avatar>
                        ...
                        <Typography
                          ...
                          fontWeight={700}
                          sx={{
                            color: r.risk > 60 ? '#ef4444' : r.risk > 40 ? '#f59e0b' : '#22c55e',
                          }}
                        >
                          {r.risk}%
                        </Typography>
          ...
          <Grid container spacing={2} mt={0.5}>
            ...
            <Grid item xs={12} sm={6}>
              <TextField
                ...
                sx={{ '& .MuiOutlinedInput-root': { borderRadius: 2 } }}
              >
                {['التأهيل', 'التمريض', 'الطب', 'الإدارة', 'التقنية', 'المالية'].map(d => (
