...
const OBJECTIVES = [
  {
    id: 1,
    title: 'رفع نسبة تعافي المرضى إلى ٩٠٪',
    progress: 88,
    target: 90,
    status: 'on-track',
    dept: 'التأهيل',
  ...
  {
    id: 3,
    title: 'تخفيض التكاليف التشغيلية ١٥٪',
    progress: 62,
    target: 100,
    status: 'behind',
    dept: 'المالية',
  ...
  {
    id: 5,
    title: 'استقطاب ٢٠ متخصصاً جديداً',
    progress: 75,
    target: 20,
    status: 'on-track',
    dept: 'الموارد البشرية',
  ...
  {
    id: 6,
    title: 'توسيع خدمات الطب عن بُعد ٥٠٪',
    progress: 40,
    target: 50,
    status: 'behind',
    dept: 'التقنية',
...
function KPICard({ label, value, target, change, trend, color, unit = '', index, isDark }) {
  const TrendIcon =
    trend === 'up' ? TrendingUpOutlined : trend === 'down' ? TrendingDownOutlined : RemoveOutlined;
  ...
  return (
    ...
    >
      ...
      >
        {/* Top bar */}
        <Box
          sx={{ position: 'absolute', top: 0, left: 0, right: 0, height: 3, background: color }}
        />
        <Box
          sx={{
            position: 'absolute',
        ...
        >
          {value}
          <span style={{ fontSize: '1rem', fontWeight: 600, color, marginRight: 2 }}>{unit}</span>
        </Typography>
        <Typography
          sx={{ fontSize: '0.72rem', color: isDark ? 'rgba(255,255,255,0.4)' : '#94A3B8', mb: 1.5 }}
        >
          {label}
        </Typography>
        ...
        >
          <Typography
            sx={{ fontSize: '0.65rem', color: isDark ? 'rgba(255,255,255,0.3)' : '#CBD5E1' }}
          >
            المستهدف: {target}
            {unit}
          </Typography>
        ...
        <LinearProgress
          variant="determinate"
          value={Math.min(100, Math.round((parseFloat(value) / parseFloat(target)) * 100))}
          sx={{
            height: 5,
            borderRadius: 3,
          ...
          <Box sx={{ position: 'relative', zIndex: 1 }}>
            ...
            <Box sx={{ display: 'flex', gap: 2, flexWrap: 'wrap' }}>
              {[
                ...
                >
                  <Typography sx={{ fontWeight: 800, fontSize: '1rem', color: s.color }}>
                    {s.value}
                  </Typography>
                  <Typography
                    sx={{
            ...
            >
              ...
              <ResponsiveContainer width="100%" height={240}>
                <ComposedChart data={MONTHLY_PERF}>
                  ...
                  <XAxis
                    dataKey="month"
                    tick={{ fontSize: 11, fill: isDark ? 'rgba(255,255,255,0.45)' : '#94A3B8' }}
                    axisLine={false}
                    tickLine={false}
                  />
                  <YAxis
                ...
                <Box sx={{ flex: 1 }}>
                  {PIE_DATA.map(d => (
                    ...
                    >
                      ...
                      <Typography sx={{ fontSize: '0.72rem', fontWeight: 700, color: d.color }}>
                        {d.value}٪
                      </Typography>
                    </Box>
                  ))}
            ...
            >
              ...
              <ResponsiveContainer width="100%" height={160}>
                <RadarChart data={DEPT_RADAR}>
                  ...
                  <PolarAngleAxis
                    dataKey="dept"
                    tick={{ fontSize: 9, fill: isDark ? 'rgba(255,255,255,0.45)' : '#94A3B8' }}
                  />
                  <Radar
                    name="الأداء"
                    dataKey="score"
