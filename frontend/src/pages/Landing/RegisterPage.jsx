  ...
  const validateStep = () => {
    ...
    if (step === 0) {
      ...
      if (!form.email || !/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(form.email))
        e.email = 'بريد إلكتروني صالح مطلوب';
      if (form.phone && !/^05\d{8}$/.test(form.phone))
        e.phone = 'رقم الجوال يبدأ بـ 05 ويتكون من 10 أرقام';
    }
    if (step === 1) {
          ...
          <div className="relative z-10 text-center max-w-md">
            ...
            <div className="w-20 h-20 mx-auto mb-6 relative">
              ...
              <div className="relative w-full h-full bg-white/10 backdrop-blur-md rounded-3xl flex items-center justify-center border border-white/20 shadow-2xl">
                <span className="text-3xl font-bold bg-gradient-to-b from-accent-300 to-accent-400 bg-clip-text text-transparent">
                  أ
                </span>
              </div>
            </div>
            ...
            <p className="text-white/55 text-base leading-relaxed max-w-sm mx-auto">
              أنشئ حسابك الآن واستفد من جميع خدمات نظام إدارة مراكز الأوائل للرعاية النهارية
            </p>
              ...
              {STEPS.map((s, i) => (
                ...
                >
                  ...
                  <div className="flex-1 text-right">
                    ...
                    {i === step && <p className="text-xs text-white/40 mt-0.5">الخطوة الحالية</p>}
                  </div>
                  {i < step && (
                    <svg
            ...
            <div className="lg:hidden flex items-center justify-center gap-2 mb-6">
              {STEPS.map((s, i) => (
                <div key={i} className="flex items-center gap-2">
                  ...
                  {i < STEPS.length - 1 && (
                    <div
                      className={`w-8 h-0.5 rounded-full ${i < step ? 'bg-primary-500' : 'bg-gray-200'}`}
                    />
                  )}
                </div>
              ))}
              ...
              {step === 1 && (
                <div className="space-y-5 animate-fade-in">
                  <div>
                    ...
                    >
                      <div className="absolute inset-y-0 right-0 flex items-center pr-4 pointer-events-none">
                        <svg
                          className={`w-5 h-5 transition-colors ${focusedField === 'password' ? 'text-primary-500' : 'text-gray-400'}`}
                          fill="none"
                          viewBox="0 0 24 24"
                          stroke="currentColor"
                          strokeWidth={1.5}
                      ...
                      <div className="mt-4">
                        <div className="flex gap-1.5 mb-2">
                          {[1, 2, 3, 4, 5].map(i => (
                            <div
                              key={i}
                              className={`h-2 flex-1 rounded-full transition-all duration-500 ${i <= strength ? strengthColors[strength] : 'bg-gray-100'}`}
                            />
                          ))}
                        </div>
                        <p
                  ...
                  <div>
                    ...
                    <div
                      className={`relative rounded-2xl transition-all duration-300 ${focusedField === 'confirmPassword' ? 'ring-2 ring-primary-500/20 shadow-lg shadow-primary-500/5' : ''}`}
                    >
                      <div className="absolute inset-y-0 right-0 flex items-center pr-4 pointer-events-none">
                        <svg
                          className={`w-5 h-5 transition-colors ${focusedField === 'confirmPassword' ? 'text-primary-500' : 'text-gray-400'}`}
                          fill="none"
                          viewBox="0 0 24 24"
                          stroke="currentColor"
                          strokeWidth={1.5}
                        ...
                        >
                          ...
                          >
                            <path
                              strokeLinecap="round"
                              strokeLinejoin="round"
                              d={r.ok ? 'M4.5 12.75l6 6 9-13.5' : 'M19.5 12h-15'}
                            />
                          </svg>
                        </div>
                        <span
                    ...
                    {ROLES.map(r => (
                      ...
                      >
                        ...
                        <div className="flex-1">
                          <h4
                            className={`font-bold text-sm ${form.role === r.value ? 'text-primary-700' : 'text-gray-800'}`}
                          >
                            {r.label}
                          </h4>
...
function InputField({
  ...
  return (
    <div>
      ...
      <div
        className={`relative rounded-2xl transition-all duration-300 ${isFocused ? 'ring-2 ring-primary-500/20 shadow-lg shadow-primary-500/5' : ''}`}
      >
        <div className="absolute inset-y-0 right-0 flex items-center pr-4 pointer-events-none">
          <span
            className={`transition-colors duration-300 ${isFocused ? 'text-primary-500' : 'text-gray-400'}`}
          >
            {icon}
          </span>
        </div>
