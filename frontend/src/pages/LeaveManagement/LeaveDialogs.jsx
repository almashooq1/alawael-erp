        ...
        <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start' }}>
          <Box sx={{ display: 'flex', alignItems: 'center', gap: 2 }}>
            ...
            <Box>
              ...
              <Chip
                icon={getLeaveTypeIcon(viewItem.leaveType)}
                label={getLeaveTypeLabel(viewItem.leaveType)}
                size="small"
                sx={{ bgcolor: `${ltc}20`, color: ltc, fontWeight: 600, mt: 0.5 }}
              />
            </Box>
          </Box>
      ...
      <DialogContent sx={{ py: 3 }}>
        <Grid container spacing={2.5}>
          {[
            {
              label: 'من تاريخ',
              value: viewItem.startDate,
              icon: <CalendarIcon color="action" sx={{ fontSize: 18 }} />,
            },
            {
              label: 'إلى تاريخ',
              value: viewItem.endDate,
              icon: <CalendarIcon color="action" sx={{ fontSize: 18 }} />,
            },
            {
              label: 'المدة',
              value: formatDays(viewItem.startDate, viewItem.endDate),
              icon: <TimerIcon color="action" sx={{ fontSize: 18 }} />,
            },
            {
              label: 'الحالة',
              value: <Chip label={st.label} color={st.color} size="small" icon={st.icon} />,
              raw: true,
            },
          ].map((item, i) => (
          ...
          <Grid item xs={12}>
            ...
            <Paper
              variant="outlined"
              sx={{ p: 2, mt: 0.5, borderRadius: 2, bgcolor: 'action.hover' }}
            >
          ...
          {viewItem.managerNote && (
            <Grid item xs={12}>
              ...
              <Paper
                variant="outlined"
                sx={{ p: 2, mt: 0.5, borderRadius: 2, bgcolor: 'warning.lighter' }}
              >
      ...
      {viewItem.status === 'pending' && (
        <DialogActions sx={{ p: 2.5, gap: 1 }}>
          ...
          <Button
            ...
            onClick={() => {
              setViewItem(null);
              openActionDialog(viewItem, 'reject');
            }}
          >
            رفض
          </Button>
          <Button
            ...
            onClick={() => {
              setViewItem(null);
              openActionDialog(viewItem, 'approve');
            }}
          >
            موافقة
          </Button>
    ...
    {actionDialog && (
      <>
        ...
        <DialogContent sx={{ pt: 2 }}>
          <Alert
            severity={actionDialog.action === 'approve' ? 'success' : 'warning'}
          ...
          >
            ...
            <Typography variant="caption">
              {actionDialog.leave.startDate} → {actionDialog.leave.endDate} (
              {formatDays(actionDialog.leave.startDate, actionDialog.leave.endDate)})
            </Typography>
          </Alert>
          <TextField
            fullWidth
            ...
            placeholder={
              actionDialog.action === 'approve' ? 'أي ملاحظات للموظف...' : 'يرجى ذكر سبب الرفض...'
            }
          />
        </DialogContent>
        <DialogActions sx={{ p: 2.5 }}>
