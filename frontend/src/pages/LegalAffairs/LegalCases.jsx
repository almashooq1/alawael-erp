...
import {
  ...
  createLegalCase,
  updateLegalCase,
} from '../../services/legalAffairs.service';
...
export default function LegalCases() {
  ...
  const handleSave = async () => {
    setSaving(true);
    try {
      if (mode === 'edit') await updateLegalCase(selectedId, form);
  ...
  return (
    <Box sx={{ p: { xs: 2, md: 3 } }}>
      ...
      {loading ? (
        ...
        >
          <Gavel sx={{ fontSize: 64, color: theme.palette.grey[300], mb: 2 }} />
          <Typography color="text.secondary">لا توجد قضايا</Typography>
          ...
          <Table>
            ...
            <TableBody>
              {list.map((c, idx) => (
                ...
                >
                  <TableCell>
                    <Typography fontWeight={600}>{c.caseNumber}</Typography>
                  </TableCell>
                  <TableCell>{c.title}</TableCell>
                  <TableCell>
                    <Chip label={TYPE_LABELS[c.type] || c.type} size="small" variant="outlined" />
                  </TableCell>
                  <TableCell>
                    <Chip
                      label={PRIORITY_LABELS[c.priority] || c.priority}
                      size="small"
                      color={PRIORITY_COLORS[c.priority] || 'default'}
                    />
                  </TableCell>
                  <TableCell>
                    <Chip
      ...
      <Dialog open={dialogOpen} onClose={() => setDialogOpen(false)} maxWidth="sm" fullWidth>
        ...
        <DialogActions>
          ...
          <Button
            variant="contained"
            onClick={handleSave}
            disabled={saving || !form.caseNumber.trim() || !form.title.trim()}
          >
