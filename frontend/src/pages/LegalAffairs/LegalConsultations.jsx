...
export default function LegalConsultations() {
  ...
  const fetch = useCallback(async () => {
    setLoading(true);
    try {
      setList(await getConsultations());
    } catch {
  ...
  const handleSave = async () => {
    setSaving(true);
    try {
      if (mode === 'edit') await updateConsultation(selectedId, form);
  ...
  return (
    <Box sx={{ p: { xs: 2, md: 3 } }}>
      ...
      >
        <Box>
          ...
          <Typography variant="body2" color="text.secondary">
            طلبات الاستشارة والآراء القانونية
          </Typography>
        </Box>
      ...
      {loading ? (
        ...
        >
          <Balance sx={{ fontSize: 64, color: theme.palette.grey[300], mb: 2 }} />
          <Typography color="text.secondary">لا توجد استشارات</Typography>
            ...
            <TableBody>
              {list.map((c, idx) => (
                <TableRow key={c._id || idx}>
                  <TableCell>
                    <Typography fontWeight={600}>{c.consultationNumber}</Typography>
                  </TableCell>
