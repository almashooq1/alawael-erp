...
const DEMO_SESSIONS = [
  {
    _id: 's1',
    student: { _id: '1', fullName: 'أحمد محمد العلي' },
    date: '2026-03-15',
    type: 'فردية',
    activities: ['تصنيف ألوان', 'مطابقة أشكال'],
    attendance: 'حاضر',
    notes: 'تفاعل ممتاز',
  },
  {
    _id: 's2',
    student: { _id: '2', fullName: 'سارة خالد المحمد' },
    date: '2026-03-15',
    type: 'جماعية',
    activities: ['لعب تعاوني'],
    attendance: 'حاضر',
    notes: '',
  },
  {
    _id: 's3',
    student: { _id: '3', fullName: 'عبدالله فهد الأحمد' },
    date: '2026-03-14',
    type: 'فردية',
    activities: ['تمارين حركية'],
    attendance: 'غائب',
    notes: 'عذر طبي',
  },
  {
    ...
    date: '2026-03-14',
    type: 'فردية',
    activities: ['قراءة بطاقات', 'لعب رمل'],
    attendance: 'حاضر',
    notes: '',
  },
  {
    _id: 's5',
    student: { _id: '4', fullName: 'لمى سعد الحربي' },
    date: '2026-03-13',
    type: 'جماعية',
    activities: ['موسيقى', 'حركة'],
    attendance: 'حاضر',
    notes: 'استجابة جيدة',
  },
];
...
const MontessoriSessions = () => {
  ...
  const loadData = useCallback(async () => {
    ...
    try {
      ...
      setEvaluations(
        arr(ev.status === 'fulfilled' && ev.value?.length ? ev.value : DEMO_EVALUATIONS)
      );
  ...
  const levelDistribution = EVAL_LEVELS.map(l => ({
    name: l,
    value: evaluations.filter(e => e.level === l).length,
  }));
  ...
  const handleSaveSession = async () => {
    try {
      ...
      if (editSession) {
        await montessoriService.updateSession(editSession._id, payload);
        showSnackbar('تم تحديث الجلسة', 'success');
      } else {
        await montessoriService.createSession(payload);
  ...
  const handleDeleteSession = sess => {
    showConfirm({
      ...
      onConfirm: async () => {
        try {
          await montessoriService.deleteSession(sess._id);
  ...
  const openEditEval = ev => {
    setEditEval(ev);
    setEvalForm({
      student: ev.student?._id || ev.student || '',
      area: ev.area || '',
      skill: ev.skill || '',
      level: ev.level || 'متوسط',
  ...
  const handleSaveEval = async () => {
    try {
      if (editEval) {
        await montessoriService.updateEvaluation(editEval._id, evalForm);
        showSnackbar('تم تحديث التقييم', 'success');
      } else {
        await montessoriService.createEvaluation(evalForm);
        showSnackbar('تم إنشاء التقييم', 'success');
      }
      setEvalDialogOpen(false);
      loadData();
    } catch {
      showSnackbar('فشل حفظ التقييم', 'error');
    }
  };
  const handleDeleteEval = ev => {
    showConfirm({
      ...
      onConfirm: async () => {
        try {
          await montessoriService.deleteEvaluation(ev._id);
  ...
  const handleExport = () => {
    if (activeTab === 0) {
      ...
      const rows = sessions.map(
        s =>
          `"${s.student?.fullName || '-'}",${s.date || '-'},${s.type},"${(s.activities || []).join(' / ')}",${s.attendance},"${s.notes || ''}"`
      );
      ...
      a.download = `montessori_sessions_${new Date().toISOString().slice(0, 10)}.csv`;
      a.click();
    } else {
      const hdr = 'الطالب,المجال,المهارة,المستوى,التاريخ';
      const rows = evaluations.map(
        e => `"${e.student?.fullName || '-'}",${e.area},${e.skill},${e.level},${e.date || '-'}`
      );
      const blob = new Blob(['\uFEFF' + [hdr, ...rows].join('\n')], {
        type: 'text/csv;charset=utf-8;',
      });
      const a = document.createElement('a');
      a.href = URL.createObjectURL(blob);
      a.download = `montessori_evaluations_${new Date().toISOString().slice(0, 10)}.csv`;
      a.click();
    }
            ...
            >
              <Box>
                ...
                <Typography variant="body2" sx={{ color: 'rgba(255,255,255,0.85)' }}>
                  تسجيل الجلسات اليومية وتقييم مستوى الطلاب
                </Typography>
              </Box>
        ...
        <Container maxWidth="xl" sx={{ pt: 5, pb: 4 }}>
              ...
              >
                ...
                <ResponsiveContainer width="100%" height={240}>
                  <PieChart>
                    ...
                    >
                      {levelDistribution.map((_, idx) => (
                        <Cell key={idx} fill={`url(#sessPie${idx})`} stroke="none" />
                      ))}
                    </Pie>
          ...
          {activeTab === 0 && (
            ...
            >
              {filteredSessions.length === 0 ? (
                <Box sx={{ p: 4 }}>
                  <EmptyState title="لا توجد جلسات" />
                </Box>
              ) : (
                <TableContainer>
                  <Table>
                        ...
                        >
                          ...
                          <TableCell>
                            <Stack direction="row" spacing={0.5} flexWrap="wrap" useFlexGap>
                              {(s.activities || []).slice(0, 3).map((a, ai) => (
                                <Chip key={ai} label={a} size="small" sx={{ borderRadius: 2 }} />
                              ))}
                              {(s.activities || []).length > 3 && (
                                <Chip
                                  label={`+${s.activities.length - 3}`}
                                  size="small"
                                  variant="outlined"
                                />
                              )}
                        ...
                        >
                          ...
                          <TableCell>
                            <Chip
                              ...
                              size="small"
                              sx={{
                                bgcolor: (levelColors[ev.level] || '#ccc') + '22',
          ...
          >
            <DialogTitle sx={{ fontWeight: 700 }}>
              {editSession ? 'تعديل الجلسة' : 'إنشاء جلسة جديدة'}
            </DialogTitle>
              ...
              <Grid container spacing={2} sx={{ mt: 0.5 }}>
                <Grid item xs={12}>
                  ...
                  >
                    ...
                    {students.length === 0 &&
                      DEMO_SESSIONS.map(s => (
                        <MenuItem key={s._id} value={s.student?._id || s._id}>
                          {s.student?.fullName || 'طالب'}
                        </MenuItem>
                      ))}
                  </TextField>
                </Grid>
          ...
          >
            <DialogTitle sx={{ fontWeight: 700 }}>
              {editEval ? 'تعديل التقييم' : 'إنشاء تقييم جديد'}
            </DialogTitle>
            <DialogContent dividers>
              <Grid container spacing={2} sx={{ mt: 0.5 }}>
                <Grid item xs={12}>
            ...
            <DialogActions>
              ...
              <Button variant="contained" onClick={handleSaveEval}>
                حفظ
              </Button>
            </DialogActions>
          </Dialog>
