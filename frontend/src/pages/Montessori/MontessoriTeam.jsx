...
const DEMO_TEAM = [
  {
    _id: 't1',
    name: 'أ. نورة محمد الشمري',
    role: 'معلم',
    contact: { phone: '0501234567', email: 'noura@alawael.com' },
  },
  {
    _id: 't2',
    name: 'د. فاطمة علي الأحمد',
    role: 'أخصائي',
    contact: { phone: '0509876543', email: 'fatima@alawael.com' },
  },
  {
    _id: 't3',
    name: 'أ. خالد سعد العتيبي',
    role: 'مشرف',
    contact: { phone: '0551112233', email: 'khaled@alawael.com' },
  },
  {
    _id: 't4',
    name: 'أ. سارة أحمد المنصور',
    role: 'معلم',
    contact: { phone: '0561234567', email: 'sara@alawael.com' },
  },
  {
    _id: 't5',
    name: 'أ. محمد عبدالله القحطاني',
    role: 'مدير',
    contact: { phone: '0541234567', email: 'mohammed@alawael.com' },
  },
];
const DEMO_PARENTS = [
  {
    ...
    students: [{ fullName: 'أحمد محمد العلي' }],
  },
  {
    _id: 'pr2',
    name: 'خالد المحمد',
    phone: '0502222222',
    email: 'km@gmail.com',
    students: [{ fullName: 'سارة خالد المحمد' }],
  },
  {
    _id: 'pr3',
    name: 'فهد الأحمد',
    phone: '0503333333',
    email: 'fa@gmail.com',
    students: [{ fullName: 'عبدالله فهد الأحمد' }],
  },
  {
    _id: 'pr4',
    name: 'سعد الحربي',
    phone: '0504444444',
    email: 'sh@gmail.com',
    students: [{ fullName: 'لمى سعد الحربي' }],
  },
];
...
const MontessoriTeam = () => {
  ...
  const roleDistribution = TEAM_ROLES.map(r => ({
    name: r,
    value: team.filter(t => t.role === r).length,
  })).filter(d => d.value > 0);

  /* ── Team CRUD ── */
  const openCreateTeam = () => {
    setEditTeamMember(null);
    setTeamForm({ name: '', role: 'معلم', contact: { phone: '', email: '' } });
  ...
  const openEditTeam = member => {
    ...
    setTeamForm({
      name: member.name || '',
      role: member.role || 'معلم',
      contact: { phone: member.contact?.phone || '', email: member.contact?.email || '' },
    });
    setTeamDialogOpen(true);
  };
  const handleSaveTeam = async () => {
    try {
      if (editTeamMember) {
        await montessoriService.updateTeamMember(editTeamMember._id, teamForm);
        showSnackbar('تم تحديث بيانات العضو', 'success');
      } else {
        await montessoriService.createTeamMember(teamForm);
        showSnackbar('تم إضافة العضو', 'success');
      }
  ...
  const handleDeleteTeam = member => {
    showConfirm({
      title: 'حذف عضو',
      message: `هل أنت متأكد من حذف "${member.name}"؟`,
      confirmText: 'حذف',
      confirmColor: 'error',
      onConfirm: async () => {
        try {
          await montessoriService.deleteTeamMember(member._id);
          showSnackbar('تم حذف العضو', 'success');
          loadData();
        } catch {
  ...
  const handleSaveParent = async () => {
    try {
      if (editParent) {
        await montessoriService.updateParent(editParent._id, parentForm);
        showSnackbar('تم تحديث بيانات ولي الأمر', 'success');
      } else {
        await montessoriService.createParent(parentForm);
        showSnackbar('تم إضافة ولي الأمر', 'success');
      }
  ...
  const handleDeleteParent = parent => {
    showConfirm({
      title: 'حذف ولي أمر',
      message: `هل أنت متأكد من حذف "${parent.name}"؟`,
      confirmText: 'حذف',
      confirmColor: 'error',
      onConfirm: async () => {
        try {
          await montessoriService.deleteParent(parent._id);
          showSnackbar('تم حذف ولي الأمر', 'success');
          loadData();
        } catch {
          showSnackbar('فشل الحذف', 'error');
        }
  ...
  const handleExport = () => {
    if (activeTab === 0) {
      ...
      const rows = team.map(
        m => `"${m.name}",${m.role},"${m.contact?.phone || '-'}","${m.contact?.email || '-'}"`
      );
      ...
      a.download = `montessori_team_${new Date().toISOString().slice(0, 10)}.csv`;
      a.click();
    } else {
      const hdr = 'الاسم,الهاتف,البريد,الأبناء';
      const rows = parents.map(
        p =>
          `"${p.name}","${p.phone || '-'}","${p.email || '-'}","${(p.students || []).map(s => s.fullName || s).join(' / ')}"`
      );
      const blob = new Blob(['\uFEFF' + [hdr, ...rows].join('\n')], {
        type: 'text/csv;charset=utf-8;',
      });
      const a = document.createElement('a');
      a.href = URL.createObjectURL(blob);
      a.download = `montessori_parents_${new Date().toISOString().slice(0, 10)}.csv`;
      a.click();
    }
            ...
            >
              <Box>
                ...
                <Typography variant="h5" fontWeight={800} color="#fff">
                  فريق العمل وأولياء الأمور
                </Typography>
                <Typography variant="body2" sx={{ color: 'rgba(255,255,255,0.85)' }}>
                  إدارة أعضاء الفريق التعليمي والتواصل مع أولياء الأمور
                </Typography>
              </Box>
        ...
        <Container maxWidth="xl" sx={{ pt: 5, pb: 4 }}>
          ...
          <Grid container spacing={2} sx={{ mb: 3 }}>
            ...
            <Grid item xs={6} sm={3}>
              <MiniKPI
                label="المعلمون"
                value={team.filter(t => t.role === 'معلم').length}
                icon={<PersonIcon />}
            ...
            <Grid item xs={6} sm={3}>
              <MiniKPI
                label="الأخصائيون"
                value={team.filter(t => t.role === 'أخصائي').length}
                icon={<BadgeIcon />}
            ...
            >
              ...
              <ResponsiveContainer width="100%" height={200}>
                <PieChart>
                  ...
                  >
                    {roleDistribution.map((_, idx) => (
                      <Cell key={idx} fill={`url(#teamPie${idx})`} stroke="none" />
                    ))}
                  </Pie>
          ...
          {activeTab === 0 &&
            (filteredTeam.length === 0 ? (
              ...
              >
                <EmptyState title="لا يوجد أعضاء في الفريق" />
              </Paper>
            ) : (
                      ...
                      <Card
                        ...
                        sx={{
                          ...
                          '&:hover': {
                            ...
                            borderColor: roleColors[member.role] || 'primary.main',
                          },
                        }}
                      >
                        {/* Gradient role strip */}
                        <Box
                          sx={{
                            height: 4,
                            background: roleGradients[member.role] || gradients.primary,
                          }}
                        />
                          ...
                          <Box sx={{ display: 'flex', alignItems: 'center', gap: 2, mb: 1.5 }}>
                            ...
                            <Box sx={{ flex: 1 }}>
                              ...
                              <Chip
                                ...
                                size="small"
                                sx={{
                                  bgcolor: (roleColors[member.role] || '#999') + '22',
                                  color: roleColors[member.role] || '#999',
                                  fontWeight: 600,
                                  borderRadius: 2,
                                }}
                              />
                        ...
                        <CardActions sx={{ px: 2.5, pb: 2, pt: 0 }}>
                          ...
                          <Tooltip title="حذف">
                            <IconButton
                              size="small"
                              color="error"
                              onClick={() => handleDeleteTeam(member)}
                            >
          ...
          {activeTab === 1 && (
            ...
            >
              {filteredParents.length === 0 ? (
                <Box sx={{ p: 4 }}>
                  <EmptyState title="لا يوجد أولياء أمور" />
                </Box>
              ) : (
                <TableContainer>
                  <Table>
          ...
          >
            <DialogTitle sx={{ fontWeight: 700 }}>
              {editTeamMember ? 'تعديل عضو الفريق' : 'إضافة عضو جديد'}
            </DialogTitle>
              ...
              <Grid container spacing={2} sx={{ mt: 0.5 }}>
                ...
                <Grid item xs={6}>
                  <TextField
                    ...
                    onChange={e =>
                      setTeamForm(f => ({ ...f, contact: { ...f.contact, phone: e.target.value } }))
                    }
                    dir="ltr"
                  />
                </Grid>
                <Grid item xs={6}>
                  <TextField
                    ...
                    value={teamForm.contact.email}
                    onChange={e =>
                      setTeamForm(f => ({ ...f, contact: { ...f.contact, email: e.target.value } }))
                    }
                    dir="ltr"
                  />
                </Grid>
            ...
            <DialogActions>
              ...
              <Button variant="contained" onClick={handleSaveTeam} disabled={!teamForm.name}>
                حفظ
              </Button>
            </DialogActions>
          </Dialog>
          ...
          >
            <DialogTitle sx={{ fontWeight: 700 }}>
              {editParent ? 'تعديل ولي الأمر' : 'إضافة ولي أمر جديد'}
            </DialogTitle>
            <DialogContent dividers>
              <Grid container spacing={2} sx={{ mt: 0.5 }}>
                <Grid item xs={12}>
            ...
            <DialogActions>
              ...
              <Button variant="contained" onClick={handleSaveParent} disabled={!parentForm.name}>
                حفظ
              </Button>
            </DialogActions>
          </Dialog>
