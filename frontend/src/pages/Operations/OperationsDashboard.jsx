...
const DATA_RECENT_MAINTENANCE = [
  {
    id: 1,
    asset: 'جهاز علاج طبيعي #12',
    type: 'صيانة دورية',
    status: 'completed',
    date: '2026-03-10',
    cost: 1500,
...
export default function OperationsDashboard() {
  ...
  const [assetsByCategory, setAssetsByCategory] = useState(DATA_ASSETS_BY_CATEGORY);
  const [maintenanceTrend, _setMaintenanceTrend] = useState(DATA_MAINTENANCE_TREND);
  const [maintenanceStatus, setMaintenanceStatus] = useState(DATA_MAINTENANCE_STATUS);
  const [recentMaintenance, setRecentMaintenance] = useState(DATA_RECENT_MAINTENANCE);
  ...
  const loadDashboard = useCallback(async () => {
    ...
    try {
      const [assetsRes, equipRes, maintRes, schedRes, licensesRes, branchesRes] = await Promise.all(
        [
          operationsService.getAssets().catch(err => {
            logger.warn('Operations: assets fetch', err);
            return null;
          }),
          operationsService.getEquipment().catch(err => {
            logger.warn('Operations: equipment fetch', err);
            return null;
          }),
          operationsService.getMaintenance().catch(err => {
            logger.warn('Operations: maintenance fetch', err);
            return null;
          }),
          operationsService.getSchedules().catch(err => {
            logger.warn('Operations: schedules fetch', err);
            return null;
          }),
          operationsService.getLicenses().catch(err => {
            logger.warn('Operations: licenses fetch', err);
            return null;
          }),
          operationsService.getBranches().catch(err => {
            logger.warn('Operations: branches fetch', err);
            return null;
          }),
        ]
      );
      ...
      if (Array.isArray(assets) && assets.length > 0) {
        ...
        if (Array.isArray(maintenance) && maintenance.length > 0) {
          ...
          const localStatusColors = {
            ...
            'in-progress': statusColors.warning,
            overdue: statusColors.error,
            pending: statusColors.info,
            scheduled: statusColors.purple,
          };
        ...
        <Grid container spacing={2} sx={{ mb: 4 }}>
          ...
          <Grid item xs={12} sm={6} md={3} lg={2}>
            <ModuleKPICard
              ...
              subtitle={`${stats.utilizationRate}% استخدام`}
          ...
          <Grid item xs={12} sm={6} md={3} lg={2}>
            <ModuleKPICard
              ...
              subtitle={`${stats.licensesExpiring} رخصة تنتهي`}
              icon={<BusinessIcon />}
              color="secondary"
            />
          </Grid>
            ...
            <Paper sx={{ p: 3, borderRadius: 3, height: '100%' }}>
              ...
              {maintenanceTrend.length > 0 ? (
                <ResponsiveContainer
                  ...
                  height={280}
                  role="img"
                  aria-label="رسم بياني لاتجاه الصيانة الشهري"
                >
            ...
            <Paper sx={{ p: 3, borderRadius: 3, height: '100%' }}>
              ...
              {assetsByCategory.length > 0 ? (
                <ResponsiveContainer
                  ...
                  height={280}
                  role="img"
                  aria-label="رسم بياني للأصول حسب الفئة"
                >
                  <PieChart>
                    <Pie
            ...
            <Paper sx={{ p: 3, borderRadius: 3 }}>
              ...
              {maintenanceStatus.length > 0 ? (
                <ResponsiveContainer
                  ...
                  height={250}
                  role="img"
                  aria-label="رسم بياني لحالة الصيانة"
                >
                  <PieChart>
                    <Pie
                    ...
                    {recentMaintenance.map(m => (
                      <TableRow key={m.id} hover>
                        ...
                        <TableCell align="center">
                          {m.cost > 0 ? `${m.cost.toLocaleString()} ر.س` : '-'}
                        </TableCell>
                        <TableCell>
                          <Chip
