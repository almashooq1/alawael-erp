...
import {
  ...
  LocalPolice as GovIcon,
  LocationCity as MunicipalIcon,
  Receipt as CommercialIcon,
  ...
  PlaylistAddCheck as ChecklistDoneIcon,
...
const RehabCenterLicenses = () => {
  ...
  const handleSetupApproval = async () => {
    ...
    try {
      const steps = (formData.approvalSteps || '')
        .split('\n')
        .filter(Boolean)
        .map((s, i) => ({
          stepNumber: i + 1,
  ...
  return (
    <Container maxWidth="xl" sx={{ mt: 3, mb: 4 }}>
      ...
      >
        ...
        >
          ...
          <Box sx={{ display: 'flex', gap: 1 }}>
            <Badge
              badgeContent={alerts.filter(a => a.priority === 'critical').length}
              color="error"
            >
              <Button
                variant="contained"
                color="warning"
                startIcon={<AlertActiveIcon />}
      ...
      <Grid container spacing={2} sx={{ mb: 3 }}>
        ...
        <Grid item xs={12} sm={6} md={3} lg={2}>
          <StatCard
            title="بانتظار التجديد"
            value={stats.pendingRenewal || 0}
            icon={<RenewIcon />}
            color="#0288d1"
          />
        </Grid>
                      ...
                      return (
                        ...
                        >
                          <TableCell>
                            ...
                            {lic.referenceNumber && (
                              <Typography variant="caption" color="text.secondary">
                                مرجع: {lic.referenceNumber}
                              </Typography>
                            )}
                          </TableCell>
                          <TableCell>
                            <Typography variant="body2">
                              {lic.licenseTypeLabel || lic.licenseType}
                            </Typography>
                          </TableCell>
                          <TableCell>
                            <Chip
                            ...
                            <Box sx={{ display: 'flex', gap: 0.5 }}>
                              ...
                              <Tooltip title="نسخ">
                                <IconButton
                                  size="small"
                                  sx={{ color: '#00695c' }}
                                  onClick={() => handleCloneLicense(lic._id)}
                                >
                              ...
                              <Tooltip title="مفضلة">
                                <IconButton
                                  size="small"
                                  sx={{ color: '#e91e63' }}
                                  onClick={() => handleToggleFavorite(lic._id)}
                                >
                              ...
                              <Tooltip title="سجل التغييرات">
                                <IconButton
                                  size="small"
                                  sx={{ color: '#37474f' }}
                                  onClick={() => handleViewChangeLog(lic._id)}
                                >
              ...
              return (
                <Grid item xs={6} sm={4} md={3} key={cat._id}>
                  <Card sx={{ textAlign: 'center', borderTop: `3px solid ${cfg.color || '#ccc'}` }}>
                    <CardContent>
                      <Avatar
                        sx={{ bgcolor: `${cfg.color}20`, color: cfg.color, mx: 'auto', mb: 1 }}
                      >
                        {cfg.icon || <DocIcon />}
                      </Avatar>
                ...
                >
                  ...
                  >
                    ...
                    <Tooltip title="تجاهل">
                      <IconButton
                        size="small"
                        onClick={() => handleDismissAlert(alert.licenseId, alert._id)}
                      >
              ...
              <TableBody>
                {(dashboard?.upcomingExpirations || []).map((lic, i) => {
                  ...
                  return (
                    <TableRow
                      key={i}
                      sx={{ bgcolor: days !== null && days <= 7 ? '#ffebee' : 'inherit' }}
                    >
                      <TableCell>
                        <Typography fontWeight="bold">{lic.licenseNumber}</Typography>
                      </TableCell>
      ...
      {activeTab === 4 && (
        <Box>
          <Box sx={{ display: 'flex', justifyContent: 'space-between', mb: 2 }}>
            <Typography variant="h6" fontWeight="bold">
              ⚖️ الغرامات والعقوبات المعلقة ({pendingPenalties.length})
            </Typography>
            <Button variant="outlined" startIcon={<RefreshIcon />} onClick={fetchPenalties}>
              تحديث
            </Button>
          </Box>
                  ...
                  {pendingPenalties.map((item, i) =>
                    (item.penalties || []).map((pen, j) => (
                      <TableRow key={`${i}-${j}`} hover>
                        <TableCell>
                          <Typography fontWeight="bold">{item.licenseNumber}</Typography>
                        </TableCell>
                            ...
                            label={
                              pen.type === 'fine'
                                ...
                                : pen.type === 'warning'
                                  ...
                                  : pen.type === 'suspension'
                                    ? 'إيقاف'
                                    : pen.type === 'restriction'
                                      ? 'تقييد'
                                      : pen.type === 'closure_threat'
                                        ? 'تهديد بالإغلاق'
                                        : pen.type
                            }
                            color={
      ...
      {activeTab === 5 && (
        <Box>
          <Box sx={{ display: 'flex', justifyContent: 'space-between', mb: 2 }}>
            ...
            <Box sx={{ display: 'flex', gap: 1 }}>
              <Button variant="outlined" startIcon={<RefreshIcon />} onClick={fetchHighRisk}>
                تحديث
              </Button>
              <Button
                variant="contained"
                color="warning"
                startIcon={<RiskIcon />}
                onClick={handleCalculateAllRisks}
              >
          ...
          <Typography variant="subtitle1" fontWeight="bold" sx={{ mb: 1 }}>
            التراخيص عالية المخاطرة ({highRiskLicenses.length})
          </Typography>
                    ...
                    >
                          ...
                          <LinearProgress
                            ...
                            sx={{
                              ...
                              '& .MuiLinearProgress-bar': {
                                bgcolor:
                                  (lic.riskScore?.score || 0) >= 75
                                    ? '#f44336'
                                    : (lic.riskScore?.score || 0) >= 50
                                      ? '#ff9800'
                                      : '#4caf50',
                              },
                            }}
                      ...
                      <TableCell>
                        <Chip
                          size="small"
                          label={
                            lic.riskScore?.level === 'critical'
                              ? 'حرج'
                              : lic.riskScore?.level === 'high'
                                ? 'عالي'
                                : lic.riskScore?.level === 'medium'
                                  ? 'متوسط'
                                  : 'منخفض'
                          }
                          color={
                      ...
                      <TableCell>
                        <Chip
                          label={STATUS_CONFIG[lic.status]?.label || lic.status}
                          color={STATUS_CONFIG[lic.status]?.color || 'default'}
                          size="small"
                        />
                      </TableCell>
                      <TableCell>
            ...
            {renewalForecast.slice(0, 6).map((m, i) => (
              <Grid item xs={6} sm={4} md={2} key={i}>
                <Card
                  sx={{
                    textAlign: 'center',
                    borderTop: `3px solid ${m.count > 5 ? '#f44336' : m.count > 2 ? '#ff9800' : '#4caf50'}`,
                  }}
                >
                  <CardContent>
                    <Typography variant="caption" color="text.secondary">
                    ...
                    <Typography variant="caption">
                      تكلفة: {(m.estimatedCost || 0).toLocaleString()} ر.س
                    </Typography>
                  </CardContent>
                </Card>
              </Grid>
              ...
              <Grid item xs={6} sm={4} md={3} key={i}>
                <Card>
                  <CardContent sx={{ textAlign: 'center' }}>
                    ...
                    <Typography variant="caption">
                      ساري: {r.active || 0} | منتهي: {r.expired || 0}
                    </Typography>
                  </CardContent>
                </Card>
              </Grid>
      ...
      {activeTab === 6 && (
        <Box>
          <Box sx={{ display: 'flex', justifyContent: 'space-between', mb: 2 }}>
            ...
            <Button variant="outlined" startIcon={<RefreshIcon />} onClick={fetchComplianceReport}>
              تحديث
            </Button>
          </Box>
          {!complianceReport ? (
                  ...
                  <CardContent>
                    ...
                    <Box sx={{ position: 'relative', display: 'inline-flex', mt: 2, mb: 1 }}>
                      <CircularProgress
                        ...
                        sx={{
                          color:
                            (complianceReport.overallCompliance || 0) >= 80 ? '#4caf50' : '#ff9800',
                        }}
                      />
                      <Box
                        sx={{
                      ...
                      >
                        <Typography variant="h5" fontWeight="bold">
                          {complianceReport.overallCompliance || 0}%
                        </Typography>
                      </Box>
                    </Box>
                  </CardContent>
                  ...
                  <Grid item xs={6} sm={3}>
                    <Card>
                      <CardContent sx={{ textAlign: 'center' }}>
                        ...
                        <Typography variant="h5" fontWeight="bold">
                          {complianceReport.totalLicenses || 0}
                        </Typography>
                      </CardContent>
                    </Card>
                  </Grid>
                    ...
                    <Card>
                      <CardContent sx={{ textAlign: 'center' }}>
                        ...
                        <Typography variant="h5" fontWeight="bold" color="success.main">
                          {complianceReport.activeLicenses || 0}
                        </Typography>
                      </CardContent>
                    </Card>
                  </Grid>
                    ...
                    <Card>
                      <CardContent sx={{ textAlign: 'center' }}>
                        ...
                        <Typography variant="h5" fontWeight="bold" color="error.main">
                          {complianceReport.expiredLicenses || 0}
                        </Typography>
                      </CardContent>
                    </Card>
                  </Grid>
                    ...
                    <Card>
                      <CardContent sx={{ textAlign: 'center' }}>
                        ...
                        <Typography variant="h5" fontWeight="bold" color="warning.main">
                          {complianceReport.totalViolations || 0}
                        </Typography>
                      </CardContent>
                    </Card>
                  </Grid>
                  ...
                  <Stack spacing={1}>
                    {complianceReport.issues.map((issue, i) => (
                      ...
                      >
                        ...
                        {issue.count && (
                          <Chip size="small" label={`${issue.count} ترخيص`} sx={{ ml: 1 }} />
                        )}
                      </Alert>
                    ))}
                  </Stack>
      ...
      {activeTab === 7 && (
        <Box>
          <Box sx={{ display: 'flex', justifyContent: 'space-between', mb: 2 }}>
            <Typography variant="h6" fontWeight="bold">
              📦 التراخيص المؤرشفة ({archivedLicenses.length})
            </Typography>
            <Button variant="outlined" startIcon={<RefreshIcon />} onClick={fetchArchived}>
              تحديث
            </Button>
          </Box>
              ...
              <Table size="small">
                ...
                <TableBody>
                  {pendingComms.map((item, i) =>
                    (item.communications || [])
                      .filter(c => c.responseRequired && !c.responseDate)
                      .map((comm, j) => (
                        <TableRow key={`${i}-${j}`} hover>
                          <TableCell>
                            <Typography fontWeight="bold">{item.licenseNumber}</Typography>
      ...
      {activeTab === 9 && (
        <Box>
          <Box sx={{ display: 'flex', justifyContent: 'space-between', mb: 2 }}>
            <Typography variant="h6" fontWeight="bold">
              📅 تقويم المواعيد والأحداث
            </Typography>
            <Button variant="outlined" startIcon={<RefreshIcon />} onClick={fetchUpcomingEvents}>
              تحديث
            </Button>
          </Box>
          ...
          {upcomingEvents.length === 0 ? (
            <Paper sx={{ p: 4, textAlign: 'center' }}>
              ...
              <Typography variant="h6" color="text.secondary">
                لا توجد أحداث قادمة خلال 30 يوم
              </Typography>
            </Paper>
          ) : (
            <Stack spacing={2}>
                  ...
                  >
                    ...
                    >
                      <Box>
                        ...
                        <Box sx={{ display: 'flex', gap: 2, mt: 1 }}>
                          <Typography variant="caption">
                            <strong>الترخيص:</strong> {item.licenseNumber}
                          </Typography>
                          <Typography variant="caption">
                            <strong>المركز:</strong> {item.center?.name || '-'}
                          </Typography>
                        </Box>
                      </Box>
          ...
          <Box sx={{ display: 'flex', justifyContent: 'space-between', mb: 2 }}>
            ...
            <Box sx={{ display: 'flex', gap: 1 }}>
              ...
              <Button
                ...
                onClick={handleCalculateAllHealth}
              >
                إعادة حساب الصحة
              </Button>
            </Box>
                    ...
                    >
                        ...
                        <Box sx={{ display: 'flex', alignItems: 'center', gap: 1 }}>
                          <LinearProgress
                            ...
                            sx={{
                              ...
                              '& .MuiLinearProgress-bar': {
                                bgcolor:
                                  (lic.healthScore?.score || 0) < 40
                                    ? '#f44336'
                                    : (lic.healthScore?.score || 0) < 60
                                      ? '#ff9800'
                                      : '#4caf50',
                              },
                            }}
                      ...
                      <TableCell>
                        <Chip
                          size="small"
                          label={
                            lic.healthScore?.grade === 'A'
                              ? 'ممتاز A'
                              : lic.healthScore?.grade === 'B'
                                ? 'جيد B'
                                : lic.healthScore?.grade === 'C'
                                  ? 'مقبول C'
                                  : lic.healthScore?.grade === 'D'
                                    ? 'ضعيف D'
                                    : lic.healthScore?.grade === 'F'
                                      ? 'راسب F'
                                      : '-'
                          }
                          color={
              ...
              <Grid container spacing={2} sx={{ mb: 3 }}>
                <Grid item xs={6} sm={4} md={3}>
                  <Card sx={{ textAlign: 'center', borderTop: '3px solid #1976d2' }}>
                    <CardContent>
                      <Typography variant="caption" color="text.secondary">
                        نسبة التجديد في الوقت
                      </Typography>
                      <Typography variant="h4" fontWeight="bold" color="primary">
                        {kpiDashboard.renewalOnTimeRate || 0}%
                      </Typography>
                    </CardContent>
                  </Card>
                </Grid>
                  ...
                  <Card sx={{ textAlign: 'center', borderTop: '3px solid #f44336' }}>
                    <CardContent>
                      ...
                      <Typography variant="h4" fontWeight="bold" color="error.main">
                        {(kpiDashboard.totalPenalties || 0).toLocaleString()}{' '}
                  ...
                  <Card sx={{ textAlign: 'center', borderTop: '3px solid #0288d1' }}>
                    <CardContent>
                      ...
                      <Typography variant="h4" fontWeight="bold" sx={{ color: '#0288d1' }}>
                        {kpiDashboard.taskCompletionRate || 0}%
                      </Typography>
                    </CardContent>
                  </Card>
                </Grid>
                  ...
                  <Card sx={{ textAlign: 'center', borderTop: '3px solid #2e7d32' }}>
                    <CardContent>
                      ...
                      <Typography variant="h4" fontWeight="bold" sx={{ color: '#2e7d32' }}>
                        {kpiDashboard.documentCompleteness || 0}%
                      </Typography>
                    </CardContent>
                  </Card>
                </Grid>
                  ...
                  <Card sx={{ textAlign: 'center', borderTop: '3px solid #ed6c02' }}>
                    <CardContent>
                      ...
                      <Typography variant="h4" fontWeight="bold" sx={{ color: '#ed6c02' }}>
                        {kpiDashboard.budgetAdherence || 0}%
                      </Typography>
                    </CardContent>
                  </Card>
                </Grid>
                    ...
                    <Grid item xs={6} sm={3}>
                      <Card sx={{ textAlign: 'center' }}>
                        <CardContent>
                          ...
                          <Typography variant="h5" fontWeight="bold">
                            {(budgetStats.totalBudget || 0).toLocaleString()} ر.س
                          </Typography>
                        </CardContent>
                      </Card>
                    </Grid>
                    <Grid item xs={6} sm={3}>
                      <Card sx={{ textAlign: 'center' }}>
                        <CardContent>
                          ...
                          <Typography variant="h5" fontWeight="bold" color="error.main">
                            {(budgetStats.totalSpent || 0).toLocaleString()} ر.س
                          </Typography>
                        </CardContent>
                      </Card>
                    </Grid>
                    <Grid item xs={6} sm={3}>
                      <Card sx={{ textAlign: 'center' }}>
                        <CardContent>
                          ...
                          <Typography variant="h5" fontWeight="bold" color="success.main">
                            {(budgetStats.remaining || 0).toLocaleString()} ر.س
                          </Typography>
                        </CardContent>
                      </Card>
                    </Grid>
                      ...
                      <Card sx={{ textAlign: 'center' }}>
                        <CardContent>
                          ...
                          <Typography
                            ...
                            fontWeight="bold"
                            color={
                              (budgetStats.spentPercentage || 0) > 90
                                ? 'error.main'
                                : (budgetStats.spentPercentage || 0) > 70
                                  ? 'warning.main'
                                  : 'success.main'
                            }
                          >
                            {budgetStats.spentPercentage || 0}%
                          </Typography>
                        </CardContent>
                      </Card>
                    </Grid>
      ...
      {activeTab === 12 && (
        <Box>
          <Box sx={{ display: 'flex', justifyContent: 'space-between', mb: 2 }}>
            <Typography variant="h6" fontWeight="bold">
              📋 القوالب والمفضلة وقائمة المتابعة
            </Typography>
            <Button
              variant="outlined"
              startIcon={<RefreshIcon />}
          ...
          {templates.length === 0 ? (
            <Paper sx={{ p: 3, mb: 3, textAlign: 'center' }}>
              <Typography color="text.secondary">
                لا توجد قوالب محفوظة بعد. قم بحفظ ترخيص كقالب من قائمة التراخيص.
              </Typography>
            </Paper>
          ) : (
            <Grid container spacing={2} sx={{ mb: 3 }}>
                ...
                <Grid item xs={12} sm={6} md={4} key={tpl._id}>
                  <Card sx={{ borderRight: '4px solid #7b1fa2' }}>
                    <CardContent>
                      ...
                      >
                        <Box>
                          <Typography fontWeight="bold">
                            {tpl.templateData?.templateName || tpl.licenseNumber}
                          </Typography>
                          <Typography variant="caption" color="text.secondary">
                            {tpl.templateData?.templateDescription || 'بدون وصف'}
                          </Typography>
                        </Box>
                        <Chip
                          label={tpl.templateData?.templateCategory || 'عام'}
                          size="small"
                          color="secondary"
                        />
                      </Box>
                      <Divider sx={{ my: 1 }} />
                      <Typography variant="caption" color="text.secondary">
                        الاستخدام: {tpl.templateData?.usageCount || 0} مرة | الفئة:{' '}
                        {CATEGORY_CONFIG[tpl.category]?.label || tpl.category}
                      </Typography>
                      <Box sx={{ mt: 1 }}>
                        <Button
                          size="small"
                ...
                <TableBody>
                  {favorites.map(lic => (
                    <TableRow key={lic._id} hover>
                      ...
                      <TableCell>
                        <Chip
                          label={CATEGORY_CONFIG[lic.category]?.label || lic.category}
                          size="small"
                        />
                      </TableCell>
                      <TableCell>
                      ...
                      <TableCell>
                        ...
                        <Tooltip title="عرض">
                          <IconButton size="small" onClick={() => setViewDialog(lic)}>
                            <ViewIcon />
                          </IconButton>
                        </Tooltip>
                      </TableCell>
                ...
                <TableBody>
                  {watchlist.map(lic => (
                    <TableRow key={lic._id} hover>
                      ...
                      <TableCell>
                        <Tooltip title="إيقاف المتابعة">
                          <IconButton
                            size="small"
                            color="primary"
                            onClick={() => handleToggleWatch(lic._id)}
                          >
                            <WatchIcon />
                          </IconButton>
                        </Tooltip>
      ...
      {activeTab === 13 && (
        <Box>
          <Box sx={{ display: 'flex', justifyContent: 'space-between', mb: 2 }}>
            <Typography variant="h6" fontWeight="bold">
              ⏱️ اتفاقيات مستوى الخدمة (SLA) والتذاكر
            </Typography>
            <Box sx={{ display: 'flex', gap: 1 }}>
              <Button
                variant="contained"
          ...
          {/* SLA Breached */}
          <Typography variant="subtitle1" fontWeight="bold" sx={{ mb: 1, color: '#f44336' }}>
            ⚠️ تراخيص مخالفة لـ SLA
          </Typography>
          {slaBreached.length === 0 ? (
            <Paper sx={{ p: 3, mb: 3, textAlign: 'center' }}>
              <Typography color="text.secondary">
                لا توجد تراخيص مخالفة لاتفاقيات الخدمة ✅
              </Typography>
            </Paper>
          ) : (
            <TableContainer component={Paper} sx={{ mb: 3 }}>
                    ...
                    <TableRow key={lic._id} hover>
                      ...
                      <TableCell>
                        <Box sx={{ display: 'flex', alignItems: 'center', gap: 1 }}>
                          <LinearProgress
                            variant="determinate"
                            value={lic.sla?.overallCompliance || 0}
                            sx={{
                              flex: 1,
                              height: 8,
                              borderRadius: 1,
                          ...
                          <Typography variant="caption" fontWeight="bold" color="error">
                            {lic.sla?.overallCompliance || 0}%
                          </Typography>
                        </Box>
                      </TableCell>
                      <TableCell>
                        <Tooltip title="إعدادات SLA">
                          <IconButton
                            size="small"
                            onClick={() => {
                              setSlaDialog(lic);
              ...
              <Grid item xs={6} sm={2.4}>
                <Card sx={{ textAlign: 'center', borderTop: '3px solid #0288d1' }}>
                  <CardContent>
                    ...
                    <Typography variant="h5" fontWeight="bold" sx={{ color: '#0288d1' }}>
                ...
                <TableBody>
                  {openTickets.map((item, idx) => (
                    <TableRow key={idx} hover>
                      ...
                      <TableCell>
                        <Chip
                          label={PRIORITY_CONFIG[item.priority]?.label || item.priority}
                          size="small"
                          sx={{
                            bgcolor: PRIORITY_CONFIG[item.priority]?.color || '#757575',
                            color: 'white',
                          }}
                        />
                      </TableCell>
      ...
      {activeTab === 14 && (
        <Box>
          <Box sx={{ display: 'flex', justifyContent: 'space-between', mb: 2 }}>
            <Typography variant="h6" fontWeight="bold">
              📈 التقارير التنفيذية وإصدارات المستندات
            </Typography>
            <Box sx={{ display: 'flex', gap: 1 }}>
              <Button
                variant="contained"
            ...
            <Paper sx={{ p: 3, mb: 3 }}>
                ...
                <Grid item xs={6} sm={3}>
                  <Card sx={{ textAlign: 'center', bgcolor: '#e3f2fd' }}>
                    <CardContent>
                      ...
                      <Typography variant="h4" fontWeight="bold" color="primary">
                        {executiveReport.totalLicenses || 0}
                      </Typography>
                    </CardContent>
                  </Card>
                </Grid>
                <Grid item xs={6} sm={3}>
                  <Card sx={{ textAlign: 'center', bgcolor: '#e8f5e9' }}>
                    <CardContent>
                      ...
                      <Typography variant="h4" fontWeight="bold" color="success.main">
                        {executiveReport.complianceRate || 0}%
                      </Typography>
                    </CardContent>
                  </Card>
                </Grid>
                <Grid item xs={6} sm={3}>
                  <Card sx={{ textAlign: 'center', bgcolor: '#fff3e0' }}>
                    <CardContent>
                      ...
                      <Typography variant="h4" fontWeight="bold" color="warning.main">
                        {executiveReport.expiringSoon || 0}
                      </Typography>
                    </CardContent>
                  </Card>
                </Grid>
                  ...
                  <Grid container spacing={1} sx={{ mb: 2 }}>
                    {Object.entries(executiveReport.statusBreakdown).map(([status, count]) => (
                      <Grid item xs={6} sm={4} md={2} key={status}>
                        <Chip
                          label={`${STATUS_CONFIG[status]?.label || status}: ${count}`}
                          size="small"
                          sx={{
                            width: '100%',
                            bgcolor:
                              STATUS_CONFIG[status]?.color === 'success' ? '#e8f5e9' : '#f5f5f5',
                          }}
                        />
                      </Grid>
                    ))}
              ...
              {executiveReport.concerns?.length > 0 && (
                <Alert severity="warning" sx={{ mb: 1 }}>
                  ...
                  {executiveReport.concerns.map((c, i) => (
                    <Typography variant="body2" key={i}>
                      ⚠️ {c}
                    </Typography>
                  ))}
              ...
              <Grid container spacing={2} sx={{ mb: 3 }}>
                <Grid item xs={12} sm={4}>
                  <Card sx={{ textAlign: 'center', borderTop: '3px solid #1976d2' }}>
                    <CardContent>
                      <Typography variant="caption" color="text.secondary">
                        التكلفة المتوقعة (شهرياً)
                      </Typography>
                      <Typography variant="h5" fontWeight="bold" color="primary">
                        {(predictiveAnalytics.monthlyCostForecast || 0).toLocaleString()}{' '}
                        <Typography component="span" variant="caption">
                          ر.س
                        </Typography>
                      </Typography>
                ...
                <Grid item xs={12} sm={4}>
                  <Card sx={{ textAlign: 'center', borderTop: '3px solid #4caf50' }}>
                    <CardContent>
                      <Typography variant="caption" color="text.secondary">
                        التكلفة المتوقعة (ربع سنوي)
                      </Typography>
                      <Typography variant="h5" fontWeight="bold" color="success.main">
                        {(predictiveAnalytics.quarterlyCostForecast || 0).toLocaleString()}{' '}
                        <Typography component="span" variant="caption">
                          ر.س
                        </Typography>
                      </Typography>
                ...
                <Grid item xs={12} sm={4}>
                  <Card sx={{ textAlign: 'center', borderTop: '3px solid #9c27b0' }}>
                    <CardContent>
                      <Typography variant="caption" color="text.secondary">
                        التكلفة المتوقعة (سنوياً)
                      </Typography>
                      <Typography variant="h5" fontWeight="bold" sx={{ color: '#9c27b0' }}>
                        {(predictiveAnalytics.annualCostForecast || 0).toLocaleString()}{' '}
                        <Typography component="span" variant="caption">
                          ر.س
                        </Typography>
                      </Typography>
                ...
                <Grid item xs={6} sm={3}>
                  <Card sx={{ textAlign: 'center', bgcolor: '#f3e5f5' }}>
                    <CardContent>
                      ...
                      <Typography variant="h4" fontWeight="bold" color="error">
                        {predictiveAnalytics.highExpiryRisk || 0}
                      </Typography>
                    </CardContent>
                  </Card>
                </Grid>
                <Grid item xs={6} sm={3}>
                  <Card sx={{ textAlign: 'center', bgcolor: '#e8eaf6' }}>
                    <CardContent>
                      ...
                      <Typography variant="h4" fontWeight="bold" color="warning.main">
                        {predictiveAnalytics.mediumExpiryRisk || 0}
                      </Typography>
                    </CardContent>
                  </Card>
                </Grid>
                <Grid item xs={6} sm={3}>
                  <Card sx={{ textAlign: 'center', bgcolor: '#e0f2f1' }}>
                    <CardContent>
                      ...
                      <Typography variant="h4" fontWeight="bold" color="success.main">
                        {predictiveAnalytics.complianceImproving || 0}
                      </Typography>
                    </CardContent>
                  </Card>
                </Grid>
                <Grid item xs={6} sm={3}>
                  <Card sx={{ textAlign: 'center', bgcolor: '#fce4ec' }}>
                    <CardContent>
                      ...
                      <Typography variant="h4" fontWeight="bold" color="error">
                        {predictiveAnalytics.complianceDeclining || 0}
                      </Typography>
                    </CardContent>
                  </Card>
                </Grid>
      ...
      {activeTab === 16 && (
        <Box>
          <Box sx={{ display: 'flex', justifyContent: 'space-between', mb: 2 }}>
            <Typography variant="h6" fontWeight="bold">
              📊 تقييم رضا المتعاملين والاجتماعات
            </Typography>
            <Button
              variant="outlined"
              startIcon={<RefreshIcon />}
              ...
              <Grid item xs={6} sm={3}>
                <Card sx={{ textAlign: 'center', borderTop: '3px solid #1976d2' }}>
                  <CardContent>
                    ...
                    <Typography variant="h4" fontWeight="bold" color="primary">
                      {globalSatisfaction.totalSurveys}
                    </Typography>
                  </CardContent>
                </Card>
              </Grid>
              <Grid item xs={6} sm={3}>
                <Card sx={{ textAlign: 'center', borderTop: '3px solid #4caf50' }}>
                  <CardContent>
                    ...
                    <Typography variant="h4" fontWeight="bold" color="success.main">
                      {globalSatisfaction.averageOverall || 0}/5
                    </Typography>
                  </CardContent>
                </Card>
              </Grid>
              <Grid item xs={6} sm={3}>
                <Card sx={{ textAlign: 'center', borderTop: '3px solid #ff9800' }}>
                  <CardContent>
                    ...
                    <Typography variant="h4" fontWeight="bold" color="warning.main">
                      {globalSatisfaction.averageService || 0}/5
                    </Typography>
                  </CardContent>
                </Card>
              </Grid>
              <Grid item xs={6} sm={3}>
                <Card sx={{ textAlign: 'center', borderTop: '3px solid #9c27b0' }}>
                  <CardContent>
                    ...
                    <Typography variant="h4" fontWeight="bold" sx={{ color: '#9c27b0' }}>
                      {globalSatisfaction.averageResponse || 0}/5
                    </Typography>
                  </CardContent>
                </Card>
              </Grid>
                ...
                <TableBody>
                  {globalMeetings.slice(0, 20).map((m, idx) => (
                    <TableRow key={idx} hover>
                      ...
                      <TableCell>
                        <Chip
                          label={`${(m.decisions || []).length} قرارات`}
                          size="small"
                          color={
                            (m.decisions || []).some(d => d.status === 'pending')
                              ? 'warning'
                              : 'success'
                          }
                        />
      ...
      {activeTab === 17 && (
        <Box>
          <Box sx={{ display: 'flex', justifyContent: 'space-between', mb: 2 }}>
            ...
            <Button variant="outlined" startIcon={<RefreshIcon />} onClick={fetchGlobalTraining}>
              تحديث
            </Button>
          </Box>
                  ...
                  <Card sx={{ textAlign: 'center', borderTop: '3px solid #4caf50' }}>
                    <CardContent>
                      ...
                      <Typography variant="h4" fontWeight="bold" color="success.main">
                        {globalTraining.length
                          ? (
                              globalTraining.reduce((s, t) => s + t.complianceRate, 0) /
                              globalTraining.length
                            ).toFixed(0)
                          : 0}
                        %
                ...
                <Grid item xs={6} sm={3}>
                  <Card sx={{ textAlign: 'center', borderTop: '3px solid #ff9800' }}>
                    <CardContent>
                      ...
                      <Typography variant="h4" fontWeight="bold" color="warning.main">
                        {globalTraining.reduce((s, t) => s + t.completed, 0)}
                      </Typography>
                    </CardContent>
                  </Card>
                </Grid>
                <Grid item xs={6} sm={3}>
                  <Card sx={{ textAlign: 'center', borderTop: '3px solid #f44336' }}>
                    <CardContent>
                      ...
                      <Typography variant="h4" fontWeight="bold" color="error">
                        {globalTraining.reduce((s, t) => s + t.total, 0)}
                      </Typography>
                    </CardContent>
                  </Card>
                </Grid>
        ...
        <Box>
          <Box sx={{ display: 'flex', justifyContent: 'space-between', mb: 2 }}>
            ...
            <Button
              variant="outlined"
              startIcon={<RefreshIcon />}
              onClick={fetchGlobalVendorRatings}
            >
              تحديث
            </Button>
          </Box>
                  ...
                  <Card sx={{ textAlign: 'center', borderTop: '3px solid #4caf50' }}>
                    <CardContent>
                      ...
                      <Typography variant="h4" fontWeight="bold" color="success.main">
                        {globalVendorRatings.length
                          ? (
                              globalVendorRatings.reduce((s, v) => s + (v.avgRating || 0), 0) /
                              globalVendorRatings.length
                            ).toFixed(1)
                          : 0}
                      </Typography>
                ...
                <Grid item xs={6} sm={3}>
                  <Card sx={{ textAlign: 'center', borderTop: '3px solid #ff9800' }}>
                    <CardContent>
                      ...
                      <Typography variant="h4" fontWeight="bold" color="warning.main">
                        {globalVendorRatings.reduce((s, v) => s + v.contracts, 0)}
                      </Typography>
                    </CardContent>
                  </Card>
                </Grid>
                  ...
                  <TableBody>
                    {globalVendorRatings.map((v, idx) => (
                      <TableRow key={idx} hover>
                        ...
                        <TableCell>
                          <Chip label={v.type || 'عام'} size="small" />
                        </TableCell>
                        ...
                        <TableCell>
                          <Chip
                            ...
                            size="small"
                            color={
                              v.avgRating >= 4 ? 'success' : v.avgRating >= 3 ? 'warning' : 'error'
                            }
                          />
                        </TableCell>
                      </TableRow>
        ...
        <Box>
          <Box sx={{ display: 'flex', justifyContent: 'space-between', mb: 2 }}>
            ...
            <Box sx={{ display: 'flex', gap: 1 }}>
              <Button
                variant="outlined"
                startIcon={<RemediationIcon />}
                onClick={handleRunAutoRemediation}
              >
                مسح تلقائي
              </Button>
              <Button
          ...
          {!globalComplaints ? (
            ...
            <>
                ...
                <Grid item xs={6} sm={3}>
                  <Card sx={{ textAlign: 'center', borderTop: '3px solid #f44336' }}>
                    <CardContent>
                      ...
                      <Typography variant="h4" fontWeight="bold" color="error">
                        {globalComplaints.totalComplaints || 0}
                      </Typography>
                    </CardContent>
                  </Card>
                </Grid>
                <Grid item xs={6} sm={3}>
                  <Card sx={{ textAlign: 'center', borderTop: '3px solid #4caf50' }}>
                    <CardContent>
                      ...
                      <Typography variant="h4" fontWeight="bold" color="success.main">
                        {globalComplaints.totalSuggestions || 0}
                      </Typography>
                    </CardContent>
                  </Card>
                </Grid>
                <Grid item xs={6} sm={3}>
                  <Card sx={{ textAlign: 'center', borderTop: '3px solid #ff9800' }}>
                    <CardContent>
                      ...
                      <Typography variant="h4" fontWeight="bold" color="warning.main">
                        {globalComplaints.avgResolutionDays || 0}
                      </Typography>
                    </CardContent>
                  </Card>
                </Grid>
              ...
              {/* Status breakdown */}
              {globalComplaints.complaintsByStatus &&
                Object.keys(globalComplaints.complaintsByStatus).length > 0 && (
                  <Paper sx={{ p: 2, mb: 3 }}>
                    <Typography variant="subtitle2" fontWeight="bold" sx={{ mb: 1 }}>
                      حسب الحالة
                    </Typography>
                    <Box sx={{ display: 'flex', gap: 1, flexWrap: 'wrap' }}>
                      {Object.entries(globalComplaints.complaintsByStatus).map(
                        ([status, count]) => (
                          <Chip
                            key={status}
                            label={`${status}: ${count}`}
              ...
              {/* Type breakdown */}
              {globalComplaints.complaintsByType &&
                Object.keys(globalComplaints.complaintsByType).length > 0 && (
                  <Paper sx={{ p: 2 }}>
                    <Typography variant="subtitle2" fontWeight="bold" sx={{ mb: 1 }}>
                      حسب النوع
                    </Typography>
      ...
      <Dialog open={createDialog} onClose={() => setCreateDialog(false)} maxWidth="md" fullWidth>
        <DialogTitle sx={{ bgcolor: '#1565c0', color: 'white' }}>إنشاء ترخيص جديد</DialogTitle>
          ...
          <Grid container spacing={2} sx={{ mt: 0.5 }}>
            ...
            <Grid item xs={12} md={6}>
              <TextField
                ...
                onChange={e =>
                  setFormData({ ...formData, center: { ...formData.center, name: e.target.value } })
                }
              />
            </Grid>
            <Grid item xs={12} md={6}>
              <TextField
                ...
                onChange={e =>
                  setFormData({ ...formData, center: { ...formData.center, city: e.target.value } })
                }
              />
            </Grid>
            <Grid item xs={12} md={6}>
              <TextField
                ...
                onChange={e =>
                  setFormData({
                    ...formData,
                    center: { ...formData.center, crNumber: e.target.value },
                  })
                }
              />
            </Grid>
            <Grid item xs={12} md={6}>
              <TextField
                ...
                onChange={e =>
                  setFormData({
                    ...formData,
                    center: { ...formData.center, unifiedNumber: e.target.value },
                  })
                }
              />
            </Grid>
            ...
            <Grid item xs={12} md={6}>
              <TextField
                ...
                onChange={e =>
                  setFormData({
                    ...formData,
                    issuingAuthority: { ...formData.issuingAuthority, name: e.target.value },
                  })
                }
              />
            </Grid>
            <Grid item xs={12} md={6}>
              <TextField
                ...
                onChange={e =>
                  setFormData({ ...formData, dates: { ...formData.dates, issued: e.target.value } })
                }
              />
            </Grid>
            <Grid item xs={12} md={6}>
              <TextField
                ...
                onChange={e =>
                  setFormData({ ...formData, dates: { ...formData.dates, expiry: e.target.value } })
                }
              />
            </Grid>
            <Grid item xs={12} md={4}>
              <TextField
                ...
                onChange={e =>
                  setFormData({
                    ...formData,
                    costs: { ...formData.costs, issueFee: Number(e.target.value) },
                  })
                }
              />
            </Grid>
            <Grid item xs={12} md={4}>
              <TextField
                ...
                onChange={e =>
                  setFormData({
                    ...formData,
                    costs: { ...formData.costs, renewalFee: Number(e.target.value) },
                  })
                }
              />
            </Grid>
      ...
      <Dialog open={!!viewDialog} onClose={() => setViewDialog(null)} maxWidth="md" fullWidth>
        ...
        <DialogContent sx={{ mt: 2 }}>
          {viewDialog && (
            <Grid container spacing={2}>
              ...
              <Grid item xs={6}>
                <Typography variant="body2">
                  <strong>الاسم:</strong> {viewDialog.center?.name || '-'}
                </Typography>
              </Grid>
              <Grid item xs={6}>
                <Typography variant="body2">
                  <strong>المدينة:</strong> {viewDialog.center?.city || '-'}
                </Typography>
              </Grid>
              <Grid item xs={6}>
                <Typography variant="body2">
                  <strong>السجل التجاري:</strong> {viewDialog.center?.crNumber || '-'}
                </Typography>
              </Grid>
              <Grid item xs={6}>
                <Typography variant="body2">
                  <strong>الرقم الموحد:</strong> {viewDialog.center?.unifiedNumber || '-'}
                </Typography>
              </Grid>
              ...
              <Grid item xs={6}>
                <Typography variant="body2">
                  <strong>النوع:</strong> {viewDialog.licenseTypeLabel || viewDialog.licenseType}
                </Typography>
              </Grid>
              <Grid item xs={6}>
                <Typography variant="body2">
                  <strong>الفئة:</strong>{' '}
                  {CATEGORY_CONFIG[viewDialog.category]?.label || viewDialog.category}
                </Typography>
              </Grid>
              <Grid item xs={6}>
                <Typography variant="body2">
                  <strong>الجهة المصدرة:</strong> {viewDialog.issuingAuthority?.name || '-'}
                </Typography>
              </Grid>
              <Grid item xs={6}>
                <Typography variant="body2">
                  ...
                  <Chip
                    label={STATUS_CONFIG[viewDialog.status]?.label || viewDialog.status}
                    color={STATUS_CONFIG[viewDialog.status]?.color}
                    size="small"
                  />
                </Typography>
              </Grid>
              ...
              <Grid item xs={6}>
                <Typography variant="body2">
                  ...
                  <span
                    style={{ color: getDaysColor(viewDialog.daysUntilExpiry), fontWeight: 'bold' }}
                  >
              ...
              <Grid item xs={6}>
                <Typography variant="body2">
                  ...
                  {PRIORITY_CONFIG[viewDialog.priority]?.label || viewDialog.priority}
                </Typography>
              </Grid>
              ...
              {viewDialog.costs && (
                <>
                  ...
                  <Grid item xs={4}>
                    <Typography variant="body2">
                      <strong>رسوم الإصدار:</strong> {viewDialog.costs.issueFee || 0} ر.س
                    </Typography>
                  </Grid>
                  <Grid item xs={4}>
                    <Typography variant="body2">
                      <strong>رسوم التجديد:</strong> {viewDialog.costs.renewalFee || 0} ر.س
                    </Typography>
                  </Grid>
                  <Grid item xs={4}>
                    <Typography variant="body2">
                      <strong>إجمالي المدفوع:</strong> {viewDialog.costs.totalPaid || 0} ر.س
                    </Typography>
                  </Grid>
                </>
              )}
              ...
              {viewDialog.compliance && (
                <>
                  ...
                  <Grid item xs={6}>
                    <Typography variant="body2">
                      <strong>حالة الامتثال:</strong> {viewDialog.compliance.status || '-'}
                    </Typography>
                  </Grid>
                  <Grid item xs={6}>
                    <Typography variant="body2">
                      <strong>آخر تفتيش:</strong>{' '}
                      {viewDialog.compliance.lastInspectionDate
                        ? new Date(viewDialog.compliance.lastInspectionDate).toLocaleDateString(
                            'ar-SA'
                          )
                        : '-'}
                    </Typography>
              ...
              {viewDialog.delegation?.hasDelegation && (
                <>
                  ...
                  <Grid item xs={6}>
                    <Typography variant="body2">
                      <strong>المفوض:</strong> {viewDialog.delegation.delegateName || '-'}
                    </Typography>
                  </Grid>
                  <Grid item xs={6}>
                    <Typography variant="body2">
                      <strong>نوع التفويض:</strong> {viewDialog.delegation.delegationType || '-'}
                    </Typography>
                  </Grid>
                  <Grid item xs={6}>
                    <Typography variant="body2">
                      <strong>الهاتف:</strong> {viewDialog.delegation.delegatePhone || '-'}
                    </Typography>
                  </Grid>
                  <Grid item xs={6}>
                    <Typography variant="body2">
                      <strong>الهوية:</strong> {viewDialog.delegation.delegateId || '-'}
                    </Typography>
                  </Grid>
                </>
              )}
                ...
                <>
                  ...
                  <Grid item xs={6}>
                    <Box sx={{ display: 'flex', alignItems: 'center', gap: 1 }}>
                      <LinearProgress
                        variant="determinate"
                        value={viewDialog.riskScore.score}
                        sx={{
                          width: 100,
                          height: 8,
                          borderRadius: 4,
                          '& .MuiLinearProgress-bar': {
                            bgcolor:
                              viewDialog.riskScore.score >= 75
                                ? '#f44336'
                                : viewDialog.riskScore.score >= 50
                                  ? '#ff9800'
                                  : '#4caf50',
                          },
                        }}
                  ...
                  <Grid item xs={6}>
                    <Chip
                      label={
                        viewDialog.riskScore.level === 'critical'
                          ? 'حرج'
                          : viewDialog.riskScore.level === 'high'
                            ? 'عالي'
                            : viewDialog.riskScore.level === 'medium'
                              ? 'متوسط'
                              : 'منخفض'
                      }
                      color={
                ...
                <>
                  ...
                  <Grid item xs={12}>
                    ...
                    {viewDialog.penalties.map((p, i) => (
                      <Paper
                        key={i}
                        sx={{ p: 1, mb: 1, bgcolor: '#fff8e1', borderRight: '3px solid #ed6c02' }}
                      >
                        <Box sx={{ display: 'flex', justifyContent: 'space-between' }}>
                          <Typography variant="body2">{p.reason || 'بدون سبب'}</Typography>
                          <Chip
                            size="small"
                            label={p.amount ? `${p.amount.toLocaleString()} ر.س` : p.type}
                            color="warning"
                          />
                        </Box>
                      </Paper>
                  ...
                  <Grid item xs={12}>
                    ...
                    {viewDialog.branches.map((b, i) => (
                      <Chip
                        key={i}
                        icon={<BranchIcon />}
                        label={`${b.branchName} - ${b.city || ''}`}
                        sx={{ m: 0.5, bgcolor: b.isMainBranch ? '#e8f5e9' : '#f5f5f5' }}
                        color={b.isMainBranch ? 'success' : 'default'}
                        variant="outlined"
                      />
                    ))}
                  </Grid>
                ...
                <>
                  ...
                  <Grid item xs={6}>
                    <Box sx={{ display: 'flex', alignItems: 'center', gap: 1 }}>
                      <LinearProgress
                        variant="determinate"
                        value={viewDialog.healthScore.score}
                        sx={{
                          width: 100,
                          height: 8,
                          borderRadius: 4,
                          '& .MuiLinearProgress-bar': {
                            bgcolor:
                              viewDialog.healthScore.score >= 80
                                ? '#4caf50'
                                : viewDialog.healthScore.score >= 60
                                  ? '#ff9800'
                                  : '#f44336',
                          },
                        }}
                  ...
                  <Grid item xs={6}>
                    <Chip
                      label={`التقدير: ${viewDialog.healthScore.grade || '-'}`}
                      color={
                        viewDialog.healthScore.grade === 'A'
                          ? 'success'
                          : viewDialog.healthScore.grade === 'B'
                            ? 'info'
                            : viewDialog.healthScore.grade === 'C'
                              ? 'warning'
                              : 'error'
                      }
                      size="small"
                      ...
                      .map((c, i) => (
                        <Paper
                          ...
                          sx={{
                            ...
                            mb: 1,
                            bgcolor: '#f5f5f5',
                            borderRight: c.isPinned ? '3px solid #ff9800' : 'none',
                          }}
                        >
              ...
              {viewDialog.budget?.allocatedAmount > 0 && (
                <>
                  ...
                  <Grid item xs={4}>
                    <Typography variant="body2">
                      <strong>المخصص:</strong>{' '}
                      {(viewDialog.budget.allocatedAmount || 0).toLocaleString()} ر.س
                    </Typography>
                  </Grid>
                  <Grid item xs={4}>
                    <Typography variant="body2">
                      <strong>المصروف:</strong>{' '}
                      {(viewDialog.budget.spentAmount || 0).toLocaleString()} ر.س
                    </Typography>
                  </Grid>
                  <Grid item xs={4}>
                    <Typography variant="body2">
                      ...
                      <span
                        style={{
                          color:
                            viewDialog.budget.allocatedAmount - viewDialog.budget.spentAmount < 0
                              ? '#f44336'
                              : '#4caf50',
                          fontWeight: 'bold',
                        }}
                      >
                        {(
                          (viewDialog.budget.allocatedAmount || 0) -
                          (viewDialog.budget.spentAmount || 0)
                        ).toLocaleString()}{' '}
                        ر.س
                      </span>
                    </Typography>
        ...
        <DialogActions sx={{ p: 2, gap: 1, flexWrap: 'wrap' }}>
          ...
          <Button
            ...
            onClick={() => {
              setRequirementDialog(viewDialog);
              setViewDialog(null);
              setFormData({});
            }}
          >
          ...
          <Button
            ...
            onClick={() => {
              setDelegationDialog(viewDialog);
              setViewDialog(null);
              setFormData({});
            }}
          >
          ...
          <Button
            variant="outlined"
            sx={{ color: '#455a64', borderColor: '#455a64' }}
            startIcon={<CommentIcon />}
            onClick={() => {
              setCommentDialog(viewDialog);
              setViewDialog(null);
              setFormData({});
            }}
          >
          ...
          <Button
            ...
            onClick={() => {
              setCommunicationDialog(viewDialog);
              setViewDialog(null);
              setFormData({});
            }}
          >
          ...
          <Button
            variant="outlined"
            sx={{ color: '#bf360c', borderColor: '#bf360c' }}
            startIcon={<ExpenseIcon />}
            onClick={() => {
              setBudgetDialog(viewDialog);
              setViewDialog(null);
              setFormData({});
            }}
          >
          ...
          <Grid container spacing={2} sx={{ mt: 0.5 }}>
            ...
            <Grid item xs={12}>
              <FormControl fullWidth size="small">
                ...
                <Select
                  value={formData.delegationType || 'full'}
                  label="نوع التفويض"
          ...
          <Grid container spacing={2} sx={{ mt: 0.5 }}>
            ...
            <Grid item xs={12} md={6}>
              <FormControl fullWidth size="small">
                ...
                <Select
                  value={formData.category || 'document'}
                  label="التصنيف"
                  onChange={e => setFormData({ ...formData, category: e.target.value })}
                >
          ...
          <Grid container spacing={2} sx={{ mt: 0.5 }}>
            ...
            <Grid item xs={12} md={6}>
              <FormControl fullWidth size="small">
                ...
                <Select
                  value={formData.isMainBranch || false}
                  label="الفرع الرئيسي؟"
          ...
          <Grid container spacing={2} sx={{ mt: 0.5 }}>
            <Grid item xs={12}>
              <Alert severity="info" sx={{ mb: 1 }}>
                أدخل اسم كل مراجع في سطر منفصل بالترتيب
              </Alert>
              <TextField
                fullWidth
                label="خطوات الموافقة *"
                size="small"
                multiline
                rows={5}
      ...
      <Dialog
        ...
        onClose={() => setCommunicationDialog(null)}
        maxWidth="sm"
        fullWidth
      >
        <DialogTitle sx={{ bgcolor: '#0288d1', color: 'white' }}>
      ...
      {/* ==================== Dialog: حدث تقويم ==================== */}
      <Dialog
        open={!!calendarEventDialog}
        onClose={() => setCalendarEventDialog(null)}
        maxWidth="sm"
        fullWidth
      >
      ...
      {/* ==================== Dialog: إضافة مستند ==================== */}
      <Dialog
        open={!!documentChecklistDialog}
        onClose={() => setDocumentChecklistDialog(null)}
        maxWidth="sm"
        fullWidth
      >
          ...
          <Grid container spacing={2} sx={{ mt: 0.5 }}>
            ...
            <Grid item xs={12} md={6}>
              <FormControl fullWidth size="small">
                ...
                <Select
                  value={formData.documentType || 'license_copy'}
                  label="نوع المستند"
          ...
          <Grid container spacing={2} sx={{ mt: 0.5 }}>
            ...
            <Grid item xs={12} md={6}>
              <FormControl fullWidth size="small">
                <InputLabel>الفئة</InputLabel>
                <Select
                  value={formData.category || 'renewal_fees'}
                  label="الفئة"
                  onChange={e => setFormData({ ...formData, category: e.target.value })}
                >
          ...
          <Grid container spacing={2} sx={{ mt: 0.5 }}>
            ...
            <Grid item xs={12}>
              <FormControl fullWidth size="small">
                ...
                <Select
                  value={formData.templateCategory || 'general'}
                  label="فئة القالب"
          ...
          <Grid container spacing={2} sx={{ mt: 0.5 }}>
            ...
            <Grid item xs={12} md={6}>
              <FormControl fullWidth size="small">
                ...
                <Select
                  value={formData.ticketType || 'inquiry'}
                  label="نوع التذكرة"
          ...
          <Grid container spacing={2} sx={{ mt: 0.5 }}>
            ...
            <Grid item xs={6}>
              <TextField
                ...
                onChange={e =>
                  setFormData({ ...formData, renewalTargetDays: Number(e.target.value) })
                }
              />
            </Grid>
            <Grid item xs={6}>
              <TextField
                ...
                onChange={e =>
                  setFormData({ ...formData, renewalWarningDays: Number(e.target.value) })
                }
              />
            </Grid>
            <Grid item xs={12}>
            ...
            <Grid item xs={6}>
              <TextField
                ...
                onChange={e =>
                  setFormData({ ...formData, responseTargetHours: Number(e.target.value) })
                }
              />
            </Grid>
            <Grid item xs={6}>
              <TextField
                ...
                onChange={e =>
                  setFormData({ ...formData, responseWarningHours: Number(e.target.value) })
                }
              />
            </Grid>
            <Grid item xs={12}>
            ...
            <Grid item xs={6}>
              <TextField
                ...
                onChange={e =>
                  setFormData({ ...formData, inspectionTargetDays: Number(e.target.value) })
                }
              />
            </Grid>
            <Grid item xs={6}>
              <TextField
                ...
                onChange={e =>
                  setFormData({ ...formData, inspectionWarningDays: Number(e.target.value) })
                }
              />
            </Grid>
          </Grid>
      ...
      <Dialog
        ...
        fullWidth
      >
        <DialogTitle sx={{ bgcolor: '#283593', color: 'white' }}>🔀 مقارنة التراخيص</DialogTitle>
        <DialogContent sx={{ mt: 2 }}>
          {!comparisonResult ? (
                ...
                <TableBody>
                  {comparisonResult.fields?.map((field, idx) => (
                    <TableRow
                      key={idx}
                      hover
                      sx={{ bgcolor: field.isDifferent ? '#fff3e0' : 'transparent' }}
                    >
      ...
      <Dialog
        ...
        onClose={() => setChangeLogDialog(null)}
        maxWidth="md"
        fullWidth
      >
        <DialogTitle sx={{ bgcolor: '#37474f', color: 'white' }}>📝 سجل التغييرات</DialogTitle>
        <DialogContent sx={{ mt: 2 }}>
      ...
      <Dialog
        open={!!docVersionDialog}
        onClose={() => setDocVersionDialog(null)}
        maxWidth="sm"
        fullWidth
      >
          ...
          <Grid container spacing={2} sx={{ mt: 0.5 }}>
            ...
            <Grid item xs={12}>
              <FormControl fullWidth size="small">
                ...
                <Select
                  value={formData.triggerEvent || 'expiry_approaching'}
                  label="حدث التفعيل"
            ...
            <Grid item xs={12}>
              <FormControl fullWidth size="small">
                ...
                <Select
                  value={formData.actionType || 'send_notification'}
                  label="نوع الإجراء"
      ...
      >
        <DialogTitle sx={{ bgcolor: '#1565c0', color: 'white' }}>
          إشعار مجدول: {notificationDialog?.licenseNumber}
        </DialogTitle>
        <DialogContent sx={{ mt: 2 }}>
          <Grid container spacing={2} sx={{ mt: 0.5 }}>
            <Grid item xs={12}>
            ...
            <Grid item xs={6}>
              <FormControl fullWidth size="small">
                ...
                <Select
                  value={formData.notificationType || 'reminder'}
                  label="نوع الإشعار"
            ...
            <Grid item xs={6}>
              <FormControl fullWidth size="small">
                ...
                <Select
                  value={formData.recurringPattern || 'none'}
                  label="التكرار"
      ...
      <Dialog open={!!surveyDialog} onClose={() => setSurveyDialog(null)} maxWidth="sm" fullWidth>
        <DialogTitle sx={{ bgcolor: '#4caf50', color: 'white' }}>
          تقييم رضا: {surveyDialog?.licenseNumber}
        </DialogTitle>
        <DialogContent sx={{ mt: 2 }}>
          <Grid container spacing={2} sx={{ mt: 0.5 }}>
            <Grid item xs={12}>
              <FormControl fullWidth size="small">
                ...
                <Select
                  value={formData.surveyType || 'general'}
                  label="نوع التقييم"
      ...
      >
        <DialogTitle sx={{ bgcolor: '#9c27b0', color: 'white' }}>
          توقيع رقمي: {signatureDialog?.licenseNumber}
        </DialogTitle>
        <DialogContent sx={{ mt: 2 }}>
          <Grid container spacing={2} sx={{ mt: 0.5 }}>
            <Grid item xs={12}>
            ...
            <Grid item xs={12}>
              <FormControl fullWidth size="small">
                ...
                <Select
                  value={formData.signatureType || 'approval'}
                  label="نوع التوقيع"
      ...
      <Dialog open={!!meetingDialog} onClose={() => setMeetingDialog(null)} maxWidth="sm" fullWidth>
        <DialogTitle sx={{ bgcolor: '#0288d1', color: 'white' }}>
          اجتماع جديد: {meetingDialog?.licenseNumber}
        </DialogTitle>
        <DialogContent sx={{ mt: 2 }}>
          <Grid container spacing={2} sx={{ mt: 0.5 }}>
            <Grid item xs={12}>
            ...
            <Grid item xs={6}>
              <FormControl fullWidth size="small">
                ...
                <Select
                  value={formData.meetingType || 'review'}
                  label="نوع الاجتماع"
      ...
      >
        <DialogTitle sx={{ bgcolor: '#ed6c02', color: 'white' }}>
          ربط خارجي: {integrationDialog?.licenseNumber}
        </DialogTitle>
        <DialogContent sx={{ mt: 2 }}>
          <Grid container spacing={2} sx={{ mt: 0.5 }}>
            <Grid item xs={12}>
            ...
            <Grid item xs={6}>
              <FormControl fullWidth size="small">
                ...
                <Select
                  value={formData.systemType || 'custom'}
                  label="نوع النظام"
      ...
      <Dialog
        open={!!trainingDialog}
        onClose={() => setTrainingDialog(null)}
        maxWidth="sm"
        fullWidth
      >
        <DialogTitle sx={{ bgcolor: '#2e7d32', color: 'white' }}>
          سجل تدريب: {trainingDialog?.licenseNumber}
        </DialogTitle>
        <DialogContent sx={{ mt: 2 }}>
          <Grid container spacing={2} sx={{ mt: 0.5 }}>
            <Grid item xs={12}>
            ...
            <Grid item xs={6}>
              <FormControl fullWidth size="small">
                ...
                <Select
                  value={formData.trainingType || 'mandatory'}
                  label="نوع التدريب"
      ...
      <Dialog open={!!vendorDialog} onClose={() => setVendorDialog(null)} maxWidth="sm" fullWidth>
        <DialogTitle sx={{ bgcolor: '#d32f2f', color: 'white' }}>
          مورد جديد: {vendorDialog?.licenseNumber}
        </DialogTitle>
        <DialogContent sx={{ mt: 2 }}>
          <Grid container spacing={2} sx={{ mt: 0.5 }}>
            <Grid item xs={12}>
            ...
            <Grid item xs={6}>
              <FormControl fullWidth size="small">
                ...
                <Select
                  value={formData.vendorType || 'other'}
                  label="نوع المورد"
      ...
      >
        <DialogTitle sx={{ bgcolor: '#7b1fa2', color: 'white' }}>
          شكوى/مقترح: {complaintDialog?.licenseNumber}
        </DialogTitle>
        <DialogContent sx={{ mt: 2 }}>
          <Grid container spacing={2} sx={{ mt: 0.5 }}>
            <Grid item xs={12}>
            ...
            <Grid item xs={6}>
              <FormControl fullWidth size="small">
                ...
                <Select
                  value={formData.complaintType || 'other'}
                  label="النوع"
        ...
        <DialogActions sx={{ p: 2 }}>
          ...
          <Button
            variant="outlined"
            onClick={() => {
              setFormData({ ...formData, isSuggestion: true });
              handleAddComplaint();
            }}
            startIcon={<SurveyIcon />}
          >
