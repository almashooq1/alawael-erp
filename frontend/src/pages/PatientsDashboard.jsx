    ...
    >
      ...
      {count !== undefined && (
        <Box
          component="span"
          sx={{
            background: active ? 'rgba(255,255,255,.25)' : alpha('#06b6d4', 0.15),
            borderRadius: 10,
            px: 0.8,
            py: 0.1,
            fontSize: 11,
...
const PatientCard = memo(
  ({ name, id, age, condition, status, progress, branch, lastVisit, delay = 0 }) => {
        ...
        <Glass
          sx={{
            ...
            '&:hover': {
              ...
              boxShadow: `0 12px 32px ${alpha(c, 0.15)}`,
            },
          }}
        >
          <Box
          ...
          <Box sx={{ p: 1.5, background: alpha(c, 0.07), borderRadius: 2, mb: 1.5 }}>
          ...
          <Box sx={{ mb: 1.5 }}>
            ...
            <LinearProgress
              ...
              sx={{
                height: 6,
                borderRadius: 3,
                '& .MuiLinearProgress-bar': { background: `linear-gradient(90deg,${c},${c}99)` },
              }}
            />
          </Box>
...
const PatientRow = memo(({ name, id, age, condition, status, progress, branch }) => {
  ...
  return (
    ...
    >
      ...
      <TableCell>
        <Typography variant="body2">{age} سنة</Typography>
      </TableCell>
      <TableCell>
    ...
    >
      ...
      <Box sx={{ textAlign: 'left' }}>
        ...
        <Chip
          size="small"
          label={status}
          sx={{ background: alpha(c, 0.15), color: c, fontWeight: 700, fontSize: 10, mt: 0.3 }}
        />
      </Box>
    </Box>
  );
...
const DEMO = {
  ...
  patients: [
    {
      ...
      id: 'P-10421',
      age: 45,
      condition: 'إصابة في الركبة - تأهيل ما بعد الجراحة',
      status: 'نشط',
      progress: 72,
      branch: 'الرياض',
      lastVisit: 'اليوم',
    ...
    },
    {
      ...
      id: 'P-10419',
      age: 58,
      condition: 'السكتة الدماغية - تأهيل حركي',
      status: 'طارئ',
      progress: 30,
      branch: 'الرياض',
      lastVisit: 'اليوم',
    ...
    },
    {
      ...
      id: 'P-10414',
      age: 54,
      condition: 'إصابة في العمود الفقري',
      status: 'معلق',
      progress: 35,
      branch: 'المدينة',
  ...
  alerts: [
    ...
    {
      type: 'warning',
      msg: 'المريضة هيا الدوسري لم تحضر جلستين متتاليتين',
      patient: 'P-10414',
      time: 'منذ ساعة',
    },
    {
      type: 'warning',
      msg: 'تجاوز 4 مرضى مدة العلاج المخططة',
      patient: 'أقسام متعددة',
      time: 'اليوم',
    },
    {
      type: 'success',
      msg: 'اكتمال علاج المريضة نورا السبيعي بنجاح',
      patient: 'P-10418',
      time: 'أمس',
    },
    {
      type: 'info',
      msg: 'مراجعة شهرية لـ 24 مريضاً مجدولة الأسبوع القادم',
      patient: 'متعدد',
      time: 'تذكير',
    },
  ],
  ...
  aiInsights: [
    {
      icon: '🧬',
      text: 'المرضى الذين يلتزمون بجدول الجلسات يحققون تعافياً أسرع بنسبة 34% مقارنة بمن يتغيبون.',
    },
    { icon: '⚠️', text: 'ارتفاع ملحوظ في إصابات الركبة (+22%) — يُنصح بمراجعة بروتوكول الوقاية.' },
    {
      icon: '📊',
      text: 'متوسط مدة التأهيل للإصابات العضلية انخفض من 21 إلى 17 يوماً بفضل البروتوكول الجديد.',
    },
    {
      icon: '💡',
      text: 'إضافة 3 أخصائيين في تأهيل الجهاز العصبي سيقلل قائمة الانتظار الحالية بنسبة 40%.',
    },
  ],
};
...
export default function PatientsDashboard() {
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      const [pRes, sRes] = await Promise.allSettled([
        apiClient.get('/beneficiary-portal?limit=8&sort=-createdAt'),
        apiClient.get('/sessions?limit=6&date=' + new Date().toISOString().slice(0, 10)),
      ]);
      const rawPatients =
        pRes.status === 'fulfilled'
          ? pRes.value.data?.data || pRes.value.data?.beneficiaries || pRes.value.data || []
          : [];
      const rawSessions =
        sRes.status === 'fulfilled'
          ? sRes.value.data?.data || sRes.value.data?.sessions || sRes.value.data || []
          : [];
      const STATUS_MAP = { active: 'نشط', completed: 'مكتمل', pending: 'معلق', emergency: 'طارئ' };
      ...
      const patients =
        Array.isArray(rawPatients) && rawPatients.length
          ? rawPatients.slice(0, 8).map(p => ({
              name: p.fullName || p.name || 'مستفيد',
              id: p.beneficiaryCode || `P-${String(p._id).slice(-5)}`,
              age:
                p.age != null
                  ? p.age
                  : p.dateOfBirth
                    ? Math.floor((Date.now() - new Date(p.dateOfBirth)) / 3.15576e10)
                    : 0,
              condition: p.primaryDiagnosis || p.disabilityType || p.condition || '',
          ...
          : D.patients;
      const appointments =
        Array.isArray(rawSessions) && rawSessions.length
          ? rawSessions.slice(0, 6).map(s => ({
              patient: s.beneficiaryName || s.patientName || 'مستفيد',
              doctor: s.therapistName || 'د. أخصائي',
              time: s.scheduledTime || s.startTime || '00:00',
              type: s.sessionType || 'جلسة تأهيل',
              status: SESSION_STATUS_MAP[s.status] || 'مؤكد',
            }))
          : D.appointments;
      setD(prev => ({ ...prev, patients, appointments }));
  ...
  const filteredPatients = D.patients.filter(
    p =>
      !searchQ ||
      p.name.includes(searchQ) ||
      p.id.includes(searchQ) ||
      p.condition.includes(searchQ)
  );
      ...
      >
        <Glass
          sx={{
            ...
            position: 'relative',
            background: isDark
              ? 'linear-gradient(135deg,rgba(6,182,212,.2),rgba(8,145,178,.14))'
              : 'linear-gradient(135deg,rgba(6,182,212,.11),rgba(8,145,178,.08))',
          }}
        >
          <Box
            sx={{
          ...
          >
            <Box sx={{ display: 'flex', alignItems: 'center', gap: 2 }}>
              ...
              <Box>
                ...
                <Typography variant="body2" color="text.secondary">
                  تتبع ذكي وشامل لكل مرضى المنظمة · آخر تحديث:{' '}
            ...
            <Box sx={{ display: 'flex', gap: 1 }}>
              <motion.div whileHover={{ scale: 1.05 }} whileTap={{ scale: 0.95 }}>
                <Button
                  ...
                  sx={{
                    ...
                    borderRadius: 2,
                    fontWeight: 700,
                    boxShadow: '0 4px 14px rgba(6,182,212,.4)',
                    fontSize: 13,
                  }}
                >
                  مريض جديد
              ...
              <Badge
                badgeContent={D.alerts.filter(a => a.type === 'critical').length}
                color="error"
              >
                <IconButton size="small" sx={{ background: alpha(theme.palette.error.main, 0.1) }}>
              ...
              <Tooltip title="تحديث">
                <IconButton
                  onClick={refresh}
                  size="small"
                  sx={{ background: alpha('#06b6d4', 0.1) }}
                >
          ...
          <Box sx={{ display: 'flex', gap: 3, mt: 2.5, flexWrap: 'wrap' }}>
            {[
              ...
              <Box key={i} sx={{ display: 'flex', alignItems: 'center', gap: 1 }}>
                ...
                <Typography variant="caption" fontWeight={700} sx={{ color: '#06b6d4' }}>
                  {s.value}
                </Typography>
              </Box>
            ))}
              ...
              <Grid item xs={12} md={7}>
                <Glass sx={{ p: 3 }}>
                  <Box sx={{ display: 'flex', alignItems: 'center', gap: 1, mb: 2 }}>
                    ...
                    <Chip
                      size="small"
                      label={`${D.appointments.length} موعد`}
                      sx={{ background: alpha('#06b6d4', 0.12), color: '#06b6d4', fontWeight: 700 }}
                    />
                  </Box>
                  {D.appointments.slice(0, 4).map((a, i) => (
                    <motion.div
                      key={i}
              ...
              >
                ...
                <Box sx={{ display: 'flex', gap: 1 }}>
                  {['cards', 'table'].map(m => (
                    <Chip
                      key={m}
                      label={m === 'cards' ? 'بطاقات' : 'جدول'}
                      onClick={() => setViewMode(m)}
                      sx={{
                        background: viewMode === m ? alpha('#06b6d4', 0.2) : 'transparent',
                        color: viewMode === m ? '#06b6d4' : 'text.secondary',
              ...
              <Grid item xs={12} md={4}>
                <Glass sx={{ p: 3 }}>
                  ...
                  {[
                    {
                      label: 'مؤكدة',
                      count: D.appointments.filter(a => a.status === 'مؤكد').length,
                      color: '#22c55e',
                    },
                    {
                      label: 'معلقة',
                      count: D.appointments.filter(a => a.status === 'معلق').length,
                      color: '#f59e0b',
                    },
                    {
                      label: 'مكتملة',
                      count: D.appointments.filter(a => a.status === 'مكتمل').length,
                      color: '#6366f1',
                    },
                    {
                      label: 'ملغاة',
                      count: D.appointments.filter(a => a.status === 'ملغى').length,
                      color: '#ef4444',
                    },
                  ].map((s, i) => (
                    <Box
                  ...
                  <Divider sx={{ my: 2 }} />
                  <Box sx={{ p: 2, background: alpha('#06b6d4', 0.08), borderRadius: 2 }}>
                    <Typography variant="caption" fontWeight={600} sx={{ color: '#06b6d4' }}>
                      الأسبوع القادم
                    </Typography>
              ...
              <Grid item xs={12}>
                <Glass
                  sx={{
                    p: 3,
                    background: isDark
                      ? 'linear-gradient(135deg,rgba(6,182,212,.18),rgba(8,145,178,.12))'
                      : 'linear-gradient(135deg,rgba(6,182,212,.1),rgba(8,145,178,.07))',
                  }}
                >
                  ...
                  <Grid container spacing={2}>
                    {[
                      {
                        label: 'معدل التعافي الشهري',
                        values: [78, 82, 80, 84, 86, 85, 87],
                        color: '#22c55e',
                      },
                      {
                        label: 'جلسات يومية',
                        values: [120, 135, 128, 142, 138, 145, 148],
                        color: '#06b6d4',
                      },
                      {
                        label: 'مرضى جدد يومياً',
                        values: [8, 11, 9, 13, 10, 12, 14],
                        color: '#6366f1',
                      },
                    ].map((chart, ci) => (
                      <Grid item xs={12} md={4} key={ci}>
                        ...
                        >
                          ...
                          <Box
                            sx={{ display: 'flex', alignItems: 'flex-end', gap: 0.7, height: 60 }}
                          >
          ...
          {tab === 5 && (
            <Grid container spacing={3}>
              <Grid item xs={12}>
                ...
                >
                  <Box sx={{ display: 'flex', alignItems: 'center', gap: 2, mb: 3 }}>
                    ...
                    <Box>
                      <Typography variant="h6" fontWeight={700}>
                        تحليل ذكاء اصطناعي — رعاية المرضى
                      </Typography>
                      <Typography variant="caption" color="text.secondary">
                        توصيات مبنية على بيانات الرعاية الصحية
                      </Typography>
                    </Box>
                    <Chip
                      label="AI-Powered"
                  ...
                  {[
                    ...
                    >
                      ...
                      >
                        ...
                        <Typography variant="body1" fontWeight={800} sx={{ color: f.color }}>
                          {f.value}
                        </Typography>
                      </Box>
                    </motion.div>
              ...
              <Grid item xs={12} md={6}>
                <Glass sx={{ p: 3 }}>
                  <Typography variant="h6" fontWeight={700} mb={2}>
                    المرضى في خطر تأخير التعافي
                  </Typography>
                  <Typography variant="caption" color="text.secondary" display="block" mb={2}>
                    بناءً على نمط الحضور والاستجابة للعلاج
                  </Typography>
                  {D.patients
                      ...
                      <Box key={i} sx={{ mb: 2 }}>
                        <Box sx={{ display: 'flex', justifyContent: 'space-between', mb: 0.5 }}>
                          <Box sx={{ display: 'flex', alignItems: 'center', gap: 1 }}>
                            <Avatar
                              sx={{
                                ...
                                height: 28,
                                fontSize: 12,
                                background: p.progress < 30 ? '#ef4444' : '#f59e0b',
                              }}
                            >
                              {p.name.charAt(0)}
                            </Avatar>
                          ...
                          <Typography
                            variant="body2"
                            fontWeight={700}
                            sx={{ color: p.progress < 30 ? '#ef4444' : '#f59e0b' }}
                          >
                            {p.progress}%
                          </Typography>
                        </Box>
          ...
          <Grid container spacing={2} mt={0.5}>
            ...
            <Grid item xs={12} sm={6}>
              <TextField
                ...
                sx={{ '& .MuiOutlinedInput-root': { borderRadius: 2 } }}
              >
                {['الرياض', 'جدة', 'الدمام', 'مكة', 'المدينة'].map(b => (
            ...
            <Grid item xs={12} sm={6}>
              <TextField
                ...
                sx={{ '& .MuiOutlinedInput-root': { borderRadius: 2 } }}
              >
                {['إصابة عضلية', 'إصابة عظام', 'ما بعد الجراحة', 'أمراض مزمنة', 'تأهيل عصبي'].map(
                  t => (
