...
export const OrgHeader = ({ title, subtitle, color }) => (
  <Box
    sx={{
      ...headerStyle,
      ...(color ? { background: `linear-gradient(135deg, ${color}, ${color}cc)` } : {}),
    }}
  >
    <Box>
...
export const Section = ({ title, color }) => (
  <Typography sx={{ ...sectionTitle, ...(color ? { borderRight: `4px solid ${color}` } : {}) }}>
    {title}
  </Typography>
);
  ...
  <Box sx={{ border: '1px dotted #999', borderRadius: 1, p: 2, minHeight }}>
    {content ? (
      ...
      <Typography variant="body2" sx={{ lineHeight: 2.5 }}>
        {Array(lines)
          .fill(0)
          .map((_, i) => `${i + 1}. ________________________________________`)
          .join('\n')}
      </Typography>
    ) : null}
  </Box>
      ...
      {(!rows || rows.length === 0) && (
        <tr>
          <td
            ...
            style={{ ...tdStyle, textAlign: 'center', color: '#999', padding: 12 }}
          >
            لا يوجد بيانات
          </td>
        </tr>
...
export const EmptyTable = ({ headers, rowCount = 5, headerBg }) => (
  <PrintTable
    ...
    rows={Array(rowCount)
      .fill(0)
      .map((_, i) => headers.map((_, ci) => (ci === 0 ? String(i + 1) : '')))}
  />
);
...
export const RefDateLine = ({ prefix = 'REF', refNum }) => (
  <Typography variant="caption" color="text.secondary" sx={{ mb: 2, display: 'block' }}>
    رقم المرجع: {prefix}-{new Date().getFullYear()}-{refNum || '________'} &nbsp;&nbsp; التاريخ:{' '}
    {today()}
  </Typography>
);
