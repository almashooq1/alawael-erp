...
export const ADMIN_TEMPLATES = [
  {
    ...
    desc: 'قرار إداري رسمي صادر من الإدارة',
    color: '#1a237e',
  },
  {
    id: 'outgoing-letter',
  ...
  },
  {
    ...
    desc: 'مذكرة تواصل داخلي بين الأقسام',
    color: '#00695c',
  },
  {
    id: 'meeting-minutes',
    ...
    desc: 'محضر اجتماع رسمي مع القرارات والتوصيات',
    color: '#37474f',
  },
  {
    id: 'delegation',
    ...
    desc: 'نموذج تفويض صلاحيات إدارية',
    color: '#6a1b9a',
  },
  {
    id: 'circular',
    name: 'تعميم',
    nameEn: 'Circular / Announcement',
    desc: 'تعميم رسمي لجميع الموظفين أو أقسام محددة',
    color: '#e65100',
  },
  {
  ...
  },
  {
    ...
    desc: 'نموذج تأكيد موعد مع المستفيد أو الزائر',
    color: '#2e7d32',
  },
];
...
export const AdminTemplateRenderer = ({ templateId, data }) => {
  ...
  switch (templateId) {
    case 'admin-decision':
      return <AdminDecision d={d} />;
    case 'outgoing-letter':
      return <OutgoingLetter d={d} />;
    case 'internal-memo':
      return <InternalMemo d={d} />;
    case 'meeting-minutes':
      return <MeetingMinutes d={d} />;
    case 'delegation':
      return <Delegation d={d} />;
    case 'circular':
      return <Circular d={d} />;
    case 'complaint-receipt':
      return <ComplaintReceipt d={d} />;
    case 'appointment-confirm':
      return <AppointmentConfirm d={d} />;
    default:
      return (
...
const AdminDecision = ({ d }) => (
  <Box sx={{ ...pageWrapper, border: '2px solid #1a237e' }}>
    ...
    <Box sx={bodyPad}>
      ...
      <Box
        sx={{ bgcolor: '#e8eaf6', p: 3, borderRadius: 2, mb: 2, borderRight: '4px solid #1a237e' }}
      >
        <Typography variant="body1" sx={{ lineHeight: 2.5, minHeight: 100 }}>
          {d.decisionText || ''}
        </Typography>
      </Box>
      ...
      <Typography variant="body1" mt={2}>
        وعلى الجهات المعنية تنفيذ ما جاء في هذا القرار كل فيما يخصه.
      </Typography>
      <Typography variant="body1" color="text.secondary" fontStyle="italic" mt={1}>
        والله الموفق
      </Typography>
  ...
  <Box sx={{ ...pageWrapper, borderTop: '4px solid #00695c' }}>
    ...
    <Box sx={bodyPad}>
      ...
      <Box sx={{ bgcolor: '#f5f5f5', p: 2, borderRadius: 2, mb: 2 }}>
        <Box sx={fieldRow}>
          <Field label="من" value={d.from} flex={2} />
          <Field label="التاريخ" value={formatDate(d.date)} />
        </Box>
        <Box sx={fieldRow}>
          <Field label="إلى" value={d.to} flex={2} />
          <Field label="الأولوية" value={d.priority || '□ عادي □ عاجل □ سري'} />
        </Box>
        <Box sx={fieldRow}>
          <Field label="الموضوع" value={d.subject} flex={3} />
        </Box>
      ...
      <Box sx={fieldRow}>
        ...
        <Field label="نسخة إلى" value={d.cc} flex={2} />
      </Box>
...
const MeetingMinutes = ({ d }) => (
  <Box sx={pageWrapper}>
    ...
    <Box sx={bodyPad}>
      ...
      <EmptyTable
        headers={[{ label: 'م', width: 30 }, 'الاسم', 'المسمى الوظيفي', 'القسم', 'التوقيع']}
        rowCount={8}
        headerBg="#eceff1"
      />
      ...
      <EmptyTable
        headers={[{ label: 'م', width: 30 }, 'البند', 'المقدم', 'القرار / التوصية']}
        rowCount={6}
        headerBg="#eceff1"
      />
      ...
      <EmptyTable
        headers={[{ label: 'م', width: 30 }, 'القرار/التوصية', 'المسؤول', 'الموعد النهائي']}
        rowCount={5}
        headerBg="#eceff1"
      />
  ...
  <Box sx={{ ...pageWrapper, border: '2px solid #6a1b9a' }}>
    ...
    <Box sx={bodyPad}>
      ...
      <Typography variant="body1" sx={{ lineHeight: 2.5 }}>
        ...
        <br />
        المسمى الوظيفي: <strong>{d.delegatorTitle || '________________'}</strong>
        <br />
        <br />
        أفوض السيد / السيدة: <strong>{d.delegate || '________________'}</strong>
        <br />
        المسمى الوظيفي: <strong>{d.delegateTitle || '________________'}</strong>
      </Typography>
  ...
  <Box sx={{ ...pageWrapper, borderTop: '4px solid #e65100' }}>
    ...
    <Box sx={bodyPad}>
      ...
      <Typography variant="body1">
        نأمل الالتزام بما ورد أعلاه اعتباراً من تاريخ <strong>{formatDate(d.effectiveDate)}</strong>
        .
      </Typography>
      <Typography variant="body1" color="text.secondary" fontStyle="italic" mt={1}>
        والله الموفق
  ...
  <Box sx={{ ...pageWrapper, borderTop: '4px solid #c62828' }}>
    ...
    <Box sx={bodyPad}>
      ...
      <Box sx={fieldRow}>
        <Field
          label="الحالة"
          value={d.status || '□ قيد المعالجة □ تمت المعالجة □ مغلقة'}
          flex={2}
        />
  ...
  <Box sx={pageWrapper}>
    ...
    <Box sx={bodyPad}>
      ...
      <Box sx={fieldRow}>
        <Field
          label="نوع الزيارة"
          value={d.visitType || '□ أولى □ متابعة □ تقييم □ أخرى'}
          flex={2}
        />
      ...
      <Box sx={{ bgcolor: '#fff3e0', p: 2, borderRadius: 1, mt: 2 }}>
        ...
        <Typography variant="body2">
          • في حال الرغبة بإلغاء أو تأجيل الموعد يرجى التواصل قبل 24 ساعة
        </Typography>
      </Box>
