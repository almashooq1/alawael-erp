...
export const FINANCE_TEMPLATES = [
  {
    ...
    name: 'فاتورة',
    nameEn: 'Invoice',
    desc: 'فاتورة مبيعات / خدمات ضريبية',
    color: '#2e7d32',
  },
  {
    id: 'payment-voucher',
    ...
    desc: 'سند صرف نقدي أو تحويل بنكي',
    color: '#c62828',
  },
  {
    id: 'receipt-voucher',
  ...
  },
  {
    ...
    desc: 'قيد محاسبي لتسجيل العمليات المالية',
    color: '#6a1b9a',
  },
  {
    id: 'trial-balance',
    ...
    desc: 'ميزان مراجعة لجميع الحسابات',
    color: '#00695c',
  },
  {
  ...
  },
  {
    ...
    desc: 'إيصال استلام تبرع نقدي أو عيني',
    color: '#e65100',
  },
  {
    id: 'credit-note',
  ...
  },
  {
    ...
    desc: 'سند صرف من صندوق المصروفات النثرية',
    color: '#ff6f00',
  },
  {
    id: 'income-statement',
    ...
    desc: 'قائمة الإيرادات والمصروفات',
    color: '#2e7d32',
  },
  {
    id: 'balance-sheet',
    ...
    desc: 'قائمة المركز المالي (الأصول والخصوم)',
    color: '#1a237e',
  },
];
...
export const FinanceTemplateRenderer = ({ templateId, data }) => {
  ...
  switch (templateId) {
    ...
    case 'payment-voucher':
      return <PaymentVoucher d={d} />;
    case 'receipt-voucher':
      return <ReceiptVoucher d={d} />;
    case 'journal-entry':
      return <JournalEntry d={d} />;
    case 'trial-balance':
      return <TrialBalance d={d} />;
    case 'account-statement':
      return <AccountStatement d={d} />;
    case 'cheque':
      return <ChequePrint d={d} />;
    case 'donation-receipt':
      return <DonationReceipt d={d} />;
    case 'credit-note':
      return <CreditNote d={d} />;
    case 'petty-cash':
      return <PettyCash d={d} />;
    case 'income-statement':
      return <IncomeStatement d={d} />;
    case 'balance-sheet':
      ...
      <Grid container spacing={2} mb={2}>
        ...
        <Grid item xs={6}>
          <Box sx={{ bgcolor: '#f5f5f5', p: 2, borderRadius: 1 }}>
            ...
            <Typography variant="caption" color="text.secondary">
              الرقم الضريبي: {d.customerTax || '________________'}
            </Typography>
          </Box>
        </Grid>
      </Grid>
      ...
      <Box sx={{ display: 'flex', justifyContent: 'flex-end', mt: 1 }}>
        <Box sx={{ display: 'flex', gap: 4, bgcolor: '#f3e5f5', px: 3, py: 1, borderRadius: 1 }}>
          <Typography variant="body2">
            <strong>إجمالي المدين:</strong> {formatMoney(d.totalDebit)} ر.س
          </Typography>
          <Typography variant="body2">
            <strong>إجمالي الدائن:</strong> {formatMoney(d.totalCredit)} ر.س
          </Typography>
        </Box>
      </Box>
      <NotesBox content={d.notes} lines={2} />
...
const TrialBalance = ({ d }) => (
  <Box sx={pageWrapper}>
    ...
    <Box sx={bodyPad}>
      ...
      <EmptyTable
        headers={[
          { label: 'رقم الحساب', width: 80 },
          'اسم الحساب',
          { label: 'مدين', center: true, width: 100 },
          { label: 'دائن', center: true, width: 100 },
        ]}
        rowCount={20}
        headerBg="#e0f2f1"
      />
      <Box sx={{ display: 'flex', justifyContent: 'flex-end', mt: 1 }}>
        <Box sx={{ display: 'flex', gap: 4, bgcolor: '#e0f2f1', px: 3, py: 1, borderRadius: 1 }}>
          <Typography variant="body2">
            <strong>إجمالي المدين:</strong> ____________ ر.س
          </Typography>
          <Typography variant="body2">
            <strong>إجمالي الدائن:</strong> ____________ ر.س
          </Typography>
        </Box>
      </Box>
      <SignatureBlock signatures={['المحاسب', 'المدير المالي']} />
...
const AccountStatement = ({ d }) => (
  <Box sx={pageWrapper}>
    ...
    <Box sx={bodyPad}>
      ...
      >
        <Typography variant="body2">
          <strong>إجمالي الحركة المدينة:</strong> ____________
        </Typography>
        <Typography variant="body2">
          <strong>إجمالي الحركة الدائنة:</strong> ____________
        </Typography>
        <Typography variant="body1" fontWeight="bold">
          <strong>الرصيد الختامي:</strong> ____________ ر.س
        </Typography>
      </Box>
      <SignatureBlock signatures={['المحاسب', 'المدير المالي']} />
      <OrgFooter />
  ...
  <Box sx={{ display: 'flex', justifyContent: 'center', py: 4 }}>
    <Box sx={{ width: 700, border: '2px solid #0d47a1', borderRadius: 2, overflow: 'hidden' }}>
      ...
      >
        ...
        <Box textAlign="left">
          <Typography variant="body2">
            رقم الشيك: <strong>{d.chequeNo || '________'}</strong>
          </Typography>
          <Typography variant="body2">
            التاريخ: <strong>{formatDate(d.date)}</strong>
          </Typography>
        </Box>
      </Box>
  ...
  <Box sx={{ ...pageWrapper, borderTop: '4px solid #e65100' }}>
    ...
    <Box sx={bodyPad}>
      ...
      <Box sx={{ bgcolor: '#fff3e0', p: 2, borderRadius: 2, mt: 2, textAlign: 'center' }}>
        <Typography variant="body1" fontWeight="bold">
          جزاكم الله خيراً على تبرعكم السخي
        </Typography>
...
const CreditNote = ({ d }) => (
  <Box sx={{ ...pageWrapper, borderTop: '4px solid #ad1457' }}>
    ...
    <Box sx={bodyPad}>
      ...
      <EmptyTable
        headers={[{ label: 'م', width: 30 }, 'البيان', { label: 'المبلغ', center: true }]}
        rowCount={5}
        headerBg="#fce4ec"
      />
      <Box sx={{ display: 'flex', justifyContent: 'flex-end', mt: 1 }}>
        <Box sx={{ bgcolor: '#fce4ec', px: 3, py: 1, borderRadius: 1 }}>
          <Typography variant="body1" fontWeight="bold">
            إجمالي الإشعار: {formatMoney(d.total)} ر.س
          </Typography>
        </Box>
      </Box>
      <Box sx={fieldRow}>
        <Field label="السبب" value={d.reason} flex={3} />
      </Box>
      <SignatureBlock signatures={['المحاسب', 'المدير المالي']} />
      <OrgFooter />
    </Box>
...
const IncomeStatement = ({ d }) => (
  <Box sx={pageWrapper}>
    ...
    <Box sx={bodyPad}>
      ...
      <EmptyTable
        headers={['البند', { label: 'المبلغ (ر.س)', center: true, width: 150 }]}
        rowCount={6}
        headerBg="#e8f5e9"
      />
      <Box sx={{ bgcolor: '#e8f5e9', px: 2, py: 1, borderRadius: 1, textAlign: 'left', mb: 2 }}>
        <Typography variant="body2" fontWeight="bold">
          إجمالي الإيرادات: ______________ ر.س
        </Typography>
      </Box>
      ...
      <Box sx={{ bgcolor: '#ffebee', px: 2, py: 1, borderRadius: 1, textAlign: 'left', mb: 2 }}>
        <Typography variant="body2" fontWeight="bold">
          إجمالي المصروفات: ______________ ر.س
        </Typography>
      </Box>

      <Box sx={{ bgcolor: '#1b5e20', color: 'white', p: 2, borderRadius: 2, textAlign: 'center' }}>
        <Typography variant="h6" fontWeight="bold">
          صافي الربح / (الخسارة): ______________ ر.س
        </Typography>
      </Box>
    ...
    <Box sx={bodyPad}>
      ...
      <Grid container spacing={2}>
        <Grid item xs={6}>
          ...
          <EmptyTable
            headers={['البند', { label: 'المبلغ', center: true }]}
            rowCount={5}
            headerBg="#e3f2fd"
          />
