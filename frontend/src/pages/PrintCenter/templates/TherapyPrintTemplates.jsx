...
export const THERAPY_TEMPLATES = [
  {
    id: 'session-report',
    name: 'تقرير جلسة علاجية',
    nameEn: 'Therapy Session Report',
    desc: 'تقرير تفصيلي عن جلسة علاج فردية أو جماعية',
    color: '#7b1fa2',
  },
  {
    id: 'rehab-plan',
    name: 'الخطة التأهيلية الفردية',
    nameEn: 'Individual Rehab Plan (IEP)',
    desc: 'خطة تأهيلية شاملة للمستفيد مع الأهداف والتدخلات',
    color: '#0277bd',
  },
  {
    id: 'assessment',
    name: 'تقرير التقييم',
    nameEn: 'Assessment Report',
    desc: 'تقرير تقييم أولي أو دوري شامل',
    color: '#00695c',
  },
  {
    id: 'treatment-auth',
    name: 'إذن علاج',
    nameEn: 'Treatment Authorization',
    desc: 'نموذج موافقة ولي الأمر على الخطة العلاجية',
    color: '#d84315',
  },
  {
    id: 'progress-report',
    name: 'تقرير التقدم العلاجي',
    nameEn: 'Therapy Progress Report',
    desc: 'تقرير متابعة دوري لتقدم المستفيد',
    color: '#2e7d32',
  },
  {
    id: 'mdt-summary',
    name: 'ملخص الفريق متعدد التخصصات',
    nameEn: 'MDT Meeting Summary',
    desc: 'ملخص اجتماع الفريق متعدد التخصصات وتوصياته',
    color: '#e65100',
  },
  {
    id: 'referral',
    ...
    desc: 'خطاب إحالة إلى جهة أو تخصص آخر',
    color: '#1565c0',
  },
  {
    id: 'discharge-report',
    name: 'تقرير الخروج',
    nameEn: 'Discharge Report',
    desc: 'تقرير ملخص عند خروج المستفيد من البرنامج',
    color: '#37474f',
  },
];
...
export const TherapyTemplateRenderer = ({ templateId, data }) => {
  ...
  switch (templateId) {
    case 'session-report':
      return <SessionReport d={d} />;
    case 'rehab-plan':
      return <RehabPlan d={d} />;
    case 'assessment':
      return <AssessmentReport d={d} />;
    case 'treatment-auth':
      return <TreatmentAuth d={d} />;
    case 'progress-report':
      return <ProgressReport d={d} />;
    case 'mdt-summary':
      return <MDTSummary d={d} />;
    case 'referral':
      return <ReferralLetter d={d} />;
    case 'discharge-report':
      return <DischargeReport d={d} />;
    default:
      return (
  ...
  <Box sx={pageWrapper}>
    ...
    <Box sx={bodyPad}>
      ...
      <Box sx={fieldRow}>
        <Field
          label="نوع العلاج"
          value={d.therapyType || '□ طبيعي □ وظيفي □ نطق □ سلوكي □ نفسي □ أخرى'}
          flex={2}
        />
      ...
      <Box sx={fieldRow}>
        <Field
          label="مستوى التعاون"
          value={d.cooperation || '□ ممتاز □ جيد □ متوسط □ ضعيف'}
          flex={2}
        />
...
const RehabPlan = ({ d }) => (
  <Box sx={pageWrapper}>
    <OrgHeader
      title="الخطة التأهيلية الفردية"
      subtitle="Individual Education/Rehab Plan (IEP)"
      color="#0277bd"
    />
    <Box sx={bodyPad}>
      <ConfidentialBanner />
      ...
      <PrintTable
        headers={['الخدمة', 'عدد الجلسات/الأسبوع', 'المدة', 'المقدم', 'ملاحظات']}
        headerBg="#e1f5fe"
        rows={[
          ['علاج طبيعي', '', '', '', ''],
          ['علاج وظيفي', '', '', '', ''],
  ...
  <Box sx={pageWrapper}>
    ...
    <Box sx={bodyPad}>
      ...
      <EmptyTable
        headers={[
          'المجال',
          { label: 'المستوى الحالي', width: 120 },
          { label: 'المستوى المتوقع', width: 120 },
          'ملاحظات',
        ]}
        rowCount={8}
        headerBg="#e0f2f1"
...
const TreatmentAuth = ({ d }) => (
  <Box sx={{ ...pageWrapper, border: '2px solid #d84315' }}>
    <OrgHeader
      title="إذن علاج / موافقة ولي الأمر"
      subtitle="Treatment Authorization / Consent"
      color="#d84315"
    />
    <Box sx={bodyPad}>
      ...
      <Box sx={fieldRow}>
        <Field label="الجوال" value={d.guardianPhone} />
      </Box>
      ...
      <DeclarationBox
        text={`أنا الموقع أدناه ${d.guardianName || '________________'} ولي أمر المستفيد ${d.beneficiaryName || '________________'} أقر بأنني اطلعت على الخطة العلاجية المقترحة وفهمت المخاطر المحتملة وأوافق على تنفيذها. كما أتعهد بالالتزام بتعليمات الفريق العلاجي ومتابعة البرنامج وفقاً للجدول المحدد.`}
      />
    ...
    <Box sx={bodyPad}>
      ...
      <EmptyTable
        headers={[
          ...
          'الهدف',
          'مستوى الأداء',
          { label: 'نسبة التحقق', center: true, width: 90 },
          'ملاحظات',
        ]}
        rowCount={6}
        headerBg="#e8f5e9"
      ...
      <Box sx={fieldRow}>
        <Field
          label="التقييم العام"
          value={d.overallRating || '□ ممتاز □ جيد □ متوسط □ محدود □ لا تقدم'}
          flex={2}
        />
...
const MDTSummary = ({ d }) => (
  <Box sx={pageWrapper}>
    <OrgHeader
      title="ملخص اجتماع الفريق متعدد التخصصات"
      subtitle="Multidisciplinary Team (MDT) Meeting Summary"
      color="#e65100"
    />
    <Box sx={bodyPad}>
      <ConfidentialBanner />
      ...
      <PrintTable
        headers={['م', 'الاسم', 'التخصص', 'التوقيع']}
        headerBg="#fff3e0"
        rows={[
  ...
  <Box sx={pageWrapper}>
    ...
    <Box sx={bodyPad}>
      ...
      <Typography variant="body1" fontWeight="bold" mb={1}>
        إلى: {d.referredTo || '________________________________'}
      </Typography>
      ...
      <Typography variant="body1" sx={{ lineHeight: 2.2 }}>
        ...
        <br />
        رقم الملف: <strong>{d.fileNo || '________'}</strong>، العمر:{' '}
        <strong>{d.age || '___'}</strong>
        {d.gender ? `، الجنس: ${d.gender}` : ''}
        <br />
        التشخيص: <strong>{d.diagnosis || '________________________________'}</strong>
      </Typography>
      ...
      <Typography variant="body1" mt={2}>
        نرجو التكرم بالاطلاع وإفادتنا بتقريركم.
      </Typography>
  ...
  <Box sx={pageWrapper}>
    ...
    <Box sx={bodyPad}>
      ...
      <Box sx={fieldRow}>
        <Field
          label="سبب الخروج"
          value={d.dischargeReason || '□ إتمام البرنامج □ بطلب ولي الأمر □ أخرى'}
          flex={3}
        />
      </Box>
      ...
      <PrintTable
        headers={['الخدمة', 'عدد الجلسات', 'الإنجازات الرئيسية']}
        headerBg="#eceff1"
        rows={[
          ['علاج طبيعي', '', ''],
          ['علاج وظيفي', '', ''],
      ...
      <EmptyTable
        headers={['المجال', 'عند القبول', 'عند الخروج', 'التغيير']}
        rowCount={6}
        headerBg="#eceff1"
      />
      ...
      <EmptyTable
        headers={['التخصص', 'التاريخ', 'المكان', 'ملاحظات']}
        rowCount={3}
        headerBg="#eceff1"
      />
      ...
      <SignatureBlock
        signatures={['الطبيب المعالج', 'منسق الفريق', 'ولي الأمر', 'مدير البرنامج']}
      />
      <OrgFooter />
    </Box>
  </Box>
