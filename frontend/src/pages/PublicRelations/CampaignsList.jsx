...
import {
  ...
  createCampaign,
  updateCampaign,
} from '../../services/publicRelations.service';
...
export default function CampaignsList() {
  ...
  const handleSave = async () => {
    setSaving(true);
    try {
      if (mode === 'edit') await updateCampaign(selectedId, form);
  ...
  return (
    <Box sx={{ p: { xs: 2, md: 3 } }}>
      ...
      >
        <Box>
          ...
          <Typography variant="body2" color="text.secondary">
            إدارة الحملات الإعلامية والتسويقية
          </Typography>
        </Box>
      ...
      {loading ? (
        ...
        >
          <Campaign sx={{ fontSize: 64, color: theme.palette.grey[300], mb: 2 }} />
          <Typography color="text.secondary">لا توجد حملات</Typography>
            ...
            <TableBody>
              {list.map((c, idx) => (
                <TableRow key={c._id || idx}>
                  <TableCell>
                    <Typography fontWeight={600}>{c.campaignCode}</Typography>
                  </TableCell>
      ...
      <Dialog open={dialogOpen} onClose={() => setDialogOpen(false)} maxWidth="sm" fullWidth>
        ...
        <DialogActions>
          ...
          <Button
            variant="contained"
            onClick={handleSave}
            disabled={saving || !form.campaignCode.trim() || !form.titleAr.trim()}
          >
