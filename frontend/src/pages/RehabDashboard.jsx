    ...
    <Glass
      sx={{
        ...
        '&:hover': {
          ...
          boxShadow: `0 16px 48px ${alpha(gradient?.[0] || '#3b82f6', 0.3)}`,
        },
      }}
    >
      <Box
      ...
      <Box
        sx={{
          ...
          height: 100,
          borderRadius: '50%',
          background: `radial-gradient(circle, ${alpha(gradient?.[0] || '#3b82f6', 0.18)}, transparent 70%)`,
          pointerEvents: 'none',
        }}
      />
      <Box
      ...
      {trend !== null && trend !== undefined && (
        <Box sx={{ display: 'flex', alignItems: 'center', gap: 0.5, mt: 1 }}>
          ...
          <Typography
            sx={{ fontSize: '0.68rem', color: trend > 0 ? '#10b981' : '#ef4444', fontWeight: 600 }}
          >
  ...
  <Box sx={{ mb: 2 }}>
    ...
    <Box
      sx={{
        height: 7,
        borderRadius: 4,
        bgcolor: alpha(gradient?.[0] || '#3b82f6', 0.12),
        overflow: 'hidden',
      }}
    >
      <motion.div
      ...
      >
        ...
        >
          <Box>
            ...
            <Typography sx={{ fontSize: '0.75rem', color: 'text.secondary' }}>
              {plan.beneficiaryName} · {plan.primaryDiagnosis}
            </Typography>
          </Box>
            ...
            {risk && (
              <Box
                sx={{
                  ...
                  fontSize: '0.68rem',
                  fontWeight: 700,
                  background: alpha(risk.color, 0.12),
                  color: risk.color,
                }}
              >
        ...
        <Box sx={{ mb: 1 }}>
          <Box sx={{ display: 'flex', justifyContent: 'space-between', mb: 0.4 }}>
            ...
            <Typography sx={{ fontSize: '0.72rem', fontWeight: 800, color: pc }}>
              {progress}%
            </Typography>
          </Box>
          ...
          <Box>
            ...
            <Tooltip title="تعديل">
              <IconButton size="small" onClick={() => onEdit(plan)}>
                <Edit sx={{ fontSize: 16, color: '#8b5cf6' }} />
              </IconButton>
            </Tooltip>
          </Box>
        </Box>
...
const AlertCard = memo(({ alert, idx }) => {
  const cm = {
    ...
    success: { border: '#10b981', bg: '#d1fae5', text: '#065f46', icon: '✅' },
  };
    ...
    >
      ...
      >
        <Typography sx={{ fontSize: '0.82rem', fontWeight: 700, color: c.text }}>
          {c.icon} {alert.title || `تنبيه ${idx + 1}`}
        </Typography>
        ...
        {alert.planCode && (
          <Box
            sx={{
              ...
              fontSize: '0.65rem',
              fontWeight: 700,
              background: alpha(c.border, 0.15),
              color: c.border,
            }}
          >
            {alert.planCode}
...
function RehabDashboardInner() {
  ...
  const handleCreatePlan = async () => {
    ...
    try {
      ...
      show('تم إنشاء الخطة بنجاح', 'success');
  ...
  const handleAddGoal = async () => {
    ...
    try {
      await apiCall(`/${selectedPlan._id}/goals`, {
        method: 'POST',
        body: JSON.stringify(newGoal),
      });
      show('تمت إضافة الهدف', 'success');
  ...
  const handleRecordSession = async () => {
    ...
    try {
      await apiCall(`/${selectedPlan._id}/services/${sid}/sessions`, {
        method: 'POST',
        body: JSON.stringify(newSession),
      });
      show('تم تسجيل الجلسة', 'success');
  ...
  const handleAI = async plan => {
    setAiLoading(true);
    try {
      const r = await apiCall(`/beneficiary/${plan.beneficiary}/ai-assessment`, { method: 'POST' });
      show(`تقييم AI مكتمل — مستوى الخطر: ${r.data?.riskLevel || 'moderate'}`, 'info');
      fetchDashboard();
    } catch (e) {
      show(e.message, 'error');
    } finally {
  ...
  const stats = dashboard?.stats || DATA_DASHBOARD.stats;
  ...
  const TABS = [
    ...
    {
      label: 'التنبيهات',
      icon: <NotificationsActive sx={{ fontSize: 15 }} />,
      badge: alerts.length,
    },
  ];
  ...
  return (
    <Box
      ...
      sx={{
        minHeight: '100vh',
        background: isDark
          ? 'linear-gradient(135deg, #0a0f1e, #0f172a, #0a1628)'
          : 'linear-gradient(135deg, #f0f9ff, #f8fafc, #faf5ff)',
        p: 3,
    ...
    >
        ...
        {error && (
          ...
          >
            ...
            >
              ...
              <Typography sx={{ fontSize: '0.8rem', color: '#92400e', flex: 1 }}>
                {error}
              </Typography>
          ...
          <Box>
            <Box sx={{ display: 'flex', alignItems: 'center', gap: 1.5, mb: 0.5 }}>
              ...
              >
                ...
                />
                <Typography sx={{ fontSize: '0.7rem', fontWeight: 700, color: '#10b981' }}>
                  AI مفعّل
                </Typography>
              </Box>
            </Box>
      ...
      >
        <StatCard
          ...
          subtitle={`من ${stats.totalPlans} إجمالي`}
        ...
        <StatCard
          ...
          gradient={alerts.length > 0 ? ['#ef4444', '#f87171'] : ['#06b6d4', '#22d3ee']}
          delay={0.15}
        />
      </Box>
      ...
      <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 0.2 }}>
        <Glass
          sx={{ mb: 2.5, p: 0.5, display: 'flex', gap: 0.5, flexWrap: 'wrap', borderRadius: 3 }}
        >
          {TABS.map((t, i) => (
      ...
      <AnimatePresence mode="wait">
        ...
        >
          ...
          {tab === 0 && (
            <Box
              sx={{ display: 'grid', gridTemplateColumns: { xs: '1fr', md: '1fr 340px' }, gap: 2 }}
            >
              <Glass sx={{ overflow: 'hidden' }}>
                ...
                >
                  <Typography sx={{ fontWeight: 700 }}>
                    قائمة خطط التأهيل ({plans.length})
                  </Typography>
                  <Box
                    component="button"
                    onClick={() => setNewPlanOpen(true)}
              ...
              <Box sx={{ display: 'flex', flexDirection: 'column', gap: 2 }}>
                ...
                <Glass sx={{ overflow: 'hidden' }}>
                  ...
                  >
                    ...
                    >
                      <VideoCall sx={{ color: '#fff', fontSize: 16 }} />
                    </Box>
                    <Typography sx={{ fontWeight: 700, fontSize: '0.88rem' }}>
                      جلسات Tele-Rehab القادمة
                    </Typography>
                  </Box>
                  <Box sx={{ p: 2 }}>
                  ...
                  <Box sx={{ p: 2 }}>
                    {alerts.length === 0 ? (
                      <Box sx={{ textAlign: 'center', py: 2 }}>
                        <CheckCircle
                          sx={{ fontSize: 32, color: '#10b981', opacity: 0.6, mb: 0.5 }}
                        />
                        <Typography sx={{ fontSize: '0.78rem', color: '#10b981', fontWeight: 600 }}>
                          جميع المرضى في المسار الصحيح
                        </Typography>
                      </Box>
                    ) : (
                ...
                {selectedPlan ? (
                  <>
                    {[
                      {
                        label: '🦵 التحرك والمشي',
                        value: selectedPlan.progressMetrics?.mobility ?? 65,
                        gradient: ['#3b82f6', '#60a5fa'],
                      },
                      {
                        label: '💪 القوة العضلية',
                        value: selectedPlan.progressMetrics?.strength ?? 50,
                        gradient: ['#8b5cf6', '#a78bfa'],
                      },
                      {
                        label: '🎯 نطاق الحركة (ROM)',
                        value: selectedPlan.progressMetrics?.ROM ?? 75,
                        gradient: ['#10b981', '#34d399'],
                      },
                      {
                        label: '😌 انخفاض الألم',
                        value: selectedPlan.progressMetrics?.painReduction ?? 80,
                        gradient: ['#f59e0b', '#fbbf24'],
                      },
                      {
                        label: '🏠 أنشطة يومية (ADL)',
                        value: selectedPlan.progressMetrics?.ADL ?? 55,
                        gradient: ['#06b6d4', '#22d3ee'],
                      },
                      {
                        label: '🧠 التوازن والتنسيق',
                        value: selectedPlan.progressMetrics?.balance ?? 70,
                    ...
                    <Box
                      sx={{
                        ...
                        p: 1.5,
                        borderRadius: 2,
                        background: isDark ? 'rgba(255,255,255,0.05)' : alpha('#3b82f6', 0.06),
                      }}
                    >
                ...
                <Box sx={{ p: 2 }}>
                  {selectedPlan?.goals?.length > 0 ? (
                    selectedPlan.goals.map((g, i) => {
                      const gs =
                        g.status === 'achieved'
                          ? { label: 'محقق', color: '#10b981', icon: '✅' }
                          : g.status === 'in_progress'
                            ? { label: 'جاري', color: '#3b82f6', icon: '🔄' }
                            : { label: 'لم يبدأ', color: '#6b7280', icon: '⭕' };
                      return (
                        <motion.div
                          key={i}
                          initial={{ opacity: 0, y: 10 }}
                        ...
                        >
                          <Box
                            sx={{
                              ...
                              mb: 1.2,
                              borderRadius: 2.5,
                              background: isDark ? 'rgba(255,255,255,0.03)' : alpha(gs.color, 0.04),
                              border: `1px solid ${alpha(gs.color, 0.2)}`,
                            }}
                          >
                            <Box
                              sx={{
                            ...
                            >
                              ...
                              <Box
                                sx={{
                                  ...
                                  fontSize: '0.65rem',
                                  fontWeight: 700,
                                  background: alpha(gs.color, 0.12),
                                  color: gs.color,
                                }}
                              >
                                {gs.icon} {gs.label}
                            ...
                            <Typography
                              sx={{ fontSize: '0.68rem', color: 'text.secondary', mt: 0.3 }}
                            >
                              الأسبوع: {g.targetWeek}
                            </Typography>
                          </Box>
                        </motion.div>
                      );
                ...
                <Glass sx={{ gridColumn: { md: '1 / -1' }, overflow: 'hidden' }}>
                  ...
                  <Box sx={{ overflowX: 'auto' }}>
                    <table style={{ width: '100%', borderCollapse: 'collapse' }}>
                      <thead>
                        <tr>
                          {['#', 'التاريخ', 'النوع', 'ألم قبل', 'ألم بعد', 'الحضور'].map(h => (
                            <Box
                              key={h}
                              component="th"
                              sx={{
                          ...
                          .map((sess, i) => (
                            ...
                            >
                              ...
                              >
                                ...
                                >
                                  {sess.sessionType === 'tele'
                                    ? 'عن بُعد'
                                    : sess.sessionType === 'in_person'
                                      ? 'حضوري'
                                      : sess.sessionType === 'home'
                                        ? 'منزلي'
                                        : sess.sessionType || '—'}
                                </Box>
                              </Box>
              ...
              <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }}>
                <Box
                  sx={{
                    p: 3,
                    borderRadius: 3,
                    background: 'linear-gradient(135deg, #0d1b2a, #1a3a5c, #0d2a1f)',
                    color: '#e8f4fd',
                    position: 'relative',
                    overflow: 'hidden',
                  }}
                >
                  <Box sx={{ display: 'flex', alignItems: 'center', gap: 1.5, mb: 2 }}>
                    ...
                    <Box>
                      <Typography sx={{ color: '#5bc8f5', fontWeight: 800, fontSize: '1.1rem' }}>
                        نظام تقييم الذكاء الاصطناعي — الإصدار 6.0
                      </Typography>
                      <Typography sx={{ color: 'rgba(232,244,253,0.7)', fontSize: '0.78rem' }}>
                  ...
                  >
                    {[
                      {
                        title: 'تحليل الحركة',
                        desc: 'كاميرات ثلاثية الأبعاد لتحليل أنماط المشي',
                        icon: '📹',
                      },
                      {
                        title: 'تنبؤ بالنتائج',
                        desc: 'توقع الوصول إلى الأهداف بناءً على معدل التحسن',
                        icon: '🔮',
                      },
                      {
                        title: 'تقييم المخاطر',
                        desc: 'اكتشاف مبكر للمرضى الذين ينحرفون عن المسار',
                        icon: '⚠️',
                      },
                      ...
                      >
                        ...
                        >
                          ...
                          <Typography
                            sx={{ fontSize: '0.85rem', fontWeight: 700, color: '#5bc8f5', mb: 0.5 }}
                          >
                            {f.title}
                          </Typography>
                          <Typography sx={{ fontSize: '0.72rem', color: 'rgba(232,244,253,0.8)' }}>
                            {f.desc}
                          </Typography>
                        </Box>
                      </motion.div>
                        ...
                        >
                          <Avatar
                            sx={{
                              width: 36,
                              height: 36,
                              background: `linear-gradient(135deg, ${risk.color}, ${risk.color}aa)`,
                              fontSize: '0.75rem',
                              fontWeight: 700,
                            }}
                          >
                            {(plan.beneficiaryName || '?').slice(0, 2)}
                          </Avatar>
                          <Box sx={{ flex: 1 }}>
                          ...
                          {plan.latestRiskLevel && (
                            <Box
                              sx={{
                                ...
                                borderRadius: 2,
                                mb: 1.5,
                                background: alpha(risk.color, 0.08),
                                border: `1px solid ${alpha(risk.color, 0.25)}`,
                              }}
                            >
                              <Typography
                                sx={{ fontSize: '0.78rem', fontWeight: 700, color: risk.color }}
                              >
                                {risk.icon} مستوى الخطر: {risk.label}
                                {plan.latestPredictedOutcome !== null &&
                                  plan.latestPredictedOutcome !== undefined &&
                                  ` · التحسن المتوقع: ${plan.latestPredictedOutcome}%`}
                              </Typography>
                            </Box>
                          )}
                            ...
                            <Box key={j}>
                              {a.recommendations?.map((r, ri) => (
                                ...
                                >
                                  <Box
                                    component="span"
                                    sx={{ color: '#3b82f6', fontWeight: 800, flexShrink: 0 }}
                                  >
                                    ›
                                  </Box>{' '}
                                  {r}
          ...
          {tab === 3 && (
            <Glass sx={{ overflow: 'hidden' }}>
              ...
              >
                ...
                >
                  <VideoCall sx={{ color: '#fff', fontSize: 20 }} />
                </Box>
                <Typography sx={{ fontWeight: 700, fontSize: '1rem' }}>
                  جلسات التأهيل عن بُعد
                </Typography>
              </Box>
              {upcomingTele.length === 0 ? (
                ...
                <Box sx={{ overflowX: 'auto' }}>
                  <table style={{ width: '100%', borderCollapse: 'collapse' }}>
                    <thead>
                      <tr>
                        {['المريض', 'التاريخ والوقت', 'المنصة', 'الحالة', 'الإجراء'].map(h => (
                          <Box
                            key={h}
                            component="th"
                            sx={{
                      ...
                      {upcomingTele.map((s, i) => (
                        ...
                        >
                          ...
                          >
                            ...
                            >
                              {s.status === 'scheduled' ? 'مجدولة' : s.status}
                            </Box>
                          </Box>
                          <Box
                            component="td"
                          ...
                          >
                            <Box
                              ...
                              sx={{
                                ...
                                borderRadius: 2,
                                textDecoration: 'none',
                                background: s.meetingLink
                                  ? 'linear-gradient(135deg, #10b981, #34d399)'
                                  : alpha('#6b7280', 0.1),
                                color: s.meetingLink ? '#fff' : '#6b7280',
                                fontSize: '0.75rem',
                                fontWeight: 700,
                              }}
                            >
              ...
              {alerts.length === 0 ? (
                <motion.div initial={{ scale: 0.9, opacity: 0 }} animate={{ scale: 1, opacity: 1 }}>
                  <Glass sx={{ p: 4, textAlign: 'center' }}>
                    ...
                    <Typography
                      sx={{ fontWeight: 700, fontSize: '1.1rem', color: '#10b981', mb: 0.5 }}
                    >
                      لا توجد تنبيهات حرجة
                    </Typography>
                    <Typography sx={{ color: 'text.secondary', fontSize: '0.85rem' }}>
                      جميع مرضاك في المسار الصحيح للوصول إلى أهدافهم
                    </Typography>
                  </Glass>
                </motion.div>
              ) : (
      ...
      >
        ...
        >
          ➕ إنشاء خطة تأهيل جديدة
        </DialogTitle>
        <DialogContent>
          <Box sx={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 2, mt: 1 }}>
            <TextField
              fullWidth
              label="رقم المستفيد / الاسم *"
              size="small"
              sx={{ gridColumn: '1/-1' }}
            ...
            <FormControl fullWidth size="small">
              ...
              >
                {[
                  ['physical', 'جسمية'],
                  ['sensory_visual', 'بصرية'],
                  ['sensory_hearing', 'سمعية'],
                  ['intellectual', 'ذهنية'],
                  ['autism', 'توحد'],
                  ['multiple', 'مركبة'],
                ].map(([v, l]) => (
            ...
            <FormControl fullWidth size="small">
              ...
              >
                {[
                  ...
                  ['vocational', 'مهني'],
                  ['educational', 'تعليمي'],
                  ['earlyIntervention', 'تدخل مبكر'],
                  ['independentLiving', 'استقلالية'],
                ].map(([v, l]) => (
                  <MenuItem key={v} value={v}>
                    {l}
                  </MenuItem>
        ...
        <DialogActions sx={{ px: 3, pb: 2.5 }}>
          ...
          <Button
            ...
            sx={{ background: 'linear-gradient(135deg, #10b981, #3b82f6)', px: 3 }}
          >
            إنشاء الخطة
          </Button>
        </DialogActions>
      ...
      >
        ...
        <DialogActions sx={{ px: 3, pb: 2.5 }}>
          ...
          <Button
            ...
            sx={{ background: 'linear-gradient(135deg, #3b82f6, #8b5cf6)', px: 3 }}
          >
            حفظ الجلسة
          </Button>
        </DialogActions>
      ...
      >
        <Alert
          ...
          variant="filled"
          sx={{ borderRadius: 2.5, fontFamily: 'Tajawal, sans-serif' }}
        >
          {snack.msg}
        </Alert>
      </Snackbar>
      ...
      {/* FAB */}
      <motion.div
        style={{ position: 'fixed', bottom: 28, insetInlineStart: 28, zIndex: 1000 }}
...
const DATA_DASHBOARD = {
  ...
  alerts: [
    {
      severity: 'warning',
      title: 'احتمال تأخر في تحقيق الهدف',
      message: 'المريض أحمد عبدالله لم يحضر 3 جلسات متتالية',
      planCode: 'RHP-2026-00001',
    },
    {
      severity: 'info',
      title: 'توصية AI',
      message: 'يُنصح بزيادة تكرار تمارين التوازن لمريضة سارة محمد بنسبة 20%',
      planCode: 'RHP-2026-00003',
    },
  ],
  upcomingTeleSessions: [
    {
      beneficiaryName: 'أحمد عبدالله',
...
const DATA_PLANS = [
  {
    ...
    planCode: 'RHP-2026-00001',
    beneficiaryName: 'أحمد عبدالله',
    primaryDiagnosis: 'إصابة نخاع شوكي T10',
    status: 'active',
    totalWeeks: 12,
    latestRiskLevel: 'moderate',
    latestPredictedOutcome: 72,
    progressMetrics: {
      mobility: 65,
      strength: 55,
      ROM: 70,
    ...
    goals: [
      {
        goalText: 'المشي 50 مترًا بعصا واحدة',
        measurableTarget: '50م',
        targetWeek: 6,
        status: 'in_progress',
      },
      {
        goalText: 'تقليل الألم إلى NRS ≤ 3',
        measurableTarget: 'NRS ≤ 3',
        targetWeek: 4,
        status: 'achieved',
      },
    ],
    ...
    aiAssessments: [
      {
        riskLevel: 'moderate',
        predictedOutcome: 72,
        recommendations: ['زيادة تمارين التوازن', 'مراجعة برنامج الألم', 'تحفيز حضور الجلسات'],
      },
    ],
    beneficiary: 'ben1',
  },
  {
    _id: 'plan2',
    planCode: 'RHP-2026-00002',
    beneficiaryName: 'يوسف عمر',
    primaryDiagnosis: 'متلازمة داون — تأخر حركي',
    status: 'active',
    totalWeeks: 12,
    latestRiskLevel: 'low',
    latestPredictedOutcome: 85,
    progressMetrics: {
      mobility: 75,
      strength: 60,
      ROM: 80,
    ...
    goals: [
      {
        goalText: 'الركض 20 مترًا دون سقوط',
        measurableTarget: '20م',
        targetWeek: 4,
        status: 'achieved',
      },
      {
        goalText: 'قطع المقص وتلوين داخل الخطوط',
        measurableTarget: 'دقة 80%',
        targetWeek: 6,
        status: 'in_progress',
      },
    ],
    ...
    aiAssessments: [
      {
        riskLevel: 'low',
        predictedOutcome: 85,
        recommendations: ['الاستمرار في برنامج NDT', 'إضافة سباحة أسبوعية'],
      },
    ],
    beneficiary: 'ben2',
  },
