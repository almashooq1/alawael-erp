...
export default function ReportsPage() {
  ...
  const filteredTemplates = templates.filter(t => {
    if (search && !((t.name || '').includes(search) || (t.description || '').includes(search)))
      return false;
    ...
    <Box sx={{ p: 3, direction: 'rtl' }}>
      <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', mb: 3 }}>
        <Box>
          <Typography variant="h5" fontWeight="bold">
            <ChartIcon sx={{ verticalAlign: 'middle', mr: 1 }} /> التقارير والتحليلات
          </Typography>
          <Typography variant="body2" color="text.secondary">
            {templates.length} قالب • {generated.length} تقرير مُنشأ
          </Typography>
        </Box>
      ...
      <Grid container spacing={2} sx={{ mb: 4 }}>
        {filteredTemplates.length === 0 && !loading ? (
          <Grid item xs={12}>
            <Alert severity="info">لا توجد قوالب مطابقة</Alert>
          </Grid>
        ) : (
          filteredTemplates.map((t, i) => (
            <Grid item xs={12} sm={6} md={4} lg={3} key={t._id || i}>
              ...
              >
                  ...
                  <Stack direction="row" spacing={0.5}>
                    {t.category && (
                      <Chip
                        ...
                        variant="outlined"
                        label={
                          REPORT_CATEGORIES.find(c => c.value === t.category)?.label || t.category
                        }
                      />
                    )}
                ...
                <CardActions>
                  <Button size="small" startIcon={<ChartIcon />}>
                    إنشاء
                  </Button>
            ...
            <Card variant="outlined">
              <CardContent>
                <Box sx={{ display: 'flex', justifyContent: 'space-between', mb: 1 }}>
                  <Typography variant="subtitle2" fontWeight="bold">
                    {r.title || r.name || 'تقرير'}
                  </Typography>
                  <Chip
                    size="small"
                    label={r.status || 'جاهز'}
                    color={r.status === 'ready' || r.status === 'completed' ? 'success' : 'default'}
                  />
                </Box>
              ...
              <CardActions>
                <IconButton size="small" onClick={() => setPreviewReport(r)}>
                  <PreviewIcon />
                </IconButton>
      ...
      <Dialog open={!!previewReport} onClose={() => setPreviewReport(null)} maxWidth="md" fullWidth>
        {previewReport && (
          <>
            <DialogTitle>
              {previewReport.title || previewReport.name || 'معاينة التقرير'}
            </DialogTitle>
