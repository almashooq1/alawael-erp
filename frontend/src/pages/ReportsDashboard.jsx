...
const KPICard = memo(({ title, value, subtitle, icon, gradient, trend, delay = 0 }) => {
  ...
  return (
    ...
    >
      <Glass
        sx={{ p: 3, height: '100%', position: 'relative', overflow: 'hidden', cursor: 'default' }}
      >
        <Box
          sx={{
            position: 'absolute',
        ...
        >
          <Box
            sx={{
              ...
              fontSize: 22,
              boxShadow: `0 4px 16px ${gradient.includes('f59e') ? '#f59e0b44' : '#6366f144'}`,
            }}
          >
            {icon}
          </Box>
          <Chip
            ...
            sx={{
              ...
              fontWeight: 700,
              fontSize: 11,
              border: `1px solid ${isPos ? '#22c55e44' : '#ef444444'}`,
            }}
          />
        </Box>
        <Typography
        ...
        <Typography
          variant="body2"
          fontWeight={600}
          sx={{ color: isDark ? 'rgba(255,255,255,0.85)' : 'rgba(0,0,0,0.8)', mb: 0.5 }}
        >
          {title}
        </Typography>
        <Typography
    ...
    >
        ...
        <Box sx={{ display: 'flex', alignItems: 'center', gap: 1.5 }}>
          ...
          <Box>
            <Typography
              variant="body2"
              fontWeight={600}
              sx={{ color: isDark ? 'rgba(255,255,255,0.9)' : 'rgba(0,0,0,0.85)' }}
            >
              {report.name}
            </Typography>
            <Typography
      ...
      <td style={{ padding: '12px 16px' }}>
        <Chip
          ...
          size="small"
          sx={{
            background: `${statusColors[report.status]}22`,
            color: statusColors[report.status],
            border: `1px solid ${statusColors[report.status]}44`,
            fontWeight: 700,
            fontSize: 11,
          }}
        />
      ...
      <td style={{ padding: '12px 16px' }}>
        <Box sx={{ display: 'flex', gap: 1 }}>
          <Tooltip title="عرض">
            <IconButton size="small" onClick={() => onView(report)} sx={{ color: '#6366f1' }}>
              👁️
            </IconButton>
          </Tooltip>
          <Tooltip title="تحميل">
            <IconButton size="small" onClick={() => onDownload(report)} sx={{ color: '#22c55e' }}>
              ⬇️
            </IconButton>
          </Tooltip>
        </Box>
    ...
    <Glass sx={{ p: 2.5, display: 'flex', alignItems: 'center', gap: 2 }}>
      ...
      <Box sx={{ flex: 1 }}>
        ...
        {change !== undefined && (
          <Typography
            variant="caption"
            sx={{ color: change >= 0 ? '#22c55e' : '#ef4444', fontSize: 10 }}
          >
            {change >= 0 ? '↑' : '↓'} {Math.abs(change)}% هذا الشهر
          </Typography>
        )}
      </Box>
    </Glass>
...
const DEMO = {
  ...
  reports: [
    {
      id: 'RPT-001',
      name: 'التقرير المالي الشهري - مارس 2026',
      type: 'مالي',
      status: 'جاهز',
      date: '30/03/2026',
      size: '2.4 MB',
    ...
    {
      id: 'RPT-002',
      name: 'تقرير الموارد البشرية الأسبوعي',
      type: 'HR',
      status: 'جاهز',
      date: '29/03/2026',
      size: '1.1 MB',
    ...
    {
      id: 'RPT-003',
      name: 'إحصائيات المرضى - الربع الأول',
      type: 'مرضى',
      status: 'قيد المعالجة',
      date: '30/03/2026',
      size: '—',
    ...
    {
      id: 'RPT-006',
      name: 'تقرير مخاطر المرضى بالذكاء الاصطناعي',
      type: 'تحليلي',
      status: 'مجدول',
      date: '01/04/2026',
      size: '—',
  ...
  monthlyTrend: [
    { label: 'يناير', value: 89 },
    { label: 'فبراير', value: 112 },
    { label: 'مارس', value: 98 },
    { label: 'أبريل', value: 134 },
    { label: 'مايو', value: 127 },
    { label: 'يونيو', value: 145 },
    { label: 'يوليو', value: 110 },
    { label: 'أغسطس', value: 138 },
    { label: 'سبتمبر', value: 151 },
    { label: 'أكتوبر', value: 163 },
    { label: 'نوفمبر', value: 142 },
    { label: 'ديسمبر', value: 178 },
  ],
  weeklyLine: [
    { value: 28 },
    { value: 35 },
  ...
  aiInsights: [
    {
      icon: '🔍',
      title: 'اكتشاف نمط',
      text: 'ارتفع معدل طلب التقارير المالية بنسبة 34% في نهايات الشهر، يُنصح بزيادة طاقة الخوادم في هذه الفترة.',
      color: '#6366f1',
    },
    {
      icon: '⚠️',
      title: 'تنبيه',
      text: 'فشل 3 تقارير HR في الأسبوع الماضي بسبب بيانات ناقصة. يُوصى بمراجعة قواعد التحقق.',
      color: '#ef4444',
    },
    {
      icon: '📈',
      title: 'توقع',
      text: 'من المتوقع ارتفاع طلبات التقارير بنسبة 20% في أبريل نظرًا لنهاية الربع المالي الأول.',
      color: '#22c55e',
    },
    {
      icon: '💡',
      title: 'اقتراح تحسين',
      text: 'يمكن تقليل وقت إنشاء التقارير التحليلية بنسبة 45% باستخدام ذاكرة تخزين مؤقت للبيانات الأكثر طلبًا.',
      color: '#f59e0b',
    },
  ],
  topUsers: [
...
export default function ReportsDashboard() {
  ...
  const fetchData = useCallback(async () => {
    setLoading(true);
    try {
      const [statsRes, execRes, jobsRes] = await Promise.allSettled([
        apiClient.get('/reports/stats'),
        apiClient.get('/reports/analytics/executive'),
        apiClient.get('/reports/jobs', { params: { limit: 20 } }),
      ]);

      const stats = statsRes.status === 'fulfilled' ? statsRes.value.data?.data || {} : {};
      const exec = execRes.status === 'fulfilled' ? execRes.value.data?.data?.kpis || {} : {};
      const jobs = jobsRes.status === 'fulfilled' ? jobsRes.value.data?.data || [] : [];

      // Map real report jobs → reports list format
      const reportsFromAPI = jobs.map(j => ({
        id: j._id,
        name: j.template_id?.name_ar || 'تقرير',
        type: j.template_id?.category || 'عام',
        status:
          j.status === 'completed'
            ? 'جاهز'
            : j.status === 'running'
      ...
      setData({
        ...DEMO,
        reports: reportsFromAPI.length > 0 ? reportsFromAPI : DEMO.reports,
        kpis: DEMO.kpis.map((k, i) => {
          if (i === 0) return { ...k, value: String(stats.jobs_this_month ?? k.value) };
          if (i === 1) return { ...k, value: String(stats.jobs_today ?? k.value) };
          if (i === 4) return { ...k, value: String(stats.active_schedules ?? k.value) };
          if (i === 5) return { ...k, value: String(exec.total_employees ?? k.value) };
          return k;
        }),
      });
    } catch {
  ...
  const handleGenerate = useCallback(async () => {
    setGenerating(true);
    try {
      // Get available templates first, pick the first matching type
      const tplRes = await apiClient.get('/reports/templates', {
        params: {
          is_active: true,
          limit: 1,
          category:
      ...
      if (templates.length > 0) {
        await apiClient.post('/reports/jobs', {
          template_id: templates[0]._id,
          export_format:
            newReport.format === 'PDF' ? 'pdf' : newReport.format === 'Excel' ? 'excel' : 'csv',
          parameters: { period: newReport.period },
        });
      }
    } catch {
      // silent — generate still closes dialog; list refreshes
    } finally {
      setGenerating(false);
      setDialogOpen(false);
      setNewReport({ name: '', type: 'مالي', period: '', format: 'PDF' });
      fetchData();
    }
  }, [newReport, fetchData]);
  ...
  return (
    <Box sx={{ minHeight: '100vh', background: bg, p: { xs: 2, md: 3 }, direction: 'rtl' }}>
            ...
            <Glass sx={{ p: 3 }}>
              <Box sx={{ display: 'flex', gap: 2, mb: 3, flexWrap: 'wrap', alignItems: 'center' }}>
                <Typography
                  variant="h6"
                  fontWeight={700}
                  sx={{ flex: 1, color: isDark ? 'rgba(255,255,255,0.9)' : 'rgba(0,0,0,0.85)' }}
                >
                  📋 جميع التقارير
                </Typography>
                <TextField
                ...
                <FormControl size="small" sx={{ minWidth: 130 }}>
                  ...
                  >
                    {['الكل', 'مالي', 'HR', 'مرضى', 'جدولة', 'عمليات', 'تحليلي'].map(t => (
                ...
                <Glass sx={{ p: 3 }}>
                  ...
                  {[
                    ...
                    },
                    {
                      ...
                      next: 'أول كل شهر 06:00',
                      type: 'تحليلي',
                      color: '#8b5cf6',
                    },
                    {
                      name: 'تقرير المخاطر بالذكاء الاصطناعي',
                      next: 'كل أحد 09:00',
                      type: 'تحليلي',
                      color: '#ec4899',
                    },
                      ...
                      >
                        ...
                        <Box sx={{ flex: 1 }}>
                          <Typography
                            variant="body2"
                            fontWeight={600}
                            sx={{ color: isDark ? 'rgba(255,255,255,0.85)' : 'rgba(0,0,0,0.85)' }}
                          >
                            {s.name}
                          </Typography>
                          <Typography
                        ...
                        />
                        <IconButton size="small" sx={{ color: '#ef4444', fontSize: 14 }}>
                          ⏸️
                        </IconButton>
                      </Box>
                    </motion.div>
                      ...
                      >
                        <Box
                          sx={{
                            ...
                            height: 36,
                            borderRadius: '50%',
                            background: i === 0 ? '#f59e0b22' : i === 1 ? '#94a3b822' : '#cd7f3222',
                            display: 'flex',
                            alignItems: 'center',
                            justifyContent: 'center',
                            fontSize: 18,
                            border: `2px solid ${i === 0 ? '#f59e0b' : i === 1 ? '#94a3b8' : '#cd7f32'}`,
                          }}
                        >
                          {u.avatar}
                        </Box>
                  ...
                  <Box sx={{ display: 'flex', alignItems: 'center', gap: 2, mb: 3 }}>
                    ...
                    <Box>
                      <Typography
                        ...
                        sx={{ color: isDark ? 'rgba(255,255,255,0.9)' : 'rgba(0,0,0,0.85)' }}
                      >
                        تحليلات الذكاء الاصطناعي للتقارير
                      </Typography>
                      <Typography
                        variant="caption"
                        sx={{ color: isDark ? 'rgba(255,255,255,0.45)' : 'rgba(0,0,0,0.45)' }}
                      >
                        رؤى ذكية مبنية على بيانات الاستخدام وأنماط التقارير
                      </Typography>
                    </Box>
                  </Box>
                ...
                <Glass sx={{ p: 3 }}>
                  ...
                  <Grid container spacing={2}>
                    {[
                      ...
                      {
                        label: 'الطلبات المتوقعة',
                        value: '+22%',
                        detail: 'نهاية الربع الأول',
                        icon: '📈',
                        color: '#22c55e',
                      },
                      {
      ...
      >
        <DialogTitle
          sx={{ background: G, color: '#fff', fontWeight: 700, borderRadius: '12px 12px 0 0' }}
        >
          📊 إنشاء تقرير جديد
        </DialogTitle>
        <DialogContent sx={{ pt: 3, display: 'flex', flexDirection: 'column', gap: 2, mt: 1 }}>
          ...
          <FormControl fullWidth>
            <InputLabel sx={{ right: 14, left: 'auto', transformOrigin: 'right top' }}>
              نوع التقرير
            </InputLabel>
            <Select
              ...
              label="نوع التقرير"
            >
              {['مالي', 'HR', 'مرضى', 'جدولة', 'عمليات', 'تحليلي'].map(t => (
                <MenuItem key={t} value={t}>
                  {t}
                </MenuItem>
              ))}
          ...
          <TextField
            ...
            onChange={e => setNewReport(p => ({ ...p, period: e.target.value }))}
          ...
          <FormControl fullWidth>
            ...
            <Select
              ...
              onChange={e => setNewReport(p => ({ ...p, format: e.target.value }))}
              label="صيغة الإخراج"
            >
        ...
        <DialogActions sx={{ p: 2, gap: 1 }}>
          ...
          >
            {generating ? (
              <>
                <CircularProgress size={16} sx={{ color: '#fff', mr: 1 }} /> جاري الإنشاء...
              </>
            ) : (
              '🚀 إنشاء التقرير'
            )}
