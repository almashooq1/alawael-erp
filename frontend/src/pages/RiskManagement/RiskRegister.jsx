...
export default function RiskRegister() {
  ...
  const handleSave = async () => {
    setSaving(true);
    try {
      if (mode === 'edit') await updateRisk(selectedId, form);
  ...
  return (
    <Box sx={{ p: { xs: 2, md: 3 } }}>
      ...
      >
        <Box>
          ...
          <Typography variant="body2" color="text.secondary">
            تسجيل وإدارة المخاطر المؤسسية
          </Typography>
        </Box>
        <Box sx={{ display: 'flex', gap: 1 }}>
          <Tooltip title="تحديث">
            <IconButton onClick={fetch} color="primary">
              <Refresh />
            </IconButton>
          </Tooltip>
          <Button
      ...
      {loading ? (
        ...
        >
          <Warning sx={{ fontSize: 64, color: theme.palette.grey[300], mb: 2 }} />
          <Typography color="text.secondary">لا توجد مخاطر مسجلة</Typography>
          <Button variant="outlined" startIcon={<Add />} onClick={openNew} sx={{ mt: 2 }}>
            تسجيل مخاطرة
          </Button>
        </Paper>
      ) : (
          ...
          <Table>
            <TableHead>
              <TableRow sx={{ bgcolor: alpha(theme.palette.primary.main, 0.04) }}>
                ...
                <TableCell sx={{ fontWeight: 700 }}>الاحتمال</TableCell>
                <TableCell sx={{ fontWeight: 700 }}>الأثر</TableCell>
            ...
            <TableBody>
              {list.map((r, idx) => (
                <TableRow key={r._id || idx}>
                  <TableCell>
                    <Typography fontWeight={600}>{r.riskCode}</Typography>
                  </TableCell>
                  ...
                  <TableCell>
                    <Chip
                      ...
                      size="small"
                      sx={{
                        bgcolor: alpha(PRIORITY_COLORS[r.priority] || '#999', 0.15),
      ...
      <Dialog open={dialogOpen} onClose={() => setDialogOpen(false)} maxWidth="sm" fullWidth>
        ...
        >
          ...
          <FormControl fullWidth>
            <InputLabel>الاحتمالية</InputLabel>
            <Select
        ...
        <DialogActions>
          ...
          <Button
            variant="contained"
            onClick={handleSave}
            disabled={saving || !form.riskCode.trim() || !form.titleAr.trim()}
          >
      ...
      <Dialog open={!!confirmDelete} onClose={() => setConfirmDelete(null)}>
        ...
        <DialogContent>
          <Typography>هل أنت متأكد من حذف هذه المخاطرة؟</Typography>
        </DialogContent>
        <DialogActions>
