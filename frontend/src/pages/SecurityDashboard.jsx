...
const ThreatRow = memo(({ threat, onBlock }) => {
  ...
  return (
    ...
    >
      ...
      <td style={{ padding: '12px 16px' }}>
        <Typography variant="caption" sx={{ color: '#6366f1', fontFamily: 'monospace' }}>
          {threat.ip}
        </Typography>
      </td>
      <td style={{ padding: '12px 16px' }}>
      ...
      <td style={{ padding: '12px 16px' }}>
        {threat.status !== 'محجوب' && (
          <Tooltip title="حجب الآن">
            <IconButton size="small" onClick={() => onBlock(threat)} sx={{ color: '#ef4444' }}>
              🚫
            </IconButton>
          </Tooltip>
        )}
        ...
        <Box sx={{ display: 'flex', alignItems: 'center', gap: 1 }}>
          <Box
            sx={{
              ...
              height: 8,
              borderRadius: '50%',
              background: session.active ? '#22c55e' : '#94a3b8',
            }}
          />
          <Typography variant="caption" sx={{ color: session.active ? '#22c55e' : '#94a3b8' }}>
            {session.active ? 'نشط' : 'منتهي'}
          </Typography>
        </Box>
      </td>
      <td style={{ padding: '12px 16px' }}>
        {session.active && (
          <Tooltip title="إنهاء الجلسة">
            <IconButton size="small" onClick={() => onRevoke(session)} sx={{ color: '#f59e0b' }}>
              ⏏️
            </IconButton>
          </Tooltip>
        )}
...
const DEMO = {
  ...
  vulnerabilities: [
    {
      title: 'SSL Certificate انتهاء الصلاحية',
      severity: 'عالي',
      deadline: '15/04/2026',
      status: 'قيد المعالجة',
      pct: 60,
    ...
    {
      title: 'ضعف في سياسة كلمات المرور',
      severity: 'منخفض',
      deadline: '01/05/2026',
      status: 'مفتوح',
      pct: 5,
...
export default function SecurityDashboard() {
  ...
  const fetchData = useCallback(async () => {
    setLoading(true);
    try {
      const res = await apiClient.get('/security/logs?limit=6');
      const raw = res.data?.data || [];
      const typeMap = { success: 'نجاح', failed: 'فشل', warning: 'تحذير', security: 'أمان' };
      const auditLog =
        raw.length > 0
          ? raw.map(l => ({
              action: l.action || l.event || l.message || 'نشاط',
              user: l.user?.name || l.userId || 'النظام',
              ip: l.ip || l.ipAddress || '',
              time: l.createdAt ? new Date(l.createdAt).toLocaleTimeString('ar-SA') : '',
              type: typeMap[l.type] || l.type || 'نجاح',
            }))
          : DEMO.auditLog;
      setData({ ...DEMO, auditLog });
    } catch {
      setData(DEMO);
    } finally {
      setLoading(false);
  ...
  const handleBlock = useCallback(threat => {
    setData(prev => ({
      ...prev,
      threats: prev.threats.map(t => (t.id === threat.id ? { ...t, status: 'محجوب' } : t)),
    }));
  }, []);
  ...
  const handleRevoke = useCallback(session => {
    setData(prev => ({
      ...prev,
      sessions: prev.sessions.map(s => (s.id === session.id ? { ...s, active: false } : s)),
    }));
  }, []);
  ...
  return (
    <Box sx={{ minHeight: '100vh', background: bg, p: { xs: 2, md: 3 }, direction: 'rtl' }}>
      ...
      >
        ...
        >
          ...
          <Box sx={{ display: 'flex', gap: 1.5, flexWrap: 'wrap', alignItems: 'center' }}>
            <Chip
              label={`🟢 ${activeSessions} جلسة نشطة`}
              size="small"
              sx={{
                background: '#22c55e22',
                color: '#22c55e',
                  ...
                  <Grid container spacing={1.5}>
                    {[
                      ...
                      <Grid item xs={6} key={i}>
                        ...
                        >
                          <Typography variant="h6" fontWeight={800} sx={{ color: m.c }}>
                            {m.v}%
                          </Typography>
                          <Typography
                            variant="caption"
                ...
                <Glass sx={{ p: 3, height: '100%' }}>
                  ...
                  {[
                    ...
                    <Box key={i} sx={{ display: 'flex', alignItems: 'center', gap: 2, mb: 2 }}>
                      ...
                      >
                        <Typography variant="body2" fontWeight={800} sx={{ color: t.color }}>
                          {t.count}
                        </Typography>
                      </Box>
                      <Box sx={{ flex: 1 }}>
                  ...
                  >
                    <Typography variant="caption" fontWeight={700} sx={{ color: '#ef4444' }}>
                      🚨 تنبيه: تهديدان حرجان قيد التحقيق
                    </Typography>
                  </Box>
                </Glass>
              </Grid>
                        ...
                        >
                          ...
                          <Box sx={{ flex: 1 }}>
                            <Typography
                              variant="caption"
                              fontWeight={600}
                              sx={{ color: isDark ? 'rgba(255,255,255,0.85)' : 'rgba(0,0,0,0.8)' }}
                            >
                              {log.action}
                            </Typography>
                          ...
                          <Chip
                            label={log.type}
                            size="small"
                            sx={{ background: `${lc}22`, color: lc, fontSize: 9, height: 18 }}
                          />
                        </Box>
                      </motion.div>
                    );
            ...
            <Glass sx={{ p: 3 }}>
              ...
              >
                <Typography
                  ...
                  sx={{ color: isDark ? 'rgba(255,255,255,0.9)' : 'rgba(0,0,0,0.85)' }}
                >
                  ⚠️ التهديدات المكتشفة
                </Typography>
                <Chip
                  label={`${(data?.threats || []).filter(t => t.status !== 'محجوب').length} نشط`}
                  size="small"
                  sx={{
                    background: '#ef444422',
                    color: '#ef4444',
              ...
              <Box sx={{ overflowX: 'auto' }}>
                <table style={{ width: '100%', borderCollapse: 'collapse' }}>
                  ...
                  <tbody>
                    {(data?.threats || []).map(t => (
                      <ThreatRow key={t.id} threat={t} onBlock={handleBlock} />
                    ))}
                  </tbody>
                </table>
              </Box>
            ...
            <Glass sx={{ p: 3 }}>
              ...
              >
                ...
                <Box sx={{ display: 'flex', gap: 1 }}>
                  <Chip
                    label={`${activeSessions} نشط`}
                    size="small"
                    sx={{ background: '#22c55e22', color: '#22c55e', fontWeight: 700 }}
                  />
                  <Chip
                    label={`${(data?.sessions?.length || 0) - activeSessions} منتهي`}
                    size="small"
                    sx={{ background: '#94a3b822', color: '#94a3b8', fontWeight: 700 }}
                  />
                </Box>
              </Box>
              <Box sx={{ overflowX: 'auto' }}>
                <table style={{ width: '100%', borderCollapse: 'collapse' }}>
                  <thead>
                    <tr
                      style={{ background: isDark ? 'rgba(255,255,255,0.04)' : 'rgba(0,0,0,0.04)' }}
                    >
                      {['المستخدم', 'عنوان IP', 'الجهاز', 'وقت الدخول', 'الحالة', 'إجراء'].map(
                        h => (
                          <th
                            key={h}
                            style={{
                      ...
                      <Glass sx={{ p: 3 }}>
                        ...
                        <Box>
                          <Box sx={{ display: 'flex', justifyContent: 'space-between', mb: 0.5 }}>
                            ...
                            <Typography variant="caption" fontWeight={700} sx={{ color: vc }}>
                              {v.pct}%
                            </Typography>
                          </Box>
                          <Box
              ...
              <Grid item xs={12} md={6}>
                <Glass sx={{ p: 3 }}>
                  <Typography
                    ...
                    sx={{ color: isDark ? 'rgba(255,255,255,0.9)' : 'rgba(0,0,0,0.85)' }}
                  >
                    📜 شهادات وإطارات الامتثال
                  </Typography>
                  {[
                    {
                      name: 'ISO/IEC 27001',
                      desc: 'إدارة أمن المعلومات',
                      status: 'معتمد',
                      color: '#22c55e',
                      expiry: '12/2026',
                    },
                    ...
                    >
                      ...
                      >
                        ...
                        <Box sx={{ flex: 1 }}>
                          <Typography variant="body2" fontWeight={700} sx={{ color: cert.color }}>
                            {cert.name}
                          </Typography>
                          <Typography
                            variant="caption"
      ...
      <Dialog
        open={scanDialog}
        onClose={() => !scanning && setScanDialog(false)}
        maxWidth="sm"
        fullWidth
        PaperProps={{
          sx: {
      ...
      >
            ...
            <Box>
              ...
              {[
                ...
                <Box key={i} sx={{ display: 'flex', alignItems: 'center', gap: 1, mb: 1 }}>
                  <Typography
                    variant="body2"
                    sx={{ color: isDark ? 'rgba(255,255,255,0.75)' : 'rgba(0,0,0,0.7)' }}
                  >
                    {item}
                  </Typography>
                </Box>
            ...
            <Box sx={{ py: 2 }}>
              ...
              <LinearProgress
                ...
                sx={{
                  ...
                  '& .MuiLinearProgress-bar': { background: G, borderRadius: 5 },
                }}
              />
              <Typography
                variant="caption"
        ...
        <DialogActions sx={{ p: 2, gap: 1 }}>
          {!scanning && (
            <Button onClick={() => setScanDialog(false)} sx={{ borderRadius: 2 }}>
              إلغاء
            </Button>
          )}
          {!scanning && (
