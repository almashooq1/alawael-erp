                ...
                <Typography variant="caption" sx={{ opacity: 0.85 }}>
                  ...
                  {trend !== null && (
                    <>
                      ...
                      {trend >= 0 ? (
                        <TrendingUpIcon
                          sx={{ fontSize: 14, verticalAlign: 'middle', color: '#c8e6c9' }}
                        />
                      ) : (
                        <TrendingDownIcon
                          sx={{ fontSize: 14, verticalAlign: 'middle', color: '#ef9a9a' }}
                        />
                      )}{' '}
                      {Math.abs(trend)}%
                    </>
...
export default function SessionAnalyticsDashboard() {
  ...
  const [overview, setOverview] = useState(DATA_OVERVIEW);
  const [trends, setTrends] = useState(DATA_TRENDS);
  const [therapistPerf, setTherapistPerf] = useState(DATA_THERAPIST_PERF);
  const [rooms, setRooms] = useState(DATA_ROOMS);
  const [attendance, setAttendance] = useState(DATA_ATTENDANCE);
  const [billing, setBilling] = useState(DATA_BILLING);
  const [goals, setGoals] = useState(DATA_GOALS);
  const [cancellations, setCancellations] = useState(DATA_CANCELLATIONS);
  ...
  const fetchAnalytics = useCallback(async () => {
    ...
    try {
      ...
      if (ovRes.status === 'fulfilled' && ovRes.value)
        setOverview(prev => ({ ...prev, ...ovRes.value }));
      if (trRes.status === 'fulfilled' && trRes.value?.trends) setTrends(trRes.value.trends);
      if (thRes.status === 'fulfilled' && Array.isArray(thRes.value?.therapists))
        setTherapistPerf(thRes.value.therapists);
      if (rmRes.status === 'fulfilled' && Array.isArray(rmRes.value?.rooms))
        setRooms(rmRes.value.rooms);
      if (atRes.status === 'fulfilled' && atRes.value)
        setAttendance(prev => ({ ...prev, ...atRes.value }));
      if (blRes.status === 'fulfilled' && blRes.value)
        setBilling(prev => ({ ...prev, ...blRes.value }));
      if (glRes.status === 'fulfilled' && glRes.value)
        setGoals(prev => ({ ...prev, ...glRes.value }));
      if (caRes.status === 'fulfilled' && caRes.value)
        setCancellations(prev => ({ ...prev, ...caRes.value }));
    } catch (err) {
  ...
  const handleExport = useCallback(async () => {
    setExporting(true);
    try {
      const result = await therapistService.exportReport({
        period,
        format: 'csv',
        sections: ['overview', 'trends', 'therapistPerformance', 'billing'],
      });
    ...
    <DashboardErrorBoundary>
      <Container maxWidth="xl" sx={{ py: 3 }}>
        ...
        <Grid container spacing={2} sx={{ mb: 4 }}>
          ...
          <KPICard
            title="نسبة الحضور"
            value={`${overview.attendanceRate?.toFixed?.(1) || overview.attendanceRate || 0}%`}
          ...
          <KPICard
            title="متوسط المدة"
            value={`${overview.avgDuration || 0} د`}
            subtitle={`${overview.totalTherapists || 0} معالج`}
            icon={<AccessTimeIcon />}
            gradient={gradients.secondary || gradients.purple}
            delay={5}
            loading={loading}
          />
        </Grid>
                ...
                <Paper sx={{ p: 3, borderRadius: 3, height: '100%' }}>
                  ...
                  {therapistPerf.length > 0 ? (
                    <ResponsiveContainer width="100%" height={320}>
                      <RadarChart
                        data={therapistPerf.slice(0, 5).map(t => ({
                          name: (t.name || t.therapistName || '').split(' ').slice(-1)[0],
                          sessions:
                            (t.totalSessions / (therapistPerf[0]?.totalSessions || 1)) * 100,
                      ...
                      >
                        ...
                        <Radar
                          name="الجلسات"
                          dataKey="sessions"
                          stroke={chartColors.category[0]}
                          fill={chartColors.category[0]}
                          fillOpacity={0.2}
                        />
                        <Radar
                          name="الإنجاز"
                          dataKey="completion"
                          stroke={chartColors.category[1]}
                          fill={chartColors.category[1]}
                          fillOpacity={0.2}
                        />
                        <Radar
                          name="التقييم"
                          dataKey="rating"
                          stroke={chartColors.category[2]}
                          fill={chartColors.category[2]}
                          fillOpacity={0.2}
                        />
                        <Legend />
                      </RadarChart>
                        ...
                        <Cell
                          ...
                          fill={
                            (r.utilization || 0) >= 85
                              ...
                              : (r.utilization || 0) >= 70
                                ? statusColors.primaryBlue || chartColors.category[0]
            ...
            <Grid container spacing={3}>
              <Grid item xs={12} md={4}>
                ...
                >
                  ...
                  >
                    {attendance.overall >= 85
                      ? 'ممتاز — نسبة حضور مرتفعة'
                      : attendance.overall >= 70
                        ? 'جيد — يمكن تحسين نسبة الحضور'
                  ...
                  <Grid item xs={12} sm={4}>
                    <Card sx={{ background: gradients.info, color: '#fff', borderRadius: 3 }}>
                      <CardContent>
                        ...
                        <Typography variant="caption" sx={{ opacity: 0.85 }}>
                          {billing.billedPercentage?.toFixed?.(1) || billing.billedPercentage || 0}%
                          من الإجمالي
                        </Typography>
                      </CardContent>
                    </Card>
                            ...
                            <Box key={i} sx={{ display: 'flex', alignItems: 'center', gap: 1.5 }}>
                              <Box
                                sx={{
                                  ...
                                  height: 14,
                                  borderRadius: '50%',
                                  bgcolor: r.color || chartColors.category[i],
                                  flexShrink: 0,
                                }}
                              />
