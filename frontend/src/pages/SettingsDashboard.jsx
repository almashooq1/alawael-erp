        ...
        <Box sx={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between' }}>
          <Box>
            ...
            <Typography
              variant="caption"
              sx={{
                color: trend > 0 ? '#22c55e' : trend < 0 ? '#ef4444' : 'text.secondary',
                fontWeight: 600,
              }}
            >
...
const UserRow = memo(({ name, role, email, avatar, status, onEdit, onDelete, delay = 0 }) => {
  const statusColor =
    status === 'active' ? '#22c55e' : status === 'inactive' ? '#94a3b8' : '#f59e0b';
  ...
  return (
    ...
    >
      <Glass sx={{ p: 2, mb: 1.5, display: 'flex', alignItems: 'center', gap: 2 }}>
        <Avatar
          src={avatar}
          sx={{ width: 44, height: 44, background: 'linear-gradient(135deg,#8b5cf6,#6366f1)' }}
        >
          {name[0]}
        </Avatar>
        <Box sx={{ flex: 1 }}>
        ...
        <Chip
          label={statusLabel}
          size="small"
          sx={{ background: statusColor + '22', color: statusColor, fontWeight: 700 }}
        />
        <Tooltip title="تعديل">
...
const BackupRow = memo(({ name, date, size, type, status, delay = 0 }) => {
  const statusMap = {
    success: { color: '#22c55e', label: 'ناجح' },
    failed: { color: '#ef4444', label: 'فشل' },
    running: { color: '#f59e0b', label: 'جارٍ' },
  };
    ...
    >
      <Glass sx={{ p: 2, mb: 1.5, display: 'flex', alignItems: 'center', gap: 2 }}>
        ...
        <Chip
          label={s.label}
          size="small"
          sx={{ background: s.color + '22', color: s.color, fontWeight: 700 }}
        />
        <Tooltip title="تنزيل">
...
const DEMO = {
  kpis: [
    ...
    {
      title: 'الأذونات النشطة',
      value: '1,847',
      subtitle: 'قاعدة صلاحية',
      icon: <VpnKey />,
      gradient: 'linear-gradient(135deg,#f59e0b,#d97706)',
      trend: -1,
    },
    {
      title: 'الجلسات المتزامنة',
      value: '37',
      subtitle: 'اتصال حالي',
      icon: <Wifi />,
  ...
  backups: [
    {
      name: 'نسخة كاملة - مارس 2026',
      date: '30/03/2026 02:00',
      size: '2.3 GB',
      type: 'full',
      status: 'success',
    },
    ...
    {
      name: 'نسخة كاملة - فبراير 2026',
      date: '28/02/2026 02:00',
      size: '2.1 GB',
      type: 'full',
      status: 'success',
    },
    {
      name: 'نسخة تدريجية',
      date: '27/03/2026 14:00',
      size: '154 MB',
      type: 'incremental',
      status: 'failed',
    },
    {
      name: 'نسخة كاملة - يناير 2026',
      date: '31/01/2026 02:00',
      size: '1.9 GB',
      type: 'full',
      status: 'success',
    },
...
export default function SettingsDashboard() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      setLoading(true);
      try {
        const [settingsRes, statsRes, usersRes] = await Promise.allSettled([
          apiClient.get('/system-settings'),
          apiClient.get('/dashboard/stats'),
          apiClient.get('/users', { params: { limit: 20, is_active: true } }),
        ]);

        const settings =
          settingsRes.status === 'fulfilled' ? settingsRes.value.data?.data || {} : {};
        const stats = statsRes.status === 'fulfilled' ? statsRes.value.data?.data || {} : {};
        const usersArr = usersRes.status === 'fulfilled' ? usersRes.value.data?.data || [] : [];

        // Hydrate general settings from API
        if (settings.general) {
          if (settings.general.systemName) setHospitalName(settings.general.systemName);
        ...
        if (settings.appearance) {
          if (settings.appearance.compactMode !== undefined)
            setCompactMode(settings.appearance.compactMode);
          if (settings.appearance.primaryColor) setAccentColor(settings.appearance.primaryColor);
        }
        if (settings.notifications) {
          if (settings.notifications.emailEnabled !== undefined)
            setEmailNotif(settings.notifications.emailEnabled);
          if (settings.notifications.smsEnabled !== undefined)
            setSmsNotif(settings.notifications.smsEnabled);
          if (settings.notifications.pushEnabled !== undefined)
            setPushNotif(settings.notifications.pushEnabled);
        }
        if (settings.security) {
          if (settings.security.twoFactorAuth !== undefined)
            setTwoFactor(settings.security.twoFactorAuth);
        }

        // Map real users to display format
        const mappedUsers = usersArr.map(u => ({
          name: u.full_name || u.name || 'مستخدم',
          role: u.role_label || u.role || 'مستخدم',
          email: u.email || '',
          avatar: u.avatar_url || '',
        ...
        setData({
          ...DEMO,
          users: mappedUsers.length > 0 ? mappedUsers : DEMO.users,
          kpis: DEMO.kpis.map((k, i) => {
            if (i === 0) return { ...k, value: String(stats.kpis?.users?.active ?? k.value) };
            if (i === 4) return { ...k, value: String(stats.kpis?.users?.total ?? k.value) };
            return k;
          }),
        });
      } catch {
  ...
  const handleSave = useCallback(async () => {
    ...
    try {
      ...
      setSnack({ open: true, msg: 'فشل الحفظ — تحقق من الاتصال', severity: 'error' });
    } finally {
      setSaving(false);
      setSaveDialog(false);
    }
  ...
  const handleBackup = useCallback(() => {
    ...
    const iv = setInterval(() => {
      setBackupProgress(p => {
        if (p >= 100) {
          clearInterval(iv);
          setTimeout(() => setBackupDialog(false), 800);
          setSnack({ open: true, msg: 'تم إنشاء النسخة الاحتياطية بنجاح ✓', severity: 'success' });
          return 100;
        }
        return p + 8;
      });
  ...
  if (loading || !data)
    return (
      ...
      >
        ...
        >
          <Avatar sx={{ width: 64, height: 64, background: GRADIENT }}>
  ...
  return (
    <Box sx={{ minHeight: '100vh', background: bg, direction: 'rtl', p: { xs: 2, md: 3 } }}>
        ...
        >
          <Box sx={{ display: 'flex', alignItems: 'center', gap: 2 }}>
            ...
            <Box>
              ...
              <Typography variant="caption" color="text.secondary">
                ضبط وتكوين منصة الأوائل الطبية · {hospitalName}
              </Typography>
            </Box>
          </Box>
              ...
              <Grid item xs={12} md={6}>
                <Glass sx={{ p: 3 }}>
                  ...
                  <TextField
                    ...
                    defaultValue="info@alawael-hospital.sa"
                    size="small"
                    sx={{ mb: 2 }}
                  />
                  <TextField
              ...
              <Grid item xs={12} md={6}>
                <Glass sx={{ p: 3 }}>
                  ...
                  <SettingRow
                    ...
                    label="تحديث تلقائي"
                    description="تحديث البيانات كل 60 ثانية"
                  >
                    <Switch
                      checked={autoRefresh}
                  ...
                  <SettingRow
                    ...
                    label="وضع الصيانة"
                    description="إيقاف الوصول مؤقتاً"
                  >
                    <Switch
                ...
                <Glass sx={{ p: 3 }}>
                  ...
                  >
                    ...
                    <Typography variant="body2" color="#22c55e" fontWeight={600}>
                      جميع الخدمات تعمل بشكل طبيعي · وقت التشغيل: 99.98% هذا الشهر
                    </Typography>
                  </Box>
                </Glass>
              </Grid>
                ...
                <Glass sx={{ p: 3 }}>
                  ...
                  >
                    <Typography variant="subtitle1" fontWeight={700}>
                      إدارة المستخدمين ({data.users.length})
                    </Typography>
                    <Button
                      variant="contained"
                      startIcon={<AddCircle />}
                  ...
                  {data.users.map((u, i) => (
                    <UserRow
                      ...
                      delay={i * 0.07}
                      onEdit={() =>
                        setSnack({ open: true, msg: `تعديل: ${u.name}`, severity: 'info' })
                      }
                      onDelete={() =>
                        setSnack({ open: true, msg: `تم حذف: ${u.name}`, severity: 'warning' })
                      }
                    />
                  ))}
                </Glass>
                  ...
                  {[
                    ...
                    <Box key={i} sx={{ mb: 1.5 }}>
                      <Box sx={{ display: 'flex', justifyContent: 'space-between', mb: 0.5 }}>
                        ...
                        <Typography variant="caption" fontWeight={700} sx={{ color: r.color }}>
                          {r.count}
                        </Typography>
                      </Box>
                      <LinearProgress
                        variant="determinate"
                        value={(r.count / 78) * 100}
                        sx={{
                          height: 6,
                          borderRadius: 3,
                          bgcolor: isDark ? 'rgba(255,255,255,0.06)' : 'rgba(0,0,0,0.05)',
                          '& .MuiLinearProgress-bar': { background: r.color, borderRadius: 3 },
                        }}
                      />
                    </Box>
                  ))}
              ...
              <Grid item xs={12} md={6}>
                <Glass sx={{ p: 3 }}>
                  ...
                  <SettingRow
                    ...
                    label="الوضع المظلم"
                    description="تبديل بين الفاتح والداكن"
                  >
                    <Switch checked={isDark} color="primary" />
                  </SettingRow>
                  <SettingRow
                    icon={<FormatSize sx={{ fontSize: 18 }} />}
                    label={`حجم الخط: ${fontSize}px`}
                    description="الحجم الافتراضي للنصوص"
                  >
                    <Box sx={{ width: 160 }}>
                      <Slider
                        value={fontSize}
                  ...
                  <SettingRow
                    ...
                    label={`نصف قطر الحواف: ${borderRadius}px`}
                    description="استدارة العناصر"
                  >
                    <Box sx={{ width: 160 }}>
                      <Slider
                        value={borderRadius}
                  ...
                  <SettingRow
                    ...
                    label={`سرعة الحركة: ${animSpeed}s`}
                    description="سرعة الانتقالات"
                  >
                    <Box sx={{ width: 160 }}>
                      <Slider
                        value={animSpeed}
                        onChange={(_, v) => setAnimSpeed(v)}
                        min={0.1}
                        max={1}
                        step={0.1}
                        size="small"
                  ...
                  <SettingRow
                    ...
                    label="إظهار الصور الشخصية"
                    description="صور المستخدمين"
                  >
                    <Switch
                      checked={showAvatars}
                ...
                <Glass sx={{ p: 3 }}>
                  ...
                  >
                    <Typography
                      variant="body2"
                      fontWeight={600}
                      sx={{ fontSize: fontSize, color: accentColor }}
                    >
                      مثال على النص الرئيسي
                    </Typography>
                    <Typography
                      variant="caption"
                      color="text.secondary"
                  ...
                  <Box sx={{ mt: 2, display: 'flex', gap: 2, alignItems: 'center' }}>
                    ...
                    <Chip
                      label={compactMode ? 'مفعّل' : 'معطّل'}
                      size="small"
                      sx={{
                        background: compactMode ? '#22c55e22' : '#94a3b822',
                        color: compactMode ? '#22c55e' : '#94a3b8',
                        fontWeight: 700,
                      }}
                    />
                  </Box>
              ...
              <Grid item xs={12} md={6}>
                <Glass sx={{ p: 3 }}>
                  ...
                  <SettingRow
                    ...
                    description="رسائل تلقائية للأحداث المهمة"
                  ...
                  <SettingRow
                    icon={<Sms sx={{ fontSize: 18 }} />}
                    label="إشعارات الرسائل النصية"
                    description="SMS للحالات الطارئة"
                    gradient="linear-gradient(135deg,#22c55e,#16a34a)"
                  >
                    <Switch
                      checked={smsNotif}
                  ...
                  <SettingRow
                    ...
                    description="Push notifications في المتصفح"
                  ...
                  <SettingRow
                    icon={<NotificationsOff sx={{ fontSize: 18 }} />}
                    label="الإشعارات العاجلة فقط"
                    description="تجاهل الإشعارات الاعتيادية"
                  ...
                  <SettingRow
                    ...
                    label="صوت الإشعارات"
                    description="تنبيه صوتي عند الاستلام"
                    gradient="linear-gradient(135deg,#8b5cf6,#6366f1)"
                  >
                    <Switch
                      checked={notifSound}
                  ...
                  <SettingRow
                    icon={<Schedule sx={{ fontSize: 18 }} />}
                    label="ملخص يومي"
                    description="تقرير يومي على البريد الإلكتروني"
                    gradient="linear-gradient(135deg,#06b6d4,#0891b2)"
                  >
                    <Switch
                  ...
                  {[
                    ...
                    >
                      <Box sx={{ display: 'flex', alignItems: 'center', gap: 1.5 }}>
                        <Box
                          sx={{ width: 8, height: 8, borderRadius: '50%', background: s.color }}
                        />
              ...
              <Grid item xs={12} md={6}>
                <Glass sx={{ p: 3 }}>
                  ...
                  <SettingRow
                    ...
                    description="طبقة حماية إضافية لجميع الحسابات"
                  >
                    <Switch
                      checked={twoFactor}
                  ...
                  <SettingRow
                    ...
                    description="AES-256 للبيانات الحساسة"
                  >
                    <Switch
                      checked={encryptData}
                  ...
                  <SettingRow
                    icon={<Schedule sx={{ fontSize: 18 }} />}
                    label={`مهلة الجلسة: ${sessionTimeout} دقيقة`}
                    description="تسجيل الخروج تلقائياً"
                  >
                    <Box sx={{ width: 160 }}>
                      <Slider
                        value={sessionTimeout}
                        onChange={(_, v) => setSessionTimeout(v)}
                        min={5}
                        max={120}
                        step={5}
                        size="small"
                  ...
                  <SettingRow
                    ...
                    label={`محاولات تسجيل الدخول: ${loginAttempts}`}
                    description="حظر بعد عدد المحاولات"
                  >
                    <Box sx={{ width: 160 }}>
                      <Slider
                        value={loginAttempts}
                        onChange={(_, v) => setLoginAttempts(v)}
                        min={3}
                        max={10}
                        step={1}
                        size="small"
                ...
                <Glass sx={{ p: 3, mb: 2 }}>
                  ...
                  {[
                    ...
                    { value: 'medium', label: 'متوسطة', desc: '8 أحرف + رقم', color: '#06b6d4' },
                    {
                      value: 'strong',
                      label: 'قوية',
                      desc: '10 أحرف + رمز + حرف كبير',
                      color: '#22c55e',
                    },
                    {
                      value: 'enterprise',
                      label: 'مؤسسية',
                      desc: '14 حرف + انتهاء صلاحية 90 يوم',
                      color: '#6366f1',
                    },
                  ].map(p => (
                    <motion.div key={p.value} whileHover={{ x: -4 }} whileTap={{ scale: 0.98 }}>
                      <Box
                        onClick={() => setPasswordPolicy(p.value)}
                        sx={{
                          p: 1.5,
                          mb: 1,
                          borderRadius: 2,
                      ...
                      >
                        <Box>
                          <Typography
                            variant="body2"
                            fontWeight={700}
                            sx={{ color: passwordPolicy === p.value ? p.color : 'text.primary' }}
                          >
                            {p.label}
                          </Typography>
                          <Typography variant="caption" color="text.secondary">
                        ...
                        {passwordPolicy === p.value && (
                          <CheckCircle sx={{ color: p.color, fontSize: 20 }} />
                        )}
                      </Box>
                    </motion.div>
                  ))}
              ...
              <Grid item xs={12} md={4}>
                <Glass sx={{ p: 3, mb: 2 }}>
                  ...
                  <SettingRow
                    ...
                    label="نسخة كاملة أسبوعية"
                    description="كل أحد 02:00 صباحاً"
                  >
                    <Switch defaultChecked color="primary" size="small" />
                  </SettingRow>
                  <SettingRow
                    icon={<CloudUpload sx={{ fontSize: 16 }} />}
                    label="نسخة تدريجية يومية"
                    description="كل يوم 14:00"
                  >
                    <Switch defaultChecked color="primary" size="small" />
                  </SettingRow>
                  <SettingRow
                ...
                <Glass sx={{ p: 3 }}>
                  ...
                  {[
                    ...
                    <Box key={i} sx={{ mb: 2 }}>
                      <Box sx={{ display: 'flex', justifyContent: 'space-between', mb: 0.5 }}>
                        ...
                        <Typography variant="caption" sx={{ color: s.color }}>
                          {s.used} / {s.total} GB
                        </Typography>
                      </Box>
                      <LinearProgress
                        variant="determinate"
                        value={(s.used / s.total) * 100}
                        sx={{
                          height: 8,
                          borderRadius: 4,
                          bgcolor: isDark ? 'rgba(255,255,255,0.08)' : 'rgba(0,0,0,0.06)',
                  ...
                  >
                    <Typography variant="caption" color="#06b6d4" fontWeight={700}>
                      إجمالي المستخدم: 104.2 GB من 260 GB
                    </Typography>
                  </Box>
                </Glass>
              </Grid>
      ...
      <Dialog
        ...
        fullWidth
        PaperProps={{
          sx: { borderRadius: 3, background: isDark ? '#1e293b' : '#fff', direction: 'rtl' },
        }}
      >
        ...
        <DialogContent>
          <Typography variant="body2" color="text.secondary">
            سيتم تطبيق جميع التغييرات على النظام. هل تريد المتابعة؟
          </Typography>
          {saving && <LinearProgress sx={{ mt: 2, borderRadius: 2 }} />}
        </DialogContent>
        <DialogActions sx={{ p: 2, gap: 1 }}>
          <Button onClick={() => setSaveDialog(false)} variant="outlined" sx={{ borderRadius: 2 }}>
            إلغاء
          </Button>
          <Button
            onClick={handleSave}
      ...
      >
        ...
        <DialogContent sx={{ display: 'flex', flexDirection: 'column', gap: 2, pt: 2 }}>
          <Grid container spacing={2}>
            <Grid item xs={6}>
              <TextField fullWidth label="الاسم الكامل" size="small" />
            </Grid>
            <Grid item xs={6}>
            ...
            <Grid item xs={6}>
              <TextField fullWidth label="كلمة المرور المؤقتة" size="small" type="password" />
            </Grid>
            <Grid item xs={12}>
              <FormControl fullWidth size="small">
        ...
        <DialogActions sx={{ p: 2, gap: 1 }}>
          ...
          <Button
            onClick={() => {
              ...
              setSnack({ open: true, msg: 'تم إضافة المستخدم بنجاح ✓', severity: 'success' });
            }}
            variant="contained"
            sx={{ background: GRADIENT, borderRadius: 2 }}
          >
      ...
      >
        ...
        <DialogContent>
          <Typography variant="body2" color="text.secondary" mb={2}>
            جارٍ حفظ النسخة الاحتياطية الكاملة للنظام...
          </Typography>
          <LinearProgress
            variant="determinate"
          ...
          <Typography
            variant="caption"
            sx={{ color: '#06b6d4', fontWeight: 700, mt: 1, display: 'block', textAlign: 'center' }}
          >
