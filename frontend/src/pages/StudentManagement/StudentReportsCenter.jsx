...
const ScoreGauge = ({ score, label, size = 80 }) => {
  const color =
    score >= 75 ? statusColors.success : score >= 50 ? statusColors.warning : statusColors.error;
  return (
...
export default function StudentReportsCenter() {
  ...
  const loadData = useCallback(async () => {
    ...
    try {
      ...
      if (summaryRes?.data?.data || summaryRes?.data)
        setReportsSummary(summaryRes?.data?.data || summaryRes?.data);
      if (analyticsRes?.data?.data || analyticsRes?.data)
        setAnalytics(analyticsRes?.data?.data || analyticsRes?.data);
      if (schedulesRes?.data?.data || schedulesRes?.data)
  ...
  return (
    <DashboardErrorBoundary>
      <Container maxWidth="xl" sx={{ py: 3 }}>
        ...
        <Box
          sx={{
            background: gradients.primary || 'linear-gradient(135deg, #1976d2 0%, #0d47a1 100%)',
            borderRadius: 3,
            p: 3,
            mb: 4,
            color: 'white',
        ...
        <Grid container spacing={2} sx={{ mb: 4 }}>
          <Grid item xs={6} sm={4} md={2}>
            <StatCard
              label="إجمالي الطلاب"
              value={overview.totalStudents || 234}
              color="#1976d2"
              icon={<PeopleIcon />}
              subtitle="طالب مسجل"
            />
          ...
          <Grid item xs={6} sm={4} md={2}>
            <StatCard
              label="نشطون"
              value={overview.activeStudents || 186}
              color="#2e7d32"
              icon={<AttendanceIcon />}
              subtitle="طالب فعال"
            />
          ...
          <Grid item xs={6} sm={4} md={2}>
            <StatCard
              label="معدل الحضور"
              value={`${overview.avgAttendance || 87}%`}
              color="#0288d1"
              icon={<TrendUpIcon />}
              subtitle="المتوسط العام"
            />
          </Grid>
          <Grid item xs={6} sm={4} md={2}>
            <StatCard
              label="إنجازات الشهر"
              value={achievements.milestonesThisMonth}
              color="#7b1fa2"
              icon={<StarIcon />}
              subtitle={`${achievements.badgesThisMonth} شارة`}
            />
          </Grid>
          <Grid item xs={6} sm={4} md={2}>
            <StatCard
              label="تغطية IEP"
              value={`${iepData.coverage}%`}
              color="#ed6c02"
              icon={<CustomIcon />}
              subtitle={`${iepData.studentsWithIEP} طالب`}
            />
          </Grid>
          <Grid item xs={6} sm={4} md={2}>
            <StatCard
              label="طلاب في خطر"
              value={overview.studentsAtRisk || riskDistribution.high}
              color="#d32f2f"
              icon={<BehaviorIcon />}
        ...
        {activeTab === 0 && (
          <>
                  ...
                  <Grid item xs={12} sm={6} md={4} lg={3} key={report.id}>
                    <Card
                      sx={{
                        height: '100%',
                        borderRadius: 2,
                        borderLeft: `4px solid ${REPORT_COLORS[report.id] || '#666'}`,
                    ...
                    >
                      <CardContent>
                        ...
                        >
                          <Box sx={{ display: 'flex', alignItems: 'center', gap: 1 }}>
                            <Box sx={{ color: REPORT_COLORS[report.id] }}>
                      ...
                      <CardActions sx={{ pt: 0, px: 2, pb: 2 }}>
                        <Button
                          ...
                          startIcon={<ViewIcon />}
                          sx={{
                            borderColor: REPORT_COLORS[report.id],
                            color: REPORT_COLORS[report.id],
                          }}
                        >
                          عرض التقرير
                        </Button>
            ...
            <Grid container spacing={2} sx={{ mb: 4 }}>
              {availableReports
                .filter(r => r.type === 'center' || r.type === 'multi')
                .map(report => (
                  <Grid item xs={12} sm={6} md={4} lg={3} key={report.id}>
                    <Card
                      sx={{
                    ...
                    >
                      <CardContent>
                        ...
                        >
                          ...
                          <Chip
                            label={report.type === 'center' ? 'مركز' : 'مقارنة'}
                            size="small"
                            variant="outlined"
                          />
                        </Box>
            ...
            >
              <SectionHeader
                icon={<CustomIcon sx={{ fontSize: 32, color: '#455a64' }} />}
                title="بناء تقرير مخصص"
              ...
              <Box sx={{ display: 'flex', gap: 1 }}>
                ...
                <Button
                  variant="outlined"
                  onClick={() => setSelectedSections(CUSTOM_SECTIONS.map(s => s.id))}
                >
                  تحديد الكل
                </Button>
          ...
          <>
            ...
            <Grid container spacing={3} sx={{ mb: 3 }}>
                    ...
                    <BarChart data={ageDistribution}>
                      ...
                      <Bar dataKey="count" name="عدد الطلاب" fill="#1976d2" radius={[4, 4, 0, 0]}>
                        {ageDistribution.map((_, i) => (
                          <Cell
                            key={i}
                            fill={chartColors.category[i % chartColors.category.length]}
                          />
                        ))}
                      </Bar>
                    </BarChart>
                ...
                <Paper sx={{ p: 3, borderRadius: 3, height: '100%' }}>
                  ...
                  <ResponsiveContainer width="100%" height={250}>
                    <PieChart>
                      ...
                      >
                        {[statusColors.error, statusColors.warning, statusColors.success].map(
                          (c, i) => (
                            <Cell key={i} fill={c} />
                          )
                        )}
              ...
              <Grid item xs={12} md={4}>
                <Paper sx={{ p: 3, borderRadius: 3, height: '100%' }}>
                  <SectionHeader
                    icon={<StarIcon sx={{ color: '#7b1fa2' }} />}
                    title="نسبة السلوك الإيجابي"
                  />
                  <Box sx={{ textAlign: 'center', py: 2 }}>
                    <ScoreGauge
                      score={behaviorData.ratio}
                      label="نسبة السلوك الإيجابي"
                      size={120}
                    />
                    <Grid container spacing={2} sx={{ mt: 2 }}>
                      <Grid item xs={6}>
                        <Typography variant="h5" fontWeight={700} color="success.main">
                          {behaviorData.totalPositive}
                        </Typography>
                        <Typography variant="caption">سلوك إيجابي</Typography>
                      </Grid>
                      <Grid item xs={6}>
                        <Typography variant="h5" fontWeight={700} color="error.main">
                          {behaviorData.totalNegative}
                        </Typography>
            ...
            <Grid container spacing={3} sx={{ mb: 3 }}>
              ...
              <Grid item xs={12} md={8}>
                <Paper sx={{ p: 3, borderRadius: 3 }}>
                  <SectionHeader
                    icon={<AnalyticsIcon color="secondary" />}
                    title="تحليل التسجيل في البرامج"
                  ...
                  {(analytics?.programEnrollment || []).length > 0 ? (
                    ...
                    <Box sx={{ textAlign: 'center', py: 4 }}>
                      <Typography color="text.secondary">
                        سيتم عرض تحليل البرامج عند توفر البيانات
                      </Typography>
                    </Box>
                  )}
                </Paper>
              ...
              <Grid item xs={12} md={4}>
                <Paper sx={{ p: 3, borderRadius: 3, height: '100%' }}>
                  <SectionHeader
                    icon={<CustomIcon sx={{ color: '#ed6c02' }} />}
                    title="تغطية خطط التدخل (IEP)"
                  />
                  <Box sx={{ textAlign: 'center', py: 3 }}>
                    <ScoreGauge score={iepData.coverage} label="نسبة التغطية" size={130} />
                    <Box sx={{ mt: 2, p: 2, bgcolor: '#f5f5f5', borderRadius: 2 }}>
                      <Typography variant="body2">
                        <strong>{iepData.studentsWithIEP}</strong> طالب لديهم خطة IEP من أصل{' '}
                        <strong>{iepData.totalActive}</strong> طالب نشط
                      </Typography>
                    </Box>
                    {iepData.coverage < 80 && (
            ...
            >
              <SectionHeader
                icon={<StarIcon sx={{ color: '#f57f17' }} />}
                title="إنجازات الشهر الحالي"
              />
              <Grid container spacing={3}>
                <Grid item xs={12} sm={6}>
                  <Box sx={{ textAlign: 'center', p: 2 }}>
                    <Typography variant="h2" fontWeight={700} color="#f57f17">
                      {achievements.milestonesThisMonth}
                    </Typography>
                    <Typography variant="h6">إنجاز ومحطة تقدم</Typography>
                    <Typography variant="body2" color="text.secondary">
                      محطات تقدم جديدة تم تحقيقها هذا الشهر
                    </Typography>
                  </Box>
                </Grid>
                <Grid item xs={12} sm={6}>
                  <Box sx={{ textAlign: 'center', p: 2 }}>
                    <Typography variant="h2" fontWeight={700} color="#7b1fa2">
                      {achievements.badgesThisMonth}
                    </Typography>
                    <Typography variant="h6">شارة ومكافأة</Typography>
                    <Typography variant="body2" color="text.secondary">
                      شارات ومكافآت تم منحها للطلاب
                    </Typography>
                  </Box>
                </Grid>
              </Grid>
        ...
        {activeTab === 2 && (
          <>
              ...
              <Grid container spacing={2} sx={{ mb: 3 }}>
                <Grid item xs={6} sm={3}>
                  ...
                  >
                    ...
                    <Typography variant="h4" fontWeight={700} color="#1976d2">
                      {subStats?.subscriptions?.active || 0}
                    </Typography>
                ...
                <Grid item xs={6} sm={3}>
                  ...
                  >
                    ...
                    <Typography variant="h4" fontWeight={700} color="#2e7d32">
                      {subStats?.deliveries?.last7d?.success || 0}
                    </Typography>
                ...
                <Grid item xs={6} sm={3}>
                  ...
                  >
                    ...
                    <Typography variant="h4" fontWeight={700} color="#25d366">
                      {subStats?.byChannel?.whatsapp || 0}
                    </Typography>
                      ...
                      {subscriptions.map(sub => (
                        <React.Fragment key={sub._id}>
                          ...
                          >
                            <TableCell>
                              <IconButton
                                size="small"
                                onClick={() =>
                                  setExpandedSub(expandedSub === sub._id ? null : sub._id)
                                }
                              >
                            ...
                            <TableCell>
                              ...
                              <Typography variant="caption" color="text.secondary">
                                {reportTypeLabels[sub.reportType] || sub.reportType}
                              </Typography>
                            </TableCell>
                            <TableCell>
                              <Chip
                            ...
                            <TableCell>
                              <Box sx={{ display: 'flex', gap: 0.5 }}>
                                {sub.channels?.email?.enabled && (
                                  <Tooltip
                                    title={`بريد: ${sub.channels.email.recipients?.length || 0} مستلم`}
                                  >
                                    <Chip
                                      label={<EmailIcon sx={{ fontSize: 16 }} />}
                                      size="small"
                                ...
                                {sub.channels?.whatsapp?.enabled && (
                                  <Tooltip
                                    title={`واتساب: ${sub.channels.whatsapp.recipients?.length || 0} مستلم`}
                                  >
                                    <Chip
                                      label={
                              ...
                              {sub.lastExecutedAt ? (
                                <Box>
                                  ...
                                  {sub.lastExecutionStatus && (
                                    <Chip
                                      label={
                                        sub.lastExecutionStatus === 'success'
                                          ? 'ناجح'
                                          : sub.lastExecutionStatus === 'partial'
                                            ? 'جزئي'
                                            : 'فشل'
                                      }
                                      size="small"
                            ...
                            <TableCell align="center">
                              <Box sx={{ display: 'flex', justifyContent: 'center', gap: 0.5 }}>
                                <Tooltip title={sub.status === 'active' ? 'إيقاف مؤقت' : 'استئناف'}>
                                  <IconButton
                                    size="small"
                                    color={sub.status === 'active' ? 'warning' : 'success'}
                          ...
                          <TableRow>
                            <TableCell
                              colSpan={8}
                              sx={{ p: 0, border: expandedSub === sub._id ? undefined : 'none' }}
                            >
                                ...
                                <Box sx={{ p: 2, bgcolor: '#fafafa' }}>
                                  <Grid container spacing={2}>
                                    <Grid item xs={12} sm={4}>
                                      ...
                                      <Typography variant="body2">
                                        الوقت: {sub.scheduledTime || '08:00'}
                                      </Typography>
                                      <Typography variant="body2">
                                        مرات التنفيذ: {sub.executionCount || 0}
                                      </Typography>
                                      {sub.lastError && (
            ...
            >
              <SectionHeader
                icon={<NotifIcon sx={{ fontSize: 28, color: '#1976d2' }} />}
              ...
              <Grid container spacing={2}>
                {[
                  ...
                  },
                  {
                    type: 'parent',
                    freq: 'quarterly',
                    title: 'تقرير ولي الأمر الفصلي',
                    desc: 'تقرير مفصل لأولياء الأمور',
                    icon: <FamilyIcon />,
                    color: '#ed6c02',
                  },
                  {
                    type: 'comprehensive',
                    freq: 'semi-annual',
                    title: 'مراجعة IEP نصف سنوية',
                    desc: 'مراجعة خطط التدخل الفردية',
                    icon: <CustomIcon />,
                    color: '#0288d1',
                  },
                  {
                ...
                <Grid item xs={12} sm={6} md={4}>
                  <Card sx={{ borderRadius: 2, height: '100%' }}>
                    <CardContent>
                      ...
                      <Typography variant="body2" color="text.secondary" sx={{ mb: 2 }}>
                        تصدير بيانات جميع الطلاب النشطين بصيغة CSV مع دعم اللغة العربية (يعمل مع
                        Excel)
                      </Typography>
            ...
            <Grid container spacing={2}>
                  ...
                  <Select
                    ...
                    onChange={e =>
                      setNewSub(prev => ({
                        ...
                        reportTitle: reportTypeLabels[e.target.value] || prev.reportTitle,
                      }))
                    }
                  >
              ...
              <Grid item xs={12} sm={6}>
                <TextField
                  ...
                  size="small"
                  label="عنوان التقرير"
                  value={newSub.reportTitle}
                  onChange={e => setNewSub(prev => ({ ...prev, reportTitle: e.target.value }))}
                />
              </Grid>
              ...
              {newSub.frequency === 'weekly' && (
                <Grid item xs={12} sm={4}>
                  <FormControl fullWidth size="small">
                    ...
                    <Select
                      value={newSub.scheduledDayOfWeek}
                      label="يوم الإرسال"
                      onChange={e =>
                        setNewSub(prev => ({ ...prev, scheduledDayOfWeek: e.target.value }))
                      }
                    >
              ...
              <Grid item xs={12}>
                <Box
                  sx={{
                    p: 2,
                    border: '1px solid',
                    borderColor: newSub.channels.email.enabled ? '#1976d2' : 'divider',
                    borderRadius: 2,
                    bgcolor: newSub.channels.email.enabled ? '#f3f8ff' : 'transparent',
                  }}
                >
                  <Box
                    sx={{
                      ...
                      alignItems: 'center',
                      justifyContent: 'space-between',
                      mb: newSub.channels.email.enabled ? 2 : 0,
                    }}
                  >
                    <Box sx={{ display: 'flex', alignItems: 'center', gap: 1 }}>
                    ...
                    <Switch
                      checked={newSub.channels.email.enabled}
                      onChange={e =>
                        setNewSub(prev => ({
                          ...prev,
                          channels: {
                            ...
                            <TextField
                              ...
                              onChange={e => {
                                ...
                                setNewSub(prev => ({
                                  ...
                                  channels: {
                                    ...prev.channels,
                                    email: { ...prev.channels.email, recipients: recs },
                                  },
                                }));
                              }}
                            />
                          ...
                          <Grid item xs={1}>
                            {i > 0 && (
                              <IconButton
                                ...
                                color="error"
                                onClick={() => {
                                  const recs = newSub.channels.email.recipients.filter(
                                    (_, idx) => idx !== i
                                  );
                                  setNewSub(prev => ({
                                    ...prev,
                          ...
                          setNewSub(prev => ({
                            ...
                            channels: {
                              ...
                              email: {
                                ...prev.channels.email,
                                recipients: [
                                  ...prev.channels.email.recipients,
              ...
              <Grid item xs={12}>
                <Box
                  sx={{
                    p: 2,
                    border: '1px solid',
                    borderColor: newSub.channels.whatsapp.enabled ? '#25d366' : 'divider',
                    borderRadius: 2,
                    bgcolor: newSub.channels.whatsapp.enabled ? '#f0faf0' : 'transparent',
                  }}
                >
                  <Box
                    sx={{
                      ...
                      alignItems: 'center',
                      justifyContent: 'space-between',
                      mb: newSub.channels.whatsapp.enabled ? 2 : 0,
                    }}
                  >
                    <Box sx={{ display: 'flex', alignItems: 'center', gap: 1 }}>
                      <WhatsAppIcon
                        sx={{ color: newSub.channels.whatsapp.enabled ? '#25d366' : '#999' }}
                      />
                      <Typography fontWeight={600}>الواتساب</Typography>
                    </Box>
                    <Switch
                      checked={newSub.channels.whatsapp.enabled}
                      color="success"
                      onChange={e =>
                        setNewSub(prev => ({
                          ...prev,
                          channels: {
                            ...prev.channels,
                            whatsapp: { ...prev.channels.whatsapp, enabled: e.target.checked },
                          },
                        }))
                      }
                    />
                            ...
                            <TextField
                              ...
                              onChange={e => {
                                ...
                                setNewSub(prev => ({
                                  ...
                                  channels: {
                                    ...prev.channels,
                                    whatsapp: { ...prev.channels.whatsapp, recipients: recs },
                                  },
                                }));
                              }}
                            />
                          ...
                          <Grid item xs={1}>
                            {i > 0 && (
                              <IconButton
                                ...
                                color="error"
                                onClick={() => {
                                  const recs = newSub.channels.whatsapp.recipients.filter(
                                    (_, idx) => idx !== i
                                  );
                                  setNewSub(prev => ({
                                    ...prev,
                          ...
                          setNewSub(prev => ({
                            ...
                            channels: {
                              ...
                              whatsapp: {
                                ...
                                recipients: [
                                  ...prev.channels.whatsapp.recipients,
                                  { phone: '', name: '', role: 'admin' },
                                ],
                              },
                            },
                          }))
                        ...
                        <TableCell>
                          <Box sx={{ display: 'flex', gap: 0.5 }}>
                            {log.deliveries?.map((d, i) => (
                              <Tooltip
                                key={i}
                                title={`${d.channel === 'email' ? 'بريد' : 'واتساب'}: ${d.recipient} — ${d.status === 'sent' ? 'تم' : 'فشل'}`}
                              >
                                <Chip
                                  size="small"
                                  icon={
