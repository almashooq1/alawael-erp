...
const DISABILITY_COLORS = {
  intellectual: chartColors.category[0],
  autism: chartColors.category[1],
  cerebral_palsy: chartColors.category[2],
  down_syndrome: chartColors.category[6],
  hearing: statusColors.warning,
  visual: chartColors.category[4],
  physical: chartColors.category[5],
  speech: statusColors.cyan,
  learning: chartColors.category[7],
  multiple: neutralColors.fallback,
};
...
const DATA_RECENT = [
  ...
  {
    id: 2,
    name: 'نورة عبدالله الغامدي',
    disability: 'cerebral_palsy',
    severity: 'severe',
    status: 'active',
    programs: 4,
  ...
  {
    id: 4,
    name: 'ريما محمد الحربي',
    disability: 'learning',
    severity: 'mild',
    status: 'active',
    programs: 2,
    enrollDate: '2026-01-25',
  },
  {
    id: 5,
    name: 'طلال ناصر العتيبي',
    disability: 'hearing',
    severity: 'moderate',
    status: 'active',
    programs: 3,
...
export default function StudentsDashboard() {
  ...
  const [byDisability, setByDisability] = useState(DATA_BY_DISABILITY);
  const [bySeverity, setBySeverity] = useState(DATA_BY_SEVERITY);
  const [enrollTrend, setEnrollTrend] = useState(DATA_ENROLLMENT_TREND);
  const [statusDist, setStatusDist] = useState(DATA_STATUS_DIST);
  const [programsDist, setProgramsDist] = useState(DATA_PROGRAMS_DIST);
  const [recentList, setRecentList] = useState(DATA_RECENT);
  ...
  const loadDashboard = useCallback(async () => {
    ...
    try {
      const [statsRes, studentsRes, analyticsRes] = await Promise.all([
        studentManagementService.getStatistics().catch(err => {
          logger.warn('Students: statistics fetch', err);
          return null;
        }),
        studentManagementService.getStudents({ limit: 200 }).catch(err => {
          logger.warn('Students: list fetch', err);
          return null;
        }),
        studentManagementService.getDashboardAnalytics('default').catch(err => {
          logger.warn('Students: analytics fetch', err);
          return null;
        }),
      ]);
      ...
      if (Array.isArray(students) && students.length > 0) {
        ...
        const progArr = Object.entries(progMap)
        ...
        const enrollArr = Object.entries(enrollByMonth)
          .slice(-6)
          .map(([month, enrolled]) => ({
            month,
            enrolled,
            graduated: Math.round(enrolled * 0.15),
            withdrawn: Math.round(enrolled * 0.05),
          }));
      ...
      <Container maxWidth="xl" sx={{ py: 3 }}>
        ...
        >
          ...
          <Button
            variant="contained"
            color="inherit"
            sx={{ color: brandColors.tealGreen, fontWeight: 600 }}
        ...
        <Grid container spacing={2} sx={{ mb: 4 }}>
          ...
          <Grid item xs={12} sm={6} md={3} lg={2}>
            <ModuleKPICard
              ...
              subtitle={`${Math.round((stats.activeStudents / Math.max(stats.totalStudents, 1)) * 100)}% من الإجمالي`}
              icon={<ActiveIcon />}
              color="success"
            />
          </Grid>
          ...
          <Grid item xs={12} sm={6} md={3} lg={2}>
            <ModuleKPICard
              title="متخرجون هذا العام"
              value={stats.graduatedThisYear}
              subtitle="إنجاز الخطة"
              icon={<TrendUpIcon />}
              color="secondary"
            />
          </Grid>
            ...
            <Paper sx={{ p: 3, borderRadius: 3, height: '100%' }}>
              ...
              {enrollTrend.length > 0 ? (
                <ResponsiveContainer
                  ...
                  height={280}
                  role="img"
                  aria-label="رسم بياني لحركة التسجيل الشهرية"
                >
            ...
            <Paper sx={{ p: 3, borderRadius: 3, height: '100%' }}>
              ...
              {bySeverity.length > 0 ? (
                <ResponsiveContainer
                  ...
                  height={280}
                  role="img"
                  aria-label="رسم بياني حسب درجة الإعاقة"
                >
                  <PieChart>
                    <Pie
                      data={bySeverity}
            ...
            <Paper sx={{ p: 3, borderRadius: 3 }}>
              ...
              {byDisability.length > 0 ? (
                <ResponsiveContainer
                  ...
                  height={300}
                  role="img"
                  aria-label="رسم بياني حسب نوع الإعاقة"
                >
                  <PieChart>
                    <Pie
                      data={byDisability}
            ...
            <Paper sx={{ p: 3, borderRadius: 3 }}>
              ...
              {statusDist.length > 0 ? (
                <ResponsiveContainer
                  ...
                  height={300}
                  role="img"
                  aria-label="رسم بياني لحالة الطلاب"
                >
                  <PieChart>
                    <Pie
                      data={statusDist}
            ...
            <Paper sx={{ p: 3, borderRadius: 3 }}>
              ...
              {programsDist.length > 0 ? (
                <ResponsiveContainer
                  ...
                  height={300}
                  role="img"
                  aria-label="رسم بياني للبرامج الأكثر تسجيلاً"
                >
        ...
        <Grid container spacing={3} sx={{ mb: 3 }}>
          ...
          <Grid item xs={12} sm={6} md={3}>
            <Paper sx={{ p: 3, borderRadius: 3, textAlign: 'center', height: '100%' }}>
              ...
              <Box sx={{ position: 'relative', display: 'inline-flex', my: 2 }}>
                ...
                >
                  <Typography
                    variant="h4"
                    fontWeight={700}
                    color={(analytics?.iep?.coverage || 72) >= 80 ? 'success.main' : 'warning.main'}
                  >
              ...
              <Typography variant="body2" color="text.secondary">
                {analytics?.iep?.studentsWithIEP || 134} طالب من{' '}
                {analytics?.iep?.totalActive || 186}
              </Typography>
              {(analytics?.iep?.coverage || 72) < 80 && (
                <Alert
                  severity="warning"
                  sx={{ mt: 1, textAlign: 'right', '& .MuiAlert-message': { fontSize: '0.75rem' } }}
                >
              ...
              <Box sx={{ position: 'relative', display: 'inline-flex', my: 2 }}>
                ...
                >
                  <Typography
                    variant="h4"
                    fontWeight={700}
                    color={(analytics?.behavior?.ratio || 84) >= 70 ? 'success.main' : 'error.main'}
                  >
              ...
              <Grid container spacing={1}>
                <Grid item xs={6}>
                  <Typography variant="h6" fontWeight={600} color="success.main">
                    {analytics?.behavior?.totalPositive || 456}
                  </Typography>
                  <Typography variant="caption">إيجابي</Typography>
                </Grid>
                <Grid item xs={6}>
                  <Typography variant="h6" fontWeight={600} color="error.main">
                    {analytics?.behavior?.totalNegative || 89}
                  </Typography>
          ...
          <Grid item xs={12} sm={6} md={3}>
            ...
            >
              ...
              <Typography variant="h3" fontWeight={700} color="#f57f17">
                {analytics?.achievements?.milestonesThisMonth || 34}
              </Typography>
              ...
              <Typography variant="h4" fontWeight={700} color="#7b1fa2">
                {analytics?.achievements?.badgesThisMonth || 22}
              </Typography>
              <Typography variant="body2" fontWeight={500}>
                شارة ومكافأة
              </Typography>
          ...
          <Grid container spacing={2}>
            {[
              ...
              },
              {
                ...
                icon: <ExportIcon />,
                color: '#00695c',
                path: '/student-reports/periodic',
              },
              {
                label: 'المقارنة',
                icon: <ExportIcon />,
                color: '#5d4037',
                path: '/student-reports/comparison',
              },
            ].map((item, idx) => (
        ...
        <Paper sx={{ p: 3, borderRadius: 3 }}>
          ...
          >
            ...
            <Button size="small" onClick={() => navigate('/student-management')}>
              عرض الكل
            </Button>
          </Box>
          <TableContainer>
              ...
              <TableBody>
                {recentList.map(s => (
                  <TableRow key={s.id} hover>
                    ...
                    <TableCell>
                      <Chip
                        label={DISABILITY_LABELS[s.disability] || s.disability}
                        size="small"
                        sx={{
                          bgcolor: DISABILITY_COLORS[s.disability] || neutralColors.fallback,
                          color: 'white',
                          fontSize: '0.7rem',
                        }}
                      />
                    ...
                    <TableCell>
                      <Chip
                        label={SEVERITY_MAP[s.severity]?.label || s.severity}
                        size="small"
                        sx={{
                          bgcolor: SEVERITY_MAP[s.severity]?.color || neutralColors.inactive,
                          color: 'white',
                        }}
                      />
                    </TableCell>
