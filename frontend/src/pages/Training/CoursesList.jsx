...
export default function CoursesList() {
  ...
  const handleSave = async () => {
    setSaving(true);
    try {
      if (mode === 'edit') await updateCourse(selectedId, form);
  ...
  return (
    <Box sx={{ p: { xs: 2, md: 3 } }}>
      ...
      {loading ? (
        ...
        >
          <School sx={{ fontSize: 64, color: theme.palette.grey[300], mb: 2 }} />
            ...
            <TableBody>
              {list.map((c, idx) => (
                <TableRow key={c._id || idx}>
                  <TableCell>
                    <Typography fontWeight={600}>{c.courseCode}</Typography>
                  </TableCell>
      ...
      <Dialog open={dialogOpen} onClose={() => setDialogOpen(false)} maxWidth="sm" fullWidth>
        ...
        <DialogActions>
          ...
          <Button
            variant="contained"
            onClick={handleSave}
            disabled={saving || !form.courseCode.trim() || !form.titleAr.trim()}
          >
      ...
      <Dialog open={!!confirmDelete} onClose={() => setConfirmDelete(null)}>
        ...
        <DialogContent>
          <Typography>هل أنت متأكد من حذف هذه الدورة؟</Typography>
        </DialogContent>
        <DialogActions>
