...
export default function TrainingPlans() {
  ...
  const handleSave = async () => {
    setSaving(true);
    try {
      if (mode === 'edit') await updatePlan(selectedId, form);
  ...
  return (
    <Box sx={{ p: { xs: 2, md: 3 } }}>
      ...
      {loading ? (
        ...
        >
          <EventNote sx={{ fontSize: 64, color: theme.palette.grey[300], mb: 2 }} />
          <Typography color="text.secondary">لا توجد خطط تدريب</Typography>
            ...
            <TableBody>
              {list.map((p, idx) => (
                <TableRow key={p._id || idx}>
                  <TableCell>
                    <Typography fontWeight={600}>{p.planCode}</Typography>
                  </TableCell>
      ...
      <Dialog open={dialogOpen} onClose={() => setDialogOpen(false)} maxWidth="sm" fullWidth>
        ...
        <DialogActions>
          ...
          <Button
            variant="contained"
            onClick={handleSave}
            disabled={saving || !form.planCode.trim() || !form.titleAr.trim()}
          >
