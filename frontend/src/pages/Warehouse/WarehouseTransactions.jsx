            ...
            <TableBody>
              {txs.map((tx, i) => (
                <TableRow key={tx._id || i}>
                  ...
                  <TableCell>
                    <Chip label={TX_TYPE[tx.type] || tx.type} size="small" variant="outlined" />
                  </TableCell>
                  <TableCell>{tx.items?.length || 0}</TableCell>
                  <TableCell>{(tx.totalValue || 0).toLocaleString('ar-SA')} ر.س</TableCell>
                  <TableCell>
                    <Chip
                      label={TX_STATUS[tx.status] || tx.status}
                      size="small"
                  ...
                  <TableCell align="center">
                    {tx.status === 'pending' && (
                      <Tooltip title="اعتماد">
                        <IconButton size="small" color="info" onClick={() => handleApprove(tx._id)}>
