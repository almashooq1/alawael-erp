...
const _MSG_TYPE_ICONS = {
  text: <Chat fontSize="small" />,
  file: <InsertDriveFile fontSize="small" />,
  image: <Image fontSize="small" />,
  audio: <MicNone fontSize="small" />,
  ...
  return (
    ...
    >
      ...
      <CardContent sx={{ textAlign: 'center', py: 2 }}>
        <Avatar sx={{ bgcolor: `${color}15`, color, mx: 'auto', mb: 1, width: 48, height: 48 }}>
          {icon}
        </Avatar>
        ...
        <Box sx={{ display: 'flex', alignItems: 'center', gap: 2 }}>
          ...
          <Box>
            ...
            <Typography variant="body2" color="text.secondary">
              تواصل فوري بين الموظفين — مجموعات ومحادثات خاصة
            </Typography>
          </Box>
        </Box>
                      ...
                      primary={
                        <Box sx={{ display: 'flex', alignItems: 'center', gap: 0.5 }}>
                          <Typography variant="subtitle2" noWrap sx={{ flex: 1 }}>
                            {conv.displayName || conv.name || 'محادثة'}
                          </Typography>
                          {conv.otherUser && (
                            <Circle
                              sx={{
                                fontSize: 10,
                                color: STATUS_COLORS[conv.otherUser.status] || '#9e9e9e',
                              }}
                            />
                          )}
                        </Box>
            ...
            >
              {selectedConv ? (
                <>
                  ...
                  >
                    <Avatar
                      sx={{ bgcolor: selectedConv.type === 'direct' ? '#1976d2' : '#2e7d32' }}
                    >
                      {selectedConv.type === 'direct' ? <Person /> : <Group />}
                    </Avatar>
                    <Box sx={{ flex: 1 }}>
                      <Typography variant="subtitle1" fontWeight="bold">
                        {selectedConv.displayName || selectedConv.name}
                      </Typography>
                ...
                >
                  ...
                  <Typography variant="body2" color="text.disabled">
                    أو أنشئ محادثة جديدة من الأزرار أعلاه
                  </Typography>
                </Box>
              )}
            </Paper>
      ...
      {tab === 1 && (
        ...
        >
          <Typography variant="h6" fontWeight={700} sx={{ mb: 2 }}>
            المستخدمون المتصلون ({dashboard?.onlineUsers?.length || 0})
          </Typography>
          <Grid container spacing={2}>
              ...
              <TableBody>
                {users.map(user => (
                  <TableRow key={user.id} hover>
                    ...
                    <TableCell>
                      <Button size="small" onClick={() => handleCreateDirect(user.id)}>
                        محادثة
                      </Button>
                    </TableCell>
                  </TableRow>
        ...
        >
          <Box sx={{ display: 'flex', gap: 1, mb: 2 }}>
            <TextField
              size="small"
              fullWidth
              placeholder="ابحث في جميع الرسائل..."
              value={searchQuery}
            ...
            />
            <Button variant="contained" sx={{ borderRadius: '12px' }} onClick={handleSearch}>
              بحث
            </Button>
          </Box>
                ...
                <ListItem key={user.id} button onClick={() => handleCreateDirect(user.id)}>
                  <ListItemAvatar>
                    <Badge
                      ...
                      variant="dot"
                      sx={{
                        '& .MuiBadge-dot': { bgcolor: STATUS_COLORS[user.status] || '#9e9e9e' },
                      }}
                    >
                      <Avatar sx={{ bgcolor: '#1976d2' }}>{user.name?.[0]}</Avatar>
                    </Badge>
