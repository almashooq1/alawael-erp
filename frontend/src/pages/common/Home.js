    ...
    >
      ...
      <div
        className={`px-5 py-4 flex items-center justify-between ${
          isGreen
            ? 'bg-gradient-to-l from-green-50/80 to-transparent'
            : 'bg-gradient-to-l from-amber-50/80 to-transparent'
        }`}
      >
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-[0.68rem] text-slate-400 mt-0.5 m-0">
              {group.items.length} نظام فرعي
            </p>
          </div>
        </div>
        <span
        ...
        {displayItems.map(item => (
          ...
          >
            <span className="flex items-center gap-2.5">
              <span
                className={`${isGreen ? 'text-green-600' : 'text-amber-600'} transition-transform duration-200 group-hover/item:scale-110`}
              >
                {item.icon}
              </span>
              {item.title}
            ...
            <OpenInNewIcon
              sx={{ fontSize: 13 }}
              className="opacity-0 group-hover/item:opacity-40 transition-opacity duration-200"
            />
          </button>
        ))}
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-xs text-slate-400 m-0 mt-0.5">
                {moduleGroups.reduce((t, g) => t + g.items.length, 0)} نظام فرعي في{' '}
                {moduleGroups.length} مجموعات
              </p>
            </div>
          </div>
        </div>
