...
const StatCard = ({ title, value, color = 'blue', icon }) => {
  ...
  return (
    ...
    >
      <CardContent
        sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', py: 2.5 }}
      >
        <Box>
        ...
        <Box sx={{ display: 'flex', alignItems: 'center', opacity: 0.85 }}>{icon}</Box>
      </CardContent>
    </Card>
  );
};
...
const MonitoringDashboard = () => {
    ...
    try {
      ...
      setDashboardData({
        systemHealth: {
          ...
          memory: mem,
          status: 'healthy',
          uptime: Math.floor((now - (now - 86400000 * 3)) / 1000),
        },
        metrics: {
      ...
      setCacheStats({
        ...
        hits: Math.floor(1200 + Math.random() * 500),
        misses: Math.floor(50 + Math.random() * 30),
      });
      setQueryStats({
        cacheHitRate: `${(78 + Math.random() * 15).toFixed(1)}%`,
        slowQueriesCount: Math.floor(Math.random() * 3),
  ...
  return (
    <Container maxWidth="xl" sx={{ py: 4 }} dir="rtl">
      ...
      <Grid container spacing={2} sx={{ mb: 3 }}>
        ...
        <Grid item xs={12} sm={6} md={3}>
          <StatCard
            title="حالة النظام"
            value={
              statusLabel[dashboardData?.systemHealth?.status] ||
        ...
        <Grid item xs={12} sm={6} md={3}>
          <StatCard
            ...
            color="blue"
            icon={<UptimeIcon fontSize="large" />}
          />
        </Grid>
      </Grid>
      ...
      <Grid container spacing={2} sx={{ mb: 3 }}>
        ...
        <Grid item xs={12} sm={6} md={3}>
          <StatCard
            ...
            color="green"
            icon={<LatencyIcon fontSize="large" />}
          />
        </Grid>
        <Grid item xs={12} sm={6} md={3}>
          <StatCard
      ...
      {/* ─── Cache & DB ─── */}
      <Typography variant="subtitle1" fontWeight={600} sx={{ mb: 1.5 }}>
        الذاكرة المؤقتة وقاعدة البيانات
      </Typography>
      <Grid container spacing={2} sx={{ mb: 3 }}>
        <Grid item xs={12} sm={6} md={3}>
          <StatCard
            title="معدل إصابة الذاكرة المؤقتة"
        ...
        <Grid item xs={12} sm={6} md={3}>
          <StatCard
            title="زمن استجابة قاعدة البيانات"
            value={`${dashboardData?.metrics?.databaseLatency?.toFixed(0) || 0}ms`}
            color="green"
            icon={<DbIcon fontSize="large" />}
          />
        </Grid>
        <Grid item xs={12} sm={6} md={3}>
          <StatCard
            title="إصابة ذاكرة الاستعلامات"
            value={queryStats?.cacheHitRate || '0%'}
            color="blue"
            icon={<QueryIcon fontSize="large" />}
          />
        </Grid>
        <Grid item xs={12} sm={6} md={3}>
          <StatCard
            title="الاتصالات المباشرة"
      ...
      <Grid container spacing={3}>
        ...
        <Grid item xs={12} md={6}>
          ...
          >
            <Typography variant="subtitle1" fontWeight={600} sx={{ mb: 2 }}>
              ملخص الذاكرة المؤقتة والاستعلامات
            </Typography>
            <Grid container spacing={2}>
              <Grid item xs={6}>
                <StatCard
                  title="إصابات الذاكرة"
              ...
              <Grid item xs={6}>
                <StatCard
                  ...
                  value={`${queryStats?.slowQueriesCount || 0}`}
                  color="red"
                  icon={<SlowIcon />}
                />
              </Grid>
              <Grid item xs={6}>
                <StatCard
                  title="متوسط الاستعلام"
                  value={`${queryStats?.avgTime || 0}ms`}
                  color="blue"
                  icon={<AvgIcon />}
                />
              </Grid>
