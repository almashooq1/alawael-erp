...
const DATA_RECENT_AUDITS = [
  {
    id: 1,
    area: 'قسم العلاج الطبيعي',
    auditor: 'أ. سلطان المقبل',
    date: '2026-03-08',
    score: 95,
    status: 'passed',
  },
...
export default function QualityDashboard() {
  ...
  const [complianceTrend, _setComplianceTrend] = useState(DATA_COMPLIANCE_TREND);
  const [issuesByCategory, _setIssuesByCategory] = useState(DATA_ISSUES_BY_CATEGORY);
  const [auditResults, setAuditResults] = useState(DATA_AUDIT_RESULTS);
  const [correctiveActions, _setCorrectiveActions] = useState(DATA_CORRECTIVE_ACTIONS);
  const [recentAudits, setRecentAudits] = useState(DATA_RECENT_AUDITS);

  const loadDashboard = useCallback(async () => {
    setLoading(true);
    try {
      const [qualityRes, auditsRes, casesRes, ticketsRes] = await Promise.all([
        qualityService.getQualityRecords().catch(err => {
          logger.warn('Quality: records fetch', err);
          return null;
        }),
        qualityService.getAudits().catch(err => {
          logger.warn('Quality: audits fetch', err);
          return null;
        }),
        qualityService.getCases().catch(err => {
          logger.warn('Quality: cases fetch', err);
          return null;
        }),
        qualityService.getTickets().catch(err => {
          logger.warn('Quality: tickets fetch', err);
          return null;
        }),
      ]);
      ...
      if (Array.isArray(audits) && audits.length > 0) {
        ...
        setStats(prev => ({
          ...
          passedAudits: passed,
          failedAudits: failed,
          openIssues: Array.isArray(cases)
            ? cases.filter(c => c.status !== 'resolved' && c.status !== 'closed').length
            : prev.openIssues,
          resolvedIssues: Array.isArray(cases)
            ? cases.filter(c => c.status === 'resolved' || c.status === 'closed').length
            : prev.resolvedIssues,
          openTickets: Array.isArray(tickets)
          ...
          <Box sx={{ display: 'flex', alignItems: 'center', gap: 2 }}>
            ...
            <Box>
              ...
              <Typography variant="body2" sx={{ opacity: 0.9 }}>
                مؤشرات الجودة ونتائج التدقيق والإجراءات التصحيحية
              </Typography>
            </Box>
          </Box>
          <Button
            variant="contained"
            color="inherit"
            sx={{ color: brandColors.primaryEnd, fontWeight: 600 }}
        ...
        <Grid container spacing={2} sx={{ mb: 4 }}>
          ...
          <Grid item xs={12} sm={6} md={3} lg={2}>
            <ModuleKPICard
              ...
              subtitle={`${stats.passedAudits} ناجحة`}
              icon={<AuditIcon />}
              color="success"
            />
          </Grid>
          ...
          <Grid item xs={12} sm={6} md={3} lg={2}>
            <ModuleKPICard
              ...
              subtitle={`${stats.resolvedIssues} تم حلها`}
              icon={<TicketIcon />}
              color="warning"
            />
          </Grid>
            ...
            <Paper sx={{ p: 3, borderRadius: 3, height: '100%' }}>
              ...
              {complianceTrend.length > 0 ? (
                <ResponsiveContainer
                  ...
                  height={280}
                  role="img"
                  aria-label="رسم بياني لاتجاه درجة الامتثال"
                >
            ...
            <Paper sx={{ p: 3, borderRadius: 3, height: '100%' }}>
              ...
              {auditResults.length > 0 ? (
                <ResponsiveContainer
                  ...
                  height={280}
                  role="img"
                  aria-label="رسم بياني لنتائج التدقيق"
                >
                  <PieChart>
                    <Pie
                      data={auditResults}
            ...
            <Paper sx={{ p: 3, borderRadius: 3 }}>
              ...
              {issuesByCategory.length > 0 ? (
                <ResponsiveContainer
                  ...
                  height={260}
                  role="img"
                  aria-label="رسم بياني للمخالفات حسب الفئة"
                >
            ...
            <Paper sx={{ p: 3, borderRadius: 3 }}>
              ...
              {correctiveActions.length > 0 ? (
                <ResponsiveContainer
                  ...
                  height={260}
                  role="img"
                  aria-label="رسم بياني للإجراءات التصحيحية"
                >
