...
import {
  ...
  automationApi,
  getDashboard,
} from '../../services/documentProPhase7Service';
...
function WatermarkTab() {
  ...
  const load = useCallback(async () => {
    setLoading(true);
    try {
      const [p, s] = await Promise.all([watermarkApi.getProfiles(), watermarkApi.getStats()]);
      setProfiles(p.data?.profiles || []);
      setStats(s.data);
    } catch (e) {
      logger.error('WatermarkTab load', e);
    }
    setLoading(false);
  }, []);
      ...
      <Paper sx={{ p: 2, mb: 3, borderRadius: 2 }}>
        ...
        <Stack direction="row" spacing={2} alignItems="center">
          <TextField
            ...
            sx={{ minWidth: 260 }}
          />
          <Button variant="outlined" startIcon={<SecurityIcon />} onClick={handleVerify}>
            تحقق
          </Button>
        </Stack>
        {verifyResult && (
            ...
            <Card sx={cardSx}>
              <CardContent>
                ...
                <Typography variant="body2" color="text.secondary" mb={1}>
                  الشفافية: {Math.round((p.settings?.opacity || 0.3) * 100)}% | الزاوية:{' '}
                  {p.settings?.rotation || 0}°
                </Typography>
                <Chip
...
function ImportExportTab() {
  ...
  const load = useCallback(async () => {
    setLoading(true);
    try {
      const [j, s] = await Promise.all([importExportApi.getJobs(), importExportApi.getStats()]);
      setJobs(j.data?.jobs || []);
      setStats(s.data);
    } catch (e) {
      logger.error('ImportExportTab load', e);
    }
    setLoading(false);
  }, []);
  ...
  const handleImport = async () => {
    try {
      let parsedData;
      try {
        parsedData = JSON.parse(importForm.data);
      } catch {
        parsedData = importForm.data;
      }
    ...
    <Box>
      ...
      <Grid container spacing={2} mb={3}>
        {[
          ...
          {
            label: 'تصدير',
            val: stats?.exportJobs || 0,
            color: '#4caf50',
            icon: <ExportFileIcon />,
          },
          {
            label: 'استيراد',
            val: stats?.importJobs || 0,
            color: '#ff9800',
            icon: <ImportFileIcon />,
          },
          {
            label: 'مكتملة',
            val: stats?.completedJobs || 0,
            color: '#9c27b0',
            icon: <CheckIcon />,
          },
        ].map((s, i) => (
          ...
          <TableBody>
            {jobs.map(j => (
              <TableRow key={j._id} hover>
                <TableCell>
                  <Chip
                    label={j.type === 'export' ? 'تصدير' : 'استيراد'}
                    color={j.type === 'export' ? 'success' : 'warning'}
                    size="small"
                  />
                </TableCell>
                ...
                <TableCell>
                  {j.status === 'processing' && (
                    <Tooltip title="إلغاء">
                      <IconButton size="small" color="error" onClick={() => handleCancel(j._id)}>
                        <StopIcon />
                      </IconButton>
                    </Tooltip>
                  )}
      ...
      <Dialog open={exportOpen} onClose={() => setExportOpen(false)} maxWidth="sm" fullWidth>
        ...
        <DialogContent>
          ...
          <TextField
            ...
            multiline
            rows={3}
            label="معرفات المستندات (مفصولة بفاصلة)"
            value={exportForm.documentIds}
            onChange={e => setExportForm(p => ({ ...p, documentIds: e.target.value }))}
          />
        </DialogContent>
        <DialogActions>
      ...
      <Dialog open={importOpen} onClose={() => setImportOpen(false)} maxWidth="sm" fullWidth>
        ...
        <DialogContent>
          ...
          <TextField
            ...
            onChange={e => setImportForm(p => ({ ...p, data: e.target.value }))}
            placeholder='[{"title": "مستند 1", ...}]'
          />
        </DialogContent>
        <DialogActions>
...
function ComplianceTab() {
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      logger.error('ComplianceTab load', e);
    }
    setLoading(false);
  }, []);
  ...
  const handleCreateRule = async () => {
    try {
      let conds;
      try {
        conds = JSON.parse(ruleForm.conditions);
      } catch {
        conds = {};
      }
  ...
  return (
    <Box>
      ...
      {/* Actions */}
      <Stack direction="row" spacing={2} mb={3}>
        <Button startIcon={<ScanIcon />} variant="contained" color="error" onClick={handleRunScan}>
          تشغيل فحص الامتثال
        </Button>
        <Button startIcon={<AddIcon />} variant="contained" onClick={() => setRuleOpen(true)}>
          إضافة قاعدة
        </Button>
        <Button startIcon={<RefreshIcon />} variant="outlined" onClick={load}>
          تحديث
      ...
      <Stack spacing={1} mb={3}>
        {alerts.slice(0, 10).map(a => (
          <Alert
            ...
            action={
              a.status === 'open' && (
                <Button size="small" color="inherit" onClick={() => handleResolve(a._id)}>
                  حل
                </Button>
              )
            }
        ...
        <DialogContent>
          <Stack spacing={2} mt={1}>
            ...
            <FormControlLabel
              control={
                <Switch
                  checked={ruleForm.autoRemediate}
                  onChange={e => setRuleForm(p => ({ ...p, autoRemediate: e.target.checked }))}
                />
              }
              label="إصلاح تلقائي"
            />
...
function KnowledgeGraphTab() {
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      logger.error('KnowledgeGraphTab load', e);
    }
    setLoading(false);
  }, []);
    ...
    <Box>
      ...
      <Grid container spacing={2} mb={3}>
        {[
          ...
          {
            label: 'الكثافة',
            val: `${((stats?.density || 0) * 100).toFixed(1)}%`,
            color: '#ff9800',
            icon: <AssessmentIcon />,
          },
        ].map((s, i) => (
      ...
      <Paper sx={{ p: 3, mb: 3, borderRadius: 2 }}>
        ...
        <Stack direction="row" spacing={2} alignItems="center" mb={2}>
          <TextField
            ...
            sx={{ minWidth: 280 }}
          />
          <Button variant="contained" startIcon={<GraphIcon />} onClick={loadDocGraph}>
            عرض الرسم
          </Button>
          <Button variant="outlined" startIcon={<AutoFixIcon />} onClick={handleAutoDiscover}>
            اكتشاف تلقائي
          </Button>
        </Stack>
        ...
        {graphData && (
          <Box>
            <Typography variant="subtitle1" fontWeight="bold" mb={1}>
              العقد المرتبطة ({graphData.nodes?.length || 0})
            </Typography>
            ...
            <Typography variant="subtitle1" fontWeight="bold" mb={1}>
              الروابط ({graphData.edges?.length || 0})
            </Typography>
            <Stack spacing={0.5} mb={2}>
              {(graphData.edges || []).slice(0, 8).map((e, i) => (
                <Chip
                  key={i}
                  label={`${e.sourceLabel || e.source} → ${e.targetLabel || e.target} (${e.type})`}
                  size="small"
                  variant="outlined"
                />
              ))}
        ...
        {impactData && (
          <Box mt={2}>
            ...
            >
              مستوى التأثير: {impactData.impactLevel || 'منخفض'} | المستندات المتأثرة:{' '}
              {impactData.affectedDocuments || 0}
            </Alert>
          </Box>
        )}
            ...
            <List dense>
              {(recommendations.recommendations || []).slice(0, 5).map((r, i) => (
                <ListItem key={i}>
                  ...
                  <ListItemText
                    primary={r.title || r.documentTitle}
                    secondary={r.reason || `درجة: ${r.score}`}
                  />
                </ListItem>
              ))}
            </List>
...
function AutomationTab() {
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      logger.error('AutomationTab load', e);
    }
    setLoading(false);
  }, []);
  ...
  return (
    <Box>
      ...
      {/* Rules */}
      <Typography variant="h6" gutterBottom>
        قواعد الأتمتة ({rules.length})
      </Typography>
      <Grid container spacing={2} mb={3}>
        {rules.map(r => (
          <Grid item xs={12} md={6} key={r._id}>
            <Card
              sx={{ ...cardSx, borderRight: `4px solid ${r.isActive ? '#4caf50' : '#bdbdbd'}` }}
            >
              <CardContent>
                <Stack direction="row" justifyContent="space-between" alignItems="center" mb={1}>
                  <Typography fontWeight="bold">{r.name}</Typography>
                  <Stack direction="row" spacing={0.5}>
                    ...
                    />
                    <Chip
                      label={`${r.actions?.length || 0} إجراء`}
                      size="small"
                      color="secondary"
                      variant="outlined"
                    />
                ...
                <Stack direction="row" spacing={1}>
                  <Chip
                    label={`نفذت ${r.executionCount || 0} مرة`}
                    size="small"
                    variant="outlined"
                  />
                  {r.lastExecution && (
                    <Chip
                      label={`آخر: ${fmtDateTime(r.lastExecution)}`}
                      size="small"
                      variant="outlined"
                    />
                  )}
              ...
              <CardActions>
                ...
                <Tooltip title="تنفيذ يدوي">
                  <IconButton color="primary" onClick={() => handleExecute(r._id)}>
                    <PlayIcon />
                  </IconButton>
                </Tooltip>
                <Tooltip title="تعديل">
              ...
              <TableRow key={e._id} hover>
                ...
                <TableCell>
                  <Chip
                    ...
                    color={
                      e.status === 'success' ? 'success' : e.status === 'failed' ? 'error' : 'info'
                    }
                    size="small"
                  />
                </TableCell>
    ...
    <Box dir="rtl" sx={{ p: { xs: 1, md: 3 } }}>
      ...
      >
        ...
        <Typography variant="body1" sx={{ opacity: 0.9 }}>
          العلامات المائية • الاستيراد والتصدير • مراقبة الامتثال • الرسم البياني المعرفي • أتمتة
          العمليات
        </Typography>
        {dashboard && (
          <Stack direction="row" spacing={3} mt={2} flexWrap="wrap">
            <Chip
              label={`${dashboard.watermark?.profiles || 0} ملف علامة مائية`}
              sx={{ bgcolor: 'rgba(255,255,255,0.2)', color: '#fff' }}
            />
            <Chip
              label={`${dashboard.importExport?.totalJobs || 0} مهمة`}
              sx={{ bgcolor: 'rgba(255,255,255,0.2)', color: '#fff' }}
            />
            <Chip
              label={`${dashboard.compliance?.score || 0}% امتثال`}
              sx={{ bgcolor: 'rgba(255,255,255,0.2)', color: '#fff' }}
            />
            <Chip
              label={`${dashboard.graph?.nodes || 0} عقدة`}
              sx={{ bgcolor: 'rgba(255,255,255,0.2)', color: '#fff' }}
            />
            <Chip
              label={`${dashboard.automation?.rules || 0} قاعدة أتمتة`}
              sx={{ bgcolor: 'rgba(255,255,255,0.2)', color: '#fff' }}
            />
          </Stack>
        )}
