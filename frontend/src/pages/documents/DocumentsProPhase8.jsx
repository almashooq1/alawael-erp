...
import {
  ...
  backupApi,
  getDashboard,
} from '../../services/documentProPhase8Service';
  ...
  return (
    <Box>
      ...
      <Stack direction="row" spacing={2} mb={3} flexWrap="wrap">
        ...
        <Button startIcon={<LanguageIcon />} variant="outlined" onClick={() => setDetectOpen(true)}>
          كشف اللغة
        </Button>
          ...
          <TableBody>
            {jobs.map(j => (
              <TableRow key={j._id} hover>
                ...
                <TableCell>
                  <Chip
                    label={languages[j.sourceLanguage]?.name || j.sourceLanguage}
                    size="small"
                  />
                </TableCell>
                <TableCell>
                  <Chip
                    label={languages[j.targetLanguage]?.name || j.targetLanguage}
                    size="small"
                    color="primary"
                  />
                </TableCell>
        ...
        <DialogContent>
          ...
          {detectResult && (
            <Alert severity="info" sx={{ mt: 2 }}>
              اللغة: <strong>{detectResult.languageName}</strong> ({detectResult.nativeName}) |
              الثقة: {Math.round(detectResult.confidence * 100)}% | الاتجاه:{' '}
              {detectResult.direction}
            </Alert>
          )}
        </DialogContent>
        <DialogActions>
          <Button onClick={() => setDetectOpen(false)}>إغلاق</Button>
          <Button variant="contained" onClick={handleDetect}>
            كشف
          </Button>
        </DialogActions>
      </Dialog>
...
function FormsTab() {
  ...
  const handleSave = async data => {
    try {
      if (selectedTemplate) {
        await formsApi.updateTemplate(selectedTemplate._id, data);
        setSnack('تم تحديث القالب');
      } else {
        await formsApi.createTemplate(data);
        setSnack('تم إنشاء القالب');
      }
      setDesignerOpen(false);
      setSelectedTemplate(null);
      load();
    } catch (e) {
      logger.error('save template', e);
    }
  };

  const handlePublish = async id => {
    try {
      await formsApi.publishTemplate(id);
      setSnack('تم النشر');
      load();
    } catch (e) {
  ...
  const handleClone = async id => {
    try {
      await formsApi.cloneTemplate(id);
      setSnack('تم النسخ');
      load();
    } catch (e) {
          ...
          <Grid item xs={12} md={6} lg={4} key={t._id}>
            <Card sx={{ ...cardSx, borderRight: `4px solid ${t.color || '#1976d2'}` }}>
              ...
              <CardActions>
                ...
                {t.status === 'draft' && (
                  <Tooltip title="نشر">
                    <IconButton color="success" onClick={() => handlePublish(t._id)}>
                      <PublishIcon />
                    </IconButton>
                  </Tooltip>
                )}
                <Tooltip title="نسخ">
                  <IconButton onClick={() => handleClone(t._id)}>
                    <CloneIcon />
                  </IconButton>
                </Tooltip>
                <Tooltip title="حذف">
...
function ApprovalTab() {
  ...
  const handleActivate = async id => {
    try {
      await approvalApi.activateChain(id);
      setSnack('تم التفعيل');
      load();
    } catch (e) {
  ...
  const handleSubmit = async () => {
    try {
      await approvalApi.submitRequest(submitForm.chainId, {
        documentId: submitForm.documentId,
        note: submitForm.note,
      });
      setSnack('تم تقديم الطلب');
      setSubmitOpen(false);
      load();
    } catch (e) {
  ...
  const handleProcess = async (id, action) => {
    try {
      await approvalApi.processStep(id, action, '');
      setSnack('تم المعالجة');
      load();
    } catch (e) {
  ...
  return (
    <Box>
            ...
            <Card sx={cardSx}>
              <CardContent>
                <Stack direction="row" justifyContent="space-between" alignItems="center" mb={1}>
                  ...
                  <Chip
                    label={c.status === 'active' ? 'نشط' : c.status === 'draft' ? 'مسودة' : 'أرشيف'}
              ...
              <CardActions>
                {c.status === 'draft' && (
                  <Tooltip title="تفعيل">
                    <IconButton color="success" onClick={() => handleActivate(c._id)}>
                      <CheckIcon />
                    </IconButton>
                  </Tooltip>
                )}
          ...
          <TableBody>
            {requests.map(r => (
              <TableRow key={r._id} hover>
                ...
                <TableCell>
                  <Chip
                    label={reqStatusLabel[r.status] || r.status}
                    color={reqStatusColor[r.status]}
                    size="small"
                  />
                </TableCell>
                <TableCell>
                  {r.currentStep} / {r.stepResults?.length || 0}
                </TableCell>
                <TableCell>
                  <Chip
                    label={
        ...
        <DialogContent>
          <Stack spacing={2} mt={1}>
            <FormControl fullWidth>
              ...
              <Select
                value={submitForm.chainId}
                label="سلسلة الموافقات"
                onChange={e => setSubmitForm(p => ({ ...p, chainId: e.target.value }))}
              >
                {chains
            ...
            <TextField
              fullWidth
              label="معرف المستند"
              value={submitForm.documentId}
              onChange={e => setSubmitForm(p => ({ ...p, documentId: e.target.value }))}
            />
            <TextField
              fullWidth
              multiline
              ...
              onChange={e => setSubmitForm(p => ({ ...p, note: e.target.value }))}
            />
          </Stack>
        </DialogContent>
        <DialogActions>
      ...
      <ApprovalChainVisualizer
        ...
        onSave={async data => {
          try {
            if (selectedChain) await approvalApi.updateChain(selectedChain._id, data);
            else await approvalApi.createChain(data);
            setVizOpen(false);
            setSnack('تم الحفظ');
            load();
          } catch (e) {
            logger.error('save chain', e);
          }
        }}
      />
      <Snackbar
...
function SecurityTab() {
  ...
  const load = useCallback(async () => {
    ...
    try {
      const [s, l, p] = await Promise.all([
        ...
        securityApi.getAccessLogs({ limit: 15 }),
        securityApi.getDLPPolicies(),
      ]);
      setStats(s.data);
      setLogs(l.data?.logs || []);
      setPolicies(p.data?.policies || []);
    } catch (e) {
      logger.error('SecurityTab', e);
    }
    setLoading(false);
  }, []);
  useEffect(() => {
  ...
  const handleEncrypt = async () => {
    if (!docId) return;
    try {
      await securityApi.encrypt(docId, {});
      setSnack('تم التشفير');
    } catch (e) {
  ...
  const handleAutoClassify = async () => {
    if (!docId) return;
    try {
      const r = await securityApi.autoClassify(docId);
      setSnack(`تصنيف تلقائي: ${r.data?.classification?.level}`);
    } catch (e) {
  ...
  const handleScan = async () => {
    if (!scanText) return;
    try {
      const r = await securityApi.scanDLP(scanText);
    ...
    <Box>
      ...
      <Paper sx={{ p: 2, mb: 3, borderRadius: 2 }}>
        ...
        <Stack direction="row" spacing={2} alignItems="center">
          ...
          <Button
            variant="contained"
            color="error"
            startIcon={<EncryptIcon />}
            onClick={handleEncrypt}
          >
            تشفير
          </Button>
          <Button variant="outlined" startIcon={<ClassifyIcon />} onClick={handleAutoClassify}>
            تصنيف تلقائي
          </Button>
        </Stack>
      </Paper>
      ...
      <Paper sx={{ p: 2, mb: 3, borderRadius: 2 }}>
        ...
        <TextField
          ...
          sx={{ mb: 1 }}
        />
        <Button variant="contained" startIcon={<DLPIcon />} onClick={handleScan}>
          فحص
        </Button>
        {scanResult && (
          <Box mt={2}>
            <Alert
              severity={
                scanResult.shouldBlock ? 'error' : scanResult.hasViolations ? 'warning' : 'success'
              }
            >
              درجة الخطورة: {scanResult.riskScore} | الانتهاكات:{' '}
          ...
          <TableBody>
            {logs.map(l => (
              <TableRow key={l._id} hover>
                ...
                <TableCell>
                  <Chip
                    label={l.status === 'allowed' ? 'مسموح' : 'محظور'}
                    color={l.status === 'allowed' ? 'success' : 'error'}
                    size="small"
                  />
                </TableCell>
                <TableCell>
                  {l.dlpViolation ? <WarningIcon color="error" fontSize="small" /> : '—'}
                </TableCell>
...
function BackupTab() {
  ...
  const handleCreate = async () => {
    try {
      ...
      setSnack('تم إنشاء النسخة');
      setCreateOpen(false);
      setTimeout(load, 2000);
    } catch (e) {
      logger.error('create backup', e);
    }
  };
  const handleVerify = async id => {
    try {
      const r = await backupApi.verifyBackup(id);
      setSnack(`التحقق: ${r.data?.verified || 0} سليم، ${r.data?.corrupted || 0} تالف`);
    } catch (e) {
  ...
  const handleCleanup = async () => {
    try {
      const r = await backupApi.cleanup();
      setSnack(`تنظيف: ${r.data?.deletedSnapshots || 0} لقطة، ${r.data?.deletedBackups || 0} نسخة`);
    } catch (e) {
  ...
  return (
    <Box>
      ...
      <Stack direction="row" spacing={2} mb={3}>
        <Button
          startIcon={<UploadBackupIcon />}
          variant="contained"
          onClick={() => setCreateOpen(true)}
        >
          إنشاء نسخة احتياطية
        </Button>
        <Button startIcon={<DeleteIcon />} variant="outlined" color="error" onClick={handleCleanup}>
          تنظيف منتهية
        </Button>
        <Button startIcon={<RefreshIcon />} variant="outlined" onClick={load}>
          تحديث
      ...
      {stats?.compressionRatio > 0 && (
        <Alert severity="info" sx={{ mb: 2 }}>
          نسبة الضغط: {stats.compressionRatio}% | إجمالي المستندات المنسوخة: {stats.totalDocsBacked}{' '}
          | السياسات النشطة: {stats.activePolicies}
        </Alert>
      )}
    ...
    <Box dir="rtl" sx={{ p: { xs: 1, md: 3 } }}>
      ...
      >
        <Typography variant="h4" fontWeight="bold" gutterBottom>
          إدارة المستندات المتقدمة — المرحلة الثامنة
        </Typography>
        <Typography variant="body1" sx={{ opacity: 0.9 }}>
          ترجمة المستندات • النماذج الديناميكية • سلاسل الموافقات • التشفير وحماية البيانات • النسخ
          الاحتياطي والاسترداد
        </Typography>
        {dashboard && (
          <Stack direction="row" spacing={3} mt={2} flexWrap="wrap">
            <Chip
              label={`${dashboard.translation?.totalJobs || 0} ترجمة`}
              sx={{ bgcolor: 'rgba(255,255,255,0.2)', color: '#fff' }}
            />
            <Chip
              label={`${dashboard.forms?.templates || 0} قالب`}
              sx={{ bgcolor: 'rgba(255,255,255,0.2)', color: '#fff' }}
            />
            <Chip
              label={`${dashboard.approval?.totalChains || 0} سلسلة`}
              sx={{ bgcolor: 'rgba(255,255,255,0.2)', color: '#fff' }}
            />
            <Chip
              label={`${dashboard.security?.encryptedDocuments || 0} مشفر`}
              sx={{ bgcolor: 'rgba(255,255,255,0.2)', color: '#fff' }}
            />
            <Chip
              label={`${dashboard.backup?.totalBackups || 0} نسخة`}
              sx={{ bgcolor: 'rgba(255,255,255,0.2)', color: '#fff' }}
            />
          </Stack>
        )}
