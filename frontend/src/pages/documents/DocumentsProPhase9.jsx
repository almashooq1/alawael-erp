...
import {
  ...
  forensicsApi,
  getDashboard,
} from '../../services/documentProPhase9Service';
...
function LifecycleTab() {
  ...
  const handleActivate = async id => {
    try {
      await lifecycleApi.activatePolicy(id);
      setSnack('تم التفعيل');
      load();
    } catch (e) {
  ...
  const handleAssign = async () => {
    try {
      await lifecycleApi.assign(assignForm);
      setSnack('تم تعيين دورة الحياة');
      setAssignOpen(false);
      load();
    } catch (e) {
  ...
  const handleAutoTransition = async () => {
    try {
      const r = await lifecycleApi.autoTransition();
      setSnack(`تم معالجة ${r.data?.processed || 0} انتقال تلقائي`);
    } catch (e) {
      logger.error('auto', e);
    }
  };
  ...
  return (
    <Box>
      ...
      <Stack direction="row" spacing={2} mb={3} flexWrap="wrap">
        ...
        <Button startIcon={<PlayIcon />} variant="outlined" onClick={handleAutoTransition}>
          انتقال تلقائي
        </Button>
            ...
            <Card sx={cardSx}>
              <CardContent>
                <Stack direction="row" justifyContent="space-between" alignItems="center" mb={1}>
                  ...
                  <Chip
                    label={p.status === 'active' ? 'نشط' : p.status === 'draft' ? 'مسودة' : 'معلق'}
                ...
                <Stack direction="row" spacing={1} flexWrap="wrap">
                  ...
                  <Chip
                    label={`${p.defaultRetentionDays} يوم احتفاظ`}
                    size="small"
                    variant="outlined"
                  />
              ...
              <CardActions>
                {p.status === 'draft' && (
                  <Tooltip title="تفعيل">
                    <IconButton color="success" onClick={() => handleActivate(p._id)}>
                      <CheckIcon />
                    </IconButton>
                  </Tooltip>
                )}
                <Tooltip title="عرض المراحل">
                  <IconButton
                    onClick={() => {
                      setSelectedDocId('');
        ...
        <Paper sx={{ p: 2, borderRadius: 2, mb: 2 }}>
          ...
          <Stack direction="row" spacing={1} flexWrap="wrap">
            {stats.byPhase.map((p, i) => (
              <Chip key={i} label={`${p._id}: ${p.count}`} color="primary" variant="outlined" />
            ))}
          </Stack>
        </Paper>
      )}
        ...
        <DialogContent>
          <Stack spacing={2} mt={1}>
            <TextField
              fullWidth
              label="معرف المستند"
              value={assignForm.documentId}
              onChange={e => setAssignForm(p => ({ ...p, documentId: e.target.value }))}
            />
            <FormControl fullWidth>
              ...
              <Select
                ...
                label="السياسة"
                onChange={e => setAssignForm(p => ({ ...p, policyId: e.target.value }))}
              >
                {policies
...
function DigitalCertTab() {
  ...
  const load = useCallback(async () => {
    ...
    try {
      const [c, s] = await Promise.all([
        digitalCertApi.getCertificates(),
        digitalCertApi.getStats(),
      ]);
      setCerts(c.data?.certificates || []);
      setStats(s.data);
    } catch (e) {
      logger.error('DigitalCertTab', e);
    }
    setLoading(false);
  }, []);
  useEffect(() => {
  ...
  const handleGenerate = async () => {
    try {
      await digitalCertApi.generate(genForm);
      setSnack('تم إنشاء الشهادة');
      setGenOpen(false);
      load();
    } catch (e) {
  ...
  const handleRevoke = async id => {
    try {
      await digitalCertApi.revoke(id, { reason: 'إلغاء يدوي' });
      setSnack('تم إلغاء الشهادة');
      load();
    } catch (e) {
  ...
  const handleSign = async () => {
    try {
      await digitalCertApi.sign(signForm);
      setSnack('تم التوقيع');
      setSignOpen(false);
    } catch (e) {
      logger.error('sign', e);
  ...
  return (
    <Box>
      ...
      <Stack direction="row" spacing={2} mb={3}>
        <Button startIcon={<KeyIcon />} variant="contained" onClick={() => setGenOpen(true)}>
          إنشاء شهادة
        </Button>
        <Button
              ...
              <TableRow key={c._id} hover>
                ...
                <TableCell>
                  {c.status === 'active' && (
                    <>
                      <Tooltip title="إلغاء">
                        <IconButton size="small" color="error" onClick={() => handleRevoke(c._id)}>
                          <CancelIcon />
                        </IconButton>
                      </Tooltip>
                      <Tooltip title="تجديد">
                        <IconButton size="small" color="primary" onClick={() => handleRenew(c._id)}>
                          <RefreshIcon />
                        </IconButton>
                      </Tooltip>
                    </>
        ...
        <DialogContent>
          <Stack spacing={2} mt={1}>
            <TextField
              ...
              onChange={e => setGenForm(p => ({ ...p, commonName: e.target.value }))}
            />
            <TextField
              fullWidth
              label="المؤسسة"
              value={genForm.organization}
              onChange={e => setGenForm(p => ({ ...p, organization: e.target.value }))}
            />
            <TextField
              fullWidth
              label="البريد الإلكتروني"
              value={genForm.email}
              onChange={e => setGenForm(p => ({ ...p, email: e.target.value }))}
            />
          </Stack>
        </DialogContent>
        <DialogActions>
        ...
        <DialogContent>
          <Stack spacing={2} mt={1}>
            <TextField
              ...
              onChange={e => setSignForm(p => ({ ...p, documentId: e.target.value }))}
            />
            <FormControl fullWidth>
              <InputLabel>الشهادة</InputLabel>
              <Select
                value={signForm.certificateId}
                label="الشهادة"
                onChange={e => setSignForm(p => ({ ...p, certificateId: e.target.value }))}
              >
                {certs
                  .filter(c => c.status === 'active')
                  .map(c => (
                    <MenuItem key={c._id} value={c._id}>
                      {c.subject?.commonName} ({c.serialNumber?.substring(0, 10)})
                    </MenuItem>
                  ))}
              </Select>
            </FormControl>
...
function ClassificationTab() {
  ...
  const load = useCallback(async () => {
    ...
    try {
      const [m, c, s] = await Promise.all([
        classificationApi.getModels(),
        classificationApi.getClusters(),
      ...
      logger.error('ClassificationTab', e);
    }
    setLoading(false);
  }, []);
  useEffect(() => {
  ...
  const handleActivate = async id => {
    try {
      await classificationApi.activateModel(id);
      setSnack('تم التفعيل');
      load();
    } catch (e) {
      logger.error('activate', e);
  ...
  const handleTrain = async id => {
    try {
      await classificationApi.trainModel(id, {});
      setSnack('تم التدريب');
      load();
    } catch (e) {
      logger.error('train', e);
    }
  };
  ...
  return (
    <Box>
      ...
      <Stack direction="row" spacing={2} mb={3}>
        <Button startIcon={<AIIcon />} variant="contained" onClick={() => setClassifyOpen(true)}>
          تصنيف مستند
        </Button>
        <Button startIcon={<RefreshIcon />} variant="outlined" onClick={load}>
          تحديث
          ...
          <Grid item xs={12} md={6} lg={4} key={m._id}>
            <Card sx={cardSx}>
              <CardContent>
                <Stack direction="row" justifyContent="space-between" mb={1}>
                  ...
                  <Chip
                    label={m.status}
                    color={
                      m.status === 'active' ? 'success' : m.status === 'ready' ? 'info' : 'default'
                    }
                    size="small"
                  />
                </Stack>
                <Stack direction="row" spacing={1} flexWrap="wrap">
                  ...
                  <Chip
                    label={`${m.categories?.length || 0} فئة`}
                    size="small"
                    variant="outlined"
                  />
                  {m.trainingData?.accuracy && (
                    <Chip
                      label={`دقة: ${Math.round(m.trainingData.accuracy * 100)}%`}
                      size="small"
                      color="primary"
                    />
                  )}
                  <Chip
                    label={`${m.usageStats?.classifications || 0} تصنيف`}
                    size="small"
                    variant="outlined"
                  />
                </Stack>
              ...
              <CardActions>
                {m.status !== 'active' && (
                  <Tooltip title="تفعيل">
                    <IconButton color="success" onClick={() => handleActivate(m._id)}>
                      <CheckIcon />
                    </IconButton>
                  </Tooltip>
                )}
                <Tooltip title="تدريب">
                  <IconButton color="primary" onClick={() => handleTrain(m._id)}>
                    <AIIcon />
                  </IconButton>
                </Tooltip>
              </CardActions>
          ...
          <Stack direction="row" spacing={1} flexWrap="wrap" mb={2}>
            {clusters.map(c => (
              <Chip
                key={c._id}
                icon={<ClusterIcon />}
                label={`${c.nameAr || c.name} (${c.metrics?.documentCount || 0})`}
                variant="outlined"
              />
            ))}
          </Stack>
      ...
      <Dialog open={classifyOpen} onClose={() => setClassifyOpen(false)} maxWidth="sm" fullWidth>
        ...
        <DialogContent>
          <Stack spacing={2} mt={1}>
            <TextField
              fullWidth
              label="معرف المستند"
              value={classifyForm.documentId}
              onChange={e => setClassifyForm(p => ({ ...p, documentId: e.target.value }))}
            />
            <FormControl fullWidth>
              <InputLabel>النموذج</InputLabel>
              <Select
                value={classifyForm.modelId}
                label="النموذج"
                onChange={e => setClassifyForm(p => ({ ...p, modelId: e.target.value }))}
              >
                {models
            ...
            <TextField
              ...
              rows={4}
              label="النص"
              value={classifyForm.text}
              onChange={e => setClassifyForm(p => ({ ...p, text: e.target.value }))}
            />
          </Stack>
          {classifyResult && (
            <Box mt={2}>
              <Alert severity="success">
                التصنيف: <strong>{classifyResult.primaryCategory}</strong> | الثقة:{' '}
                {Math.round((classifyResult.confidence || 0) * 100)}%
              </Alert>
              {classifyResult.predictions?.map((pred, i) => (
                <Chip
                  key={i}
                  label={`${pred.categoryAr || pred.category}: ${Math.round(pred.confidence * 100)}%`}
                  sx={{ mt: 0.5, mr: 0.5 }}
                />
              ))}
            </Box>
        ...
        <DialogActions>
          ...
          <Button variant="contained" onClick={handleClassify}>
            تصنيف
          </Button>
        </DialogActions>
      </Dialog>
...
function WorkflowOrchTab() {
  ...
  const load = useCallback(async () => {
    ...
    try {
      const [d, inst, t, s] = await Promise.all([
        workflowOrchApi.getDefs(),
        workflowOrchApi.getInstances({ limit: 20 }),
        workflowOrchApi.getMyTasks(),
        workflowOrchApi.getStats(),
      ]);
      ...
      logger.error('WorkflowOrchTab', e);
    }
    setLoading(false);
  }, []);
  useEffect(() => {
  ...
  const handleActivate = async id => {
    try {
      await workflowOrchApi.activateDef(id);
      setSnack('تم التفعيل');
      load();
    } catch (e) {
      logger.error('activate', e);
  ...
  const handleClone = async id => {
    try {
      await workflowOrchApi.cloneDef(id);
      setSnack('تم النسخ');
      load();
    } catch (e) {
  ...
  const handleStart = async () => {
    try {
      await workflowOrchApi.start({ definitionId: startDefId });
      setSnack('تم البدء');
      setStartOpen(false);
      load();
    } catch (e) {
  ...
  const handleSuspend = async id => {
    try {
      await workflowOrchApi.suspend(id, {});
      setSnack('تم التعليق');
      load();
    } catch (e) {
  ...
  const handleResume = async id => {
    try {
      await workflowOrchApi.resume(id);
      setSnack('تم الاستئناف');
      load();
    } catch (e) {
      logger.error('resume', e);
    }
  };
  const handleCancel = async id => {
    try {
      await workflowOrchApi.cancel(id, {});
      setSnack('تم الإلغاء');
      load();
    } catch (e) {
  ...
  return (
    <Box>
            ...
            <Card sx={cardSx}>
              <CardContent>
                <Stack direction="row" justifyContent="space-between" mb={1}>
                  ...
                  <Chip
                    label={d.status === 'active' ? 'نشط' : 'مسودة'}
                ...
                <Stack direction="row" spacing={1}>
                  ...
                  <Chip
                    label={`${d.usageStats?.executions || 0} تنفيذ`}
                    size="small"
                    variant="outlined"
                  />
                </Stack>
              ...
              <CardActions>
                {d.status === 'draft' && (
                  <Tooltip title="تفعيل">
                    <IconButton color="success" onClick={() => handleActivate(d._id)}>
                      <CheckIcon />
                    </IconButton>
                  </Tooltip>
                )}
                ...
                <Tooltip title="نسخ">
                  <IconButton onClick={() => handleClone(d._id)}>
                    <CloneIcon />
                  </IconButton>
                </Tooltip>
              </CardActions>
      ...
      {/* Instances */}
      <Typography variant="h6" gutterBottom>
        آخر التنفيذات ({instances.length})
      </Typography>
      <TableContainer component={Paper} sx={{ borderRadius: 2 }}>
        <Table size="small">
          <TableHead>
            <TableRow sx={{ bgcolor: 'action.hover' }}>
              ...
              <TableCell>الحالة</TableCell>
              <TableCell>العقد الحالية</TableCell>
          ...
          <TableBody>
            {instances.map(inst => (
              <TableRow key={inst._id} hover>
                ...
                <TableCell>
                  <Chip
                    label={instLabels[inst.status] || inst.status}
                    color={instColors[inst.status]}
                    size="small"
                  />
                </TableCell>
                ...
                <TableCell>
                  {inst.status === 'running' && (
                    <>
                      <Tooltip title="تعليق">
                        <IconButton size="small" onClick={() => handleSuspend(inst._id)}>
                          <PauseIcon />
                        </IconButton>
                      </Tooltip>
                      <Tooltip title="إلغاء">
      ...
      <WorkflowOrchestrator
        ...
        onSave={async data => {
          try {
            if (selectedDef) await workflowOrchApi.updateDef(selectedDef._id, data);
            else await workflowOrchApi.createDef(data);
            setOrchOpen(false);
            setSnack('تم الحفظ');
            load();
          } catch (e) {
            logger.error('save def', e);
          }
        }}
      />
      <Snackbar
...
function ForensicsTab() {
  ...
  const load = useCallback(async () => {
    ...
    try {
      const [a, s] = await Promise.all([
        forensicsApi.getAlerts({ limit: 20 }),
        forensicsApi.getStats(),
      ]);
      setAlerts(a.data?.alerts || []);
      setStats(s.data);
    } catch (e) {
      logger.error('ForensicsTab', e);
    }
    setLoading(false);
  }, []);
  useEffect(() => {
  ...
  const handleIntegrityCheck = async () => {
    if (!docId) return;
    try {
      const r = await forensicsApi.checkIntegrity({
        documentId: docId,
        content: `doc-${docId}-${Date.now()}`,
      });
      setIntegrityResult(r.data);
      setSnack('تم فحص السلامة');
    } catch (e) {
      logger.error('integrity', e);
    }
  };
  const handleAnalyze = async () => {
    if (!docId) return;
    try {
      const r = await forensicsApi.analyze(docId);
  ...
  const handleUpdateAlert = async (id, status) => {
    try {
      await forensicsApi.updateAlert(id, { status });
      setSnack('تم التحديث');
      load();
    } catch (e) {
      logger.error('update alert', e);
    }
  };
    ...
    <Box>
      ...
      <Paper sx={{ p: 2, mb: 3, borderRadius: 2 }}>
        ...
        <Stack direction="row" spacing={2} alignItems="center">
          ...
          <Button variant="outlined" startIcon={<AnalysisIcon />} onClick={handleAnalyze}>
            تحليل جنائي
          </Button>
        </Stack>
        ...
        {integrityResult && (
          <Alert
            severity={integrityResult.integrityStatus === 'verified' ? 'success' : 'error'}
            sx={{ mt: 2 }}
          >
            السلامة:{' '}
            <strong>
              {integrityResult.integrityStatus === 'verified' ? 'سليم' : 'مُتلاعب به'}
            </strong>
            {integrityResult.hashes?.sha256 && (
              <Typography variant="caption" display="block" sx={{ fontFamily: 'monospace' }}>
                SHA-256: {integrityResult.hashes.sha256.substring(0, 32)}...
              </Typography>
            )}
          </Alert>
        )}
        ...
        {analysisResult && (
          <Box mt={2}>
            ...
            >
              درجة الخطورة: <strong>{analysisResult.riskScore}/100</strong> | النتائج:{' '}
          ...
          <TableBody>
            {alerts.map(a => (
              <TableRow
                key={a._id}
                hover
                sx={{ bgcolor: a.severity === 'critical' ? 'error.50' : 'inherit' }}
              >
                ...
                <TableCell>
                  <Chip
                    ...
                    size="small"
                    sx={{ bgcolor: `${sevColor[a.severity]}20`, color: sevColor[a.severity] }}
                  />
                </TableCell>
                ...
                <TableCell>
                  {a.status === 'new' && (
                    <Tooltip title="بدء تحقيق">
                      <IconButton
                        size="small"
                        onClick={() => handleUpdateAlert(a._id, 'investigating')}
                      >
                        <AnalysisIcon />
                      </IconButton>
                    </Tooltip>
                    ...
                    <>
                      <Tooltip title="تأكيد">
                        <IconButton
                          size="small"
                          color="error"
                          onClick={() => handleUpdateAlert(a._id, 'confirmed')}
                        >
                          <CheckIcon />
                        </IconButton>
                      </Tooltip>
                      <Tooltip title="إنذار كاذب">
                        <IconButton
                          size="small"
                          onClick={() => handleUpdateAlert(a._id, 'false_positive')}
                        >
                          <CancelIcon />
                        </IconButton>
                      </Tooltip>
    ...
    <Box dir="rtl" sx={{ p: { xs: 1, md: 3 } }}>
      ...
      >
        <Typography variant="h4" fontWeight="bold" gutterBottom>
          إدارة المستندات المتقدمة — المرحلة التاسعة
        </Typography>
        <Typography variant="body1" sx={{ opacity: 0.9 }}>
          دورة حياة المستند • التوقيع الرقمي بالشهادات • التصنيف الذكي بالذكاء الاصطناعي • تنسيق سير
          العمل • التحليل الجنائي
        </Typography>
        {dashboard && (
          <Stack direction="row" spacing={3} mt={2} flexWrap="wrap">
            <Chip
              label={`${dashboard.lifecycle?.totalLifecycles || 0} دورة حياة`}
              sx={{ bgcolor: 'rgba(255,255,255,0.2)', color: '#fff' }}
            />
            <Chip
              label={`${dashboard.digitalCert?.totalCertificates || 0} شهادة`}
              sx={{ bgcolor: 'rgba(255,255,255,0.2)', color: '#fff' }}
            />
            <Chip
              label={`${dashboard.classification?.totalClassifications || 0} تصنيف`}
              sx={{ bgcolor: 'rgba(255,255,255,0.2)', color: '#fff' }}
            />
            <Chip
              label={`${dashboard.workflowOrch?.running || 0} سير عمل نشط`}
              sx={{ bgcolor: 'rgba(255,255,255,0.2)', color: '#fff' }}
            />
            <Chip
              label={`${dashboard.forensics?.criticalAlerts || 0} تنبيه حرج`}
              sx={{ bgcolor: 'rgba(255,255,255,0.2)', color: '#fff' }}
            />
          </Stack>
        )}
