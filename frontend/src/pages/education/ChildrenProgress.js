...
const ChildrenProgress = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const data = await parentService.getChildrenProgress(userId);
        setProgressData(data);
        if (data?.children?.length > 0) {
          setSelectedChild(data.children[0]);
        }
      } catch (err) {
        logger.error('Failed to load children progress:', err);
        setError(err.message || 'حدث خطأ في تحميل البيانات');
        showSnackbar('حدث خطأ في تحميل بيانات التقدم', 'error');
      }
    };
    fetchData();
  }, [userId, showSnackbar]);
        ...
        <>
          ...
          <Grid container spacing={3} sx={{ mb: 4 }}>
            ...
            <Grid item xs={12} sm={6} md={4}>
              <Card>
                <CardContent sx={{ textAlign: 'center' }}>
                  <Typography
                    variant="h6"
                    sx={{ color: brandColors.accentTeal, fontWeight: 'bold' }}
                  >
            ...
            <Grid item xs={12} sm={6} md={4}>
              <Card>
                <CardContent sx={{ textAlign: 'center' }}>
                  <Typography variant="h6" sx={{ color: statusColors.info, fontWeight: 'bold' }}>
                ...
                <Grid item xs={12} md={6} key={skill.id}>
                  <Card sx={{ height: '100%' }}>
                    <CardContent>
                        ...
                        <LinearProgress
                          ...
                          sx={{
                            ...
                            '& .MuiLinearProgress-bar': {
                              background:
                                skill.progress >= 80
                                  ? `linear-gradient(90deg, ${brandColors.accentGreen} 0%, ${brandColors.accentTeal} 100%)`
                                  : skill.progress >= 50
                                    ? `linear-gradient(90deg, ${brandColors.accentRose} 0%, #fee140 100%)`
                                    : `linear-gradient(90deg, ${brandColors.accentCoral} 0%, ${brandColors.accentPink} 100%)`,
                            },
                          }}
                        />
                      </Box>
                      ...
                      <Typography
                        variant="caption"
                        sx={{ display: 'block', color: neutralColors.textSecondary, mt: 1 }}
                      >
                    ...
                    <CardContent>
                      <Box sx={{ display: 'flex', gap: 2 }}>
                        ...
                        <Box sx={{ flex: 1 }}>
                          ...
                          <Typography
                            variant="body2"
                            sx={{ color: neutralColors.textSecondary, mb: 1 }}
                          >
                            {area.description}
                          </Typography>
                          <Box sx={{ display: 'flex', gap: 1 }}>
                            <Chip
                              label={`تحسن: +${area.improvement}%`}
                              size="small"
                              color="success"
                            />
