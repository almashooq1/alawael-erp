      ...
      <Stack direction="row" justifyContent="space-between" alignItems="center" mb={3}>
        <Box>
          ...
          <Typography variant="body2" color="text.secondary">
            إدارة خطط استمرارية الأعمال والاستجابة للأزمات
          </Typography>
        </Box>
      ...
      {tab === 0 && (
        <Box>
          ...
          <Grid container spacing={2}>
              ...
              <Grid item xs={12} md={4} key={p._id}>
                <Card variant="outlined">
                  <CardContent>
                    ...
                    <Divider sx={{ my: 1 }} />
                    <Typography variant="caption">النطاق: {p.scope}</Typography>
                    <br />
                    <Typography variant="caption">الإصدار: {p.version}</Typography>
                    <br />
            ...
            {!plans.length && (
              <Grid item xs={12}>
                <Alert severity="info">لا توجد خطط BCP</Alert>
              </Grid>
            )}
          </Grid>
        </Box>
            ...
            <Table size="small">
              <TableHead>
                <TableRow>
                  ...
                  <TableCell>الأثر المالي</TableCell>
                ...
                {bias.map(b => (
                  <TableRow key={b._id} hover>
                    ...
                    <TableCell>
                      <Chip
                        size="small"
                        color={severityColors[b.criticality] || 'default'}
                        label={b.criticality}
                      />
                    </TableCell>
                    <TableCell>
                      {b.financialImpact ? `${b.financialImpact.toLocaleString()} ر.س` : '—'}
                    </TableCell>
            ...
            <Table size="small">
              <TableHead>
                <TableRow>
                  ...
                  <TableCell>الحالة</TableCell>
                  <TableCell>بدء الأزمة</TableCell>
                  <TableCell>المتأثرون</TableCell>
                </TableRow>
              </TableHead>
              <TableBody>
                {crises.map(c => (
                  <TableRow
                    ...
                    hover
                    sx={{
                      bgcolor: ['detected', 'escalated'].includes(c.status)
                        ? 'error.50'
                        : 'inherit',
                    }}
                  >
                    ...
                    <TableCell>
                      <Chip size="small" label={c.incidentType?.replace(/_/g, ' ')} />
                    </TableCell>
                    <TableCell>
                      <Chip
                        size="small"
                        color={severityColors[c.severity] || 'default'}
                        label={c.severity}
        ...
        <Box>
          ...
          <TableContainer component={Paper}>
            <Table size="small">
              <TableHead>
                <TableRow>
                  <TableCell>اسم التمرين</TableCell>
                  <TableCell>النوع</TableCell>
          ...
          <Grid container spacing={2}>
            {drPlans.map(d => (
              <Grid item xs={12} md={4} key={d._id}>
                <Card
                  variant="outlined"
                  sx={{ borderColor: d.status === 'active' ? 'success.main' : 'divider' }}
                >
                  <CardContent>
                    <Stack
                      direction="row"
                    ...
                    <Divider sx={{ my: 1 }} />
                    <Typography variant="caption">RTO: {d.rto || '—'}</Typography>
                    <br />
                    ...
                    <Chip
                      size="small"
                      color={statusColors[d.testStatus] || 'default'}
                      label={d.testStatus?.replace(/_/g, ' ') || '—'}
                      sx={{ mt: 1 }}
                    />
                  </CardContent>
                </Card>
            ...
            {!drPlans.length && (
              <Grid item xs={12}>
                <Alert severity="info">لا توجد خطط DR</Alert>
              </Grid>
            )}
          </Grid>
        </Box>
                ...
                <Grid container spacing={1}>
                  <Grid item xs={6}>
                    <TextField
                      ...
                      type="number"
                      value={form.rto || ''}
                      onChange={e => setForm({ ...form, rto: e.target.value })}
                    />
                  </Grid>
                  <Grid item xs={6}>
                    <TextField
                      ...
                      type="number"
                      value={form.rpo || ''}
                      onChange={e => setForm({ ...form, rpo: e.target.value })}
                    />
                  </Grid>
                </Grid>
              </>
            ...
            {dialog.type === 'drill' && (
              <>
                <TextField
                  ...
                  onChange={e => setForm({ ...form, drillName: e.target.value })}
                />
                <TextField
                  select
                  label="النوع"
                ...
                >
                  {['tabletop', 'walkthrough', 'simulation', 'full_scale', 'functional'].map(t => (
                    <MenuItem key={t} value={t}>
                      {t.replace(/_/g, ' ')}
                    </MenuItem>
                  ))}
