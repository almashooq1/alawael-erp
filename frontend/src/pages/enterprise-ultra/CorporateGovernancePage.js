...
export default function CorporateGovernancePage() {
  ...
  const fetchData = useCallback(async () => {
    setLoading(true);
    try {
      const [meetingsRes, committeesRes, resolutionsRes, policiesRes, reportsRes, statsRes] =
        await Promise.all([
  ...
  return (
    <Box sx={{ p: 3 }}>
      ...
      <Stack direction="row" justifyContent="space-between" alignItems="center" mb={3}>
        <Box>
          ...
          <Typography variant="body2" color="text.secondary">
            إدارة مجلس الإدارة واللجان والقرارات والسياسات
          </Typography>
        </Box>
      ...
      >
        <Tab
          label={`الاجتماعات (${meetings.length})`}
          icon={<MeetingIcon />}
          iconPosition="start"
        />
              ...
              <TableBody>
                {meetings.map(m => (
                  <TableRow key={m._id} hover>
                    ...
                    <TableCell>
                      {m.quorumMet ? <CheckIcon color="success" fontSize="small" /> : '—'}
                    </TableCell>
      ...
      {tab === 1 && (
        <Box>
          ...
          <Grid container spacing={2}>
                ...
                <Card variant="outlined">
                  <CardContent>
                    ...
                    <Typography variant="caption">
                      النوع: {c.committeeType?.replace(/_/g, ' ')}
                    </Typography>
                    <br />
                    <Typography variant="caption">الأعضاء: {c.members?.length || 0}</Typography>
                    <br />
            ...
            {!committees.length && (
              <Grid item xs={12}>
                <Alert severity="info">لا توجد لجان</Alert>
              </Grid>
            )}
          </Grid>
        </Box>
              ...
              <TableBody>
                {resolutions.map(r => (
                  <TableRow key={r._id} hover>
                    ...
                    <TableCell>
                      <Chip size="small" label={r.resolutionType?.replace(/_/g, ' ')} />
                    </TableCell>
                    <TableCell>
                      <Chip
                        size="small"
                      ...
                      {r.votingResult ? (
                        <Stack direction="row" spacing={0.5}>
                          <Chip
                            size="small"
                            color="success"
                            label={`✓ ${r.votingResult?.inFavor || 0}`}
                          />
                          <Chip
                            size="small"
                            color="error"
                            label={`✗ ${r.votingResult?.against || 0}`}
                          />
                        </Stack>
                      ) : (
                        '—'
      ...
      {tab === 3 && (
        <Box>
          ...
          <Grid container spacing={2}>
              ...
              <Grid item xs={12} md={6} key={p._id}>
                <Card variant="outlined">
                  <CardContent>
                    ...
                    <Typography variant="caption">
                      الفئة: {p.category?.replace(/_/g, ' ')}
                    </Typography>
                    <br />
                    <Typography variant="caption">الإصدار: {p.version}</Typography>
                    <br />
                    <Typography variant="caption">
                      الجهة المسؤولة: {p.owner?.department || '—'}
                    </Typography>
                    {p.nextReviewDate && (
                      <Typography
                        variant="caption"
            ...
            {!policies.length && (
              <Grid item xs={12}>
                <Alert severity="info">لا توجد سياسات</Alert>
              </Grid>
            )}
          </Grid>
        </Box>
              ...
              <TableBody>
                {reports.map(r => (
                  <TableRow key={r._id} hover>
                    <TableCell>{r.title}</TableCell>
                    <TableCell>
                      <Chip size="small" label={r.reportType?.replace(/_/g, ' ')} />
                    </TableCell>
          ...
          <Stack spacing={2} sx={{ mt: 1 }}>
            {dialog.type === 'meeting' && (
              <>
                ...
                >
                  {['regular', 'extraordinary', 'annual_general', 'special'].map(t => (
            ...
            {dialog.type === 'resolution' && (
              <>
                ...
                <TextField
                  ...
                  onChange={e => setForm({ ...form, resolutionType: e.target.value })}
                >
                  {['ordinary', 'special', 'written', 'emergency'].map(t => (
                    <MenuItem key={t} value={t}>
                      {t}
                    </MenuItem>
                  ))}
