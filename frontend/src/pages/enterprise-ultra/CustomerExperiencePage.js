...
export default function CustomerExperiencePage() {
  ...
  const fetchData = useCallback(async () => {
    setLoading(true);
    try {
      const [surveysRes, feedbackRes, complaintsRes, journeysRes, benchRes, statsRes] =
        await Promise.all([
  ...
  return (
    <Box sx={{ p: 3 }}>
      ...
      <Stack direction="row" justifyContent="space-between" alignItems="center" mb={3}>
        <Box>
          ...
          <Typography variant="body2" color="text.secondary">
            استبيانات وتحليلات رضا العملاء وإدارة الشكاوى
          </Typography>
        </Box>
      ...
      >
        ...
        <Tab
          label={`التغذية الراجعة (${feedback.length})`}
          icon={<FeedbackIcon />}
          iconPosition="start"
        />
        <Tab
          label={`الشكاوى (${complaints.length})`}
          icon={<ComplaintIcon />}
          iconPosition="start"
        />
        <Tab
          label={`رحلة العميل (${journeys.length})`}
          icon={<JourneyIcon />}
          iconPosition="start"
        />
        <Tab
          label={`المعايير (${benchmarks.length})`}
          icon={<BenchmarkIcon />}
          iconPosition="start"
        />
      </Tabs>
      ...
      {tab === 0 && (
        <Box>
          ...
          <Grid container spacing={2}>
                ...
                <Card variant="outlined">
                  <CardContent>
                    ...
                    <Typography variant="caption">
                      النوع: {s.surveyType?.replace(/_/g, ' ')}
                    </Typography>
                    <br />
                    <Typography variant="caption">الأسئلة: {s.questions?.length || 0}</Typography>
                    <br />
            ...
            {!surveys.length && (
              <Grid item xs={12}>
                <Alert severity="info">لا توجد استبيانات</Alert>
              </Grid>
            )}
          </Grid>
        </Box>
              ...
              <TableBody>
                {feedback.map(f => (
                  <TableRow key={f._id} hover>
                    ...
                    <TableCell>
                      <Chip size="small" label={f.channel?.replace(/_/g, ' ')} />
                    </TableCell>
                    <TableCell>
                      <Chip
                        size="small"
                        color={sentimentColors[f.sentiment] || 'default'}
                        label={f.sentiment}
                        icon={
        ...
        <Box>
          ...
          {complaints.filter(c => c.status === 'escalated').length > 0 && (
            <Alert severity="error" sx={{ mb: 2 }}>
              يوجد {complaints.filter(c => c.status === 'escalated').length} شكوى مصعّدة تتطلب
              اهتمام فوري!
            </Alert>
          )}
          <TableContainer component={Paper}>
              ...
              <TableBody>
                {complaints.map(c => (
                  <TableRow
                    key={c._id}
                    hover
                    sx={{ bgcolor: c.status === 'escalated' ? 'error.50' : 'inherit' }}
                  >
                    <TableCell>
                    ...
                    <TableCell>
                      <Chip size="small" label={c.category?.replace(/_/g, ' ')} />
                    </TableCell>
                    <TableCell>
                      <Chip
                        size="small"
      ...
      {tab === 3 && (
        <Box>
          ...
          <Grid container spacing={2}>
              ...
              <Grid item xs={12} md={6} key={j._id}>
                <Card variant="outlined">
                  <CardContent>
                    ...
                    <Divider sx={{ my: 1 }} />
                    <Typography variant="caption">
                      شريحة العملاء: {j.customerSegment || '—'}
                    </Typography>
                    <br />
                    <Typography variant="caption">
                      نقاط التماس: {j.touchpoints?.length || 0}
                    </Typography>
                    <br />
                    <Typography variant="caption">
                      نقاط الألم: {j.painPoints?.length || 0}
                    </Typography>
                    {j.satisfactionScore !== null && (
                      <Box mt={1}>
                        <Rating
                          value={j.satisfactionScore / 20}
                          readOnly
                          size="small"
                          precision={0.5}
                        />
            ...
            {!journeys.length && (
              <Grid item xs={12}>
                <Alert severity="info">لا توجد رحلات عملاء</Alert>
              </Grid>
            )}
          </Grid>
        </Box>
            ...
            <Table size="small">
              <TableHead>
                <TableRow>
                  ...
                  <TableCell>متوسط الصناعة</TableCell>
              ...
              <TableBody>
                {benchmarks.map(b => (
                  <TableRow key={b._id} hover>
                    ...
                    <TableCell>
                      <Chip size="small" label={b.category?.replace(/_/g, ' ')} />
                    </TableCell>
                    <TableCell>
                      {b.currentValue !== null ? b.currentValue : '—'}
                      {b.unit || ''}
                    </TableCell>
                    <TableCell>
                    ...
                    <TableCell>
                      {b.industryAverage !== null ? b.industryAverage : '—'}
                      {b.unit || ''}
                    </TableCell>
                    <TableCell>
          ...
          <Stack spacing={2} sx={{ mt: 1 }}>
            ...
            {dialog.type === 'feedback' && (
              <>
                ...
                >
                  {['phone', 'email', 'web', 'mobile_app', 'social_media', 'in_person', 'chat'].map(
                    t => (
                      <MenuItem key={t} value={t}>
                        {t.replace(/_/g, ' ')}
                      </MenuItem>
            ...
            {dialog.type === 'complaint' && (
              <>
                <TextField
                  ...
                  onChange={e => setForm({ ...form, complaintNumber: e.target.value })}
                />
                <TextField
                  label="الموضوع"
                  fullWidth
            ...
            {dialog.type === 'journey' && (
              <>
                <TextField
                  ...
                  onChange={e => setForm({ ...form, journeyName: e.target.value })}
                />
                <TextField
                  label="الوصف"
                  fullWidth
