    ...
    <Box sx={{ p: 3 }}>
      ...
      <Stack direction="row" justifyContent="space-between" alignItems="center" mb={3}>
        <Box>
          ...
          <Typography variant="body2" color="text.secondary">
            تقييم النضج الرقمي وإدارة الأفكار والمشاريع الابتكارية
          </Typography>
        </Box>
      ...
      <Grid container spacing={2} mb={3}>
        {[
          {
            label: 'مستوى النضج',
            value: stats.maturityLevel ? `${stats.maturityLevel}/5` : '—',
            color: '#1976d2',
          },
      ...
      >
        <Tab
          label={`تقييم النضج (${assessments.length})`}
          icon={<MaturityIcon />}
          iconPosition="start"
        />
      ...
      {tab === 0 && (
        <Box>
          ...
          <Grid container spacing={2}>
                  ...
                  <CardContent>
                    ...
                    {a.dimensions?.map((d, idx) => (
                      ...
                      >
                        ...
                        <Chip
                          size="small"
                          color={maturityLevels[d.level]?.color || 'default'}
                          label={`${d.level}/5 - ${maturityLevels[d.level]?.label || ''}`}
                        />
                      </Stack>
                    ))}
                    {a.overallScore !== null && (
                      <Box mt={1}>
                        ...
                        <Chip
                          size="small"
                          color={maturityLevels[Math.round(a.overallScore)]?.color || 'info'}
                          label={`${a.overallScore}/5`}
                        />
                      </Box>
                    )}
                  </CardContent>
            ...
            {!assessments.length && (
              <Grid item xs={12}>
                <Alert severity="info">لا توجد تقييمات</Alert>
              </Grid>
            )}
          </Grid>
        </Box>
      ...
      {tab === 1 && (
        <Box>
          ...
          <Grid container spacing={2}>
                ...
                <Card variant="outlined">
                  <CardContent>
                    ...
                    <Typography variant="caption">
                      الفئة: {idea.category?.replace(/_/g, ' ')}
                    </Typography>
                    <br />
                    <Typography variant="caption">
                      الأثر المتوقع: {idea.expectedImpact || '—'}
                    </Typography>
                    <br />
                    <Typography variant="caption">
                      المقدم: {idea.submittedBy?.name || '—'}
                    </Typography>
                    <Stack direction="row" spacing={1} mt={1} alignItems="center">
                      <VoteIcon fontSize="small" color="action" />
                      <Typography variant="caption">{idea.votes || 0} تصويت</Typography>
                      {idea.estimatedROI && (
                        <Chip size="small" color="success" label={`ROI: ${idea.estimatedROI}%`} />
                      )}
                    </Stack>
                  </CardContent>
                </Card>
            ...
            {!ideas.length && (
              <Grid item xs={12}>
                <Alert severity="info">لا توجد أفكار</Alert>
              </Grid>
            )}
          </Grid>
        </Box>
              ...
              <TableBody>
                {projects.map(p => (
                  <TableRow key={p._id} hover>
                    ...
                    <TableCell>
                      <Chip size="small" label={p.stage?.replace(/_/g, ' ')} />
                    </TableCell>
                    <TableCell>
                      <Chip
                        size="small"
                  ...
                  {items.map(r => (
                    <Grid item xs={12} sm={6} md={3} key={r._id}>
                      <Card
                        variant="outlined"
                        sx={{
                          borderLeft: `3px solid ${statusColors[r.ring] === 'success' ? '#2e7d32' : statusColors[r.ring] === 'warning' ? '#ed6c02' : '#1976d2'}`,
                        }}
                      >
                        <CardContent sx={{ py: 1.5 }}>
                          <Stack direction="row" justifyContent="space-between" alignItems="center">
                            ...
                            <Chip
                              size="small"
                              color={statusColors[r.ring] || 'default'}
                              label={r.ring}
                            />
                          </Stack>
                          <Typography variant="caption" color="text.secondary">
              ...
              <TableBody>
                {kpis.map(k => (
                  <TableRow key={k._id} hover>
                    ...
                    <TableCell>
                      <Chip size="small" label={k.category?.replace(/_/g, ' ')} />
                    </TableCell>
                    <TableCell>
                    ...
                    <TableCell>
                      {k.targetValue !== null ? k.targetValue : '—'}
                      {k.unit || ''}
                    </TableCell>
                    <TableCell>
                      <Chip
                        size="small"
                        color={statusColors[k.status] || 'default'}
                        label={k.status?.replace(/_/g, ' ')}
                      />
                    </TableCell>
                    <TableCell>
                      {k.trend === 'up' && (
                        <Chip size="small" color="success" icon={<TrendUpIcon />} label="تحسن" />
                      )}
          ...
          <Stack spacing={2} sx={{ mt: 1 }}>
            ...
            {dialog.type === 'idea' && (
              <>
                ...
                <TextField
                  ...
                  onChange={e => setForm({ ...form, estimatedROI: e.target.value })}
                />
              </>
            )}
            {dialog.type === 'project' && (
              <>
                ...
                >
                  {['ideation', 'proof_of_concept', 'pilot', 'scaling', 'production'].map(t => (
                    <MenuItem key={t} value={t}>
                      {t.replace(/_/g, ' ')}
                    </MenuItem>
                  ))}
            ...
            {dialog.type === 'radar' && (
              <>
                ...
                >
                  {['techniques', 'platforms', 'tools', 'languages_frameworks'].map(t => (
                    <MenuItem key={t} value={t}>
                      {t.replace(/_/g, ' ')}
                    </MenuItem>
                  ))}
