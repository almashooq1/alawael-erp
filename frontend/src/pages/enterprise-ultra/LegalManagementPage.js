  ...
  return (
    <Box sx={{ p: 3 }}>
      ...
      <Stack direction="row" justifyContent="space-between" alignItems="center" mb={3}>
        <Box>
          ...
          <Typography variant="body2" color="text.secondary">
            إدارة القضايا والجلسات والتوكيلات والفتاوى القانونية
          </Typography>
        </Box>
      ...
      >
        ...
        <Tab
          label={`الفتاوى القانونية (${opinions.length})`}
          icon={<FilingIcon />}
          iconPosition="start"
        />
        <Tab
          label={`الإيداعات التنظيمية (${filings.length})`}
          icon={<EventIcon />}
          iconPosition="start"
        />
      </Tabs>
        ...
        <Box>
          ...
          <TableContainer component={Paper}>
            <Table size="small">
              <TableHead>
                <TableRow>
                  <TableCell>رقم القضية</TableCell>
                  <TableCell>العنوان</TableCell>
                  <TableCell>النوع</TableCell>
                  <TableCell>الحالة</TableCell>
                  <TableCell>الأولوية</TableCell>
                  <TableCell>المحكمة</TableCell>
                  <TableCell>الجلسة القادمة</TableCell>
              ...
              <TableBody>
                {cases.map(c => (
                  <TableRow key={c._id} hover>
                    ...
                    <TableCell>
                      <Chip size="small" label={c.caseType?.replace(/_/g, ' ')} />
                    </TableCell>
                    <TableCell>
                      <Chip
                        size="small"
        ...
        <Box>
          ...
          <TableContainer component={Paper}>
            <Table size="small">
              <TableHead>
                <TableRow>
                  <TableCell>القضية</TableCell>
                  <TableCell>تاريخ الجلسة</TableCell>
                  <TableCell>النوع</TableCell>
                  <TableCell>الحالة</TableCell>
                  <TableCell>المحكمة</TableCell>
                ...
                {hearings.map(h => (
                  <TableRow key={h._id} hover>
                    ...
                    <TableCell>
                      <Chip
                        size="small"
                        color={statusColors[h.status] || 'default'}
                        label={h.status}
                      />
                    </TableCell>
      ...
      {tab === 2 && (
        <Box>
          ...
          <Grid container spacing={2}>
              ...
              <Grid item xs={12} md={4} key={p._id}>
                <Card variant="outlined">
                  <CardContent>
                    ...
                    <Typography variant="caption" color="text.secondary">
                      النوع: {p.poaType?.replace(/_/g, ' ')}
                    </Typography>
                    <Box mt={1}>
                      <Typography variant="caption">المانح: {p.grantor?.name || '—'}</Typography>
                      <br />
            ...
            {!poas.length && (
              <Grid item xs={12}>
                <Alert severity="info">لا توجد توكيلات</Alert>
              </Grid>
            )}
          </Grid>
        </Box>
              ...
              <TableBody>
                {opinions.map(o => (
                  <TableRow key={o._id} hover>
                    ...
                    <TableCell>
                      <Chip size="small" label={o.category?.replace(/_/g, ' ')} />
                    </TableCell>
                    <TableCell>
                      <Chip
                        size="small"
                    ...
                    <TableCell>
                      <Chip
                        size="small"
                        color={priorityColors[o.priority] || 'default'}
                        label={o.priority}
                      />
                    </TableCell>
        ...
        <Box>
          ...
          {filings.filter(f => f.status === 'overdue').length > 0 && (
            <Alert severity="error" icon={<WarningIcon />} sx={{ mb: 2 }}>
              يوجد {filings.filter(f => f.status === 'overdue').length} إيداع متأخر يتطلب اهتمام
              فوري!
            </Alert>
          )}
          <TableContainer component={Paper}>
            <Table size="small">
              <TableHead>
                <TableRow>
                  <TableCell>الرقم</TableCell>
                  <TableCell>العنوان</TableCell>
                  <TableCell>الجهة التنظيمية</TableCell>
                  <TableCell>النوع</TableCell>
                  <TableCell>الحالة</TableCell>
              ...
              <TableBody>
                {filings.map(f => (
                  <TableRow
                    key={f._id}
                    hover
                    sx={{ bgcolor: f.status === 'overdue' ? 'error.50' : 'inherit' }}
                  >
                    <TableCell>{f.filingNumber}</TableCell>
                    <TableCell>{f.title}</TableCell>
                    <TableCell>{f.regulatoryBody}</TableCell>
                    <TableCell>
                      <Chip size="small" label={f.filingType?.replace(/_/g, ' ')} />
                    </TableCell>
                    <TableCell>
                      <Chip
                        size="small"
            ...
            {dialog.type === 'poa' && (
              <>
                ...
                <TextField
                  ...
                  fullWidth
                  value={form.poaType || ''}
                  onChange={e => setForm({ ...form, poaType: e.target.value })}
                >
                  {[
                    'general',
                    'special',
