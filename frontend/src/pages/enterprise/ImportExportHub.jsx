...
export default function ImportExportHub() {
  ...
  useEffect(() => {
    if (activeTab === 3) loadTemplates();
  }, [activeTab, loadTemplates]);
  useEffect(() => {
    if (activeTab === 4) loadScheduledExports();
  ...
  const handleExportExecute = async () => {
    try {
      ...
      if (jobId) {
        setLiveProgress({
          status: 'processing',
          progress: { percentage: 0, processed: 0, total: 0 },
        });
  ...
  const handleCreateScheduled = async data => {
    try {
      ...
      showSnackbar(
        'خطأ في إنشاء الجدولة: ' + (err.response?.data?.message || err.message || ''),
        'error'
      );
    }
  };
    ...
    <Box sx={{ p: 3 }}>
      ...
      <Grid container spacing={2} sx={{ mb: 3 }}>
        {[
          ...
          },
          {
            ...
            value: stats?.overview?.totalExports || 0,
            icon: <DownloadIcon />,
            color: '#2E7D32',
            sub: 'تصدير',
          },
          {
            ...
            value: stats?.overview?.totalImports || 0,
            icon: <UploadIcon />,
            color: '#ED6C02',
            sub: 'استيراد',
          },
          {
            ...
            value: (stats?.overview?.totalRowsProcessed || 0).toLocaleString(),
            icon: <TrendIcon />,
            color: '#9C27B0',
            sub: 'سجل',
          },
          ...
          {
            label: 'متوسط الوقت',
            value: stats?.overview?.avgDuration
              ? `${(stats.overview.avgDuration / 1000).toFixed(1)}ث`
              : '—',
            icon: <SpeedIcon />,
            color: '#FF5722',
            sub: '',
      ...
      <Paper sx={{ borderRadius: 2, mb: 2 }}>
        ...
        >
          ...
          <Tab
            icon={<HistoryIcon />}
            label={
              <Badge badgeContent={jobs.total} color="primary" max={99}>
                سجل العمليات
              </Badge>
            }
            iconPosition="start"
      ...
      {activeTab === 1 && (
        <ModulesTab
          modules={modules}
          onExport={mod => {
            setExportConfig(prev => ({ ...prev, module: mod }));
            setExportDialog(true);
            setExportStep(0);
          }}
          onImport={mod => {
            setImportConfig(prev => ({ ...prev, module: mod }));
            setImportDialog(true);
            setImportStep(0);
          }}
  ...
  if (!stats) {
    return (
      <Paper sx={{ p: 4, textAlign: 'center', borderRadius: 2 }}>
        ...
        <Typography variant="body2" color="text.secondary">
          ابدأ بتصدير أو استيراد البيانات لعرض الإحصائيات
        </Typography>
      </Paper>
    );
  }
          ...
          {(stats.byFormat || []).map((item, i) => (
            <Box key={i} sx={{ display: 'flex', alignItems: 'center', mb: 1.5 }}>
              <Chip
                ...
                size="small"
                sx={{
                  bgcolor: (FORMAT_CONFIG[item._id]?.color || '#666') + '20',
                  color: FORMAT_CONFIG[item._id]?.color,
                  fontWeight: 600,
                  minWidth: 100,
                }}
              />
              <LinearProgress
                variant="determinate"
                value={
                  stats.overview?.totalJobs ? (item.count / stats.overview.totalJobs) * 100 : 0
                }
                sx={{
                  flex: 1,
                  mx: 2,
                  ...
                  borderRadius: 4,
                  bgcolor: '#f0f0f0',
                  '& .MuiLinearProgress-bar': { bgcolor: FORMAT_CONFIG[item._id]?.color },
                }}
              />
              <Typography variant="body2" fontWeight="bold">
                {item.count}
                ...
                <Box sx={{ position: 'relative', display: 'inline-flex', mb: 2 }}>
                  ...
                  >
                    <Typography
                      ...
                      fontWeight="bold"
                      color={
                        rate >= 80 ? 'success.main' : rate >= 50 ? 'warning.main' : 'error.main'
                      }
                    >
                      {rate}%
                    </Typography>
            ...
            const statusData = [
              {
                ...
                status: 'completed',
                color: '#4CAF50',
                count: stats?.overview?.completed || 0,
              },
              {
                label: 'قيد التنفيذ',
                status: 'processing',
                color: '#2196F3',
                count: stats?.overview?.processing || 0,
              },
              {
                label: 'فشل',
                status: 'failed',
                color: '#F44336',
                count: stats?.overview?.failed || 0,
              },
              {
                label: 'جزئي',
                status: 'partial',
                color: '#FF9800',
                count: stats?.overview?.partial || 0,
              },
              {
                label: 'ملغى',
                status: 'cancelled',
                color: '#9E9E9E',
                count: stats?.overview?.cancelled || 0,
              },
            ];
                    ...
                    <TableRow key={i} hover>
                      ...
                      <TableCell>
                        <Chip
                          ...
                          size="small"
                          color={job.type === 'export' ? 'success' : 'warning'}
                          variant="outlined"
                        />
                      </TableCell>
                      <TableCell>
                        <Chip
                          label={FORMAT_CONFIG[job.format]?.label || job.format}
                          size="small"
                      ...
                      <TableCell>
                        <Chip
                          label={STATUS_CONFIG[job.status]?.label || job.status}
                          size="small"
...
function ModulesTab({ modules, onExport, onImport, onDownloadTemplate }) {
  ...
  const filtered = modules.filter(
    m =>
      ...
      m.labelEn?.toLowerCase().includes(search.toLowerCase()) ||
      m.key.includes(search)
  );
                ...
                {mod.hasTemplate && (
                  <Tooltip title="تحميل القالب">
                    <IconButton
                      size="small"
                      color="primary"
                      onClick={() => onDownloadTemplate(mod.key)}
                    >
                      <TemplateIcon />
                    </IconButton>
                  </Tooltip>
                ...
                <TableRow key={i} hover>
                  ...
                  <TableCell>
                    <Chip
                      label={FORMAT_CONFIG[job.format]?.label || job.format}
                      size="small"
                      icon={FORMAT_CONFIG[job.format]?.icon}
                      sx={{ bgcolor: (FORMAT_CONFIG[job.format]?.color || '#666') + '10' }}
                    />
                  </TableCell>
                  ...
                  <TableCell align="center">
                    <Box sx={{ display: 'flex', gap: 0.5, justifyContent: 'center' }}>
                      {['processing', 'pending'].includes(job.status) && (
                        <Tooltip title="إلغاء">
                          <IconButton size="small" color="error" onClick={() => onCancel(job._id)}>
                      ...
                      {job.status === 'failed' && (
                        <Tooltip title="إعادة المحاولة">
                          <IconButton size="small" color="primary" onClick={() => onRetry(job._id)}>
                            <RetryIcon fontSize="small" />
                          </IconButton>
                        </Tooltip>
                      )}
                      <Tooltip title="حذف">
                        <IconButton size="small" color="error" onClick={() => onDelete(job._id)}>
      ...
      {jobs.total > 0 && (
        ...
        >
          ...
          <Box sx={{ display: 'flex', gap: 1 }}>
            ...
            <Button
              size="small"
              disabled={filters.page >= (jobs.pages || 1)}
              onClick={() => onFilterChange({ ...filters, page: filters.page + 1 })}
            >
  ...
  return (
    <Box>
      ...
      <Grid container spacing={2}>
            ...
            <Grid item xs={12} sm={6} md={4} key={`sys-${i}`}>
              <Card sx={{ borderRadius: 2, border: '2px solid', borderColor: 'primary.light' }}>
                <CardContent>
                  ...
                  <Typography variant="body2" color="text.secondary">
                    {mod.labelEn} — قالب استيراد جاهز مع تعليمات
                  </Typography>
                </CardContent>
        ...
        {modules.filter(m => m.hasTemplate).length === 0 &&
              ...
              >
                <CardContent>
                  ...
                  <Box sx={{ display: 'flex', gap: 0.5, flexWrap: 'wrap', mb: 1 }}>
                    ...
                    <Chip
                      label={
                        FREQ_OPTIONS.find(f => f.value === job.schedule?.frequency)?.label ||
          ...
          <Step>
            ...
            <StepContent>
                ...
                <Grid container spacing={2}>
                  ...
                  <Grid item xs={12}>
                    ...
                    <FormControl size="small" sx={{ minWidth: 150, mr: 2 }}>
                      ...
                      <Select
                        value={config.options?.language || 'both'}
                        label="اللغة"
                        onChange={e =>
                          setConfig({
                            ...config,
                            options: { ...config.options, language: e.target.value },
                          })
                        }
                      >
                  ...
                  <Grid item xs={12}>
                    ...
                    <Collapse in={showFieldSelect}>
                      {moduleFields.length > 0 ? (
                        <Paper
                          variant="outlined"
                          sx={{ p: 2, borderRadius: 2, maxHeight: 220, overflow: 'auto' }}
                        >
                          <Box sx={{ display: 'flex', justifyContent: 'space-between', mb: 1 }}>
                            ...
                            <Button
                              ...
                              onClick={() => {
                                ...
                                setConfig({
                                  ...config,
                                  fields: config.fields?.length === allKeys.length ? [] : allKeys,
                                });
                              }}
                            >
                              {config.fields?.length === moduleFields.length
                                ? 'إلغاء الكل'
                                : 'تحديد الكل'}
                            </Button>
                          </Box>
                    ...
                    {modules.map(m => (
                      <Chip
                        ...
                        onClick={() => {
                          ...
                          const newSel = sel.includes(m.key)
              ...
              <Box sx={{ mt: 2 }}>
                <Button
                  variant="contained"
                  onClick={() => (bulkMode ? onBulkExport() : onPreview())}
                  disabled={loading}
              ...
              {preview && (
                <Box>
                  <Alert severity="info" sx={{ mb: 2 }}>
                    ...
                    إجمالي السجلات: <strong>{preview.totalRecords}</strong> — الحقول:{' '}
                    <strong>{preview.fields?.length || 0}</strong> — الصيغة:{' '}
                    <strong>{FORMAT_CONFIG[config.format]?.label}</strong>
                  </Alert>
                        ...
                        <TableHead>
                          <TableRow>
                            {Object.keys(preview.preview[0])
                              ...
                              .slice(0, 8)
                              .map(key => (
                                <TableCell key={key} sx={{ fontWeight: 700, bgcolor: '#e3f2fd' }}>
                                  {key}
                                </TableCell>
                              ))}
                          </TableRow>
                  ...
                  <Box sx={{ display: 'flex', gap: 1 }}>
                    <Button onClick={() => setStep(0)} startIcon={<BackIcon />}>
                      رجوع
                    </Button>
                    <Button
                      variant="contained"
            ...
            <StepContent>
              ...
              <Box sx={{ mt: 2, display: 'flex', gap: 1 }}>
                <Button
                  variant="contained"
                  onClick={onParse}
                  disabled={!config.file || !config.module || loading}
                  endIcon={<ArrowIcon />}
                >
                  تحليل الملف
                </Button>
          ...
          <Step>
            ...
            <StepContent>
              {preview && (
                <Box>
                  <Alert
                    severity={preview.validation?.isValid ? 'success' : 'warning'}
                    sx={{ mb: 2 }}
                  >
                    <AlertTitle>
                      {preview.validation?.isValid ? 'البيانات سليمة' : 'توجد تحذيرات'}
                    </AlertTitle>
                    إجمالي الصفوف: <strong>{preview.totalRows}</strong> — الأعمدة:{' '}
                    <strong>{preview.detectedColumns?.length}</strong> —
                    {preview.validation && (
                      <>
                        صالحة: <strong>{preview.validation.validRows}</strong> — أخطاء:{' '}
                        <strong>{preview.validation.errors?.length || 0}</strong>
                      </>
                    )}
                  </Alert>
                                ...
                                >
                                  ...
                                  {(transformRules || []).map((rule, ri) => (
                                    <MenuItem key={ri} value={rule.key || rule.name}>
                                      <Tooltip
                                        title={rule.descriptionAr || rule.description || ''}
                                        placement="left"
                                      >
                                        <span>
                                          {rule.labelAr || rule.label || rule.key || rule.name}
                                        </span>
                                      </Tooltip>
                                    </MenuItem>
                                  ))}
                            ...
                            <TableHead>
                              <TableRow>
                                {preview.detectedColumns?.slice(0, 8).map(col => (
                                  <TableCell key={col} sx={{ fontWeight: 700, fontSize: 11 }}>
                                    {col}
                                  </TableCell>
                                ))}
                              </TableRow>
                  ...
                  {preview.validation?.errors?.length > 0 && (
                    <Accordion
                      ...
                      defaultExpanded={preview.validation.errors.length <= 5}
                    >
                      <AccordionSummary expandIcon={<ExpandIcon />}>
                        <Box sx={{ display: 'flex', alignItems: 'center', gap: 1 }}>
                          ...
                          {preview.validation.errors.length > 20 && (
                            <Chip
                              label={`+${preview.validation.errors.length - 20} أخرى`}
                              size="small"
                              color="error"
                              variant="outlined"
                            />
                        ...
                        <TableContainer>
                          <Table size="small">
                            <TableHead>
                              <TableRow
                                sx={{
                                  '& th': { bgcolor: '#ffebee', fontWeight: 700, fontSize: 11 },
                                }}
                              >
                            ...
                            <TableBody>
                              {preview.validation.errors.slice(0, 30).map((err, i) => (
                                <TableRow
                                  key={i}
                                  sx={{ bgcolor: i % 2 ? '#fff5f5' : 'transparent' }}
                                >
                                  <TableCell>
                                    <Chip
                                      label={`#${err.row}`}
                        ...
                        >
                          <Box sx={{ display: 'flex', alignItems: 'center', gap: 2, mb: 2 }}>
                            <Box sx={{ position: 'relative', display: 'inline-flex' }}>
                              <CircularProgress
                                ...
                                sx={{
                                  color:
                                    (qualityReport.qualityScore || qualityReport.score || 0) >= 80
                                      ? '#4CAF50'
                                      : (qualityReport.qualityScore || qualityReport.score || 0) >=
                                          50
                                        ? '#FF9800'
                                        : '#F44336',
                                }}
                            ...
                            <Box>
                              ...
                              <Typography variant="caption" color="text.secondary">
                                ...
                                {qualityReport.completeness
                                  ? `${Math.round(qualityReport.completeness.overallCompleteness || 0)}%`
                                  : '—'}{' '}
                                | التفرد:{' '}
                                {qualityReport.uniqueness
                                  ? `${Object.keys(qualityReport.uniqueness).length} حقول`
                                  : '—'}
                              </Typography>
                            </Box>
                          </Box>
                          {qualityReport.completeness?.fieldCompleteness && (
                            <Box sx={{ maxHeight: 120, overflow: 'auto' }}>
                              {Object.entries(qualityReport.completeness.fieldCompleteness)
                                .slice(0, 8)
                                .map(([field, pct], idx) => (
                                  <Box
                                    key={idx}
                  ...
                  <Box sx={{ display: 'flex', gap: 2, mb: 2, flexWrap: 'wrap' }}>
                    <FormControl size="small" sx={{ minWidth: 150 }}>
                      ...
                      <Select
                        value={config.options?.mode || 'insert'}
                        label="وضع الاستيراد"
                        onChange={e =>
                          setConfig({
                            ...config,
                            options: { ...config.options, mode: e.target.value },
                          })
                        }
                      >
                    ...
                    <FormControlLabel
                      control={
                        <Switch
                          checked={config.options?.skipDuplicates !== false}
                          onChange={e =>
                            setConfig({
                              ...config,
                              options: { ...config.options, skipDuplicates: e.target.checked },
                            })
                          }
                        />
                      }
                      ...
                      <FormControl size="small" sx={{ minWidth: 200 }}>
                        ...
                        <Select
                          ...
                          onChange={e =>
                            setConfig({
                              ...config,
                              options: { ...config.options, matchField: e.target.value },
                            })
                          }
                        >
                          <MenuItem value="">
                          ...
                          {(config.columnMappings || [])
                            ...
                            .map((m, i) => (
                              <MenuItem key={i} value={m.targetField}>
                ...
                <Box>
                  ...
                  >
                    <AlertTitle>
                      {result.results?.failed > 0
                        ? 'اكتمل الاستيراد مع بعض الأخطاء'
                  ...
                  <Grid container spacing={2} sx={{ mb: 2 }}>
                    {[
                      {
                        label: 'تم الإدراج',
                        value: result.results?.inserted || 0,
                        color: 'success.main',
                        icon: <SuccessIcon />,
                      },
                      {
                        label: 'تم التحديث',
                        value: result.results?.updated || 0,
                        color: 'info.main',
                        icon: <RefreshIcon />,
                      },
                      {
                        label: 'تم التخطي',
                        value: result.results?.skipped || 0,
                        color: 'warning.main',
                        icon: <WarningIcon />,
                      },
                      {
                        label: 'فشل',
                        value: result.results?.failed || 0,
                        color: 'error.main',
                        icon: <ErrorIcon />,
                      },
                    ].map((stat, i) => (
                    ...
                    <Accordion>
                      <AccordionSummary expandIcon={<ExpandIcon />}>
                        <Typography color="error">
                          تفاصيل الأخطاء ({result.results.errors.length})
                        </Typography>
                      </AccordionSummary>
                      <AccordionDetails>
