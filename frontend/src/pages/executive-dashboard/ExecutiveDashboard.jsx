...
function KPICard({ kpi, snapshot }) {
  ...
  const TrendIcon =
    trend === 'improving' ? TrendUpIcon : trend === 'declining' ? TrendDownIcon : TrendFlatIcon;
  const trendColor =
    trend === 'improving'
      ? 'success.main'
      : trend === 'declining'
  ...
  return (
    <Card
      sx={{
        height: '100%',
        borderLeft: `4px solid ${KPI_STATUS_COLORS[snapshot?.status] || '#9e9e9e'}`,
      }}
    >
      <CardContent sx={{ pb: 1 }}>
        ...
        <Box display="flex" alignItems="center" gap={0.5} mt={0.5}>
          ...
          {snapshot?.changePercentage != null && (
            <Typography variant="caption" sx={{ color: trendColor }}>
              ...
              {snapshot.changePercentage.toFixed(1)}%
            </Typography>
          )}
          <Typography variant="caption" color="text.secondary" sx={{ ml: 'auto' }}>
            الهدف: {kpi.target?.value}
            {kpi.unit}
          </Typography>
        </Box>
...
export default function ExecutiveDashboard() {
  ...
  const fetchAll = useCallback(async () => {
    ...
    try {
      ...
      if (alertAnalRes.status === 'fulfilled')
        setAlertStats(alertAnalRes.value?.data?.data || null);
  ...
  return (
    <Box sx={{ p: 3 }}>
      ...
      <Box display="flex" alignItems="center" justifyContent="space-between" mb={3}>
        ...
        <Box display="flex" gap={1}>
          <Button
            variant="outlined"
            startIcon={<AIIcon />}
            onClick={() => dashboardsAPI.runAllRules().then(fetchAll)}
          >
            تشغيل محرك القرار
          </Button>
          <Tooltip title="تحديث">
      ...
      <Grid container spacing={2} mb={3}>
        ...
        <Grid item xs={12} sm={6} md={3}>
          <SummaryCard
            title="الحلقات العلاجية النشطة"
            value={summary?.activeEpisodes}
            icon={EpisodeIcon}
            color="#9c27b0"
          />
        </Grid>
        ...
        <Grid item xs={12} sm={6} md={3}>
          <SummaryCard
            ...
            subtitle={
              summary?.alerts?.bySeverity?.critical
                ? `${summary.alerts.bySeverity.critical} حرجة`
                : null
            }
          />
        </Grid>
              ...
              <Box display="flex" gap={1} mb={2} flexWrap="wrap">
                {alertStats.bySeverity?.critical > 0 && (
                  <Chip
                    size="small"
                    color="error"
                    label={`${alertStats.bySeverity.critical} حرجة`}
                  />
                )}
                {alertStats.bySeverity?.high > 0 && (
                  <Chip
                    size="small"
                    color="warning"
                    label={`${alertStats.bySeverity.high} عالية`}
                  />
                )}
                {alertStats.bySeverity?.medium > 0 && (
                  <Chip
                    size="small"
                    color="info"
                    label={`${alertStats.bySeverity.medium} متوسطة`}
                  />
                )}
                {alertStats.averageResponseMinutes != null && (
                  <Chip
                    size="small"
                    variant="outlined"
                    label={`متوسط الاستجابة: ${Math.round(alertStats.averageResponseMinutes)} د`}
                  />
                )}
              </Box>
            )}
