...
function StepPersonal({ form, setField, errors, touched = {}, readOnly }) {
  ...
  return (
    <Grid container spacing={2}>
      ...
      <Grid item xs={12} sm={6}>
        {field('nationalId', 'رقم الهوية', { dir: 'ltr' })}
      </Grid>
      <Grid item xs={12} sm={6}>
        {field('phone', 'رقم الجوال', { dir: 'ltr' })}
      </Grid>
      <Grid item xs={12}>
        {field('email', 'البريد الإلكتروني', { type: 'email', dir: 'ltr' })}
      </Grid>
    </Grid>
  );
}
...
function StepEmployment({ form, setField, errors, readOnly, positionsList }) {
  if (readOnly) {
    ...
    return (
      <Grid container spacing={2}>
        <Grid item xs={12} sm={6}>
          <ReadOnly label="القسم" value={form.department} />
        </Grid>
        <Grid item xs={12} sm={6}>
          <ReadOnly label="المسمى الوظيفي" value={form.position} />
        </Grid>
        <Grid item xs={12} sm={6}>
          <ReadOnly label="تاريخ الالتحاق" value={form.joinDate} />
        </Grid>
        <Grid item xs={12} sm={6}>
          <Typography variant="caption" color="text.secondary">
            الحالة
        ...
        <Grid item xs={12} sm={6}>
          <ReadOnly label="الراتب" value={form.salary} />
        </Grid>
        <Grid item xs={12}>
          <ReadOnly label="ملاحظات" value={form.notes} />
        </Grid>
      </Grid>
    );
  }
...
export default function EmployeeFormDialog({
  ...
  return (
    ...
    >
      ...
      {/* ── Content ── */}
      <DialogContent sx={{ pt: 3, minHeight: 280 }}>{stepContent()}</DialogContent>
