...
const meetTypeLabels = {
  REGULAR: 'دوري',
  EMERGENCY: 'طارئ',
  CASE_REVIEW: 'مراجعة حالة',
  CARE_PLANNING: 'تخطيط رعاية',
  DISCHARGE_PLANNING: 'تخطيط خروج',
  PROGRESS_REVIEW: 'مراجعة تقدم',
  ...
  const kpis = [
    {
      ...
      sub: `${overview.completedMeetings || 0} مكتمل`,
      icon: <MeetingIcon sx={{ fontSize: 36 }} />,
    ...
    },
    {
      ...
      sub: `${overview.activePlans || 0} نشط`,
      icon: <PlanIcon sx={{ fontSize: 36 }} />,
    ...
    },
    {
      ...
      sub: `${overview.pendingReferrals || 0} قيد الانتظار`,
      icon: <RefIcon sx={{ fontSize: 36 }} />,
    ...
    },
    {
      ...
      sub: `تقدم الخطط: ${overview.averageProgress || 0}%`,
    ...
    <Box p={3} dir="rtl">
      <Box display="flex" justifyContent="space-between" alignItems="center" mb={3}>
        <Box>
          ...
          <Typography variant="body2" color="text.secondary">
            إدارة شاملة لاجتماعات الفريق، خطط التأهيل، والإحالات الداخلية
          </Typography>
        </Box>
        <Box display="flex" gap={1}>
          ...
          <Button
            ...
            onClick={() => navigate('/mdt-coordination/meetings')}
            sx={{ borderRadius: 2 }}
          >
            الاجتماعات
          </Button>
          <Button
            variant="outlined"
            startIcon={<PlanIcon />}
            onClick={() => navigate('/mdt-coordination/plans')}
            sx={{ borderRadius: 2 }}
          >
            الخطط
          </Button>
          <Button
            variant="outlined"
            startIcon={<RefIcon />}
            onClick={() => navigate('/mdt-coordination/referrals')}
            sx={{ borderRadius: 2 }}
          >
            الإحالات
          </Button>
      ...
      {overdueTotal > 0 && (
        <Alert severity="warning" sx={{ mb: 2, borderRadius: 2 }} icon={<WarningIcon />}>
          يوجد <strong>{overdueTotal}</strong> عنصر متأخر — {overdue.overdueReviews?.length || 0}{' '}
          مراجعة، {overdue.overdueReferrals?.length || 0} إحالة،{' '}
          {overdue.overdueActions?.length || 0} مهمة
        </Alert>
      )}
      ...
      <Grid container spacing={2} mb={3}>
        ...
        <Grid item xs={12} md={4}>
          <Paper sx={{ p: 2, borderRadius: 2 }}>
            <Box display="flex" alignItems="center" gap={1} mb={1}>
              <RefIcon color="secondary" />
              <Typography variant="subtitle1" fontWeight="bold">
                إحالات هذا الشهر
              </Typography>
            </Box>
            <Typography variant="h3" fontWeight="bold" color="secondary">
              {overview.monthlyReferrals || 0}
            </Typography>
          </Paper>
        </Grid>
        <Grid item xs={12} md={4}>
      ...
      <Paper sx={{ p: 2, borderRadius: 2, mb: 3 }}>
        <Tabs value={tab} onChange={(_, v) => setTab(v)} sx={{ mb: 2 }}>
          <Tab
            icon={
              <Badge badgeContent={recentMeetings.length} color="primary">
                <EventIcon />
              </Badge>
            }
            label="الاجتماعات"
          ...
          <Tab
            icon={
              <Badge badgeContent={recentPlans.length} color="success">
                <PlanIcon />
              </Badge>
            }
            label="الخطط"
          ...
          <Tab
            icon={
              <Badge badgeContent={overview.pendingReferrals || 0} color="warning">
                <RefIcon />
              </Badge>
            }
            label="الإحالات"
          ...
          <Tab
            icon={
              <Badge badgeContent={overdueTotal} color="error">
                <WarningIcon />
              </Badge>
            }
            label="المتأخرات"
              ...
              <TableBody>
                {recentMeetings.map((m, i) => (
                  ...
                  >
                    ...
                    <TableCell>
                      <Chip
                        label={meetTypeLabels[m.type] || m.type || '-'}
                        size="small"
                        variant="outlined"
                      />
                    </TableCell>
                ...
                <Grid item xs={12} sm={6} md={4} key={p._id}>
                  ...
                  >
                    <CardContent>
                      ...
                      <Typography variant="body2" color="text.secondary">
                        {p.beneficiaryName || p.beneficiary?.name || '-'}
                      </Typography>
                      <Box mt={1}>
                      ...
                      <Box display="flex" gap={1} mt={1}>
                        <Chip
                          icon={<MeetingIcon />}
                          label={`${p.teamMembers?.length || 0} أعضاء`}
                          size="small"
                          variant="outlined"
                        />
                        <Chip
                          icon={<TaskIcon />}
                          label={`${p.goals?.length || 0} أهداف`}
                          size="small"
                          variant="outlined"
                        />
                      </Box>
                  ...
                  <TableRow key={r._id || i} hover>
                    ...
                    <TableCell>
                      <Chip
                        ...
                        size="small"
                        sx={{
                          bgcolor: priorityColors[r.priority] || '#999',
                          color: '#fff',
                          fontWeight: 'bold',
                        }}
                      />
          ...
          <>
            ...
            {overdueTotal === 0 ? (
              <Box textAlign="center" py={4}>
                ...
                <Typography color="text.secondary">لا توجد عناصر متأخرة</Typography>
              </Box>
            ) : (
              <Grid container spacing={2}>
                {(overdue.overdueReviews || []).length > 0 && (
                  <Grid item xs={12} md={4}>
                    <Typography variant="subtitle2" color="error" gutterBottom>
                      مراجعات متأخرة ({overdue.overdueReviews.length})
                    </Typography>
                    <List dense>
                ...
                {(overdue.overdueReferrals || []).length > 0 && (
                  <Grid item xs={12} md={4}>
                    <Typography variant="subtitle2" color="error" gutterBottom>
                      إحالات متأخرة ({overdue.overdueReferrals.length})
                    </Typography>
                    <List dense>
                      {overdue.overdueReferrals.slice(0, 5).map((r, i) => (
                        <ListItem key={i}>
                          ...
                          <ListItemText
                            primary={r.ticketNumber || r.beneficiaryName}
                            secondary={`${r.fromDepartment} → ${r.toDepartment}`}
                          />
                        </ListItem>
                      ))}
                    </List>
                ...
                {(overdue.overdueActions || []).length > 0 && (
                  <Grid item xs={12} md={4}>
                    <Typography variant="subtitle2" color="error" gutterBottom>
                      مهام متأخرة ({overdue.overdueActions.length})
                    </Typography>
                    <List dense>
                      {overdue.overdueActions.slice(0, 5).map((a, i) => (
                        <ListItem key={i}>
                          ...
                          <ListItemText
                            primary={a.actionItem?.description || 'مهمة'}
                            secondary={`اجتماع: ${a.meetingNumber}`}
                          />
                        </ListItem>
                      ))}
                    </List>
      ...
      {(overview.upcomingMeetings || []).length > 0 && (
        <Paper sx={{ p: 2, borderRadius: 2 }}>
          <Box display="flex" alignItems="center" gap={1} mb={2}>
            ...
            <Typography variant="h6">الاجتماعات القادمة</Typography>
          </Box>
          <Grid container spacing={2}>
