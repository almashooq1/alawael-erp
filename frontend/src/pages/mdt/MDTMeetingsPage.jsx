...
const TYPES = [
  ...
  { value: 'CASE_REVIEW', label: 'مراجعة حالة' },
  { value: 'CARE_PLANNING', label: 'تخطيط رعاية' },
  { value: 'DISCHARGE_PLANNING', label: 'تخطيط خروج' },
  { value: 'PROGRESS_REVIEW', label: 'مراجعة تقدم' },
...
const specLabels = {
  ...
  EDUCATION: 'تعليم',
  NUTRITION: 'تغذية',
  BEHAVIORAL_THERAPY: 'سلوكي',
  CASE_MANAGEMENT: 'إدارة حالات',
...
export default function MDTMeetingsPage() {
  ...
  const handleSave = async () => {
    if (!form.title || !form.date || !form.startTime) {
      setError('العنوان والتاريخ ووقت البداية مطلوبة');
      return;
    }
    setSaving(true);
    try {
  ...
  const DECISION_CATEGORIES = [
    { value: 'TREATMENT_PLAN', label: 'خطة علاج' },
    { value: 'MEDICATION', label: 'دواء' },
    { value: 'REFERRAL', label: 'إحالة' },
    { value: 'DISCHARGE', label: 'خروج' },
    { value: 'ASSESSMENT', label: 'تقييم' },
    { value: 'GOAL_CHANGE', label: 'تغيير هدف' },
    { value: 'EQUIPMENT', label: 'معدات' },
    { value: 'FAMILY_MEETING', label: 'اجتماع عائلة' },
    ...
    <Box p={3} dir="rtl">
        ...
        <Box display="flex" alignItems="center" gap={2}>
          ...
          <Box>
            ...
            <Typography variant="body2" color="text.secondary">
              إنشاء وإدارة الاجتماعات، المحاضر، القرارات، وجدول الأعمال
            </Typography>
          </Box>
        </Box>
      ...
      <Grid container spacing={2} mb={3}>
        {[
          ...
          {
            label: 'متوسط الحضور',
            value: Math.round(stats.averageAttendees || 0),
            color: '#9c27b0',
          },
        ].map(s => (
              ...
              <TableBody>
                {meetings.map(m => (
                  ...
                  >
                    ...
                    <TableCell>
                      <Chip label={typeLabels[m.type] || m.type} size="small" variant="outlined" />
                    </TableCell>
                    <TableCell>
                      <Chip
              ...
              {detailTab === 0 && (
                <Grid container spacing={2}>
                  <Grid item xs={6}>
                    ...
                    <Typography>{typeLabels[detail.type] || detail.type}</Typography>
                  </Grid>
                  <Grid item xs={6}>
                    <Typography variant="caption" color="text.secondary">
                      المكان
                    </Typography>
                    <Typography>{detail.isVirtual ? 'افتراضي' : detail.location || '-'}</Typography>
                  </Grid>
                  <Grid item xs={6}>
                    <Typography variant="caption" color="text.secondary">
                      الوقت
                    ...
                    <Typography>
                      {detail.startTime}
                      {detail.endTime ? ` - ${detail.endTime}` : ''} ({detail.duration || 60} دقيقة)
                    </Typography>
                  </Grid>
                  <Grid item xs={6}>
                    <Typography variant="caption" color="text.secondary">
                          ...
                          <ListItemText
                            primary={
                              <Box display="flex" gap={1} alignItems="center">
                                <Chip label={a.order || i + 1} size="small" />
                                <Typography>{a.topic}</Typography>
                              </Box>
                            }
              ...
              {detailTab === 3 && (
                <Box>
                  <Box display="flex" justifyContent="space-between" alignItems="center" mb={1}>
                    ...
                    <Button
                      size="small"
                      startIcon={<MinutesIcon />}
                      onClick={() => {
                        setMinutesContent(detail.minutes?.content || '');
                        setMinutesOpen(true);
                      }}
                    >
                  ...
                  {detail.minutes?.content ? (
                    ...
                    >
                      ...
                      <Box display="flex" gap={2} mt={1}>
                        <Chip
                          label={detail.minutes.approved ? 'معتمد' : 'غير معتمد'}
                          size="small"
                          color={detail.minutes.approved ? 'success' : 'warning'}
                        />
                        {detail.minutes.recordedAt && (
                          <Typography variant="caption" color="text.secondary">
                            {new Date(detail.minutes.recordedAt).toLocaleString('ar')}
                          </Typography>
                        )}
                      </Box>
                    </Paper>
                  ...
                  <Box display="flex" justifyContent="space-between" alignItems="center" mb={1}>
                    <Typography variant="subtitle1" fontWeight="bold">
                      القرارات ({detail.generalDecisions?.length || 0})
                    </Typography>
                    <Button
                      size="small"
                  ...
                  <Typography variant="subtitle1" fontWeight="bold" mb={1}>
                    المهام ({detail.generalActionItems?.length || 0})
                  </Typography>
                    ...
                    <Card key={i} variant="outlined" sx={{ mb: 1, borderRadius: 2 }}>
                      <CardContent sx={{ py: 1 }}>
                        ...
                        <Box display="flex" gap={2} mt={0.5}>
                          {a.assignedToName && (
                            <Typography variant="caption">المسؤول: {a.assignedToName}</Typography>
                          )}
                          {a.dueDate && (
                            <Typography variant="caption" color="text.secondary">
          ...
          <Grid container spacing={2} sx={{ mt: 0.5 }}>
            ...
            <Grid item xs={12}>
              <TextField
                ...
                select
                label="التخصص"
                value={attendeeForm.specialty}
                onChange={e => setAttendeeForm(f => ({ ...f, specialty: e.target.value }))}
              >
            ...
            <Grid item xs={12}>
              <TextField
                ...
                onChange={e => setAttendeeForm(f => ({ ...f, department: e.target.value }))}
              />
            </Grid>
          </Grid>
        </DialogContent>
        <DialogActions>
          <Button onClick={() => setAttendeeOpen(false)}>إلغاء</Button>
      ...
      <Dialog open={minutesOpen} onClose={() => setMinutesOpen(false)} maxWidth="sm" fullWidth>
        ...
        <DialogActions>
          <Button onClick={() => setMinutesOpen(false)}>إلغاء</Button>
          <Button variant="contained" onClick={handleAddMinutes}>
            حفظ
          </Button>
        </DialogActions>
      </Dialog>
      ...
      <Dialog open={agendaOpen} onClose={() => setAgendaOpen(false)} maxWidth="xs" fullWidth>
          ...
          <Grid container spacing={2} sx={{ mt: 0.5 }}>
            <Grid item xs={12}>
              <TextField
                ...
                onChange={e => setAgendaForm(f => ({ ...f, topic: e.target.value }))}
                required
              />
            </Grid>
            <Grid item xs={12}>
              <TextField
                ...
                onChange={e => setAgendaForm(f => ({ ...f, duration: +e.target.value }))}
              />
            </Grid>
            <Grid item xs={12}>
              <TextField
                ...
                onChange={e => setAgendaForm(f => ({ ...f, notes: e.target.value }))}
                multiline
                rows={2}
              />
            </Grid>
        ...
        <DialogActions>
          <Button onClick={() => setAgendaOpen(false)}>إلغاء</Button>
