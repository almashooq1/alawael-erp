...
const DOMAINS = [
  ...
  { value: 'SELF_CARE', label: 'رعاية ذاتية' },
...
const SPECIALTIES = [
  ...
  { value: 'SPEECH_THERAPY', label: 'نطق ولغة' },
  ...
  { value: 'BEHAVIORAL_THERAPY', label: 'سلوكي' },
  { value: 'CASE_MANAGEMENT', label: 'إدارة حالات' },
...
export default function MDTPlansPage() {
  ...
  const reloadDetail = async id => {
    try {
      const res = await plansService.getById(id);
  ...
  const openDetail = async p => {
    try {
      const res = await plansService.getById(p._id);
      setDetail(res.data || res);
    } catch {
      setDetail(p);
    }
  ...
  const handleAddMember = async () => {
    ...
    try {
      await plansService.addTeamMember(detail._id, memberForm);
      reloadDetail(detail._id);
      setMemberOpen(false);
      setMemberForm({ name: '', specialty: '', role: 'CONTRIBUTOR' });
    } catch {
      /* */
    }
  };
  ...
  const handleAddGoal = async () => {
    ...
    try {
      await plansService.addGoal(detail._id, goalForm);
      reloadDetail(detail._id);
      setGoalOpen(false);
      setGoalForm({
        title: '',
  ...
  const handleUpdateProgress = async () => {
    ...
    try {
      await plansService.updateGoalProgress(detail._id, progressGoalId, {
  ...
  const handleApprove = async () => {
    try {
      await plansService.approve(detail._id, { decision: 'APPROVED', notes: 'تمت الموافقة' });
      reloadDetail(detail._id);
    } catch {
      /* */
    }
  ...
  const handleAddReview = async () => {
    ...
    try {
      await plansService.addReview(detail._id, reviewForm);
      reloadDetail(detail._id);
      setReviewOpen(false);
      setReviewForm({ summary: '', recommendations: '', overallStatus: 'ON_TRACK' });
    } catch {
      /* */
    }
  };
    ...
    <Box p={3} dir="rtl">
      ...
      <Box display="flex" justifyContent="space-between" alignItems="center" mb={3}>
        <Box display="flex" alignItems="center" gap={2}>
          <IconButton onClick={() => navigate('/mdt-coordination')}>
            <BackIcon />
          </IconButton>
          <Box>
            ...
            <Typography variant="body2" color="text.secondary">
              إنشاء وإدارة خطط التأهيل، الأهداف، وتقدم العلاج
            </Typography>
          </Box>
        </Box>
      ...
      <Grid container spacing={2} mb={3}>
        {[
          ...
          <Grid item xs={6} sm={3} key={s.label}>
            <Paper
              sx={{ p: 2, borderRadius: 2, borderTop: `3px solid ${s.color}`, textAlign: 'center' }}
            >
              ...
              <TableBody>
                {plans.map(p => (
                  ...
                  >
                    ...
                    <TableCell>
                      <Chip label={p.teamMembers?.length || 0} size="small" />
                    </TableCell>
                    <TableCell>
                ...
                {plans.length === 0 && (
                  <TableRow>
                    <TableCell colSpan={9} align="center" sx={{ py: 4 }}>
                      <Typography color="text.secondary">لا توجد خطط</Typography>
                    </TableCell>
                  </TableRow>
                )}
              </TableBody>
      ...
      <Dialog open={formOpen} onClose={() => setFormOpen(false)} maxWidth="sm" fullWidth>
          ...
          <Grid container spacing={2} sx={{ mt: 0.5 }}>
            ...
            <Grid item xs={6}>
              <TextField
                ...
                onChange={e => setForm(f => ({ ...f, reviewCycle: e.target.value }))}
              >
                {REVIEW_CYCLES.map(c => (
        ...
        <DialogActions>
          ...
          <Button variant="contained" onClick={handleSave} disabled={saving}>
            {saving ? <CircularProgress size={20} /> : editId ? 'تحديث' : 'إنشاء'}
          </Button>
        </DialogActions>
      </Dialog>
          ...
          <>
            <DialogTitle>
              ...
              <Box mt={1} display="flex" alignItems="center" gap={2}>
                ...
                <LinearProgress
                  variant="determinate"
                  value={detail.overallProgress || 0}
                  sx={{ flex: 1, height: 10, borderRadius: 5 }}
                />
                <Typography variant="body2" fontWeight="bold">
                  {Math.round(detail.overallProgress || 0)}%
                </Typography>
              </Box>
            </DialogTitle>
            <DialogContent>
              ...
              {detailTab === 0 && (
                <Grid container spacing={2}>
                  <Grid item xs={6}>
                    ...
                    <Typography>
                      {REVIEW_CYCLES.find(c => c.value === detail.reviewCycle)?.label ||
                        detail.reviewCycle ||
                        '-'}
                    </Typography>
                  </Grid>
                  <Grid item xs={6}>
                    ...
                    <Typography>{detail.department || '-'}</Typography>
                  </Grid>
                  <Grid item xs={6}>
                    <Typography variant="caption" color="text.secondary">
                      تاريخ النهاية
                  ...
                  {(detail.linkedMeetings || []).length > 0 && (
                    <Grid item xs={12}>
                      ...
                      <Typography variant="subtitle2" mb={1}>
                        الاجتماعات المرتبطة ({detail.linkedMeetings.length})
                      </Typography>
                      {detail.linkedMeetings.map((m, i) => (
                        <Chip
                          key={i}
                          label={m.title || m.meetingNumber || 'اجتماع'}
                          size="small"
                          variant="outlined"
                          sx={{ mr: 0.5, mb: 0.5 }}
                        />
                  ...
                  <Table size="small">
                    <TableHead>
                      <TableRow>
                        ...
                        <TableCell>الأهداف المسندة</TableCell>
                      </TableRow>
                    </TableHead>
                    <TableBody>
                      {(detail.teamMembers || []).map((m, i) => (
                        <TableRow key={i}>
                          ...
                          <TableCell>
                            {SPECIALTIES.find(s => s.value === m.specialty)?.label ||
                              m.specialty ||
                              '-'}
                          </TableCell>
                          <TableCell>
                            <Chip
                              label={ROLES.find(r => r.value === m.role)?.label || m.role}
                              size="small"
              ...
              {detailTab === 2 && (
                <>
                  <Box display="flex" justifyContent="flex-end" mb={1}>
                    <Button size="small" startIcon={<GoalIcon />} onClick={() => setGoalOpen(true)}>
                      إضافة هدف
                    </Button>
                  </Box>
                      ...
                      <CardContent>
                        <Box display="flex" justifyContent="space-between" alignItems="flex-start">
                          <Box flex={1}>
                            ...
                            <Box display="flex" gap={1} mt={0.5} mb={1}>
                              <Chip
                                label={domainLabels[g.domain] || g.domain}
                                size="small"
                                variant="outlined"
                              />
                              {g.icfCode && (
                          ...
                          <Tooltip title="تحديث التقدم">
                            <IconButton
                              size="small"
                              onClick={() => {
                                setProgressGoalId(g._id);
                                setProgressForm({ progressValue: g.progress || 0, note: '' });
                        ...
                        <Box display="flex" alignItems="center" gap={1}>
                          <LinearProgress
                            ...
                            sx={{ flex: 1, height: 8, borderRadius: 4 }}
                            color={
                              g.progress >= 80 ? 'success' : g.progress >= 50 ? 'warning' : 'error'
                            }
                          />
                          <Typography variant="body2" fontWeight="bold">
                          ...
                          <Chip
                            label={
                              r.overallStatus === 'ON_TRACK'
                                ? 'على المسار'
                                : r.overallStatus === 'BEHIND'
                                  ? 'متأخر'
                                  : r.overallStatus === 'AHEAD'
                                    ? 'متقدم'
                                    : r.overallStatus || '-'
                            }
                            size="small"
      ...
      <Dialog open={memberOpen} onClose={() => setMemberOpen(false)} maxWidth="xs" fullWidth>
          ...
          <Grid container spacing={2} sx={{ mt: 0.5 }}>
            ...
            <Grid item xs={12}>
              <TextField
                ...
                onChange={e => setMemberForm(f => ({ ...f, specialty: e.target.value }))}
              >
                {SPECIALTIES.map(s => (
        ...
        <DialogActions>
          <Button onClick={() => setMemberOpen(false)}>إلغاء</Button>
      ...
      <Dialog open={goalOpen} onClose={() => setGoalOpen(false)} maxWidth="sm" fullWidth>
          ...
          <Grid container spacing={2} sx={{ mt: 0.5 }}>
            ...
            <Grid item xs={6}>
              <TextField
                fullWidth
                label="كود ICF"
                value={goalForm.icfCode}
                onChange={e => setGoalForm(f => ({ ...f, icfCode: e.target.value }))}
                placeholder="مثال: b710.2"
              />
            </Grid>
            <Grid item xs={6}>
              <TextField
                ...
                onChange={e => setGoalForm(f => ({ ...f, baseline: e.target.value }))}
              />
            </Grid>
            <Grid item xs={6}>
              <TextField
        ...
        <DialogActions>
          <Button onClick={() => setGoalOpen(false)}>إلغاء</Button>
        ...
        <DialogContent>
          <Box sx={{ mt: 2 }}>
            ...
            <Slider
              ...
              onChange={(_, v) => setProgressForm(f => ({ ...f, progressValue: v }))}
              min={0}
              max={100}
              step={5}
          ...
          <Grid container spacing={2} sx={{ mt: 0.5 }}>
            <Grid item xs={12}>
              <TextField
                ...
                onChange={e => setReviewForm(f => ({ ...f, summary: e.target.value }))}
                multiline
                rows={3}
                required
              />
            ...
            <Grid item xs={12}>
              <TextField
                ...
                onChange={e => setReviewForm(f => ({ ...f, overallStatus: e.target.value }))}
              >
