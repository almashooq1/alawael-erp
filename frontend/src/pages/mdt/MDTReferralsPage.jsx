...
const TYPES = [
  ...
  { value: 'SPECIALIST', label: 'تخصصية' },
  { value: 'ASSESSMENT', label: 'تقييم' },
  { value: 'THERAPY', label: 'علاج' },
...
export default function MDTReferralsPage() {
  ...
  const handleEdit = r => {
    setForm({
      beneficiary: r.beneficiary?._id || r.beneficiary || '',
  ...
  const handleDelete = async id => {
    ...
    try {
      await referralsService.remove(id);
      loadReferrals();
    } catch {
      /* */
    }
  ...
  const openDetail = async r => {
    try {
      const res = await referralsService.getById(r._id);
  ...
  const reloadDetail = async id => {
    try {
      const res = await referralsService.getById(id);
      setDetail(res.data || res);
    } catch {
      /* */
    }
    ...
    try {
      if (actionType === 'accept') {
        ...
        await referralsService.complete(detail._id, {
          ...
          recommendations: actionForm.recommendations,
          followUp: actionForm.followUp,
        });
      }
        ...
        <Box display="flex" alignItems="center" gap={2}>
          ...
          <Box>
            <Typography variant="h4" fontWeight="bold">
              تذاكر الإحالة الداخلية
            </Typography>
            <Typography variant="body2" color="text.secondary">
              إنشاء وإدارة الإحالات، القبول، الرفض، والإكمال
            </Typography>
          </Box>
        </Box>
        <Box display="flex" gap={1}>
          <Tooltip title="تحديث">
            <IconButton onClick={loadReferrals}>
              <RefreshIcon />
            </IconButton>
          </Tooltip>
              ...
              <TableBody>
                {referrals.map(r => (
                  ...
                  >
                    ...
                    <TableCell>
                      <Chip
                        label={TYPES.find(t => t.value === r.type)?.label || r.type}
                        size="small"
                        variant="outlined"
                      />
                    </TableCell>
        ...
        <DialogContent>
          ...
          <Grid container spacing={2} sx={{ mt: 0.5 }}>
            ...
            <Grid item xs={12}>
              <TextField
                fullWidth
                label="الأخصائي المحول إليه (اختياري)"
                value={form.toSpecialist}
                onChange={e => setForm(f => ({ ...f, toSpecialist: e.target.value }))}
                placeholder="ID الأخصائي"
              />
            </Grid>
            <Grid item xs={12}>
              <TextField
              ...
              <Box display="flex" justifyContent="space-between" alignItems="center">
                <Box>
                  ...
                  <Typography variant="caption" color="text.secondary">
                    ...
                    {TYPES.find(t => t.value === detail.type)?.label || detail.type}
                  </Typography>
                </Box>
                <Box display="flex" gap={1} alignItems="center">
                  <Chip
                    ...
                    size="small"
                    sx={{ bgcolor: PRIORITY_COLORS[detail.priority], color: '#fff' }}
                  />
                  <Chip
              ...
              {detailTab === 0 && (
                <Grid container spacing={2}>
                  <Grid item xs={6}>
                    ...
                    <Typography>
                      {detail.referredByName || detail.referredBy?.name || '-'}
                    </Typography>
                  </Grid>
                  <Grid item xs={6}>
                    <Typography variant="caption" color="text.secondary">
                  ...
                  <Grid item xs={6}>
                    ...
                    <Typography>{detail.fromDepartment || '-'}</Typography>
                  </Grid>
                  <Grid item xs={6}>
                    <Typography variant="caption" color="text.secondary">
                      إلى قسم
                    </Typography>
                    <Typography>{detail.toDepartment || '-'}</Typography>
                  </Grid>
                  <Grid item xs={12}>
                    <Divider />
                  </Grid>
                      ...
                      <CardContent>
                        <Grid container spacing={1}>
                          <Grid item xs={6}>
                            ...
                            <Typography>{detail.response.respondedByName || '-'}</Typography>
                          </Grid>
                          <Grid item xs={6}>
                            <Typography variant="caption" color="text.secondary">
                              التاريخ
                          ...
                          {detail.response.acceptanceNote && (
                            <Grid item xs={12}>
                              <Typography variant="caption" color="text.secondary">
                                ملاحظة القبول
                              </Typography>
                              <Typography>{detail.response.acceptanceNote}</Typography>
                            </Grid>
                          )}
                          {detail.response.rejectionReason && (
                            <Grid item xs={12}>
                              <Typography variant="caption" color="text.secondary">
                                سبب الرفض
                              </Typography>
                        ...
                        <Grid container spacing={1}>
                          {detail.outcome.summary && (
                            <Grid item xs={12}>
                              ...
                              <Typography>{detail.outcome.summary}</Typography>
                            </Grid>
                          )}
                          {detail.outcome.findings && (
                            <Grid item xs={12}>
                              ...
                              <Typography>{detail.outcome.findings}</Typography>
                            </Grid>
                          )}
                          {detail.outcome.recommendations && (
                            <Grid item xs={12}>
                              <Typography variant="caption" color="text.secondary">
                                التوصيات
                              </Typography>
                              <Typography>{detail.outcome.recommendations}</Typography>
                            </Grid>
                          )}
                          <Grid item xs={12}>
                            <Typography variant="caption" color="text.secondary">
                            ...
                            <Chip
                              label={detail.outcome.followUp ? 'نعم' : 'لا'}
                              size="small"
                              color={detail.outcome.followUp ? 'warning' : 'default'}
                            />
                          </Grid>
                        </Grid>
                      </CardContent>
                        ...
                        >
                          ...
                          <Box flex={1}>
                            <Box display="flex" justifyContent="space-between" alignItems="center">
                              ...
                              <Typography variant="caption" color="text.secondary">
                                {h.date ? new Date(h.date).toLocaleString('ar') : ''}
                              </Typography>
                            </Box>
                            {h.performedByName && (
                              <Typography variant="caption" color="text.secondary">
                                بواسطة: {h.performedByName}
                              </Typography>
                            )}
                            {h.notes && (
      ...
      <Dialog open={actionOpen} onClose={() => setActionOpen(false)} maxWidth="sm" fullWidth>
        ...
        <DialogContent>
          {actionType === 'complete' ? (
            <Grid container spacing={2} sx={{ mt: 0.5 }}>
              <Grid item xs={12}>
                <TextField
                  ...
                  onChange={e => setActionForm(f => ({ ...f, summary: e.target.value }))}
                  multiline
                  rows={2}
                  required
                />
              ...
              <Grid item xs={12}>
                <TextField
                  fullWidth
                  label="النتائج"
                  value={actionForm.findings}
                  onChange={e => setActionForm(f => ({ ...f, findings: e.target.value }))}
                  multiline
                  rows={2}
                />
              </Grid>
              <Grid item xs={12}>
                <TextField
                  fullWidth
                  label="التوصيات"
                  value={actionForm.recommendations}
                  onChange={e => setActionForm(f => ({ ...f, recommendations: e.target.value }))}
                  multiline
                  rows={2}
                />
              </Grid>
              <Grid item xs={12}>
                <TextField
                  ...
                  select
                  label="متابعة مطلوبة"
                  value={actionForm.followUp ? 'yes' : 'no'}
                  onChange={e => setActionForm(f => ({ ...f, followUp: e.target.value === 'yes' }))}
                >
            ...
            <TextField
              fullWidth
              label={actionType === 'accept' ? 'ملاحظة القبول' : 'سبب الرفض'}
