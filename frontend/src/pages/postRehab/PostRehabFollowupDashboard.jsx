...
import {
  FollowTheSigns as FollowIcon,
  EventNote as VisitIcon,
  TrendingUp as ImpactIcon,
  ReplayCircleFilled as ReEnrollIcon,
...
export default function PostRehabFollowupDashboard() {
  ...
  const loadData = useCallback(async () => {
    ...
    setError(null);
    try {
      const [dashRes, caseRes, visitRes, impactRes, surveyRes, reRes, overdueRes] =
      ...
      setCases(
        caseRes.status === 'fulfilled'
          ? caseRes.value?.data?.data?.cases || caseRes.value?.data?.data || []
          : []
      );
      setVisits(
        visitRes.status === 'fulfilled'
          ? visitRes.value?.data?.data?.visits || visitRes.value?.data?.data || []
          : []
      );
      setImpacts(
        impactRes.status === 'fulfilled'
          ? impactRes.value?.data?.data?.measurements || impactRes.value?.data?.data || []
          : []
      );
      setSurveys(
        surveyRes.status === 'fulfilled'
          ? surveyRes.value?.data?.data?.surveys || surveyRes.value?.data?.data || []
          : []
      );
      setReEnrollments(
        reRes.status === 'fulfilled'
          ? reRes.value?.data?.data?.requests || reRes.value?.data?.data || []
          : []
      );
  ...
  const handleCreateReEnroll = async () => {
    try {
      ...
      setError('فشل تقديم طلب إعادة التسجيل');
    }
  };
  ...
  if (loading)
    return (
      <Box p={4} textAlign="center">
        <CircularProgress />
        <Typography mt={2}>جاري تحميل بيانات المتابعة...</Typography>
      </Box>
    );
  ...
  return (
    <Box p={3} dir="rtl">
      ...
      <Box display="flex" justifyContent="space-between" alignItems="center" mb={3}>
        <Box>
          <Typography variant="h4" fontWeight="bold">
            <FollowIcon sx={{ mr: 1, verticalAlign: 'middle' }} />
            متابعة ما بعد التأهيل
          </Typography>
          <Typography color="text.secondary">
            تتبع الحالات، الزيارات، قياس الأثر، وإعادة التسجيل
          </Typography>
        </Box>
        <Box display="flex" gap={1}>
          <Button variant="contained" startIcon={<AddIcon />} onClick={() => setCaseDialog(true)}>
            حالة جديدة
          </Button>
      ...
      <Grid container spacing={2} mb={3}>
        <Grid item xs={6} md={2.4}>
          <KPI
            ...
            value={dashboard?.activeCases || cases.filter(c => c.status === 'active').length}
            icon={<FollowIcon />}
          />
        </Grid>
        <Grid item xs={6} md={2.4}>
          <KPI
            ...
            value={
              dashboard?.scheduledVisits || visits.filter(v => v.status === 'scheduled').length
            }
            icon={<VisitIcon />}
            color="info.main"
          />
        ...
        <Grid item xs={6} md={2.4}>
          <KPI
            title="طلبات إعادة تسجيل"
            value={reEnrollments.filter(r => r.status === 'pending').length}
            icon={<ReEnrollIcon />}
            color="warning.main"
          />
        </Grid>
          ...
          <Table size="small">
            <TableHead>
              <TableRow>
                ...
                <TableCell>البرنامج المكتمل</TableCell>
                <TableCell>تاريخ الإنشاء</TableCell>
                <TableCell>الحالة</TableCell>
                <TableCell>التنبيهات</TableCell>
              </TableRow>
            </TableHead>
            <TableBody>
              {cases.length === 0 ? (
      ...
      <TabPanel value={tab} index={1}>
        <Box display="flex" justifyContent="flex-end" mb={2}>
          <Button variant="contained" startIcon={<AddIcon />} onClick={() => setVisitDialog(true)}>
            جدولة زيارة
          </Button>
        </Box>
        <TableContainer component={Paper} variant="outlined">
                ...
                impacts.map(m => (
                  <TableRow key={m._id || m.id}>
                    ...
                    <TableCell>
                      {m.improvement !== null ? (
                        <Chip
                          label={`${m.improvement > 0 ? '+' : ''}${m.improvement}%`}
                          color={m.improvement > 0 ? 'success' : 'error'}
                          size="small"
                        />
                      ) : (
                        '—'
              ...
              {surveys.length === 0 ? (
                ...
                surveys.map(s => (
                  <TableRow key={s._id || s.id}>
                    ...
                    <TableCell>
                      {getChip(s.status, {
                        submitted: { label: 'مكتمل', color: 'success' },
      ...
      <TabPanel value={tab} index={4}>
        <Box display="flex" justifyContent="flex-end" mb={2}>
          <Button
            variant="contained"
            startIcon={<AddIcon />}
            onClick={() => setReEnrollDialog(true)}
          >
            طلب إعادة تسجيل
          </Button>
        </Box>
          ...
          <Table size="small">
            <TableHead>
              <TableRow>
                ...
                <TableCell>البرنامج المطلوب</TableCell>
                <TableCell>الحالة</TableCell>
                <TableCell>التاريخ</TableCell>
              </TableRow>
            </TableHead>
        ...
        <DialogContent>
          <Grid container spacing={2} sx={{ mt: 1 }}>
            <Grid item xs={12}>
              <TextField
                fullWidth
                label="رقم/معرف المستفيد"
                value={caseForm.beneficiaryId}
                onChange={e => setCaseForm({ ...caseForm, beneficiaryId: e.target.value })}
              />
            </Grid>
            <Grid item xs={12}>
              <TextField
                fullWidth
                label="البرنامج المكتمل"
                value={caseForm.programCompleted}
                onChange={e => setCaseForm({ ...caseForm, programCompleted: e.target.value })}
              />
            </Grid>
            <Grid item xs={12}>
              <TextField
          ...
          <Grid container spacing={2} sx={{ mt: 1 }}>
            <Grid item xs={12}>
              <TextField
                fullWidth
                label="معرف الحالة"
                value={visitForm.caseId}
                onChange={e => setVisitForm({ ...visitForm, caseId: e.target.value })}
              />
            </Grid>
            <Grid item xs={6}>
              <TextField
      ...
      <Dialog
        open={reEnrollDialog}
        onClose={() => setReEnrollDialog(false)}
        maxWidth="sm"
        fullWidth
      >
          ...
          <Grid container spacing={2} sx={{ mt: 1 }}>
            <Grid item xs={12}>
              <TextField
                fullWidth
                label="معرف الحالة"
                value={reEnrollForm.caseId}
                onChange={e => setReEnrollForm({ ...reEnrollForm, caseId: e.target.value })}
              />
            </Grid>
            <Grid item xs={12}>
              <TextField
                ...
                value={reEnrollForm.requestedProgram}
                onChange={e =>
                  setReEnrollForm({ ...reEnrollForm, requestedProgram: e.target.value })
                }
              />
            </Grid>
            <Grid item xs={12}>
              <TextField
                ...
                rows={3}
                label="السبب"
                value={reEnrollForm.reason}
                onChange={e => setReEnrollForm({ ...reEnrollForm, reason: e.target.value })}
              />
            </Grid>
          </Grid>
        </DialogContent>
        <DialogActions>
          ...
          <Button variant="contained" onClick={handleCreateReEnroll}>
            تقديم
          </Button>
        </DialogActions>
      </Dialog>
