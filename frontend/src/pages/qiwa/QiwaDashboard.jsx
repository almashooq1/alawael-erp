...
import {
  ...
  PersonSearch as VerifyIcon,
  Description as ContractIcon,
  TrendingUp as NitaqatIcon,
...
export default function QiwaDashboard() {
    ...
    try {
      ...
      setEmployees(
        empRes.status === 'fulfilled'
          ? empRes.value?.data?.data?.employees || empRes.value?.data?.data || []
          : []
      );
      setContracts(
        conRes.status === 'fulfilled'
          ? conRes.value?.data?.data?.contracts || conRes.value?.data?.data || []
          : []
      );
  ...
  const handleVerify = async () => {
    try {
      ...
      setError('فشل التحقق من بيانات الموظف');
    }
  };

  const handleRegisterContract = async () => {
    try {
      await qiwaApi.registerContract({
        ...contractForm,
        basicSalary: Number(contractForm.basicSalary),
      });
      setNewContractDialog(false);
      loadData();
    } catch {
      setError('فشل تسجيل العقد');
    }
  };
  ...
  const loadWPSCompliance = async () => {
    try {
      ...
      setError('فشل تحميل تقرير حماية الأجور');
    }
  };
  ...
  if (loading)
    return (
      <Box p={4} textAlign="center">
        <CircularProgress />
        <Typography mt={2}>جاري تحميل بيانات قوى...</Typography>
      </Box>
    );
  ...
  return (
    <Box p={3} dir="rtl">
      ...
      <Grid container spacing={2} mb={3}>
        <Grid item xs={6} md={3}>
          <KPI title="الموظفون" value={employees.length} icon={<VerifyIcon />} />
        </Grid>
        <Grid item xs={6} md={3}>
          <KPI
            title="العقود النشطة"
        ...
        <Grid item xs={6} md={3}>
          <KPI
            title="نطاقات"
            value={nitaqat?.band || nitaqat?.zone || '—'}
            icon={<NitaqatIcon />}
            color={
              nitaqat?.band === 'green' || nitaqat?.band === 'platinum'
                ? 'success.main'
                : 'warning.main'
            }
          />
        ...
        <Grid item xs={6} md={3}>
          <KPI
            title="نسبة السعودة"
            value={nitaqat?.saudizationRate !== null ? `${nitaqat.saudizationRate}%` : '—'}
            icon={<NitaqatIcon />}
            color="info.main"
          />
        </Grid>
      ...
      <TabPanel value={tab} index={0}>
        ...
        <TableContainer component={Paper} variant="outlined">
          <Table size="small">
            <TableHead>
              <TableRow>
                <TableCell>رقم الإقامة / الهوية</TableCell>
              ...
              {employees.length === 0 ? (
                ...
                employees.map((e, i) => (
                  <TableRow key={i}>
                    ...
                    <TableCell>
                      <Chip
                        label={e.status === 'active' ? 'نشط' : e.status || '—'}
      ...
      <TabPanel value={tab} index={1}>
        <Box display="flex" justifyContent="space-between" mb={2}>
          ...
          <Button
            variant="contained"
            startIcon={<AddIcon />}
            onClick={() => setNewContractDialog(true)}
          >
            تسجيل عقد
          </Button>
        </Box>
              ...
              {contracts.length === 0 ? (
                ...
                contracts.map((c, i) => (
                  <TableRow key={i}>
                    ...
                    <TableCell>
                      {c.contractType === 'unlimited' ? 'غير محدد المدة' : 'محدد المدة'}
                    </TableCell>
                    <TableCell>{c.basicSalary?.toLocaleString() || '—'} ر.س</TableCell>
                    <TableCell>
                      <Chip
                        label={c.status === 'active' ? 'نشط' : c.status || '—'}
      ...
      <TabPanel value={tab} index={2}>
        <Box display="flex" gap={2} mb={2} alignItems="center">
          <TextField
            ...
            value={wpsPeriod}
            onChange={e => setWpsPeriod(e.target.value)}
            size="small"
            sx={{ width: 200 }}
          />
          <Button variant="contained" onClick={loadWPSCompliance}>
            عرض التقرير
          </Button>
        </Box>
        {wpsCompliance ? (
          <Card>
            <CardContent>
              <Typography variant="h6" mb={2}>
                تقرير امتثال حماية الأجور — {wpsCompliance.period || wpsPeriod}
              </Typography>
              <Grid container spacing={2}>
                <Grid item xs={4}>
                  ...
                  <Box>
                    <Chip
                      label={wpsCompliance.compliant ? 'ملتزم' : 'غير ملتزم'}
                      color={wpsCompliance.compliant ? 'success' : 'error'}
                    />
                  </Box>
                </Grid>
                <Grid item xs={4}>
                  ...
                  <Typography variant="h5" fontWeight="bold">
                    {wpsCompliance.totalPaid?.toLocaleString() || '—'} ر.س
                  </Typography>
                </Grid>
                <Grid item xs={4}>
                  <Typography variant="caption">الموظفون المشمولون</Typography>
                  <Typography variant="h5" fontWeight="bold">
                    {wpsCompliance.employeesCovered || '—'}
                  </Typography>
                </Grid>
              </Grid>
                ...
                <CardContent>
                  ...
                  <Box display="flex" alignItems="center" gap={2} mb={2}>
                    ...
                    <Box>
                      ...
                      <Typography variant="h4" fontWeight="bold">
                        {nitaqat.saudizationRate ?? '—'}%
                      </Typography>
                    </Box>
                  </Box>
                  <LinearProgress
                    variant="determinate"
                    value={nitaqat.saudizationRate || 0}
                ...
                <CardContent>
                  ...
                  <Box display="flex" flexDirection="column" gap={1}>
                    <Box display="flex" justifyContent="space-between">
                      <Typography>الموظفون السعوديون</Typography>
                      <Typography fontWeight="bold">{nitaqat.saudiCount ?? '—'}</Typography>
                    </Box>
                    <Divider />
                    <Box display="flex" justifyContent="space-between">
                      <Typography>الموظفون غير السعوديين</Typography>
                      <Typography fontWeight="bold">{nitaqat.nonSaudiCount ?? '—'}</Typography>
                    </Box>
                    <Divider />
                    <Box display="flex" justifyContent="space-between">
                      <Typography>الإجمالي</Typography>
                      <Typography fontWeight="bold">{nitaqat.totalEmployees ?? '—'}</Typography>
                    </Box>
                    <Divider />
                    <Box display="flex" justifyContent="space-between">
                      <Typography>النسبة المطلوبة</Typography>
                      <Typography fontWeight="bold">{nitaqat.requiredRate ?? '—'}%</Typography>
                    </Box>
                  </Box>
                </CardContent>
              </Card>
          ...
          {verifyResult && (
            <Box mt={2} p={2} bgcolor="grey.50" borderRadius={1}>
              ...
              <Grid container spacing={1}>
                <Grid item xs={6}>
                  <Typography variant="caption">الاسم</Typography>
                  <Typography>{verifyResult.name || verifyResult.fullName || '—'}</Typography>
                </Grid>
                <Grid item xs={6}>
                  <Typography variant="caption">الحالة</Typography>
                  <Chip
                    label={verifyResult.verified ? 'تم التحقق' : 'غير مؤكد'}
                    color={verifyResult.verified ? 'success' : 'warning'}
                    size="small"
                  />
                </Grid>
                <Grid item xs={6}>
                  <Typography variant="caption">المهنة</Typography>
                  <Typography>{verifyResult.occupation || '—'}</Typography>
                </Grid>
                <Grid item xs={6}>
                  <Typography variant="caption">الكفيل</Typography>
                  <Typography>{verifyResult.sponsor || verifyResult.employer || '—'}</Typography>
                </Grid>
              </Grid>
            </Box>
          )}
      ...
      {/* ─── New Contract Dialog ─── */}
      <Dialog
        open={newContractDialog}
        onClose={() => setNewContractDialog(false)}
        maxWidth="sm"
        fullWidth
      >
          ...
          <Grid container spacing={2} sx={{ mt: 1 }}>
            <Grid item xs={6}>
              <TextField
                ...
                value={contractForm.employeeIqama}
                onChange={e => setContractForm({ ...contractForm, employeeIqama: e.target.value })}
              />
            </Grid>
            <Grid item xs={6}>
              <TextField
            ...
            <Grid item xs={6}>
              <TextField
                ...
                value={contractForm.jobTitleArabic}
                onChange={e => setContractForm({ ...contractForm, jobTitleArabic: e.target.value })}
              />
            </Grid>
            <Grid item xs={6}>
              <TextField
