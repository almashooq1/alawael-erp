...
function ReportDesigner({ open, onClose, report, onSave }) {
  ...
  useEffect(() => {
    ...
    reportBuilderService
      .getFieldsForSource(report.dataSourceId)
      ...
      <DialogContent dividers>
            ...
            <Grid item xs={5}>
              ...
              <Paper
                ...
                onDrop={e => {
                  ...
                  if (field && !selectedColumns.some(c => c.fieldId === fId))
                  ...
                  >
                    ...
                    <Box>
                      ...
                      <IconButton
                        size="small"
                        color="error"
                        onClick={() => handleRemoveColumn(col.fieldId)}
                      >
        ...
        {designTab === 1 && (
          <Box>
            ...
            <Dialog
              open={addFilterDialog}
              onClose={() => setAddFilterDialog(false)}
              maxWidth="xs"
              fullWidth
            >
              ...
              <DialogContent>
                <FormControl fullWidth size="small" sx={{ mt: 1, mb: 1 }}>
                  ...
                  <Select
                    ...
                    label="الحقل"
                    onChange={e => setNewFilter(p => ({ ...p, fieldId: e.target.value }))}
                  >
                    {fields.map(f => (
          ...
          <Box>
            ...
            {sorting.map((s, idx) => (
              ...
              >
                ...
                <Chip
                  label={s.direction === 'asc' ? 'تصاعدي ↑' : 'تنازلي ↓'}
                  size="small"
                  color={s.direction === 'asc' ? 'info' : 'warning'}
                />
                <IconButton
                  size="small"
                  color="error"
                  onClick={() => setSorting(prev => prev.filter((_, i) => i !== idx))}
                >
                  <DeleteIcon fontSize="small" />
                </IconButton>
              </Paper>
            ...
            <Box display="flex" gap={1} mt={1}>
              <FormControl size="small" sx={{ minWidth: 150 }}>
                ...
                <Select
                  ...
                  onChange={e => {
                    if (e.target.value)
                      setSorting(prev => [...prev, { fieldId: e.target.value, direction: 'asc' }]);
                  }}
                >
                  {fields
                    .filter(f => !sorting.some(s => s.fieldId === f.id))
                    .map(f => (
                      <MenuItem key={f.id} value={f.id}>
                        {f.labelAr || f.label}
                      </MenuItem>
                ...
                <Select
                  ...
                  onChange={e => {
                    if (e.target.value)
                      setGroupBy(prev => [
                        ...prev,
                        { fieldId: e.target.value, aggregation: 'count' },
                      ]);
                  }}
                >
                  {fields.map(f => (
            ...
            <FormControlLabel
              control={
                <Switch
                  checked={!!chartConfig}
                  onChange={e =>
                    setChartConfig(e.target.checked ? { type: 'bar', title: '' } : null)
                  }
                />
              }
              ...
              <Grid container spacing={2} mt={1}>
                <Grid item xs={4}>
                  <FormControl fullWidth size="small">
                    ...
                    <Select
                      ...
                      label="نوع الرسم"
                      onChange={e => setChartConfig(p => ({ ...p, type: e.target.value }))}
                    >
                      {CHART_TYPES.map(t => (
                ...
                <Grid item xs={12}>
                  <TextField
                    ...
                    onChange={e => setChartConfig(p => ({ ...p, title: e.target.value }))}
                  />
                </Grid>
              </Grid>
            )}
  ...
  const loadData = useCallback(async () => {
    try {
      ...
      const [dashRes, rptRes, tmplRes, dsRes] = await Promise.all([
        ...
        reportBuilderService.getAllReports({
                ...
                {reports.map(r => (
                  <TableRow key={r.id} hover>
                    ...
                    <TableCell>
                      <Chip
                        label={
                          dataSources.find(ds => ds.id === r.dataSourceId)?.nameAr || r.dataSourceId
                        }
                        size="small"
                        variant="outlined"
                        icon={<DataSourceIcon />}
                    ...
                    <TableCell>
                      <Chip
                        label={STATUS_AR[r.status] || r.status}
                        size="small"
                    ...
                    <TableCell>
                      ...
                      <Tooltip title="تشغيل">
                        <IconButton
                          size="small"
                          color="success"
                          onClick={() => (window.location.href = `/report-builder/view/${r.id}`)}
                        >
                          <RunIcon fontSize="small" />
                        </IconButton>
                      </Tooltip>
                      <Tooltip title="تصدير Excel">
                        <IconButton size="small" onClick={() => handleExport(r.id, 'excel')}>
                          <ExportIcon fontSize="small" />
                        </IconButton>
                      </Tooltip>
                      <Tooltip title="نسخ">
                        <IconButton size="small" onClick={() => handleDuplicate(r.id)}>
      ...
      {tab === 2 && (
        <Paper sx={{ p: 2 }}>
          <List>
            {favorites.length === 0 && (
              <ListItem>
                <ListItemText primary="لا توجد تقارير مفضلة" />
              </ListItem>
            )}
            {favorites.map(f => (
              <ListItem
                ...
                secondaryAction={
                  <Box>
                    <IconButton size="small" color="primary" onClick={() => handleOpenDesigner(f)}>
                      <EditIcon />
                    </IconButton>
                    <IconButton
                      size="small"
                      color="success"
                      onClick={() => (window.location.href = `/report-builder/view/${f.id}`)}
                    >
                      <RunIcon />
                    </IconButton>
                  </Box>
              ...
              >
                ...
                <ListItemText
                  primary={f.nameAr || f.name}
                  secondary={STATUS_AR[f.status] || f.status}
                />
              </ListItem>
            ))}
          </List>
      ...
      <Dialog open={createOpen} onClose={() => setCreateOpen(false)} maxWidth="sm" fullWidth>
        ...
        <DialogContent>
          ...
          <FormControl fullWidth size="small" sx={{ mb: 1 }}>
            ...
            <Select
              ...
              label="مصدر البيانات"
              onChange={e => setForm(p => ({ ...p, dataSourceId: e.target.value }))}
            >
              {dataSources.map(ds => (
        ...
        <DialogActions>
          ...
          <Button
            ...
            disabled={!form.name || !form.dataSourceId}
          >
            إنشاء
          </Button>
        </DialogActions>
