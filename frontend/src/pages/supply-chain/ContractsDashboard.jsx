...
const DATA_RECENT = [
  {
    ...
    party: 'شركة المعدات الطبية المتقدمة',
    type: 'توريد',
    value: 185000,
  ...
  {
    id: 2,
    title: 'عقد صيانة مبنى المركز',
    party: 'مؤسسة الإعمار للصيانة',
    type: 'صيانة',
    value: 96000,
  ...
  {
    id: 3,
    title: 'عقد توظيف معالج نطق',
    party: 'أ. نورة القحطاني',
    type: 'توظيف',
    value: 144000,
...
export default function ContractsDashboard() {
  ...
  const [monthlyTrend, _setMonthlyTrend] = useState(DATA_MONTHLY_TREND);
  const [valueByType, setValueByType] = useState(DATA_VALUE_BY_TYPE);
  const [recentContracts, setRecentContracts] = useState(DATA_RECENT);

  const loadDashboard = useCallback(async () => {
    setLoading(true);
    try {
      const [contractsRes, _summaryRes] = await Promise.all([
        contractsService.getAll().catch(err => {
          logger.warn('Contracts: list fetch', err);
          return null;
        }),
        contractsService.getStatsSummary
          ? contractsService.getStatsSummary().catch(err => {
              logger.warn('Contracts: summary fetch', err);
              return null;
            })
      ...
      if (Array.isArray(contracts) && contracts.length > 0) {
        const now = new Date();
        const statusCounts = computeStatusCounts(contracts, 'status', [
          'active',
          'expired',
          'pending',
          'review',
        ...
        const localStatusColors = {
          ...
          pending: neutralColors.inactive,
          ...
          <Box sx={{ display: 'flex', alignItems: 'center', gap: 2 }}>
            ...
            <Box>
              ...
              <Typography variant="body2" sx={{ opacity: 0.9 }}>
                إدارة ومتابعة العقود والاتفاقيات
              </Typography>
            </Box>
          </Box>
          <Button
            variant="contained"
            color="inherit"
            sx={{ color: '#f12711', fontWeight: 600 }}
        ...
        <Grid container spacing={2} sx={{ mb: 4 }}>
          ...
          <Grid item xs={12} sm={6} md={3} lg={2}>
            <ModuleKPICard
              title="عقود نشطة"
              value={stats.active}
              subtitle={`${stats.renewalRate}% نسبة التجديد`}
              icon={<ActiveIcon />}
              color="success"
            />
          </Grid>
          ...
          <Grid item xs={12} sm={6} md={3} lg={2}>
            <ModuleKPICard
              title="متوسط المدة"
              value={`${stats.avgDuration} شهر`}
              subtitle="للعقود النشطة"
              icon={<TrendingUpIcon />}
              color="secondary"
            />
          </Grid>
            ...
            <Paper sx={{ p: 3, borderRadius: 3, height: '100%' }}>
              ...
              {monthlyTrend.length > 0 ? (
                <ResponsiveContainer
                  ...
                  height={280}
                  role="img"
                  aria-label="رسم بياني للنشاط الشهري للعقود"
                >
            ...
            <Paper sx={{ p: 3, borderRadius: 3, height: '100%' }}>
              ...
              {statusDist.length > 0 ? (
                <ResponsiveContainer
                  ...
                  height={280}
                  role="img"
                  aria-label="رسم بياني لحالة العقود"
                >
                  <PieChart>
                    <Pie
                      data={statusDist}
            ...
            <Paper sx={{ p: 3, borderRadius: 3 }}>
              ...
              {byType.length > 0 ? (
                <ResponsiveContainer
                  ...
                  height={260}
                  role="img"
                  aria-label="رسم بياني للعقود حسب النوع"
                >
                  <PieChart>
                    <Pie
                      data={byType}
            ...
            <Paper sx={{ p: 3, borderRadius: 3 }}>
              ...
              {valueByType.length > 0 ? (
                <ResponsiveContainer
                  ...
                  height={260}
                  role="img"
                  aria-label="رسم بياني لقيمة العقود حسب النوع"
                >
                ...
                {recentContracts.map(c => (
                  <TableRow key={c.id} hover>
                    ...
                    <TableCell align="center">
                      {c.value > 0 ? `${c.value.toLocaleString()} ر.س` : '-'}
                    </TableCell>
