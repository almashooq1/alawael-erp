...
export default function TelehealthVideoRoom() {
  ...
  const handleAddNote = async () => {
    ...
    try {
      const { data } = await telehealthService.addNote(sessionId, {
        content: noteInput,
        type: 'observation',
      });
  ...
  const handleRecordVitals = async () => {
    try {
      ...
      if (vitalForm.oxygenSaturation)
        payload.oxygenSaturation = parseInt(vitalForm.oxygenSaturation);
          ...
          <Paper sx={{ p: 1.5, display: 'flex', justifyContent: 'center', gap: 2 }}>
            <Tooltip title={micOn ? 'كتم الصوت' : 'تشغيل الصوت'}>
              <IconButton
                ...
                sx={{
                  bgcolor: micOn ? 'transparent' : 'error.main',
                  color: micOn ? 'inherit' : '#fff',
                }}
              >
            ...
            <Tooltip title={camOn ? 'إيقاف الكاميرا' : 'تشغيل الكاميرا'}>
              <IconButton
                ...
                sx={{
                  bgcolor: camOn ? 'transparent' : 'error.main',
                  color: camOn ? 'inherit' : '#fff',
                }}
              >
                  ...
                  {vitals.map((v, i) => (
                    <Card key={i} sx={{ mb: 1 }} variant="outlined">
                      <CardContent sx={{ py: 1, '&:last-child': { pb: 1 } }}>
                        ...
                        <Typography variant="caption" color="text.secondary">
                          {new Date(v.timestamp).toLocaleTimeString('ar-SA', {
                            hour: '2-digit',
                            minute: '2-digit',
                          })}
                        </Typography>
