...
const statusLabels = {
  draft: 'مسودة',
  pending_review: 'قيد المراجعة الداخلية',
  submitted: 'مقدّم للتأمين',
  under_review: 'قيد مراجعة التأمين',
  approved: 'مُوافق',
  partially_approved: 'موافق جزئياً',
  denied: 'مرفوض',
  appealed: 'تم الاستئناف',
  appeal_approved: 'الاستئناف مُوافق',
  appeal_denied: 'الاستئناف مرفوض',
                    ...
                    >
                      ...
                      <TableCell>
                        {(req.services || []).slice(0, 2).map((s, i) => (
                          <Chip
                            key={i}
                            label={serviceCategories[s.serviceCategory] || s.serviceCategory}
                            size="small"
                            sx={{ mr: 0.5 }}
                          />
                        ))}
                        ...
                        {req.status === 'draft' && (
                          <Tooltip title="إرسال للمراجعة">
                            <IconButton
                              ...
                              onClick={e => {
                                e.stopPropagation();
                                treatmentAuthorizationService
                                  .submitForReview(req._id)
                                  .then(fetchData);
                              }}
                            >
                              <Send fontSize="small" />
          ...
          <>
            ...
            <DialogContent>
              ...
              <Grid container spacing={2} sx={{ mb: 2 }}>
                <Grid item xs={6}>
                  <Typography variant="body2">
                    <strong>الاسم:</strong> {selectedRequest.beneficiary?.name || '—'}
                  </Typography>
                </Grid>
                <Grid item xs={6}>
                  <Typography variant="body2">
                    <strong>رقم الملف:</strong> {selectedRequest.beneficiary?.fileNumber || '—'}
                  </Typography>
                </Grid>
              </Grid>
              ...
              <Grid container spacing={2} sx={{ mb: 2 }}>
                <Grid item xs={6}>
                  <Typography variant="body2">
                    <strong>الشركة:</strong> {selectedRequest.insurance?.providerName || '—'}
                  </Typography>
                </Grid>
                <Grid item xs={6}>
                  <Typography variant="body2">
                    <strong>رقم البوليصة:</strong> {selectedRequest.insurance?.policyNumber || '—'}
                  </Typography>
                </Grid>
              </Grid>
              ...
              {(selectedRequest.services || []).map((svc, idx) => (
                <Box key={idx} sx={{ p: 1.5, mb: 1, bgcolor: 'grey.50', borderRadius: 1 }}>
                  ...
                  <Typography variant="caption">
                    الجلسات: {svc.sessionsUsed || 0}/
                    {svc.sessionsApproved || svc.sessionsRequested || 0} | التكرار:{' '}
                    {svc.frequencyPerWeek || 0}/أسبوع | المدة: {svc.durationMinutes || 0} دقيقة
                  </Typography>
                  {svc.sessionsRequested > 0 && (
                    <LinearProgress
                      ...
                      value={
                        ((svc.sessionsUsed || 0) /
                          (svc.sessionsApproved || svc.sessionsRequested)) *
                        100
                      }
