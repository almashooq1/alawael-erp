...
export default function WorkflowFormBuilder() {
  ...
  const openEditForm = form => {
    ...
    setFormData({
      ...
      fields: form.fields || [],
      layout: form.layout || { type: 'single_column' },
    });
    setFormDialog(true);
  };
    ...
    <Box sx={{ p: 3, direction: 'rtl' }}>
          ...
          <Grid item xs={12}>
            <Paper sx={{ p: 4, textAlign: 'center' }}>
              ...
              <Typography variant="body2" color="text.secondary" sx={{ mb: 2 }}>
                قم بإنشاء نموذج جديد لربطه بخطوات سير العمل
              </Typography>
              <Button variant="contained" startIcon={<AddIcon />} onClick={openNewForm}>
                إنشاء أول نموذج
              </Button>
            ...
            <Grid item xs={12} sm={6} md={4} key={form._id}>
              ...
              >
                  ...
                  <Box sx={{ display: 'flex', gap: 0.5, flexWrap: 'wrap', mb: 1 }}>
                    {(form.fields || []).slice(0, 4).map((f, i) => (
                      <Chip
                        ...
                        variant="outlined"
                        sx={{
                          borderColor: FIELD_TYPE_COLORS[f.fieldType] || '#999',
                          color: FIELD_TYPE_COLORS[f.fieldType] || '#999',
                        }}
                      />
                    ))}
                ...
                <CardActions sx={{ justifyContent: 'flex-end' }}>
                  <Tooltip title="تعديل">
                    <IconButton size="small" onClick={() => openEditForm(form)}>
                      <EditIcon fontSize="small" />
                    </IconButton>
                  </Tooltip>
                  <Tooltip title="نسخ">
                    <IconButton size="small" onClick={() => handleCloneForm(form._id)}>
      ...
      <Dialog open={formDialog} onClose={() => setFormDialog(false)} maxWidth="md" fullWidth>
        ...
        <DialogContent>
              ...
              <FormControl fullWidth>
                ...
                <Select
                  ...
                  onChange={e =>
                    setFormData(prev => ({
                      ...prev,
                      layout: { ...prev.layout, type: e.target.value },
                    }))
                  }
                >
          ...
          {formData.fields.length === 0 ? (
            <Alert severity="info">
              لم يتم إضافة أي حقول بعد. اضغط "إضافة حقل" لبدء تصميم النموذج.
            </Alert>
          ) : (
            <TableContainer>
              <Table size="small">
                  ...
                  {formData.fields.map((f, idx) => (
                    <TableRow key={idx}>
                      ...
                      <TableCell>
                        <Chip
                          label={
                            fieldTypes.find(t => t.value === f.fieldType)?.label || f.fieldType
                          }
                          size="small"
                          sx={{
                            bgcolor: alpha(FIELD_TYPE_COLORS[f.fieldType] || '#999', 0.1),
                            color: FIELD_TYPE_COLORS[f.fieldType] || '#999',
                          }}
                        />
                      </TableCell>
      ...
      <Dialog open={fieldDialog} onClose={() => setFieldDialog(false)} maxWidth="sm" fullWidth>
          ...
          <Grid container spacing={2} sx={{ mt: 0.5 }}>
            <Grid item xs={6}>
              <TextField
                ...
                onChange={e => setFieldData(prev => ({ ...prev, nameAr: e.target.value }))}
              />
            </Grid>
            <Grid item xs={6}>
              <TextField
                fullWidth
                label="اسم الحقل (إنجليزي)"
            ...
            <Grid item xs={6}>
              <TextField
                ...
                onChange={e => setFieldData(prev => ({ ...prev, section: e.target.value }))}
              />
            </Grid>
            <Grid item xs={6}>
              <FormControlLabel
            ...
            <Grid item xs={6}>
              <TextField
                ...
                onChange={e => setFieldData(prev => ({ ...prev, order: Number(e.target.value) }))}
              />
            </Grid>
            <Grid item xs={12}>
              <TextField
                fullWidth
                label="نص المساعدة"
                value={fieldData.helpTextAr || ''}
                onChange={e => setFieldData(prev => ({ ...prev, helpTextAr: e.target.value }))}
              />
            </Grid>
          </Grid>
        </DialogContent>
        <DialogActions>
          ...
          <Button variant="contained" onClick={saveField} disabled={!fieldData.nameAr}>
            حفظ الحقل
          </Button>
        </DialogActions>
      </Dialog>
