...
export const getVendors = async (params = {}) => {
  try {
    const { data } = await apiClient.get(`${BASE}/vendors`, { params });
    return data;
  } catch {
    return {
      success: true,
...
export const createVendor = async body => {
  try {
    const { data } = await apiClient.post(`${BASE}/vendors`, body);
    return data;
  } catch {
...
export const updateVendor = async (id, body) => {
  try {
    const { data } = await apiClient.put(`${BASE}/vendors/${id}`, body);
    return data;
  } catch {
      ...
      data: [
        {
          ...
          date: '2026-03-01',
          vendor: { name: 'شركة التقنية المتقدمة' },
          total: 75000,
          status: 'confirmed',
...
export const createPurchaseOrder = async body => {
  try {
    const { data } = await apiClient.post(`${BASE}/orders`, body);
    return data;
  } catch {
    return { success: true, data: { _id: Date.now().toString(), ...body } };
  }
...
export const getPurchaseRequests = async (params = {}) => {
  try {
    const { data } = await apiClient.get(`${BASE}/requests`, { params });
    return data;
  } catch {
    return {
      success: true,
...
export const createPurchaseRequest = async body => {
  try {
    const { data } = await apiClient.post(`${BASE}/requests`, body);
    return data;
  } catch {
    return { success: true, data: { _id: Date.now().toString(), ...body } };
  }
