        ...
        recentApplications: [
          {
            ...
            applicant: { firstName: 'أحمد', lastName: 'محمد' },
            stage: 'interview',
            rating: 4,
          },
          {
            _id: '2',
            applicationNumber: 'APP-000002',
            applicant: { firstName: 'سارة', lastName: 'العلي' },
            stage: 'screening',
            rating: 3,
          },
        ],
...
export const getJobs = async (params = {}) => {
  try {
    const { data } = await apiClient.get(`${BASE}/jobs`, { params });
    return data;
  } catch {
    return {
      success: true,
...
export const createJob = async body => {
  try {
    const { data } = await apiClient.post(`${BASE}/jobs`, body);
    return data;
  } catch {
...
export const updateJob = async (id, body) => {
  try {
    const { data } = await apiClient.put(`${BASE}/jobs/${id}`, body);
    return data;
  } catch {
...
export const deleteJob = async id => {
  try {
    const { data } = await apiClient.delete(`${BASE}/jobs/${id}`);
    return data;
  } catch {
      ...
      data: [
        {
          ...
          jobPosting: { title: { ar: 'مطور أنظمة أول' } },
          stage: 'interview',
          source: 'linkedin',
          rating: 4,
        ...
        },
        {
          ...
          jobPosting: { title: { ar: 'أخصائي موارد بشرية' } },
          stage: 'screening',
          source: 'website',
          rating: 3,
...
export const createApplication = async body => {
  try {
    const { data } = await apiClient.post(`${BASE}/applications`, body);
    return data;
  } catch {
    return { success: true, data: { _id: Date.now().toString(), ...body } };
  }
...
export const updateApplicationStage = async (id, body) => {
  try {
    const { data } = await apiClient.patch(`${BASE}/applications/${id}/stage`, body);
    return data;
  } catch {
    return { success: true };
  }
