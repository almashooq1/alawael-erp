  ...
  const selectAllPage = useCallback(() => {
    ...
    const pageDocs = documents
      .filter(doc => {
        ...
        const matchesCategory = filters.categoryFilter === 'الكل' || doc.category === filters.categoryFilter;
    ...
    <>
        ...
        <Stack spacing={2}>
          <Box sx={{ display: 'flex', gap: 2, alignItems: 'center', flexWrap: 'wrap' }}>
            ...
            <FormControl size="small" sx={{ minWidth: 150 }}>
              ...
              <Select value={filters.categoryFilter} onChange={e => filters.setCategoryFilter(e.target.value)} label="الفئة">
                {categories.map(cat => (
            ...
            <FormControl size="small" sx={{ minWidth: 150 }}>
              ...
              <Select value={filters.sortBy} onChange={e => filters.setSortBy(e.target.value)} label="الترتيب">
                ...
                <TableCell colSpan={7} align="center" sx={{ py: 8 }}>
                  <Box sx={{ textAlign: 'center' }}>
                    ...
                    <Typography variant="h6" color="textSecondary" gutterBottom>
                      {filters.searchQuery || filters.categoryFilter !== 'الكل' ? '🔍 لا توجد نتائج' : 'لا توجد مستندات'}
                    </Typography>
                    <Typography variant="body2" color="textSecondary">
                      {filters.searchQuery || filters.categoryFilter !== 'الكل' ? 'جرب تغيير معايير البحث' : 'ابدأ برفع أول مستند لك'}
                    </Typography>
                  </Box>
                </TableCell>
              </TableRow>
      ...
      {selection.selected.length > 0 && (
        <SpeedDial ariaLabel="الإجراءات الجماعية" sx={{ position: 'fixed', bottom: 24, left: 24 }} icon={<SpeedDialIcon />}>
          ...
          />
          <SpeedDialAction icon={<ShareIcon />} tooltipTitle={`مشاركة ${selection.selected.length}`} onClick={handleBulkShare} />
          <SpeedDialAction icon={<LocalOfferIcon />} tooltipTitle="تحرير الوسوم" onClick={() => handleBulkEdit('tags')} />
          <SpeedDialAction icon={<CategoryIcon />} tooltipTitle="تغيير الفئة" onClick={() => handleBulkEdit('category')} />
        </SpeedDial>
      )}
        ...
        <DialogContent sx={{ mt: 3 }}>
          {dialogs.bulkEditType === 'tags' ? (
            ...
            <FormControl fullWidth>
              ...
              <Select value={dialogs.bulkEditCategory} onChange={e => dialogs.setBulkEditCategory(e.target.value)} label="الفئة الجديدة">
              ...
              <Paper elevation={0} sx={{ p: 2, backgroundColor: '#f8f9ff', borderRadius: 2 }}>
                ...
                <Stack spacing={2}>
                  ...
                  <Box>
                    ...
                    <Typography variant="body2">{dialogs.selectedDoc.description || 'لا يوجد وصف'}</Typography>
                  </Box>
