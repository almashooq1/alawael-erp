...
export const resolvers = {
  Query: {
    ...
    projectStats: async (_: any, { projectId }: any) => {
      ...
      const predictions = db.predictions.filter((p: any) => models.some((m: any) => m.id === p.modelId));
    ...
    search: async (_: any, { query, type }: any) => {
      ...
      if (!type || type === 'User') {
        const users = db.users.filter((u: any) => u.username.includes(query) || u.email.includes(query));
        results.push(...users);
      }

      if (!type || type === 'Project') {
        const projects = db.projects.filter((p: any) => p.name.includes(query) || p.description?.includes(query));
        results.push(...projects);
      }

      if (!type || type === 'Dataset') {
        const datasets = db.datasets.filter((d: any) => d.name.includes(query));
        results.push(...datasets);
      }

      if (!type || type === 'Model') {
        const models = db.models.filter((m: any) => m.name.includes(query));
        results.push(...models);
      }
  ...
  // Field Resolvers
  User: {
    projects: (user: any) => db.projects.filter((p: any) => p.userId === user.id),
  ...
  Project: {
    user: (project: any) => db.users.find((u: any) => u.id === project.userId),

    datasets: (project: any) => db.datasets.filter((d: any) => d.projectId === project.id),

    models: (project: any) => db.models.filter((m: any) => m.projectId === project.id),
  ...
  Dataset: {
    project: (dataset: any) => db.projects.find((p: any) => p.id === dataset.projectId),
  },

  Model: {
    project: (model: any) => db.projects.find((p: any) => p.id === model.projectId),

    predictions: (model: any) => db.predictions.filter((p: any) => p.modelId === model.id),
  },

  Prediction: {
    model: (prediction: any) => db.models.find((m: any) => m.id === prediction.modelId),
  },
