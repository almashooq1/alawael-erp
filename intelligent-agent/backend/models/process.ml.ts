...
export function classifyProcessRisk(process: Process): 'high' | 'medium' | 'low' {
...
export function predictDelayProbability(process: Process): number {
  const features = extractProcessFeatures(process);
...
interface ProcessFeatures {
  ...
  // ميزات السرعة
  velocity: number; // خطوات مكتملة في الأسبوع
  acceleration: number; // تغير السرعة
...
function extractProcessFeatures(process: Process): ProcessFeatures {
  const now = Date.now();
  ...
  // حساب التعقيد
  const approvalSteps = process.steps.filter(s =>
    s.name?.includes('موافقة') || s.name?.includes('اعتماد')
  ).length;
  const documentSteps = process.steps.filter(s =>
    s.name?.includes('مستند') || s.name?.includes('وثيقة')
  ).length;
...
function tree1Predict(features: ProcessFeatures): number {
  // شجرة 1: التركيز على التأخير الحالي
  if (features.delayedSteps > 5) return 0.9;
  if (features.delayedSteps > 3) return 0.7;
  if (features.delayRatio > 0.3) return 0.6;
  if (features.avgDelayDays > 7) return 0.5;
  return features.delayRatio * 0.5;
}

function tree2Predict(features: ProcessFeatures): number {
  // شجرة 2: التركيز على السرعة والزخم
  if (features.velocity < 0.3 && features.pendingRatio > 0.5) return 0.8;
  if (features.velocity < 0.5) return 0.6;
  if (features.inProgressRatio === 0 && features.pendingRatio > 0) return 0.7;
  return (1 - features.velocity) * features.pendingRatio;
}

function tree3Predict(features: ProcessFeatures): number {
  // شجرة 3: التركيز على التعقيد والموارد
  if (features.complexityScore > 0.7 && features.velocity < 0.5) return 0.75;
  if (features.unassignedSteps > features.totalSteps * 0.3) return 0.65;
  if (features.approvalCount > 5 && features.inProgressRatio < 0.2) return 0.6;
  return features.complexityScore * 0.5;
}

function tree4Predict(features: ProcessFeatures): number {
  // شجرة 4: التركيز على الأنماط التاريخية (محاكاة)
  if (features.ageInDays > 30 && features.completionRatio < 0.5) return 0.85;
  if (features.ageInDays > 14 && features.completionRatio < 0.3) return 0.7;
...
function tree5Predict(features: ProcessFeatures): number {
  // شجرة 5: التركيز على العوامل الخارجية
  ...
  if (features.assigneeCount === 0) return 0.8 + weekendPenalty;
  if (features.assigneeCount < features.inProgressSteps) return 0.6 + weekendPenalty;
  return (features.unassignedSteps / Math.max(features.totalSteps, 1)) * 0.5 + weekendPenalty;
}

// ================ توصيات ذكية متقدمة ================

export function generateAIRecommendation(process: Process): string {
...
export function identifyBottlenecks(process: Process): Bottleneck[] {
