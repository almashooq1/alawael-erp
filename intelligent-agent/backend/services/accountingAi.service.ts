...
const ACCOUNTING_API_BASE_URL =
  process.env.ACCOUNTING_API_BASE_URL || 'http://localhost:5001/api/accounting';
...
type CashFlowStatement = Record<string, any>;
...
const normalizeNumber = (value: any) => (Number.isFinite(value) ? Number(value) : 0);
...
export const getBalanceSheet = async (asOfDate: string) => {
  ...
  return fetchJson<BalanceSheet>(url);
};
...
export const getIncomeStatement = async (startDate: string, endDate: string) => {
  ...
  return fetchJson<IncomeStatement>(url);
};

export const getCashFlowStatement = async (startDate: string, endDate: string) => {
  const url = buildUrl('reports/cash-flow', { startDate, endDate });
  return fetchJson<CashFlowStatement>(url);
};

export const getFinancialSnapshot = async (params: {
  asOfDate: string;
  startDate: string;
  endDate: string;
  ...
  const [balanceSheet, incomeStatement, cashFlowStatement] = await Promise.all([
    getBalanceSheet(params.asOfDate),
    getIncomeStatement(params.startDate, params.endDate),
    params.includeCashFlow ? getCashFlowStatement(params.startDate, params.endDate) : Promise.resolve(undefined),
  ]);
...
export const getFinancialInsights = async (params: {
  ...
  const totalCurrentLiabilities = normalizeNumber(
    snapshot.balanceSheet.liabilities?.totalCurrentLiabilities
  );
  const totalLongTermLiabilities = normalizeNumber(
    snapshot.balanceSheet.liabilities?.totalLongTermLiabilities
  );
  ...
  let riskScore = 0;
  if (currentRatio > 0 && currentRatio < 1) riskScore += 40;
  if (debtToAssets > 0.6) riskScore += 30;
  if (netMargin < 0.05) riskScore += 20;
  if (netProfit < 0) riskScore += 30;

  const riskLevel = riskScore >= 60 ? 'high' : riskScore >= 35 ? 'medium' : 'low';

  const recommendations: string[] = [];
  if (currentRatio > 0 && currentRatio < 1) recommendations.push('تحسين السيولة من خلال زيادة الأصول المتداولة أو خفض الخصوم المتداولة.');
  if (debtToAssets > 0.6) recommendations.push('خفض المديونية عبر إعادة هيكلة الالتزامات أو تحسين التدفقات النقدية.');
  if (netMargin < 0.05) recommendations.push('مراجعة هيكل التكاليف لتحسين هامش الربح.');
  if (netProfit < 0) recommendations.push('خفض المصروفات التشغيلية أو زيادة الإيرادات لتحويل الربحية إلى إيجابية.');
...
export const getAdvancedInsightsReport = async (params: {
  asOfDate: string;
  startDate: string;
  endDate: string;
}) => {
  const insights = await getFinancialInsights(params);

  const liquiditySignal =
    insights.metrics.currentRatio > 0 && insights.metrics.currentRatio < 1
      ? 'ضعيفة'
      : insights.metrics.currentRatio >= 1.2
      ? 'مستقرة'
      : 'متوسطة';

  const leverageSignal =
    insights.metrics.debtToAssets > 0.6
      ? 'مرتفعة'
  ...
  const alerts = [] as Array<{ level: 'info' | 'warning' | 'critical'; message: string }>;
...
const monthLabel = (date: Date) =>
  date.toLocaleDateString('ar-SA', { year: 'numeric', month: 'short' });

export const getFinancialForecast = async (params: {
  asOfDate: string;
  ...
  const history: Array<{ month: string; revenue: number; expenses: number; netProfit: number }> = [];
  ...
  for (let i = historyMonths - 1; i >= 0; i -= 1) {
    ...
    const incomeStatement = await getIncomeStatement(range.start.toISOString(), range.end.toISOString());
  ...
  const calcAverageGrowth = (series: number[]) => {
  ...
  const forecast: Array<{ month: string; revenue: number; expenses: number; netProfit: number }> = [];
  ...
  const confidence = Math.max(
    0.3,
    1 - (revenueVolatility + expenseVolatility) / (Math.max(...revenueSeries, 1) + Math.max(...expenseSeries, 1))
  );
