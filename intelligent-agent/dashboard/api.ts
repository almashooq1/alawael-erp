...
router.get('/export/pdf', (req, res) => {
  ...
  const topQuestions = Object.entries(
    ...
    .sort((a, b) => b[1] - a[1])
    .slice(0, 5);
  const errorCount = weekLogs.filter(
    l => l.output && (l.output.toLowerCase().includes('خطأ') || l.output.toLowerCase().includes('error')),
  ).length;
...
router.get('/export/excel', async (req, res) => {
  ...
  await exportReportExcel(res, { total, weekCount: weekLogs.length, topQuestions, errorCount });
});
// API: إحصائيات عامة
