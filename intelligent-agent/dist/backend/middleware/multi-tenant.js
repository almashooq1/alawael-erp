...
const logger = (0, logger_1.createLogger)('MultiTenant');
// Plan-based limits
const planLimits = {
    free: {
        users: 3,
...
async function tenantMiddleware(req, res, next) {
    try {
        ...
        let tenant = await cache_1.cacheService.get(`tenant:${tenantId}`);
        if (!tenant) {
            // Fetch from database
            ...
            // Cache tenant data
            await cache_1.cacheService.set(`tenant:${tenantId}`, tenant, 300);
        }
        // Check tenant status
 ...
 */
async function checkLimit(tenantId, resource, currentCount) {
 ...
 */
function limitMiddleware(resource) {
...
// ==================== HELPER FUNCTIONS ====================
async function fetchTenantFromDB(tenantId) {
...
async function getCurrentResourceCount(tenantId, resource) {
    // Mock implementation - replace with actual database query
    return 0;
}
// Example usage:
