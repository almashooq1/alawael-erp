...
exports.classifyProcessRiskAdvanced = classifyProcessRiskAdvanced;
exports.predictDelayAdvanced = predictDelayAdvanced;
exports.trainProcessModel = trainProcessModel;
const process_deeplearning_1 = require("./process.deeplearning");
/**
 * Enhanced ML Service
 */
class EnhancedMLServiceClass {
    ...
    async initializeModels() {
        this.dlModel = new process_deeplearning_1.DeepLearningModel({
            inputSize: 10,
     ...
     */
    async classifyRiskAdvanced(process) {
     ...
     */
    combineRisks(dlRisk, ruleRisk) {
        const riskMap = { high: 2, medium: 1, low: 0 };
        const dlScore = riskMap[dlRisk] || 0;
        const ruleScore = riskMap[ruleRisk] || 0;
     ...
     */
    calculateRuleConfidence(features) {
     ...
     */
    riskToProbability(risk) {
        return risk === 'high' ? 0.85 : risk === 'medium' ? 0.5 : 0.15;
    }
    /**
     * Pattern Identification
     */
    identifyPatterns(features, process) {
        const patterns = [];
     ...
     * Generate Explanation
     */
    generateExplanation(risk, features, patterns) {
     ...
     * Generate Recommendations
     */
    generateRecommendations(risk, features, patterns) {
     ...
     */
    async predictDelayAdvanced(process) {
        const features = this.extractFeatures(process);
     ...
     */
    async predictDelayWithDL(features) {
        if (!this.dlModel) {
     ...
     * Estimate Completion Date
     */
    estimateCompletionDate(process, features) {
        const now = new Date();
    ...
    identifyBottlenecks(process) {
        ...
        process.steps.forEach(step => {
            if (step.status === 'in_progress' && step.dueDate) {
                ...
                if (delay > 0) {
                    const severity = delay > 7 * 24 * 60 * 60 * 1000 ? 'high' :
     ...
     */
    predictResourceNeeds(process, features) {
        const predictions = [];
     ...
     */
    async predictRisks(process, features) {
        const risks = [];
        // Delay risk
     ...
     */
    async trainModel(historicalProcesses) {
     ...
     */
    calculateDetailedMetricsFromRecords(records) {
     ...
     */
    calculateMetricsFromFeedback(records) {
        const labels = ['high', 'medium', 'low'];
        let correct = 0;
        for (const record of records) {
...
async function classifyProcessRiskAdvanced(process) {
    return mlService.classifyRiskAdvanced(process);
}
async function predictDelayAdvanced(process) {
    return mlService.predictDelayAdvanced(process);
}
async function trainProcessModel(historicalProcesses) {
