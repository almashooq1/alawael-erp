...
exports.executeRetraining = executeRetraining;
exports.getRetrainingStatus = getRetrainingStatus;
exports.scheduleAutoCheck = scheduleAutoCheck;
...
async function shouldRetrain(config = DEFAULT_CONFIG) {
    const reasons = [];
    ...
    const [feedback, driftEvents, currentMetrics] = await Promise.all([
        ml_feedback_model_1.default.countDocuments({ createdAt: { $gte: recentDate } }),
        ml_drift_model_1.default.countDocuments({ createdAt: { $gte: recentDate } }),
    ...
    if (accuracyDrop >= config.triggerConditions.minAccuracyDrop) {
        reasons.push(`Accuracy dropped by ${(accuracyDrop * 100).toFixed(1)}% (threshold: ${(config.triggerConditions.minAccuracyDrop * 100).toFixed(1)}%)`);
    }
    // شرط 3: انخفاض F1
    const baselineF1 = 0.82;
    const f1Drop = baselineF1 - currentMetrics.f1Score;
    if (f1Drop >= config.triggerConditions.minF1Drop) {
        reasons.push(`F1 Score dropped by ${(f1Drop * 100).toFixed(1)}% (threshold: ${(config.triggerConditions.minF1Drop * 100).toFixed(1)}%)`);
    }
    // شرط 4: أحداث الانحراف
    if (driftEvents >= config.triggerConditions.minDriftEvents) {
        reasons.push(`Drift events detected: ${driftEvents} (threshold: ${config.triggerConditions.minDriftEvents})`);
    }
...
// تنفيذ إعادة التدريب
async function executeRetraining(config = DEFAULT_CONFIG, triggeredBy = 'manual') {
    ...
    try {
        ...
        const improvementPercent = ((evaluation.accuracy - oldMetrics.accuracy) / oldMetrics.accuracy) * 100;
        const result = {
            success: true,
...
// جمع بيانات التدريب
async function collectTrainingData(config) {
...
// معالجة البيانات
async function preprocessData(data, config) {
...
// حفظ النموذج
async function saveModel(model, metrics) {
    const version = `v${Date.now()}`;
    const modelDir = path.join(__dirname, '..', 'models', 'saved');
    try {
        await fs.mkdir(modelDir, { recursive: true });
        await fs.writeFile(path.join(modelDir, `${version}.json`), JSON.stringify({ model, metrics, savedAt: new Date() }, null, 2));
    }
    catch (error) {
...
function scheduleAutoCheck(config = DEFAULT_CONFIG) {
