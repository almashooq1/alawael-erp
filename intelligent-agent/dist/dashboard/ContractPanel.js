...
const ContractActivityPanel_1 = __importDefault(require("./ContractActivityPanel"));
const SignaturePad_1 = __importDefault(require("./SignaturePad"));
const ExportFieldsSelector_1 = __importDefault(require("./ExportFieldsSelector"));
function ContractPanel() {
    const [contracts, setContracts] = useState([]);
    const [form, setForm] = useState({ title: '', parties: '', startDate: '', endDate: '', value: '', terms: '', ownerId: '' });
    const [file, setFile] = useState(null);
    // Export fields selector
    const [exportFields, setExportFields] = useState(['title', 'parties', 'startDate', 'endDate', 'value', 'status']);
    ...
    function handleAdd(e) {
        ...
        fetch('/dashboard/contract/add', {
            ...
            body: JSON.stringify({ ...form, parties: form.parties.split(',').map((p) => p.trim()), value: Number(form.value) })
    ...
    <ExportFieldsSelector_1.default selected={exportFields} onChange={setExportFields}/>
    ...
    <table style={{ width: '100%' }}><thead><tr><th>العنوان</th><th>الأطراف</th><th>البداية</th><th>النهاية</th><th>القيمة</th><th>الحالة</th><th>المخاطر</th><th>ملف</th><th>سجل النشاطات</th><th>توقيع إلكتروني</th></tr></thead>
      <tbody>
        {contracts.map(c => <React.Fragment key={c.id}>
          ...
          {showLog === c.id && <tr><td colSpan={10}><ContractActivityPanel_1.default contractId={c.id}/></td></tr>}
          {signingId === c.id && <tr><td colSpan={10} style={{ background: '#f9f9f9' }}>
            <SignaturePad_1.default onSave={dataUrl => {
