...
router.get('/requests', (req, res) => {
    const stats = performance_monitor_1.performanceMonitor.getStats();
...
router.get('/alerts', (req, res) => {
    ...
    // Response Time Alert
    if (stats.avgResponseTime && stats.avgResponseTime > 500) {
        alerts.push({
            type: 'warning',
            message: `Slow response time: ${(stats.avgResponseTime).toFixed(0)}ms`,
            timestamp: new Date().toISOString(),
        });
    }
    res.json(alerts);
