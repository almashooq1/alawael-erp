...
export function getContractsWithRisk() {
...
import { ContractManager, Contract } from './contract-manager';
...
export function analyzeContractsSmartly(): SmartContractAnalysis[] {
  const manager = new ContractManager();
  const contracts = manager.listContracts();
  const now = new Date();
  ...
  return contracts.map((c: Contract) => {
    ...
    let risk: 'منخفض' | 'متوسط' | 'مرتفع' = 'منخفض';
    ...
    let aiScore = 0;
    if (c.value > avgValue * 2) aiScore += 1;
    const terminatedParties = contracts.filter(x=>x.status==='terminated').flatMap(x=>x.parties);
    if (c.parties.some(p=>terminatedParties.includes(p))) aiScore += 1;
    if (daysToExpire < 0) aiScore += 1;
    if (aiScore >= 2) {
      risk = 'مرتفع';
