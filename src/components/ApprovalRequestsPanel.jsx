  ...
  return (
    <Box p={2}>
          ...
          <Grid item xs={12} md={6} key={req._id}>
            <Paper sx={{ p: 2 }}>
              ...
              <Typography variant="body2">
                تاريخ الإنشاء: {new Date(req.createdAt).toLocaleString()}
              </Typography>
              <Typography variant="body2">
                سلسلة الموافقات: {req.steps.map((s, i) => `${i + 1}: ${s.action}`).join(' | ')}
              </Typography>
            </Paper>
          </Grid>
        ))}
      ...
      <Dialog
        ...
        onClose={() => setActionDialog({ open: false, approval: null, action: '', comment: '' })}
      >
        <DialogTitle>تأكيد الإجراء</DialogTitle>
        <DialogContent>
          <Typography>
            هل أنت متأكد من أنك تريد {actionDialog.action === 'approved' ? 'الموافقة' : 'الرفض'} على
            هذا الطلب؟
          </Typography>
          <TextField
            label="تعليق (اختياري)"
