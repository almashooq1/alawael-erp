...
const CustomReportsDashboard = () => {
  ...
  const handleExportDashboard = () => {
    try {
      exportService.toJSON([selectedDashboard], `${selectedDashboard.name}-dashboard`);
    } catch (error) {
  ...
  const WidgetRenderer = ({ widget }) => {
    switch (widget.type) {
      ...
      case 'table':
        return (
          <Paper sx={{ p: 2 }}>
            <Typography variant="body2">جدول: {widget.title}</Typography>
          </Paper>
        );
      case 'kpis':
        return (
          <Paper sx={{ p: 2 }}>
            <Typography variant="body2">KPIs: {widget.title}</Typography>
          </Paper>
        );
      default:
                      ...
                      sx={{
                        ...
                        bgcolor:
                          selectedDashboard?.id === dashboard.id
                            ? 'primary.light'
                            : 'background.paper',
                        border: selectedDashboard?.id === dashboard.id ? '2px solid' : '1px solid',
                        borderColor: 'divider',
                      }}
