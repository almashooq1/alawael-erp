...
const AuditTrail = ({ organizationId }) => {
  ...
  const fetchAuditTrail = useCallback(async () => {
    try {
      ...
      const response = await fetch(`/api/finance/audit-trail?${queryParams.toString()}`, {
        headers: {
  ...
  const handleLogClick = async log => {
    try {
      const response = await fetch(`/api/finance/audit-trail/${log.id}`, {
        headers: {
          Authorization: `Bearer ${localStorage.getItem('token')}`,
        },
      });
  ...
  const handleExport = async format => {
    try {
      const response = await fetch(`/api/finance/audit-trail/export`, {
        method: 'POST',
        headers: {
  ...
  const SearchAndFilters = () => (
    <div className="search-and-filters">
      ...
      <div className="export-actions">
        <button className="btn btn-export" onClick={() => handleExport('pdf')}>
          📄 PDF
        </button>
        <button className="btn btn-export" onClick={() => handleExport('xlsx')}>
          📊 Excel
        </button>
        <button className="btn btn-secondary" onClick={fetchAuditTrail} disabled={loading}>
          {loading ? '⏳...' : '🔄 تحديث'}
        </button>
      </div>
    </div>
        ...
        <thead>
          <tr>
            ...
            <th onClick={() => handleSort('operation')} className="sortable">
              العملية{' '}
              {sortConfig.key === 'operation' && (sortConfig.direction === 'asc' ? '▲' : '▼')}
            </th>
            <th>النوع</th>
            <th>المعرف</th>
            <th>الوصف</th>
          ...
          {auditLogs.length === 0 ? (
            ...
            auditLogs.map(log => (
              <tr key={log.id} className={`log-row severity-${log.severity.toLowerCase()}`}>
                <td className="timestamp">{new Date(log.timestamp).toLocaleString('ar-SA')}</td>
                ...
                <td>
                  <button className="btn-view" onClick={() => handleLogClick(log)}>
        ...
        <div className="audit-main">
          ...
          {auditLogs.length > 0 && (
            <div className="pagination-controls">
              ...
              <span className="page-info">صفحة {pagination.page}</span>
              <button
                onClick={() => setPagination({ ...pagination, page: pagination.page + 1 })}
                className="btn-pagination"
              >
