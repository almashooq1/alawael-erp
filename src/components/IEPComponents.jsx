  ...
  return (
    <Card className="iep-generator mb-4">
      <Card.Header className="bg-primary text-white">
        <Card.Title className="mb-0">🔧 مولد الخطط التعليمية الذكية</Card.Title>
      </Card.Header>
      <Card.Body>
...
export const IEPGoalsManager = ({ iepId, goals = [], onUpdate }) => {
  ...
  const handleAddGoal = async () => {
    try {
      const response = await axios.post(`/api/intelligent-iep/${iepId}/goals`, newGoal, {
        headers: { Authorization: `Bearer ${localStorage.getItem('token')}` },
      });
            ...
            <tbody>
              {goals.map(goal => (
                <tr key={goal.id}>
                  ...
                  <td>
                    <Badge bg={priorityColors[goal.priority] || 'secondary'}>{goal.priority}</Badge>
                  </td>
                  <td>
          ...
          <Form>
            <Form.Group className="mb-3">
              ...
              <Form.Control
                ...
                onChange={e => setNewGoal({ ...newGoal, goal_statement: e.target.value })}
              />
            </Form.Group>
...
export const ProgressTracker = ({ iepId, goals = [] }) => {
  ...
  const fetchProgress = async () => {
    setLoading(true);
    try {
      const response = await axios.get(`/api/intelligent-iep/${iepId}/progress`, {
        headers: { Authorization: `Bearer ${localStorage.getItem('token')}` },
      });
      ...
      <Card.Body>
        ...
        <Tabs defaultActiveKey="goals" className="mb-3">
          ...
          <Tab eventKey="interventions" title="فعالية التدخلات">
            <div className="mt-3">
              <Alert variant="info">قيد التطوير - سيتم عرض تقييم فعالية التدخلات هنا</Alert>
            </div>
          </Tab>
        </Tabs>
...
export const AIRecommendations = ({ iepId }) => {
  ...
  const fetchRecommendations = async () => {
    setLoading(true);
    try {
      const response = await axios.get(`/api/intelligent-iep/${iepId}/ai-recommendations`, {
        headers: { Authorization: `Bearer ${localStorage.getItem('token')}` },
      });
            ...
            {recommendations.map((rec, idx) => (
              <Card key={idx} className="mb-3 border-left">
                <Card.Body>
                  ...
                  {!rec.approved && (
                    <Button size="sm" variant="success" onClick={() => handleApprove(rec.id)}>
                      ✓ الموافقة
                    </Button>
                  )}
                  {rec.approved && <Badge bg="success">موافق عليه</Badge>}
                </Card.Body>
              </Card>
            ))}
          </div>
  ...
  return (
    <div className="iep-dashboard">
      <div className="container-fluid p-4">
        ...
        {!iepId ? (
          <IEPGenerator studentId={studentId} onGenerate={handleIEPGenerated} />
        ) : (
          <div>
            ...
            <Tabs activeKey={activeTab} onSelect={k => setActiveTab(k)} className="mb-4">
              <Tab eventKey="overview" title="نظرة عامة">
                <div className="mt-4">
                  <IEPGoalsManager iepId={iepId} goals={iepData?.goals} onUpdate={() => {}} />
                </div>
              </Tab>

              <Tab eventKey="progress" title="التقدم">
                <div className="mt-4">
                  <ProgressTracker iepId={iepId} goals={iepData?.goals} />
                </div>
              </Tab>
