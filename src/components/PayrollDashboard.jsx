      ...
      {stats && (
        <div className="statistics-grid">
          <div className="stat-card">
            ...
            <div className="stat-value">{formatCurrency(stats.monthly?.totalGross || 0)}</div>
          </div>
          <div className="stat-card">
            <div className="stat-label">إجمالي الخصومات</div>
            <div className="stat-value">{formatCurrency(stats.monthly?.totalDeductions || 0)}</div>
          </div>
          <div className="stat-card">
...
const PayrollRow = ({ payroll, formatCurrency, onUpdate }) => {
  ...
  const handleApprove = async () => {
    ...
    try {
      await axios.put(
        `${API_BASE}/payroll/${payroll._id}/approve`,
        {},
        {
          headers: { Authorization: `Bearer ${localStorage.getItem('token')}` },
        }
