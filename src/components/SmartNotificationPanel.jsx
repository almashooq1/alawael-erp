  ...
  const filteredNotifications = notifications.filter(
    n =>
      ...
      (search.trim() === '' ||
        ...
        (n.message && n.message.includes(search)))
  );

  return (
    <>
      {/* Notification Button */}
      <IconButton onClick={handleOpenMenu} sx={{ position: 'relative', color: 'primary.main' }}>
          ...
          {error && !loading && (
            <Box sx={{ p: 3, textAlign: 'center' }}>
              <Typography color="error">
                {typeof error === 'string' ? error : (error && 'An internal error occurred') || 'Failed to load'}
              </Typography>
            </Box>
          )}
          {/* Notification List */}
