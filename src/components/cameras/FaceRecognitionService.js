...
const FaceRecognitionService = ({ camera, onClose, onDetectFace }) => {
  ...
  const handleConfirmFace = useCallback((faceId, name) => {
    setFaceDatabase(prev =>
      prev.map(f =>
        f.id === faceId
          ? { ...f, confidence: Math.min(f.confidence + 10, 99), lastSeen: Date.now() }
          : f
      )
    );
  }, []);
        ...
        <Stack spacing={3}>
                    ...
                    <CardContent>
                      ...
                      {face.status === 'recognized' && (
                        <Button
                          size="small"
                          fullWidth
                          onClick={() => handleConfirmFace(face.id, face.name)}
                          sx={{ mt: 1 }}
                        >
                          تأكيد
                        </Button>
          ...
          {alerts.length > 0 && (
            <Alert
              severity="warning"
              icon={<WarningIcon />}
              sx={{ borderRadius: 2, display: 'flex', alignItems: 'center', gap: 1 }}
            >
              <Box>
                  ...
                  <TableRow key={face.id} hover>
                    ...
                    <TableCell>
                      <Typography variant="caption">
                        {new Date(face.lastSeen).toLocaleTimeString('ar-SA')}
                      </Typography>
                    </TableCell>
                    <TableCell>
