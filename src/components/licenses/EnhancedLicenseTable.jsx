    ...
    <Box sx={{ width: '100%', p: 2 }}>
      ...
      <Card sx={{ mb: 3 }}>
        <CardContent>
          ...
          <Box sx={{ mt: 2, display: 'flex', gap: 2, flexWrap: 'wrap' }}>
            <Chip label={`الكل: ${filtered.length}`} variant="outlined" size="small" />
            <Chip
      ...
      <TableContainer component={Paper}>
        ...
        {!loading && paginatedData.length === 0 && (
          ...
          >
            <Typography variant="body1">لم يتم العثور على رخص مطابقة</Typography>
          </Box>
        )}
            ...
            <TableHead>
              <TableRow sx={{ backgroundColor: '#f5f5f5' }}>
                <TableCell padding="checkbox">
                  <Checkbox
                    indeterminate={
                      selectedLicenses.size > 0 && selectedLicenses.size < paginatedData.length
                    }
                    checked={
                      selectedLicenses.size === paginatedData.length && paginatedData.length > 0
                    }
                ...
                {visibleColumns.licenseType && <TableCell>نوع الرخصة</TableCell>}

                {visibleColumns.issueDate && <TableCell>تاريخ الإصدار</TableCell>}
                ...
                {visibleColumns.status && <TableCell>الحالة</TableCell>}

                {visibleColumns.action && <TableCell align="center">الإجراءات</TableCell>}
              </TableRow>
            </TableHead>
            ...
            <TableBody>
              {paginatedData.map(license => (
                ...
                >
                  ...
                  {visibleColumns.licenseNumber && (
                    <TableCell sx={{ fontWeight: 600 }}>{license.licenseNumber}</TableCell>
                  )}

                  {visibleColumns.entityName && <TableCell>{license.entity?.name}</TableCell>}
                  ...
                  {visibleColumns.action && (
                    <TableCell align="center">
                      <Tooltip title="المزيد">
                        <IconButton size="small" onClick={e => handleMenuOpen(e, license)}>
                          <MoreVertIcon />
                        </IconButton>
                      </Tooltip>
                    </TableCell>
