...
const ABTestingFramework = () => {
  ...
  const handleStopTest = testId => {
    setTests(
      tests.map(t =>
        t.id === testId
          ? { ...t, status: 'completed', endDate: new Date().toISOString().split('T')[0] }
          : t
      )
    );
  };
                ...
                <CardContent>
                  ...
                  >
                    <Box>
                      ...
                      <Typography variant="caption" color="textSecondary">
                        بدأ: {test.startDate} | المدة:{' '}
                        {Math.floor(
                          (new Date() - new Date(test.startDate)) / (1000 * 60 * 60 * 24)
                        )}{' '}
                        يوم
                      </Typography>
                    </Box>
                      ...
                      <TableBody>
                        {test.variants.map((variant, idx) => (
                          <TableRow
                            key={idx}
                            sx={{
                              backgroundColor: variant.name === test.winner ? '#c8e6c9' : 'white',
                            }}
                          >
                    ...
                    {test.variants.map((variant, idx) => (
                      <Grid item xs={12} key={idx}>
                        <Paper
                          sx={{
                            p: 1.5,
                            backgroundColor: variant.name === test.winner ? '#c8e6c9' : '#f8f9ff',
                          }}
                        >
                          <Box
                            sx={{
                          ...
                          <LinearProgress
                            variant="determinate"
                            value={variant.conversionRate * 20}
              ...
              <Grid item xs={12} md={6} key={idx}>
                <Card sx={{ borderRadius: 2 }}>
                  <CardContent>
                    ...
                    <Box
                      sx={{ backgroundColor: '#f8f9ff', p: 2, borderRadius: 1.5, minHeight: 200 }}
                    >
                      <Typography
                        variant="body2"
                        color="textSecondary"
      ...
      <Dialog open={openDialog} onClose={() => setOpenDialog(false)} maxWidth="sm" fullWidth>
        ...
        <DialogContent sx={{ mt: 2 }}>
          <TextField
            ...
            variant="outlined"
            sx={{ mb: 2 }}
            placeholder="مثال: اختبار لون الزر"
          />
          <TextField
            fullWidth
            label="الهدف من الاختبار"
            variant="outlined"
            multiline
            rows={3}
            sx={{ mb: 2 }}
