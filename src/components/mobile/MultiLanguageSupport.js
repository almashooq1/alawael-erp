...
const MultiLanguageSupport = () => {
  ...
  const stats = {
    ...
    avgCompletion: Math.round(
      languages.reduce((sum, l) => sum + l.completion, 0) / languages.length
    ),
  };
              ...
              <CardContent>
                ...
                >
                  ...
                  {lang.code === currentLanguage && (
                    <Chip label="الحالية" color="success" size="small" icon={<CheckIcon />} />
                  )}
                </Box>
                ...
                <Box sx={{ mb: 2 }}>
                  <Box sx={{ display: 'flex', justifyContent: 'space-between', mb: 0.5 }}>
                    ...
                    <Typography
                      variant="caption"
                      sx={{ fontWeight: 600, color: lang.code === 'ar' ? '#4caf50' : '#ff9800' }}
                    >
                      {lang.completion}%
                    </Typography>
                  </Box>
        ...
        <List>
          {currencies.map(curr => (
            <ListItem
              key={curr.code}
              sx={{
                backgroundColor: curr.code === selectedCurrency ? '#e3f2fd' : 'transparent',
                borderRadius: 1,
                mb: 0.5,
              }}
            >
              <ListItemIcon>
                {curr.code === 'USD' ? '$' : curr.code === 'EUR' ? '€' : curr.symbol}
              </ListItemIcon>
              <ListItemText
                primary={curr.name}
                secondary={`${curr.code} • السعر: 1 SAR = ${curr.rate} ${curr.code}`}
              />
        ...
        {regions.map(region => (
          <Grid item xs={12} sm={6} key={region.name}>
            <Card
              sx={{
                borderRadius: 2,
                borderLeft: `4px solid ${region.selected ? '#667eea' : '#ccc'}`,
              }}
            >
              <CardContent>
                <Box
