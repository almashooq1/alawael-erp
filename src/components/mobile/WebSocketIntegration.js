...
const WebSocketIntegration = () => {
  ...
  const [events, setEvents] = useState([
    ...
    {
      id: 2,
      timestamp: '2026-01-16 15:29:20',
      type: 'data_sync',
      user: 'system',
      data: 'Synchronized 250 records',
      status: 'success',
    },
    {
      id: 3,
    ...
    },
    {
      ...
      type: 'error',
      user: 'system',
      data: 'Connection timeout on endpoint 2',
      status: 'error',
    },
  ]);
          ...
          {events.map((event, idx) => (
            <Box key={event.id}>
              <ListItem>
                ...
                <ListItemText
                  primary={event.type}
                  secondary={`${event.timestamp} • ${event.user}`}
                />
                <Typography variant="caption" color="textSecondary">
                  {event.data}
                </Typography>
