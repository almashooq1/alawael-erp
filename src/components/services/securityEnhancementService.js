    ...
    if (userData.requests.length >= this.securityConfig.rateLimit.maxRequests) {
      ...
      return {
        ...
        retryAfter: Math.ceil(
          (userData.requests[0] + this.securityConfig.rateLimit.windowMs - now) / 1000
        ),
      };
    }
  ...
  validateInput(data, schema = {}) {
    ...
    if (typeof data === 'string') {
      if (data.length > this.securityConfig.inputValidation.maxStringLength) {
        errors.push(
          `String exceeds maximum length of ${this.securityConfig.inputValidation.maxStringLength}`
        );
      }
    }
    ...
    if (Array.isArray(data)) {
      if (data.length > this.securityConfig.inputValidation.maxArrayLength) {
        errors.push(
          `Array exceeds maximum length of ${this.securityConfig.inputValidation.maxArrayLength}`
        );
      }
    }
    ...
    return {
      ...
      blockRate:
        this.requestStats.totalRequests > 0
          ? ((this.requestStats.blockedRequests / this.requestStats.totalRequests) * 100).toFixed(
              2
            ) + '%'
          : '0%',
