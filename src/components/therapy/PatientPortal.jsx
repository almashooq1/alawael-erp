...
import { CheckCircle, Schedule, TrendingUp, Chat, Download, Cancel } from '@mui/icons-material';
...
export default function PatientPortal() {
  ...
  const handleSubmitFeedback = async () => {
    try {
      await api.post(`/therapy-sessions/${selectedSession._id}/feedback`, feedback);
  ...
  return (
    <Box sx={{ p: 3 }}>
      ...
      {/* Upcoming Sessions */}
      <Card sx={{ mb: 3 }}>
        <CardHeader title="Upcoming Sessions" avatar={<Schedule />} />
        <CardContent>
            ...
            <List>
              {upcomingSessions.map(session => (
                ...
                >
                  ...
                  />
                  <Button size="small" color="error" onClick={() => cancelSession(session._id)}>
                    Cancel
                  </Button>
                </ListItem>
              ))}
      ...
      {progress?.goalsProgress && (
        <Card sx={{ mb: 3 }}>
          <CardHeader title="Your Goals Progress" avatar={<TrendingUp />} />
          <CardContent>
            <Timeline>
              {progress.goalsProgress.map((goal, index) => (
                <TimelineItem key={index}>
                  <TimelineSeparator>
                    ...
                    {index < progress.goalsProgress.length - 1 && <TimelineConnector />}
                  </TimelineSeparator>
                  <TimelineContent>
                    <Typography variant="h6">{goal.name}</Typography>
                    <Box sx={{ mb: 1 }}>
                      <LinearProgress variant="determinate" value={goal.progress || 0} />
                    </Box>
      ...
      <Card sx={{ mb: 3 }}>
        <CardHeader title="Recent Sessions" avatar={<CheckCircle />} />
        <CardContent>
      ...
      {progress?.homeProgram && (
        <Card>
          <CardHeader title="Your Home Program" avatar={<Download />} />
          <CardContent>
