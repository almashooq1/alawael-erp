...
export default function QualityReviewInterface() {
  ...
  const handleSubmitReview = async () => {
    try {
      await api.post(`/therapy-sessions/${selectedDoc.session._id}/documentation/review`, {
                    ...
                    <TableCell align="center">
                      {doc.quality?.scoreComment ? (
                        <Box sx={{ textAlign: 'center' }}>
                          <Rating value={doc.quality.scoreComment} readOnly size="small" />
                          <Typography variant="caption">{doc.quality.scoreComment}/5</Typography>
                        </Box>
                      ) : (
                        <Typography variant="caption" color="textSecondary">
                          -
          ...
          {selectedDoc && (
            <Box>
              ...
              <Paper sx={{ p: 2, mb: 2, backgroundColor: '#f9f9f9' }}>
                ...
                <Typography variant="body2">
                  <strong>Date:</strong>{' '}
                  {format(new Date(selectedDoc.session.date), 'MMMM dd, yyyy')}
                </Typography>
                  ...
                  {selectedDoc.soapNote?.objective && (
                    <Box sx={{ mb: 1 }}>
                      ...
                      <Typography variant="body2" sx={{ ml: 2 }}>
                        Accuracy: {selectedDoc.soapNote.objective.accuracy}% | Reps:{' '}
                        {selectedDoc.soapNote.objective.repetitions} | Assistance:{' '}
                        {selectedDoc.soapNote.objective.assistanceLevel}
                      </Typography>
                    </Box>
                  )}
