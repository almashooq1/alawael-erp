...
import { Download, TrendingUp, CheckCircle, Clock, WarningAmber } from '@mui/icons-material';
...
export default function SessionAnalyticsDashboard() {
  ...
  const { overview, trends, breakdown, therapistComparison, goalAchievementRate, topPerformers } =
    analytics;
    ...
    <Box sx={{ p: 3 }}>
      ...
      <Box sx={{ mb: 3, display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
        ...
        <Box sx={{ display: 'flex', gap: 1 }}>
          ...
          <Button variant="contained" startIcon={<Download />} onClick={downloadReport}>
            Export Report
          </Button>
        </Box>
      </Box>
      ...
      <Grid container spacing={2} sx={{ mb: 3 }}>
        ...
        <Grid item xs={12} sm={6} md={3}>
          <Paper sx={{ p: 2 }}>
            ...
            <Typography variant="caption">Based on {overview.ratingCount || 0} ratings</Typography>
          </Paper>
        </Grid>
        ...
        <Grid item xs={12} sm={6} md={3}>
          <Paper sx={{ p: 2 }}>
            ...
            <Typography variant="caption">{overview.noShowCount || 0} sessions</Typography>
          </Paper>
        </Grid>
      </Grid>
                  ...
                  <TableBody>
                    <TableRow>
                      ...
                      <TableCell align="right">
                        {breakdown.byStatus?.total > 0
                          ? (
                              (breakdown.byStatus.completed / breakdown.byStatus.total) *
                              100
                            ).toFixed(1)
                          : 0}
                        %
                    ...
                    <TableRow>
                      ...
                      <TableCell align="right">
                        {breakdown.byStatus?.total > 0
                          ? (
                              (breakdown.byStatus.scheduled / breakdown.byStatus.total) *
                              100
                            ).toFixed(1)
                          : 0}
                        %
                    ...
                    <TableRow>
                      ...
                      <TableCell align="right">
                        {breakdown.byStatus?.total > 0
                          ? (
                              (breakdown.byStatus.cancelled / breakdown.byStatus.total) *
                              100
                            ).toFixed(1)
                          : 0}
                        %
                    ...
                    <TableRow>
                      <TableCell>
                        <Chip label="No-Show" color="warning" size="small" />
                      </TableCell>
                      <TableCell align="right">{breakdown.byStatus?.noShow || 0}</TableCell>
                      <TableCell align="right">
                        {breakdown.byStatus?.total > 0
                          ? ((breakdown.byStatus.noShow / breakdown.byStatus.total) * 100).toFixed(
                              1
                            )
                          : 0}
                        %
                ...
                <TableBody>
                  {topPerformers.map((therapist, index) => (
                    <TableRow key={therapist._id}>
                      ...
                      <TableCell align="right">
                        {(therapist.goalAchievementRate || 0).toFixed(1)}%
                      </TableCell>
                    </TableRow>
                  ))}
                </TableBody>
