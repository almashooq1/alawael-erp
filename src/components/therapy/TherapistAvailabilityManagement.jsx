...
export default function TherapistAvailabilityManagement() {
  ...
  const handleEditDay = day => {
    const existing = availability?.recurringSchedule?.find(s => s.dayOfWeek === day);
  ...
  const handleSaveDay = async () => {
    ...
    try {
      ...
      const updatedSchedule = currentSchedule.filter(s => s.dayOfWeek !== daySchedule.dayOfWeek);
  ...
  const handleRemoveDay = async day => {
    ...
    try {
      const updatedSchedule = availability.recurringSchedule.filter(s => s.dayOfWeek !== day);
          ...
          <Grid container spacing={2}>
            ...
            <Grid item xs={12}>
              ...
              {daySchedule.breaks?.map((breakPeriod, index) => (
                <Paper key={index} sx={{ p: 2, mb: 1, display: 'flex', gap: 1, alignItems: 'end' }}>
                  <TextField
                    type="time"
                    label="Break Start"
                    InputLabelProps={{ shrink: true }}
                  ...
                  <TextField
                    ...
                    size="small"
                  />
                  <Button color="error" onClick={() => removeBreak(index)} startIcon={<Delete />}>
                    Remove
                  </Button>
                </Paper>
              ))}
              <Button variant="outlined" startIcon={<Add />} onClick={addBreak} sx={{ mt: 1 }}>
                Add Break
              </Button>
            </Grid>
          </Grid>
