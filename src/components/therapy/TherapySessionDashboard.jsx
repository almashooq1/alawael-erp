...
const TherapySessionDashboard = () => {
  ...
  const handleUpdateStatus = async (sessionId, newStatus) => {
    try {
      const response = await axios.patch(`${API_BASE_URL}/therapy-sessions/${sessionId}/status`, {
        status: newStatus,
      });
  ...
  const handleMarkAttendance = async sessionId => {
    try {
      const response = await axios.post(`${API_BASE_URL}/therapy-sessions/${sessionId}/attend`, {
                  ...
                  {sessions.length === 0 ? (
                    ...
                    sessions.map(session => (
                      <TableRow key={session._id}>
                        <TableCell>
                          {session.beneficiary?.firstName} {session.beneficiary?.lastName}
                        </TableCell>
                        <TableCell>
                          ...
                          {session.status !== 'COMPLETED' && session.status !== 'NO_SHOW' && (
                            <Tooltip title="Cancel">
                              <IconButton
                                size="small"
                                onClick={() =>
                                  handleUpdateStatus(session._id, 'CANCELLED_BY_CENTER')
                                }
                              >
          ...
          >
            <Box sx={{ p: 3 }}>
              ...
              <Grid container spacing={2}>
                <Grid item xs={12}>
                  <strong>Beneficiary:</strong> {selectedSession.beneficiary?.firstName}{' '}
                  {selectedSession.beneficiary?.lastName}
                </Grid>
                <Grid item xs={12}>
                  <strong>Therapist:</strong> {selectedSession.therapist?.firstName}{' '}
                  {selectedSession.therapist?.lastName}
                </Grid>
                <Grid item xs={12}>
