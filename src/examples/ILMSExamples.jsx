...
export function EnrollmentManagementExample({ studentId }) {
  const { enrollments, loading, enrollInCourse, unenrollFromCourse } = useEnrollments(studentId);
                  ...
                  {lessons.map(lesson => (
                    ...
                    >
                      <div className="d-flex justify-content-between">
                        ...
                        <span className="badge bg-secondary">{lesson.progress}%</span>
                      </div>
                    </button>
                  ))}
                </div>
...
export function RecommendationsExample({ studentId }) {
  const { recommendations, loading, acceptRecommendation, dismissRecommendation } =
          ...
          {recommendations.map(rec => (
            <Col md={6} key={rec.id} className="mb-4">
              <Card>
                ...
                <Card.Footer>
                  <Button variant="success" size="sm" onClick={() => acceptRecommendation(rec.id)}>
                    قبول
                  </Button>
                  <Button
                    variant="danger"
        ...
        <Card.Body>
          {editMode ? (
            ...
            <div>
              ...
              <p>
                <strong>البريد:</strong> {profile?.email}
              </p>
              <p>
                <strong>أسلوب التعلم:</strong> {profile?.learning_style}
              </p>
            </div>
          )}
        </Card.Body>
