  ...
  const fetchInvoices = async () => {
    try {
      const response = await axios.get(
        'http://localhost:3001/api/accounting/invoices?status=sent,partial'
      );
  ...
  const handleOpenDialog = (payment = null) => {
    if (payment) {
      ...
      setFormData({
        ...
        chequeDate: payment.chequeDate ? format(new Date(payment.chequeDate), 'yyyy-MM-dd') : '',
      });
    } else {
                    ...
                    <Grid item xs={6}>
                      ...
                      <Typography variant="body1">
                        {selectedPayment.chequeDate
                          ? format(new Date(selectedPayment.chequeDate), 'dd/MM/yyyy', {
                              locale: ar,
                            })
                          : '-'}
                      </Typography>
