...
const ProgramDetails = ({ programId, onBack }) => {
  ...
  const fetchProgramData = async () => {
    try {
      ...
      // جلب تفاصيل البرنامج
      const programResponse = await axios.get(`${API_ENDPOINT}/programs/${programId}`, {
      ...
      // جلب التقرير المفصل
      const reportResponse = await axios.get(`${API_ENDPOINT}/programs/${programId}/report`, {
        headers: { Authorization: `Bearer ${token}` },
      });
  ...
  const handleAddSession = async () => {
    try {
      ...
      await axios.post(`${API_ENDPOINT}/programs/${programId}/sessions`, sessionData, {
        headers: { Authorization: `Bearer ${token}` },
      });
  ...
  const handleAddAssessment = async () => {
    try {
      ...
      await axios.post(`${API_ENDPOINT}/programs/${programId}/assessments`, assessmentData, {
        headers: { Authorization: `Bearer ${token}` },
      });
  ...
  if (!program) {
    return <Alert variant="danger">البرنامج غير موجود</Alert>;
  }
              ...
              {program.program_status === 'active' && (
                <>
                  <Button
                    size="sm"
                    variant="warning"
                    onClick={() => updateProgramStatus('paused')}
                    className="me-2"
                  >
                    ⏸️ إيقاف مؤقت
                  </Button>
                          ...
                          {goal.progress_percentage !== undefined && (
                            <div className="mt-2">
                              <ProgressBar
                                ...
                                label={`${goal.progress_percentage}%`}
                              />
                            </div>
                          )}
                        </Col>
          ...
          <Form>
            <Form.Group className="mb-3">
              ...
              <Form.Control
                ...
                onChange={e =>
                  setAssessmentData({ ...assessmentData, assessment_date: e.target.value })
                }
              />
            </Form.Group>
            ...
            <Form.Group className="mb-3">
              ...
              <Form.Select
                ...
                onChange={e =>
                  setAssessmentData({ ...assessmentData, assessment_type: e.target.value })
                }
              >
            ...
            <Form.Group className="mb-3">
              ...
              <Form.Control
                ...
                onChange={e =>
                  setAssessmentData({ ...assessmentData, recommendations: e.target.value })
                }
              />
            </Form.Group>
          </Form>
          ...
          <Form>
            ...
            <Form.Group className="mb-3">
              ...
              <Form.Control
                ...
                onChange={e =>
                  setGoalUpdate({ ...goalUpdate, progress_percentage: e.target.value })
                }
              />
              <Form.Range
                min="0"
