  ...
  return (
    <Container maxWidth="lg" sx={{ py: 3 }}>
      ...
      <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', mb: 3 }}>
        ...
        <Button variant="contained" startIcon={<Add />} onClick={() => navigate('/programs/new')}>
          برنامج جديد
        </Button>
      </Box>
          ...
          {programs.length > 0 ? (
            programs.map(program => (
              <Grid item xs={12} sm={6} md={4} key={program.id}>
                <Card sx={{ height: '100%', display: 'flex', flexDirection: 'column' }}>
                  ...
                  <CardActions>
                    <Button size="small" onClick={() => navigate(`/programs/${program.id}`)}>
                      عرض التفاصيل
                    </Button>
                  </CardActions>
                </Card>
            ...
            <Grid item xs={12}>
              <Paper sx={{ p: 4, textAlign: 'center' }}>
                <Typography color="text.secondary">لا توجد برامج</Typography>
              </Paper>
            </Grid>
          )}
        </Grid>
      ...
      <Dialog open={openDelete} onClose={() => setOpenDelete(false)}>
        ...
        <DialogContent>هل أنت متأكد من حذف هذا البرنامج؟</DialogContent>
        <DialogActions>
