...
const brandingManager = {
    ...
    themesGrid.innerHTML = currentThemes.map(theme => `
        <div class="col-md-6 col-lg-4 mb-4">
                    ...
                    <div class="d-flex justify-content-between align-items-center">
                        <div class="form-check form-switch">
                            <input class="form-check-input" type="checkbox" id="theme-${theme.id}" 
                                   ...
                                   onchange="toggleThemeStatus(${theme.id})">
                            <label class="form-check-label" for="theme-${theme.id}">نشط</label>
                        </div>
                        <div class="btn-group btn-group-sm">
                            <button class="btn btn-outline-success" onclick="applyTheme(${theme.id})" 
                                    ${theme.is_default ? 'disabled' : ''}>
                                <i class="fas fa-check"></i>
                            </button>
                            <button class="btn btn-outline-primary" onclick="editTheme(${theme.id})">
                                <i class="fas fa-edit"></i>
                            </button>
                            <button class="btn btn-outline-danger" onclick="deleteTheme(${theme.id})" 
                                    ${theme.is_default ? 'disabled' : ''}>
                                <i class="fas fa-trash"></i>
                            </button>
                        </div>
...
async function loadBranchesForLogo() {
    try {
...
// تبديل حالة الشعار
async function toggleLogoStatus(logoId) {
    try {
...
// تبديل حالة الثيم
async function toggleThemeStatus(themeId) {
    try {
