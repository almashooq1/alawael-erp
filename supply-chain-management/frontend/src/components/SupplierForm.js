...
function SupplierForm({ onAdd, initialData, editMode, user, notify }) {
  ...
  return (
    <form onSubmit={handleSubmit} style={{ marginBottom: 16 }}>
      <input name="name" placeholder="اسم المورد" value={form.name} onChange={handleChange} required />{' '}
      ...
      <input name="rating" type="number" min="0" max="5" placeholder="تقييم" value={form.rating} onChange={handleChange} />{' '}
      <button type="submit" disabled={loading}>
        {loading ? (editMode ? 'جاري التعديل...' : 'جاري الإضافة...') : editMode ? 'حفظ التعديلات' : 'إضافة مورد'}
      </button>
    </form>
  );
}
